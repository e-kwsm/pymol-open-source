...
/// Gets properties
void Get_Property_Values(
    GRASSP* grassp, float* properties, float* colores, int nvert)
{
  long i;
  const char* name[] = {
      "potential", "curvature", "distance", "property1", "property2"};
  SHORTWORD index;
  int k = 0, j = 0;
...
#define TRASH fread(trash, 4, 1, infile)
  ...
  if (gfiletype == '1') {
    ...
    printf("graspplugin) GRASP file is in format %c, but only '1' or '2' is "
           "supported\n",
        gfiletype);
    return MOLFILE_ERROR;
  }
  ...
  /// Gets properties:  potentials, curvature, distances, gproperty, g2property
  /// or vertexcolor
  ...
  for (int tri_count = 0; tri_count < ntriangles; tri_count++) {
    ...
    if (vert1 < 0 || vert2 < 0 || vert3 < 0 || vert1 >= nvert ||
        vert2 >= nvert || vert3 >= nvert) {
