    ...
    void* v, int* nelem, const molfile_graphics_t** data)
{
  ...
  nextNoncommentLine(buff, BUFFLEN, infile);
  if (sscanf(buff, " %d %d %d", &nVert, &nFaces, &nEdges) < 2 || nVert <= 0 ||
      nFaces <= 0) {
  ...
  for (i = 0; i < nVert; i++) {
    nextNoncommentLine(buff, BUFFLEN, infile);
    int n = sscanf(buff, " %g %g %g %g %g %g", &vertices[3 * i],
        &vertices[3 * i + 1], &vertices[3 * i + 2], &vertColors[3 * i],
        &vertColors[3 * i + 1], &vertColors[3 * i + 2]);
    if (n != 3 && n != 6) {
  ...
  graphics =
      (molfile_graphics_t*) calloc(6 * nFaces, sizeof(molfile_graphics_t));
  n = 0;
  for (i = 0; i < nFaces; i++) {
    int idx[4];
    float c[3];
    ...
    if (k > 4) {
      ...
      fprintf(stderr, "offplugin) error: TODO -- handling polygons with more "
                      "than 4 vertices.\n");
      goto error;
    }

    if (k == 3) {
      j = sscanf(buff, "%d %d %d %d %g %g %g", &k, &idx[0], &idx[1], &idx[2],
          &c[0], &c[1], &c[2]);
      bool hasColor =
          ((j == 7) || (vertHasColor[idx[0]] && vertHasColor[idx[1]] &&
                           vertHasColor[idx[2]]));
      ...
      j = sscanf(buff, "%d %d %d %d %d %g %g %g", &k, &idx[0], &idx[1], &idx[2],
          &idx[3], &c[0], &c[1], &c[2]);
      bool hasColor =
          ((j == 8) || (vertHasColor[idx[0]] && vertHasColor[idx[1]] &&
                           vertHasColor[idx[2]] && vertHasColor[idx[3]]));
  ...
  *nelem = n;
  *data =
      (molfile_graphics_t*) realloc(graphics, n * sizeof(molfile_graphics_t));
