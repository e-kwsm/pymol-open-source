...
#define LINESIZE 180
  ...
  while (msmsgets(inbuf, LINESIZE, msms->vfd) != NULL) {
    if (sscanf(inbuf, "%f %f %f %f %f %f %d %d %d", &tf, &tf, &tf, &tf, &tf,
            &tf, &t, &t, &t) == 9)
      vertexcount++;
  }
  rewind(msms->vfd);
  ...
  while (msmsgets(inbuf, LINESIZE, msms->vfd) != NULL) {
    ...
    if (inbuf[0] != '#') {
      if (sscanf(inbuf, "%f %f %f %f %f %f %d %d %d", &vertex[addr],
              &vertex[addr + 1], &vertex[addr + 2], &normal[addr],
              &normal[addr + 1], &normal[addr + 2], &l0fa, &atomid, &l) == 9)
        i++;
    }
  }
  ...
  while (msmsgets(inbuf, LINESIZE, msms->ffd) != NULL) {
    ...
    if (inbuf[0] != '#') {
      ...
      if (sscanf(inbuf, "%d %d %d %d %d", &v0, &v1, &v2, &surftype, &ana) ==
          5) {
