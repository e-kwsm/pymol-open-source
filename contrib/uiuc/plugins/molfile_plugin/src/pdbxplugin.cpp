...
static int parseStructureFaster(
    molfile_atom_t* atoms, int* optflags, pdbxParser* parser);
...
static const char atomSiteHeader[] = "loop_\n"
                                     "_atom_site.group_PDB\n"
                                     "_atom_site.id\n"
                                     "_atom_site.type_symbol\n"
                                     "_atom_site.label_atom_id\n"
...
static void addAtoms(
    const molfile_atom_t* atoms, int optflags, pdbxWriter* writer);
...
static int parseNumberAtoms(pdbxParser* parser)
{
  ...
  while (!(NULL == strstr(buffer, "_atom_site."))) {
    sscanf(
        buffer + 11, "%s", wordbuffer); // table is used in parseStructure too
...
static inline int getUniqueResID(char* chainstr, int resid)
{
  ...
  // Each char can be respresented by <= 6 bits since only a-z, A-Z, and 0-9 are
  // valid values (62 possible values)
  ...
  uid <<= 12;
  uid += (0xFFF & resid); // add 12 least significant bits of resid to fill the
                          // remaining 10 bits of uid
    ...
    molfile_atom_t* atoms, int* optflags, pdbxParser* parser)
{
  ...
  int hashCount = 1;
  int head;
  int chainAuthIdx = MAX_COLUMNS - 1, typeIdx = MAX_COLUMNS - 1,
      resnameIdx = MAX_COLUMNS - 1;
  ...
  do {
    ...
    if (doBonds && residAuthbuffer[0] != '.' && residAuthbuffer[0] != '?') {
      ...
      if (-1 !=
          (head = inthash_insert(&parser->bondHash, hashTemp, hashCount))) {
        /* key already exists, so we have to "add" a node to the linked list for
         * this residue */
        /* Since we can't change the pointer in the hash table, we insert the
         * node at the second position in the list
         */
...
// TODO: figure out what this conditional should be
  ...
  if (parseFlags & FLAG_CHAIN_LENGTH) {
    printf(
        "pdbxplugin) WARNING: This plugin ABI does not support chain names "
        "longer than two characters. Some chain names have been truncated.\n");
  }
#endif
...
static bool readAngleBonds(molfile_atom_t* atoms, pdbxParser* parser)
{
  ...
  while (NULL != strstr(buffer, "_pdbx_validate_rmsd_angle.")) {
    sscanf(
        buffer + 26, "%s", wordbuffer); // table is used in parseStructure too
    /* assign integer values to each column */
  ...
  while (buffer[0] != '#') {
    ...
    if (((uid1 = inthash_lookup(&parser->bondHash, uid1)) != -1) &&
      ...
      if (k == 2) {
        parser->bondsFrom[parser->nbonds + bnum] =
            aIdx1 + 1; // vmd doesn't use 0 based index for bond info?
...
static bool readRMSDBonds(molfile_atom_t* atoms, pdbxParser* parser)
{
  ...
  while (isValidateRMSDBond(buffer)) {
    sscanf(
        buffer + 25, "%s", wordbuffer); // table is used in parseStructure too
    /* assign integer values to each column */
    if (0 == strcmp(wordbuffer, "auth_atom_id_1")) {
      columns[bondTableSize] = (char*) name1;
    } else if (0 == strcmp(wordbuffer, "auth_asym_id_1")) {
  ...
  while (buffer[0] != '#') {
    ...
    if (((uid1 = inthash_lookup(&parser->bondHash, uid1)) != -1) &&
        ((uid2 = inthash_lookup(&parser->bondHash, uid2)) != -1)) {
      /* because the hashtable is residue specifc, loop through all atoms in the
       * residue to find the correct one */
      /* Find atom 1 */
      do {
        aIdx1 = parser->hashMem[uid1].index;
        if (strcmp(name1, parser->type_auth + aIdx1 * TYPE_SIZE) == 0 &&
...
static inline bool isValidateRMSDBond(char* str)
{
  ...
  return (str[0] == '_' && str[1] == 'p' && str[2] == 'd' && str[3] == 'b' &&
          ...
          str[12] == 't' && str[13] == 'e' && str[14] == '_' &&
          str[15] == 'r' && str[16] == 'm' && str[17] == 's' &&
          str[18] == 'd' && str[19] == '_' && str[20] == 'b' &&
          str[21] == 'o' && str[22] == 'n' && str[23] == 'd' && str[24] == '.');
}

static inline bool isAtomSite(char* str)
{
  return (str[0] == '_' && str[1] == 'a' && str[2] == 't' && str[3] == 'o' &&
          str[4] == 'm' && str[5] == '_' && str[6] == 's' && str[7] == 'i' &&
          str[8] == 't' && str[9] == 'e' && str[10] == '.');
}
...
static void addAtoms(
    const molfile_atom_t* atomlist, int optflags, pdbxWriter* writer)
{
  int i;
...
static void writeAtomSite(pdbxWriter* writer)
{
  ...
  for (i = 0; i < writer->numatoms; ++i) {
    sprintf(lineBuffer, "ATOM %d %s %s . %s %s . %d ? %f %f %f %f %f %s\n",
        ...
        atoms[i].resid, *x, *y, *z, atoms[i].occupancy, atoms[i].charge,
        atoms[i].chain);
    x += 3;
    y += 3;
    z += 3;
...
static void write(const char* str, pdbxWriter* writer)
{
  ...
  if (length + writer->bufferCount < WB_SIZE) {
    ...
    do {
      ...
      memcpy(
          writer->writeBuf + writer->bufferCount, str + num_copied, copy_size);
...
static int read_pdbx_structure(
...
static int read_bonds(void* v, int* nbonds, int** fromptr, int** toptr,
    float** bondorder, int** bondtype, int* nbondtypes, char*** bondtypename)
{
...
static void* open_file_write(
    const char* path, const char* filetypye, int natoms)
{
  FILE* fd;
...
int main(int argc, char* argv[])
{
  ...
  if (*argv != NULL)
    ...
    //   v = (pdbx_data*)open_pdbx_read("/Users/Brendan/pdbx/3j3q.cif", "pdbx",
    //   &natoms);
    v = (pdbx_data*) open_pdbx_read(
        "/home/brendanbc1/Downloads/3j3q.cif", "pdbx", &natoms);
  if (!v) {
...
f = fopen("pdbx.txt", "w");
for(i=0; i<natoms; i++) {
fprintf(f, "%i %d %s %s %s %f %f %f\n", atoms[i].atomicnumber, atoms[i].resid,
atoms[i].chain, atoms[i].resname, atoms[i].type, *x,*y,*z);
//fprintf(stderr, "%i\t%s  %s\t%s  %s  %i  %f\t%f\t%f\t%f\t%f\t%f\n", i+1,
atoms[i].name, atoms[i].type,
 //      atoms[i].chain, atoms[i].resname, atoms[i].resid, *x, *y, *z,
atoms[i].occupancy, atoms[i].bfactor, atoms[i].charge); x+=3; y+=3; z+=3;
}
fclose(f);
