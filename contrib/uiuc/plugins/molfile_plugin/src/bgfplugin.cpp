...
// functions.
static void* open_bgf_read(const char* path, const char* filetype, int* natoms)
{
  FILE* fd;
  bgfdata* bgf;
  char line[LINESIZE];
...
static void get_bgf_coordinates(
    const char* record, float* x, float* y, float* z)
{
...
static void get_bgf_fields(const char* record, char* name, char* resname,
    char* chain, char* segname, int* resid, char* type, float* charge, float* x,
    float* y, float* z)
{
  char tempresid[6];
  char tempcharge[9];
  ...
  *optflags = bgf->optflags;
  ...
  do {
    ...
    get_bgf_fields(line, atom->name, atom->resname, atom->chain, atom->segid,
        &atom->resid, atom->type, &atom->charge, NULL, NULL, NULL);
  } while (strncmp(line, "END", 3));
    ...
    const char* filename, const char* filetype, int natoms)
{
  ...
  if ((fd = fopen(filename, "w")) == NULL) {
    printf(
        "bgfplugin) Error: unable to open bgf file %s for writing\n", filename);
    return NULL;
  }
...
static int write_bgf_structure(
    void* mydata, int optflags, const molfile_atom_t* atoms)
{
  bgfdata* data = (bgfdata*) mydata;
  data->atomlist =
      (molfile_atom_t*) malloc(data->natoms * sizeof(molfile_atom_t));
...
{
  if ((strncmp(resname, "ALA", 3) == 0) || (strncmp(resname, "ASP", 3) == 0) ||
      (strncmp(resname, "ARG", 3) == 0) || (strncmp(resname, "ASN", 3) == 0) ||
      (strncmp(resname, "CYS", 3) == 0) || (strncmp(resname, "GLN", 3) == 0) ||
      (strncmp(resname, "GLU", 3) == 0) || (strncmp(resname, "GLY", 3) == 0) ||
      (strncmp(resname, "HIS", 3) == 0) || (strncmp(resname, "ILE", 3) == 0) ||
      (strncmp(resname, "LEU", 3) == 0) || (strncmp(resname, "LYS", 3) == 0) ||
      (strncmp(resname, "MET", 3) == 0) || (strncmp(resname, "PHE", 3) == 0) ||
      (strncmp(resname, "PRO", 3) == 0) || (strncmp(resname, "SER", 3) == 0) ||
      (strncmp(resname, "THR", 3) == 0) || (strncmp(resname, "TRP", 3) == 0) ||
      (strncmp(resname, "TYR", 3) == 0) || (strncmp(resname, "VAL", 3) == 0) ||
      (strncmp(resname, "ADE", 3) == 0) || (strncmp(resname, "THY", 3) == 0) ||
      (strncmp(resname, "GUA", 3) == 0) || (strncmp(resname, "CYT", 3) == 0) ||
      (strncmp(resname, "URA", 3) == 0) || (strncmp(resname, "HSD", 3) == 0) ||
      (strncmp(resname, "HSE", 3) == 0) || (strncmp(resname, "HSP", 3) == 0)) {
...
static int read_bgf_bonds_aux(
    void* v, int* nbonds, int** fromptr, int** toptr, float** bondorderptr)
{
  bgfdata* bgf = (bgfdata*) v;
  char line[LINESIZE];
    ...
    float** bondorderptr, int** bondtype, int* nbondtypes, char*** bondtypename)
{
  ...
  *nbonds = bgf->nbonds;
  if (bgf->nbonds > 0) {
    ...
    if ((read_bgf_bonds_aux(bgf, nbonds, &(bgf->from), &(bgf->to),
            &(bgf->bondorder))) != MOLFILE_SUCCESS) {
      fclose(bgf->file);
      bgf->file = NULL;
      return MOLFILE_ERROR;
    }
...
static int write_bgf_timestep(void* mydata, const molfile_timestep_t* ts)
{
  ...
  fprintf(data->file, "FORMAT ATOM   "
                      "(a6,1x,i5,1x,a5,1x,a3,1x,a1,1x,a5,3f10.5,1x,a5,i3,i2,1x,"
                      "f8.5,i2,i4,f10.5)\n");
  ...
  for (i = 0; i < data->natoms; i++) {
    ...
    fprintf(data->file,
        "%-6s %5i %5s %3.3s %1s %5i%10.5f%10.5f%10.5f %-5s%3i%2i %8.5f%2i%4i\n",
        atomfield, i + 1, atom->name, atom->resname, atom->chain, atom->resid,
        pos[0], pos[1], pos[2], atom->type, numbonds, lp, atom->charge, 0, 0);
    ++atom;
    pos += 3;
  }
  ...
  fprintf(
      data->file, "FORMAT CONECT (a6,14i6) \nFORMAT ORDER (a6,i6,13f6.3)\n");
