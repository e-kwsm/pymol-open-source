    ...
    const char* filepath, const char* filetype, int* natoms)
{
  ...
  if (fgets(mapString, 4, fd) == NULL) {
    printf("ccp4plugin) Error: unable to read 'MAP' string, not a valid "
           "CCP4/IMOD MRC file.\n");
    return NULL;
  }
  if ((strcmp(mapString, "MAP") != 0) && (imodstamp != IMOD_MAGIC_STAMP)) {
    ...
    printf(
        "ccp4plugin) Warning: 'MAP' string missing which usually indicates that this is\n\
...
string so file loading will continue but may fail.\n");
  ...
  default:
    printf("ccp4plugin) Error: Only byte, short (16-bit integer) or float "
           "(32-bit real) data types are supported.\n");
    return NULL;
  }

#if 1
  printf(
      "ccp4plugin)    extent: %d x %d x %d\n", extent[0], extent[1], extent[2]);
  printf("ccp4plugin) nxyzstart: %d x %d x %d\n", nxyzstart[0], nxyzstart[1],
      nxyzstart[2]);
  printf("ccp4plugin)  origin2k: %f x %f x %f\n", origin2k[0], origin2k[1],
      origin2k[2]);
  printf("ccp4plugin)      grid: %d x %d x %d\n", grid[0], grid[1], grid[2]);
  printf("ccp4plugin)   celldim: %f x %f x %f\n", cellDimensions[0],
      cellDimensions[1], cellDimensions[2]);
  printf("cpp4plugin)cellangles: %f, %f, %f\n", cellAngles[0], cellAngles[1],
      cellAngles[2]);
  printf(
      "ccp4plugin)   crs2xyz: %d %d %d\n", crs2xyz[0], crs2xyz[1], crs2xyz[2]);
  ...
  if (voxtype == MRC_TYPE_BYTE) {
    ...
    dataOffset =
        filesize - sizeof(uchar3) * (extent[0] * extent[1] * extent[2]);
  } else {
  ...
  if (dataOffset != (CCP4HDSIZE + symBytes)) {
    if (dataOffset == CCP4HDSIZE) {
      ...
      printf("ccp4plugin) Error: File appears truncated and doesn't match "
             "header.\n");
      return NULL;
      ...
      printf("ccp4plugin) Warning: File is larger than expected and doesn't "
             "match header.\n");
      printf("ccp4plugin) Warning: Continuing file load, good luck!\n");
    } else {
      printf("ccp4plugin) Error: File is MUCH larger than expected and doesn't "
             "match header.\n");
      return NULL;
    }
  }
  ...
  *natoms = MOLFILE_NUMATOMS_NONE;
  ...
  if (cellDimensions[0] == 0.0 && cellDimensions[1] == 0.0 &&
    ...
    printf("ccp4plugin) Warning: Map file will not align with other "
           "structures.\n");
  ...
  if (origin2k[0] == 0.0f && origin2k[1] == 0.0f && origin2k[2] == 0.0f) {
    ...
    ccp4->vol[0].origin[1] =
        yaxis[1] * nxyzstart[yIndex] + zaxis[1] * nxyzstart[zIndex];
...
static int read_ccp4_data(void* v, int set, float* datablock, float* colorblock)
{
  ccp4_t* ccp4 = (ccp4_t*) v;
  ...
  if ((ccp4->voxtype == MRC_TYPE_BYTE) &&
      (ccp4->imodflags & IMOD_FLAG_SIGNED)) {
    ...
    delete[] rowdata;
  } else if ((ccp4->voxtype == MRC_TYPE_BYTE) &&
             !(ccp4->imodflags & IMOD_FLAG_SIGNED)) {
    ...
    /* IMOD developers said that this is not used anymore and not worth our time
     * to implement */
    ...
    for (coord[2] = 0; coord[2] < extent[2]; coord[2]++) {
      for (coord[1] = 0; coord[1] < extent[1]; coord[1]++) {
        ...
        if (fread(rowdata, sizeof(unsigned short), extent[0], fd) !=
            extent[0]) {
          printf("ccp4plugin) Error reading data row.\n");
          delete[] rowdata;
          return MOLFILE_ERROR;
    ...
    printf(
        "ccp4plugin) reading unsigned char * 3 (8-bit uchar * 3) voxel data\n");
    ...
    for (coord[2] = 0; coord[2] < extent[2]; coord[2]++) {
      for (coord[1] = 0; coord[1] < extent[1]; coord[1]++) {
        ...
        for (coord[0] = 0; coord[0] < extent[0]; coord[0]++) {
          ...
          y = coord[ccp4->xyz2crs[1]];
          z = coord[ccp4->xyz2crs[2]];
          grayscale = rowdata[coord[0]].red + rowdata[coord[0]].blue +
                      rowdata[coord[0]].green;
