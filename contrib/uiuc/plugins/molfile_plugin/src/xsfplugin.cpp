...
static const char* xsf_symtab[] = {"(unknown keyword)", "#", "BEGIN_INFO",
    "END_INFO", "BEGIN_BLOCK_DATAGRID_2D", "END_BLOCK_DATAGRID_2D",
    "BEGIN_DATAGRID_2D", "END_DATAGRID_2D", "BEGIN_BLOCK_DATAGRID_3D",
    "END_BLOCK_DATAGRID_3D", "BEGIN_DATAGRID_3D", "END_DATAGRID_3D",
    "BEGIN_BLOCK_BANDGRID_3D", "END_BLOCK_BANDGRID_3D", "ATOMS", "ANIMSTEPS",
    "BAND", "MOLECULE", "POLYMER", "SLAB", "CRYSTAL", "PRIMVEC", "CONVVEC",
    "PRIMCOORD", "CONVCOORD"};
...
static const struct {
  const char* name;
  xsf_keyword_t kw;
} xsf_aliases[] = {{"DATAGRID_2D", xsf_BEG_2D_DATA},
    ...
    {"END_BLOCK_DATAGRID3D", xsf_END_3D_BLOCK}, {NULL, xsf_UNKNOWN}};
...
static int xsf_readbox(xsf_box* box, float* x, float* y, float* z)
{
  ...
  box->gamma =
      acos((x[0] * y[0] + x[1] * y[1] + x[2] * y[2]) / (A * B)) * 90.0 / M_PI_2;
  box->beta =
      acos((x[0] * z[0] + x[1] * z[1] + x[2] * z[2]) / (A * C)) * 90.0 / M_PI_2;
  box->alpha =
      acos((y[0] * z[0] + y[1] * z[1] + y[2] * z[2]) / (B * C)) * 90.0 / M_PI_2;
...
static void xsf_buildrotmat(xsf_t* xsf, float* a, float* b)
{
  ...
  const double psi = atan2(-sph * cth * b[0] - sph * sth * b[1] + cph * b[2],
      -sth * b[0] + cth * b[1]);
  const double cps = cos(psi);
  const double sps = sin(psi);

  const double r[3][3] = {{cph * cth, cph * sth, sph},
...
{
  return (9 == fscanf(fd, "%f%f%f%f%f%f%f%f%f", &a[0], &a[1], &a[2], &b[0],
                   &b[1], &b[2], &c[0], &c[1], &c[2]));
}
    ...
    const char* filepath, const char* filetype, int* natoms)
{
  ...
  do {
    if (NULL == fgets(readbuf, 256, xsf->fd))
      break;
  ...
  again:
    ...
    case xsf_PRIMVEC: // store primitive cell info for rotation of the
                      // volumetric data grid vectors
    {
      float a[3], b[3], c[3];
      ...
      if (xsf_read_cell(xsf->fd, a, b, c)) {
        ...
        fprintf(stderr, "xsfplugin) WARNING: error reading unit cell. ignoring "
                        "unit cell info.\n");
      }
    } break;
    ...
    case xsf_BEG_3D_BLOCK: // analyse number of 3d-data sets
      // ordinarily the parsing of the metadata would be done in
      // read_xsf_metadata() but since we have to move through the whole file to
      // count its contents, it is faster to parse it here and just pass the
      // data later.
      ...
      do { // loop until we reach the end of the whole block
        ...
        case xsf_BEG_3D_DATA: // fallthrough
        {
          ...
          if (xsf->nvolsets > xsf->numvolmeta) {
            ...
            memcpy((void*) xsf->vol, (void*) ptr,
                xsf->numvolmeta * sizeof(molfile_volumetric_t));
            xsf->numvolmeta *= 2;
            delete[] ptr;
          }
          ...
          fgets(readbuf, 256, xsf->fd);
          sscanf(
              readbuf, "%d%d%d", &(set->xsize), &(set->ysize), &(set->zsize));
          fgets(readbuf, 256, xsf->fd);
          ...
          for (n = 0; n < 3; ++n) {
            set->origin[n] = xsf->rotmat[n][0] * orig[0] +
                             xsf->rotmat[n][1] * orig[1] +
                             xsf->rotmat[n][2] * orig[2];
...
static int read_xsf_structure(void* v, int* optflags, molfile_atom_t* atoms)
{
  ...
  do {
      ...
      *optflags = MOLFILE_ATOMICNUMBER | MOLFILE_MASS | MOLFILE_RADIUS;
      ...
      for (i = 0; i < xsf->numatoms; ++i) {
        ...
        if (k == NULL) {
          ...
          fprintf(stderr,
              "xsfplugin) missing type or coordinate(s) in file '%s' for atom "
              "'%d'\n",
              xsf->file_name, i + 1);
          return MOLFILE_ERROR;
        }
    ...
    case xsf_PRIMVEC: {
      ...
      if (xsf_read_cell(xsf->fd, a, b,
              c)) { // ignore unit cell info,if we cannot parse it.
        ...
        if ((fabs((double) a[1]) + fabs((double) a[2]) + fabs((double) b[2])) >
            0.001) {
          fprintf(stderr,
              "xsfplugin) WARNING: Coordinates will be rotated to comply \n"
      ...
      for (i = 0; i < natoms; ++i) {
        ...
        if (k == NULL) {
          ...
          if (ts != NULL) {
            ...
            switch (xsf->pbctype) {
            case xsf_CRYSTAL:
              xf = xsf->invmat[0][0] * x + xsf->invmat[0][1] * y +
                   xsf->invmat[0][2] * z;
              xf = xf - floor((double) xf);
              yf = xsf->invmat[1][0] * x + xsf->invmat[1][1] * y +
                   xsf->invmat[1][2] * z;
              yf = yf - floor((double) yf);
              zf = xsf->invmat[2][0] * x + xsf->invmat[2][1] * y +
                   xsf->invmat[2][2] * z;
              zf = zf - floor((double) zf);
              x = xsf->box.cell[0][0] * xf + xsf->box.cell[0][1] * yf +
                  xsf->box.cell[0][2] * zf;
              y = xsf->box.cell[1][0] * xf + xsf->box.cell[1][1] * yf +
                  xsf->box.cell[1][2] * zf;
              z = xsf->box.cell[2][0] * xf + xsf->box.cell[2][1] * yf +
                  xsf->box.cell[2][2] * zf;
              break;
...
static int read_xsf_metadata(
    void* v, int* nvolsets, molfile_volumetric_t** metadata)
{
  xsf_t* xsf = (xsf_t*) v;
...
static int read_xsf_data(void* v, int set, float* datablock, float* colorblock)
{
  ...
  fprintf(
      stderr, "xsfplugin) trying to read xsf data set %d: %s\n", set, block);
  ...
  rewind(xsf->fd);
  do {
    if (NULL == fgets(readbuf, 1024, xsf->fd))
      return MOLFILE_ERROR;
    ...
    for (y = 0; y < ysize + 1; y++) {
      for (x = 0; x < xsize + 1; x++) {
        if ((x >= xsize) || (y >= ysize) || (z >= zsize)) {
          if (fscanf(xsf->fd, "%f", &dummy) != 1)
            return MOLFILE_ERROR;
        } else {
          if (fscanf(xsf->fd, "%f", &datablock[n]) != 1)
            return MOLFILE_ERROR;
          ++n;
        }
      }
