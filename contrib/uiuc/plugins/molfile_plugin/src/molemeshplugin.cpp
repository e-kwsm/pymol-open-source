...
static int read_rawgraphics(
    void* v, int* nelem, const molfile_graphics_t** data)
{
  molfile_graphics_list *gListPtr = NULL, *tmpPtr = NULL;
  int i = 0, ntriangles = 0, j = 0;
  int error = 0, numVerts = 0, numFacets = 0, facetType = 0, facet = 0,
      tmpFacet[5];
  ...
  if (numVerts < 1) {
    fprintf(stderr,
        "molespmeshplugin) error: expected \"Positive Number of Vertices\".\n");
    error = 1;
  ...
  // Read in the total number of facets.  For pmesh files the next number is the
  // total number of triangle or quadrilaterals If triangles, then the next
  // number after the total is a 4, and the next four numbers after that
  // describe the vertex index for each point of the triangle.  If the set
  // begins with a 5 then the next 5 numbers describe the vertex number for each
  // point of the quadrilateral.  So far this reader does not support
  // multiple/concatenated mpesh files yet.
  fgets(line, 80, infile);
  sscanf(line, "%d", &numFacets);
  // printf("numFacets %d \n",numFacets);
  if (numFacets < 1) {
    fprintf(stderr,
        "molespmeshplugin) error: expected \"Positive Number of Facets\".\n");
    error = 1;
    return MOLFILE_ERROR;
  } else {
    gListPtr = new molfile_graphics_list;
  ...
  // Read in the facet type 4=triangle 5=quadrilateral. need to do a while loop
  // here, as long as xxxx<numFacets keep reading
  while (!feof(infile) && (error == 0)) {
    ...
    if (facetType == 4) {
      ...
      // Create a new list item and initialize it for first triangle of the
      ...
      // Find out if this is a concatenated file by reading the next value and
      // testing for a series of three floats. If so, free tmpData and
      // reallocate according to this number. then read all the vertices into
      // tmpData and do a drop back into the while loop. If not, exit with
      // error.
      fgets(line, 80, infile);
      float t1 = 0.0f, t2 = 0.0f, t3 = 0.0f;
      if (sscanf(line, "%f %f %f", &t1, &t2, &t3) == 3) {
        ...
        for (j = 1; j < numVerts; j++) {
          ...
          if (sscanf(line, "%f %f %f", &t1, &t2, &t3) == 3) {
            ...
            fprintf(stderr, "molespmeshplugin) error: problem reading vertices "
        ...
        fprintf(stderr,
            "molespmeshplugin) error: problem reading concatenated file?\n");
        error = 1;
        break;
      }
      fgets(line, 80, infile);
      sscanf(line, "%d", &numFacets);
      if (numFacets < 1) {
        fprintf(stderr, "molespmeshplugin) error: expected \"Positive Number "
                        "of Facets\".\n");
        error = 1;
      }
    }
    // go back into the while loop with error=0 so that we can reuse the facet
    // code for additional meshes
    error = 0;
  }
