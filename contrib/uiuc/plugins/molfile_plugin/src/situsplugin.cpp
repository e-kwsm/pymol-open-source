...
static int read_situs_metadata(
...
float situs_voxel_value_safe(int x, int y, int z, const int xsize,
    const int ysize, const int zsize, const float* data)
{
  int xx, yy, zz;
...
float situs_voxel_value_interpolate(float xv, float yv, float zv,
    const int xsize, const int ysize, const int zsize, const float* data)
{
  int x = (int) xv;
  int y = (int) yv;
  int z = (int) zv;
  ...
  xlerps[0] = tmp + xf * (situs_voxel_value_safe(
                              x + 1, y, z, xsize, ysize, zsize, data) -
                             tmp);

  tmp = situs_voxel_value_safe(x, y + 1, z, xsize, ysize, zsize, data);
  xlerps[1] = tmp + xf * (situs_voxel_value_safe(
                              x + 1, y + 1, z, xsize, ysize, zsize, data) -
                             tmp);

  tmp = situs_voxel_value_safe(x, y, z + 1, xsize, ysize, zsize, data);
  xlerps[2] = tmp + xf * (situs_voxel_value_safe(
                              x + 1, y, z + 1, xsize, ysize, zsize, data) -
                             tmp);

  tmp = situs_voxel_value_safe(x, y + 1, z + 1, xsize, ysize, zsize, data);
  xlerps[3] = tmp + xf * (situs_voxel_value_safe(
                              x + 1, y + 1, z + 1, xsize, ysize, zsize, data) -
                             tmp);
...
float situs_voxel_value_interpolate_from_coord(float xpos, float ypos,
    float zpos, const float* origin, const float* xdelta, const float* ydelta,
    const float* zdelta, const int xsize, const int ysize, const int zsize,
    float* data)
{
  ...
  return situs_voxel_value_interpolate(
      xpos, ypos, zpos, xsize, ysize, zsize, data);
}

static int write_situs_data(void* v, molfile_volumetric_t* metadata,
    float* datablock, float* colorblock)
{
  ...
  if (fabs(xaxis[1]) > TOLERANCE || fabs(xaxis[2]) > TOLERANCE ||
    ...
    fprintf(stderr, "situsplugin) Could not write situs file: this format "
  ...
  float xres =
      xdelta[0] * xdelta[0] + xdelta[1] * xdelta[1] + xdelta[2] * xdelta[2];
  float yres =
      ydelta[0] * ydelta[0] + ydelta[1] * ydelta[1] + ydelta[2] * ydelta[2];
  float zres =
      zdelta[0] * zdelta[0] + zdelta[1] * zdelta[1] + zdelta[2] * zdelta[2];
  if ((fabs(xres - yres) > TOLERANCE) || (fabs(xres - zres) > TOLERANCE)) {

    // fprintf(stderr, "situsplugin) Could not write situs file: this format
    // requires the same grid spacing in all dimensions.\n"); return
    // MOLFILE_ERROR;

    fprintf(stderr,
        "situsplugin) Warning: This format requires the same grid spacing in "
        "all dimensions. The map will be re-sampled to meet this requirement. "
        "The resulting cell may be slightly smaller than the original one.\n");
    ...
    for (i = 0; i < new_xsize; i++) {
      ...
      for (j = 0; j < new_ysize; j++) {
        ...
        for (k = 0; k < new_zsize; k++) {
          ...
          new_data[i + j * new_xsize + k * new_xysize] =
              situs_voxel_value_interpolate_from_coord(xpos, ypos, zpos, origin,
                  xdelta, ydelta, zdelta, xsize, ysize, zsize, datablock);
        }
      }
    }

    /* Write a situs header */
    fprintf(fd, "%g %g %g %g %d %d %d\n\n", new_delta, origin[0], origin[1],
        origin[2], new_xsize, new_ysize, new_zsize);
    ...
    /* Write a situs header */
    fprintf(fd, "%g %g %g %g %d %d %d\n\n", xdelta[0], origin[0], origin[1],
        origin[2], xsize, ysize, zsize);
