...
// functions.
static void* open_xbgf_read(const char* path, const char* filetype, int* natoms)
{
  FILE* fd;
  xbgfdata* bgf;
  char line[LINESIZE];
  ...
  do {
    ...
    optflags = MOLFILE_INSERTION | MOLFILE_CHARGE | MOLFILE_BFACTOR |
               MOLFILE_OCCUPANCY | MOLFILE_ATOMICNUMBER;
...
static void get_xbgf_coordinates(
...
static void get_xbgf_fields(const char* record, char* name, char* resname,
    char* chain, char* segname, float* occupancy, float* bfactor,
    int* elementindex, int* resid, char* type, float* charge, float* x,
    float* y, float* z, char* insert)
{
  char tempresid[6];
  char tempcharge[8];
  char tempbeta[7];
  ...
  *bfactor = atof(tempbeta);
  ...
  tempocc[6] = '\0';
  adjust_xbgf_field_string(
      tempocc); /* remove spaces from the occupancy field */
  ...
  *optflags = bgf->optflags;
  ...
  do {
    ...
    get_xbgf_fields(line, atom->name, atom->resname, atom->chain, atom->segid,
        &atom->occupancy, &atom->bfactor, &atom->atomicnumber, &atom->resid,
        atom->type, &atom->charge, NULL, NULL, NULL, atom->insertion);
  } while (strncmp(line, "END", 3));
...
static int write_xbgf_structure(
...
static int read_xbgf_bonds_aux(
    ...
    float** bondorderptr, int** bondtype, int* nbondtypes, char*** bondtypename)
{
  ...
  *nbonds = bgf->nbonds;
  if (bgf->nbonds > 0) {
    ...
    if ((read_xbgf_bonds_aux(bgf, nbonds, &(bgf->from), &(bgf->to),
...
static int write_xbgf_timestep(void* mydata, const molfile_timestep_t* ts)
{
  ...
  fprintf(data->file, "FORMAT ATOM   "
                      "(a6,1x,i6,1x,a5,1x,a4,1x,a1,1x,i5,3f10.5,1x,a5,i3,i2,1x,"
                      "f8.5,1x,f6.3,1x,f6.3,1x,i3,1x,a4)\n");
  ...
  for (i = 0; i < data->natoms; i++) {
    fprintf(data->file,
        "%-6s %6i %5s %4s %1s %5i%10.5f%10.5f%10.5f %-5s%3i%2i %8.5f %6.3f "
        "%6.3f %3i %4s\n",
        "ATOM", i + 1, atom->name, atom->resname, atom->chain, atom->resid,
        pos[0], pos[1], pos[2], atom->type, numbonds, lp, atom->charge,
        atom->bfactor, atom->occupancy, atom->atomicnumber, atom->segid);
    ++atom;
    pos += 3;
  }
  ...
  float* orders =
      (float*) malloc((data->natoms + 1) * sizeof(float) * MAXBONDS);
  ...
  for (i = 0; i < data->nbonds; i++) {
    ...
    if (numcons[j] > MAXBONDS) {
      printf(
          "xbgfplugin) Warning: Bond overflow. Not all bonds were written\n");
      numcons[j]--;
      numcons[k]--;
      continue;
    }
