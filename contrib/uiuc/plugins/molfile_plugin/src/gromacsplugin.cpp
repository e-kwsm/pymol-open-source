...
static void convert_vmd_box_for_writing(
    const molfile_timestep_t* ts, float* x, float* y, float* z)
{
  ...
  z[2] =
      (ts->C / ANGS_PER_NM) *
      sqrt((double) (1.0 + 2.0 * ca * cb * cg - ca * ca - cb * cb - cg * cg) /
           (1.0 - cg * cg));
}

static void* open_gro_read(const char* filename, const char*, int* natoms)
{
  ...
  if (gro_header(mf, mdh.title, MAX_MDIO_TITLE, &mdh.timeval, &mdh.natoms, 0) <
      0) {
...
static int write_gro_structure(
    void* v, int optflags, const molfile_atom_t* atoms)
{
...
static int write_gro_timestep(void* v, const molfile_timestep_t* ts)
{
  ...
  for (i = 0; i < gmx->natoms; i++) {
    fprintf(gmx->mf->f, "%5d%-5s%5s%5d%8.3f%8.3f%8.3f", atom->resid,
        atom->resname, atom->name, i + 1, pos[0] / ANGS_PER_NM,
        pos[1] / ANGS_PER_NM, pos[2] / ANGS_PER_NM);
    if (vel) {
      fprintf(gmx->mf->f, "%8.4f%8.4f%8.4f", vel[0] / ANGS_PER_NM,
          vel[1] / ANGS_PER_NM, vel[2] / ANGS_PER_NM);
      vel += 3;
    }
    fprintf(gmx->mf->f, "\n");
    ++atom;
...
static void* open_g96_read(const char* filename, const char*, int* natoms)
{
...
static int read_trr_timestep(void* v, int natoms, molfile_timestep_t* ts)
{
  ...
  if (mdts.natoms != natoms) {
    fprintf(stderr,
        "gromacsplugin) Timestep in file contains wrong number of atoms\n");
    fprintf(
        stderr, "gromacsplugin) Found %d, expected %d\n", mdts.natoms, natoms);
    mdio_tsfree(&mdts);
    return MOLFILE_ERROR;
  }
...
static int write_trr_timestep(void* mydata, const molfile_timestep_t* ts)
{
  ...
  if (gmx->mf->fmt == MDFMT_TRR) {
    ...
    if (put_trx_int(gmx->mf, TRX_MAGIC)            // ID
        ...
        || put_trx_real(
               gmx->mf, 0.1 * gmx->step) // current time. (dummy value: 0.1)
