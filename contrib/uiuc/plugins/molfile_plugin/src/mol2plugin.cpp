...
// functions.
static void* open_mol2_read(const char* path, const char* filetype, int* natoms)
{
  FILE* fd;
  mol2data* mol2;
  char line[LINESIZE];
...
static int read_mol2(void* v, int* optflags, molfile_atom_t* atoms)
{
  ...
  *optflags = mol2->optflags;
  ...
  for (i = 0; i < mol2->natoms; i++) {
    ...
    match = sscanf(line, " %*d %s %*f %*f %*f %s %d %s %f", atom->name,
        atom->type, &atom->resid, atom->resname, &atom->charge);
...
static int read_mol2_bonds_aux(
    void* v, int* nbonds, int** fromptr, int** toptr, float** bondorderptr)
{
  ...
  for (i = 0; i < mol2->nbonds; i++) {
    ...
    if (strncmp(bond_type, "nc", 2) == 0) {
      ...
      if (curr_order < 1.0 || curr_order > 4.0)
        curr_order = 1;
    ...
    const char* filename, const char* filetype, int natoms)
{
  ...
  if (!fd) {
    fprintf(stderr,
        "mol2plugin) Error: unable to open mol2 file %s for writing\n",
        filename);
    return NULL;
  }
...
static int write_mol2_structure(
...
#else
    fprintf(data->file,
        "%7d %-4s      %8.4f  %8.4f  %8.4f %4s %4d  %3s        %8.6f\n", i + 1,
        atom->name, pos[0], pos[1], pos[2], atom->type, atom->resid,
        atom->resname, atom->charge);
#endif
    ++atom;
    pos += 3;
  }
    ...
    float** bondorderptr, int** bondtype, int* nbondtypes, char*** bondtypename)
{
  ...
  if (mol2->nbonds > 0) {
    ...
    if (mol2->from == NULL || mol2->to == NULL || mol2->bondorder == NULL) {
      fprintf(
          stderr, "mol2plugin) ERROR: Failed to allocate memory for bonds\n");
      fclose(mol2->file);
      mol2->file = NULL;
      return MOLFILE_ERROR;
    }
    if ((read_mol2_bonds_aux(mol2, nbonds, &(mol2->from), &(mol2->to),
            &(mol2->bondorder))) != MOLFILE_SUCCESS) {
      fclose(mol2->file);
      mol2->file = NULL;
      return MOLFILE_ERROR;
    }
