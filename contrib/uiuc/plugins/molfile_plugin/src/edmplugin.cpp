    ...
    const char* filepath, const char* filetype, int* natoms)
{
  ...
  *natoms = MOLFILE_NUMATOMS_NONE;
  ...
  convcnt = fscanf(edm->fd, "%d %d %d %d %d %d %d %d %d", &na, &amin, &amax,
      &nb, &bmin, &bmax, &nc, &cmin, &cmax);
  if (convcnt != 9) {
  ...
  convcnt =
      fscanf(edm->fd, "%f %f %f %f %f %f", &a, &b, &c, &alpha, &beta, &gamma);
  if (convcnt != 6) {
...
static int read_edm_data(void* v, int set, float* datablock, float* colorblock)
{
  edm_t* edm = (edm_t*) v;
    ...
    const char* filepath, const char* filetype, int natoms)
{
  ...
  if (!fd) {
    fprintf(
        stderr, "edmplugin) Could not open path '%s' for writing.\n", filepath);
  }
  return fd;
}
...
float edm_voxel_value_safe(int x, int y, int z, const int xsize,
...
float edm_voxel_value_interpolate(float xv, float yv, float zv, const int xsize,
    const int ysize, const int zsize, const float* data)
{
  int x = (int) xv;
  int y = (int) yv;
...
float edm_voxel_value_interpolate_from_coord(float xpos, float ypos, float zpos,
    const float* origin, const float* xdelta, const float* ydelta,
    ...
    float* data)
{
  ...
  return edm_voxel_value_interpolate(
      xpos, ypos, zpos, xsize, ysize, zsize, data);
}

static int write_edm_data(void* v, molfile_volumetric_t* metadata,
    float* datablock, float* colorblock)
{
  ...
  if (fabs(xaxis[1]) > TOLERANCE || fabs(xaxis[2]) > TOLERANCE ||
    ...
    fprintf(stderr, "edmplugin) Could not write X-PLOR file: only orthogonal "
                    "cells are currently supported.\n");
  ...
  for (i = 0; i < na; i++) {
    ...
    for (j = 0; j < nb; j++) {
      ...
      for (k = 0; k < nc; k++) {
        ...
        pdata[i + j * na + k * pxysize] =
            edm_voxel_value_interpolate_from_coord(xpos, ypos, zpos, origin,
