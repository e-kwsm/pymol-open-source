...
static int read_parm7_structure(
  ...
  *optflags = MOLFILE_NOOPTIONS; /* no optional data to start with */
  ...
  while (fgets(buf, 85, file)) {
    ...
    if (!strcmp(field, "ATOM_NAME")) {
      if (!parse_parm7_atoms(buf, prm->Natom, atoms, file))
        break;
    } else if (!strcmp(field, "CHARGE")) {
      *optflags |= MOLFILE_CHARGE;
      if (!parse_parm7_charge(buf, prm->Natom, atoms, file))
        break;
    } else if (!strcmp(field, "MASS")) {
      *optflags |= MOLFILE_MASS;
      if (!parse_parm7_mass(buf, prm->Natom, atoms, file))
        break;
    } else if (!strcmp(field, "AMBER_ATOM_TYPE")) {
      if (!parse_parm7_atype(buf, prm->Natom, atoms, file))
        break;
    } else if (!strcmp(field, "RESIDUE_LABEL")) {
      resnames = new char[4 * prm->Nres];
      if (!parse_parm7_resnames(buf, prm->Nres, resnames, file))
        break;
      ...
      if (!parse_parm7_respointers(
              buf, prm->Natom, atoms, prm->Nres, resnames, file))
        break;
      // XXX: we could count the bonded parameters and assign bond types.
    } else if (!strcmp(field, "BONDS_WITHOUT_HYDROGEN")) {
      if (!parse_parm7_bonds(
              buf, prm->Nbona, p->from + p->nbonds, p->to + p->nbonds, file))
        break;
      p->nbonds += prm->Nbona;
    } else if (!strcmp(field, "BONDS_INC_HYDROGEN")) {
      if (!parse_parm7_bonds(
              buf, prm->Nbonh, p->from + p->nbonds, p->to + p->nbonds, file))
        break;
      p->nbonds += prm->Nbonh;
    }
  }
