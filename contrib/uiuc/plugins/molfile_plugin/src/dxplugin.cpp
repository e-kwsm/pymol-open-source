 ...
 * object 3 class array type double rank 0 items { xn*yn*zn } [binary] data
 follows
 * f1 f2 f3
 * f4 f5 f6 f7 f8 f9
 * .
    ...
    const char* filepath, const char* filetype, int* natoms)
{
  ...
  if (sscanf(inbuf, "object 1 class gridpositions counts %d %d %d", &xsize,
          &ysize, &zsize) != 3) {
...
static int read_binary_dx_data(dx_t* dx, int set, float* datablock)
{
  ...
  if (fread(tmp, sizeof(float), total, dx->fd) != total) {
    vmdcon_printf(
        VMDCON_ERROR, "dxplugin) Failed to read %d binary floats\n", total);
    free(tmp);
...
static int read_dx_data(void* v, int set, float* datablock, float* colorblock)
{
  dx_t* dx = (dx_t*) v;
  FILE* fd = dx->fd;
  char inbuf[LINESIZE];
  ...
  for (count = 0; count < total;) {
    ...
    if (p == NULL) {
      ...
      vmdcon_printf(VMDCON_ERROR,
          "dxplugin) line: %d. item: %d/%d. last data: %s\n", line, count,
          total, inbuf);
      return MOLFILE_ERROR;
    }
    ...
    float* datablock, float* colorblock)
{
  ...
  fprintf(fd,
      "object 3 class array type double rank 0 items %d %sdata follows\n",
  ...
  for (i = 0; i < xsize; i++) {
    for (j = 0; j < ysize; j++) {
      for (k = 0; k < zsize; k++) {
        if (useBinary) {
          fwrite(datablock + k * xysize + j * xsize + i, sizeof(float), 1, fd);
        } else {
...
VMDPLUGIN_API int VMDPLUGIN_init(void)
{
  ...
  dxplugin.author =
      "Eamon Caddigan, Justin Gullingsrud, John Stone, Leonardo Trabuco";
  dxplugin.majorv = 2;
  dxplugin.minorv = 0;
