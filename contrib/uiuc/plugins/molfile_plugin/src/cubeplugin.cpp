    ...
    const char* filepath, const char* filetype, int* natoms)
{
  ...
  if ((fgets(readbuf, 255, cube->fd) == NULL) ||
      (sscanf(readbuf, "%d%f%f%f", &cube->numatoms, &voltmpl.origin[0],
           &voltmpl.origin[1], &voltmpl.origin[2]) != 4)) {
  ...
  *natoms = cube->numatoms;
  ...
  if ((fabs((double) a[1]) + fabs((double) a[2]) + fabs((double) b[2])) >
      0.001) {
    vmdcon_printf(
        VMDCON_WARN, "cubeplugin) Coordinates will be rotated to comply \n");
  ...
  // store aligned axes.
  for (i = 0; i < 3; ++i) {
    voltmpl.xaxis[i] = cube->rotmat[i][0] * a[0] + cube->rotmat[i][1] * a[1] +
                       cube->rotmat[i][2] * a[2];

    voltmpl.yaxis[i] = cube->rotmat[i][0] * b[0] + cube->rotmat[i][1] * b[1] +
                       cube->rotmat[i][2] * b[2];

    voltmpl.zaxis[i] = cube->rotmat[i][0] * c[0] + cube->rotmat[i][1] * c[1] +
                       cube->rotmat[i][2] * c[2];
  }
...
static int read_cube_structure(void* v, int* optflags, molfile_atom_t* atoms)
{
  ...
  *optflags =
      MOLFILE_ATOMICNUMBER | MOLFILE_MASS | MOLFILE_RADIUS | MOLFILE_CHARGE;
  ...
  for (i = 0; i < cube->numatoms; i++) {
    ...
    if (k == NULL) {
      vmdcon_printf(VMDCON_ERROR,
          "cube structure) missing atom(s) in "
          "file '%s'\n",
          cube->file_name);
      vmdcon_printf(VMDCON_ERROR,
          "cube structure) expecting '%d' atoms,"
          " found only '%d'\n",
          cube->numatoms, i + 1);
      return MOLFILE_ERROR;
    } else if (j < 2) {
      vmdcon_printf(VMDCON_INFO,
          "cube structure) missing atom data in file"
          " '%s' for atom '%d'\n",
          cube->file_name, i + 1);
      return MOLFILE_ERROR;
    }
...
static int read_cube_timestep(void* v, int natoms, molfile_timestep_t* ts)
{
  ...
  for (i = 0; i < cube->numatoms; i++) {
    ...
    if (k == NULL) {
      ...
      vmdcon_printf(VMDCON_ERROR,
          "cube timestep) missing type or coordinate(s)"
      ...
      if (ts != NULL) {
        ...
        for (n = 0; n < 3; ++n) {
          ts->coords[3 * i + n] =
              bohr * (cube->origin[n] + cube->rotmat[n][0] * x +
                         cube->rotmat[n][1] * y + cube->rotmat[n][2] * z);
        }
      }
    } else {
      break;
...
static int read_cube_data(void* v, int set, float* datablock, float* colorblock)
{
  ...
  vmdcon_printf(
      VMDCON_INFO, "cubeplugin) trying to read cube data set %d\n", set);
  ...
  if (cube->nsets == 1) { // density cube file
    for (x = 0; x < xsize; x++) {
      for (y = 0; y < ysize; y++) {
        for (z = 0; z < zsize; z++) {
          if (fscanf(cube->fd, "%f", &datablock[z * xysize + y * xsize + x]) !=
              1) {
            return MOLFILE_ERROR;
          }
        }
    ...
    // Since the orbital cube file stores the data orb1(a1,b1,c1),
    // orb2(a1,b1,c1),
    // ... orbn(a1,b1,c1), orb1(a1,b1,c2), orb2(a1,a1,c2), ..., orbn(ai,bj,ck)
    // we have to cache the whole data set of have any kind of reasonable
    // performance. otherwise we would have to read (and parse!) the whole file
    // over and over again. this way we have to do it only once at the temporary
    // expense of some memory.
    if (cube->datacache == NULL) {
      int points =
          xsize * ysize * zsize * nsize; // number of grid cells * nsets
      int i;

      // let people know what is going on.
      vmdcon_printf(VMDCON_INFO,
          "cubeplugin) creating %d MByte cube orbital"
          " cache.\n",
      ...
      for (y = 0; y < ysize; y++) {
        for (z = 0; z < zsize; z++) {
          datablock[z * xysize + y * xsize + x] =
              cube->datacache[x * nyzsize + y * nzsize + z * nsize + set];
        }
      }
    }
  }
