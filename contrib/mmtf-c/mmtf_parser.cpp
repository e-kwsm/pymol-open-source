...
#define IF_NULL_ALLOCERROR_RETURN_NULL(ptr) IF_NULL_ALLOCERROR_RETURN(ptr, NULL)
...
#define MAP_ITERATE_BEGIN_RV(object, returnvalue)                              \
  if (object->type != MMTF_MSGPACK_TYPE(MAP)) {                                \
    fprintf(stderr,                                                            \
        "Error in %s: the entry encoded in the MMTF is not a map.\n",          \
        __FUNCTION__);                                                         \
    return returnvalue;                                                        \
  }                                                                            \
  {                                                                            \
    msgpack_object_kv* current_key_value = object->via.map.ptr;                \
    msgpack_object_kv* last_key_value =                                        \
        current_key_value + object->via.map.size;                              \
...
#define MAP_ITERATE_BEGIN(object) MAP_ITERATE_BEGIN_RV(object, )
...
#define generic_destroy(ptr) free(*(ptr))
...
#define CODEGEN_BODY_fetch_OBJECT_ARRAY(type_, RESULT_I_ASSIGN)                \
  {                                                                            \
    if (object->type != MMTF_MSGPACK_TYPE(ARRAY)) {                            \
      fprintf(stderr,                                                          \
          "Error in %s: the entry encoded in the MMTF is not an array.\n",     \
          __FUNCTION__);                                                       \
      return NULL;                                                             \
    }                                                                          \
...
/*
 * Generate "initialize", "new", "empty" and "destroy" functions for MMTF struct
 * types.
 */
// clang-format off
...
static inline void assign_bigendian_4(void* dst, const char* src)
{
  *((uint32_t*) dst) = ntohl(*((uint32_t*) src));
}

static inline void assign_bigendian_2(void* dst, const char* src)
{
  *((uint16_t*) dst) = ntohs(*((uint16_t*) src));
}

static void array_copy_bigendian_4(void* dst, const char* src, size_t n)
{
  size_t i;
  for (i = 0; i < n; i += 4) {
...
static void array_copy_bigendian_2(void* dst, const char* src, size_t n)
{
  size_t i;
  for (i = 0; i < n; i += 2) {
...
static float* MMTF_parser_float_from_bytes(
    const char* input, uint32_t input_length, uint32_t* output_length)
{
...
static int8_t* MMTF_parser_int8_from_bytes(
    const char* input, uint32_t input_length, uint32_t* output_length)
{
...
static int16_t* MMTF_parser_int16_from_bytes(
    const char* input, uint32_t input_length, uint32_t* output_length)
{
...
static int32_t* MMTF_parser_int32_from_bytes(
    const char* input, const uint32_t input_length, uint32_t* output_length)
{
  IF_NOT_MULTIPLE_ERROR_RETURN(input_length, 4, NULL);
...
static char** MMTF_parser_strings_from_bytes(const char* input,
    uint32_t input_length, uint32_t parameter, uint32_t* output_length)
{
...
//*** Array decoders
// Run-length decode
static int32_t* MMTF_parser_run_length_decode(
    const int32_t* input, uint32_t input_length, uint32_t* output_length)
{
  (*output_length) = 0;
  ...
  int32_t* output = MALLOC_ARRAY(int32_t,
      (*output_length)); // The output needs to be freed by the calling process
  IF_NULL_ALLOCERROR_RETURN_NULL(output);
...
// Delta decode
static int32_t* MMTF_parser_delta_decode(
    const int32_t* input, uint32_t input_length, uint32_t* output_length)
{
  (*output_length) = input_length;
  int32_t* output = MALLOC_ARRAY(int32_t,
...
static int32_t* MMTF_parser_recursive_indexing_decode_from_16(
    const int16_t* input, uint32_t input_length, uint32_t* output_length)
{
  (*output_length) = 0;
  uint32_t i;
  ...
  int32_t* output = (int32_t*) MALLOC_ARRAY(int32_t,
      (*output_length)); // The output needs to be freed by the calling process
  IF_NULL_ALLOCERROR_RETURN_NULL(output);
...
static int32_t* MMTF_parser_recursive_indexing_decode_from_8(
    const int8_t* input, uint32_t input_length, uint32_t* output_length)
{
  (*output_length) = 0;
  uint32_t i;
  for (i = 0; i < input_length; ++i) {
...
// Integer decoding
static float* MMTF_parser_integer_decode_from_16(const int16_t* input,
    uint32_t input_length, int32_t parameter, uint32_t* output_length)
{
  (*output_length) = input_length;
...
static float* MMTF_parser_integer_decode_from_32(const int32_t* input,
    uint32_t input_length, int32_t parameter, uint32_t* output_length)
{
  (*output_length) = input_length;
  float* output = (float*) MALLOC_ARRAY(float, (*output_length));
...
static void* MMTF_parser_decode_apply_strategy(const char* input,
    ...
    int32_t parameter, int* typecode)
{
  ...
  case 5:
    *typecode = MMTF_TYPE_string;
    return MMTF_parser_strings_from_bytes(
        input, input_length, parameter, output_length);
  case 6: {
    // pass
  }
  case 7: {
    ...
    int32_t* step1 =
        MMTF_parser_int32_from_bytes(input, input_length, &step1_length);

    int32_t* output =
        MMTF_parser_run_length_decode(step1, step1_length, output_length);
    free(step1);
  ...
  case 8: {
    ...
    int32_t* step2 =
        MMTF_parser_run_length_decode(step1, step1_length, &step2_length);
    free(step1);

    int32_t* output =
        MMTF_parser_delta_decode(step2, step2_length, output_length);
    free(step2);
  ...
  case 9: {
    ...
    float* output = MMTF_parser_integer_decode_from_32(
        step2, step2_length, parameter, output_length);
    free(step2);
  ...
  case 10: {
    uint32_t step1_length;
    int16_t* step1 =
        MMTF_parser_int16_from_bytes(input, input_length, &step1_length);

    uint32_t step2_length;
    int32_t* step2 = MMTF_parser_recursive_indexing_decode_from_16(
        step1, step1_length, &step2_length);
    free(step1);

    uint32_t step3_length;
    int32_t* step3 =
        MMTF_parser_delta_decode(step2, step2_length, &step3_length);
    free(step2);

    float* output = MMTF_parser_integer_decode_from_32(
        step3, step3_length, parameter, output_length);
    free(step3);
  ...
  case 11: {
    ...
    float* output = MMTF_parser_integer_decode_from_16(
        step1, step1_length, parameter, output_length);
    free(step1);
  ...
  case 13: {
    uint32_t step1_length;
    int8_t* step1 =
        MMTF_parser_int8_from_bytes(input, input_length, &step1_length);

    uint32_t step2_length;
    int32_t* step2 = MMTF_parser_recursive_indexing_decode_from_8(
        step1, step1_length, &step2_length);
    free(step1);
  ...
  case 14: {
    ...
    int32_t* output = MMTF_parser_recursive_indexing_decode_from_16(
        step1, step1_length, output_length);
    free(step1);
  ...
  case 15: {
    ...
    int32_t* output = MMTF_parser_recursive_indexing_decode_from_8(
        step1, step1_length, output_length);
    free(step1);
 ...
 */
static void MMTF_parser_put_string(const msgpack_object* object, char** out)
{
...
static char* MMTF_parser_fetch_string(const msgpack_object* object)
{
  switch (object->type) {
#ifdef MMTF_ACCEPT_MSGPACK_BIN_AS_STR
  case MMTF_MSGPACK_TYPE(BIN):
    fprintf(stderr, "Warning in %s: type BIN, expected STR ('%.*s')\n",
        __FUNCTION__, object->via.bin.size, object->via.bin.ptr);
#endif
  case MMTF_MSGPACK_TYPE(STR):
    break;
  default:
    fprintf(stderr,
        "Error in %s: the entry encoded in the MMTF is not a string.\n",
        __FUNCTION__);
    return NULL;
  }
...
static char MMTF_parser_fetch_char(const msgpack_object* object)
{
  switch (object->type) {
#ifdef MMTF_ACCEPT_MSGPACK_BIN_AS_STR
  case MMTF_MSGPACK_TYPE(BIN):
...
static int64_t MMTF_parser_fetch_int(const msgpack_object* object)
{
  int64_t result;
  ...
  if (object->type == MMTF_MSGPACK_TYPE(POSITIVE_INTEGER)) {
    ...
    fprintf(stderr,
        "Error in %s: the entry encoded in the MMTF is not an integer.\n",
        __FUNCTION__);
    return 0;
  }
...
static float MMTF_parser_fetch_float(const msgpack_object* object)
{
  switch (object->type) {
  ...
  default:
    fprintf(stderr,
        "Error in %s: the entry encoded in the MMTF is not a float.\n",
        __FUNCTION__);
    return NAN;
  }
}
 ...
 */
static void* MMTF_parser_fetch_typed_array(
    const msgpack_object* object, size_t* length, int typecode)
{
  if (object->type != MMTF_MSGPACK_TYPE(BIN)) {
    fprintf(stderr,
        "Error in %s: the entry encoded in the MMTF is not binary data.\n",
        __FUNCTION__);
    return NULL;
  }
  ...
  *length = len_int32;
  ...
  void* result = MMTF_parser_decode_apply_strategy(bytes + 12,
      object->via.bin.size - 12, &out_length, strategy, parameter, &typecheck);
...
bool MMTF_parser_compare_msgpack_string_char_array(const msgpack_object_str* m_string, const char* string) {
  // clang-format on
  return (m_string->size == strlen(string) &&
          strncmp(m_string->ptr, string, m_string->size) == 0);
}

static void MMTF_parser_put_entity(
    const msgpack_object* object, MMTF_Entity* entity)
{
...
static void MMTF_parser_put_group(
    const msgpack_object* object, MMTF_GroupType* group_type)
{
...
static MMTF_Transform* MMTF_parser_fetch_transformList(
    const msgpack_object*, size_t*);

static void MMTF_parser_put_bioAssembly(
    const msgpack_object* object, MMTF_BioAssembly* bio_assembly)
{
...
static void MMTF_parser_put_transform(
    const msgpack_object* object, MMTF_Transform* transform)
{
  MAP_ITERATE_BEGIN(object);
...
bool MMTF_unpack_from_msgpack_object(const msgpack_object* object, MMTF_container* thing) {
  ...
  if (thing->mmtfVersion &&
    ...
    fprintf(
        stderr, "Error: unsupported MMTF version '%s'.\n", thing->mmtfVersion);
    return false;
  }
 ...
 */
bool MMTF_unpack_from_string(
    const char* buffer, size_t msgsize, MMTF_container* thing)
{
  bool status;
...
bool MMTF_unpack_from_file(const char* name, MMTF_container* thing)
{
  ...
  if (!file) {
    fprintf(
        stderr, "Error in %s: unable to open file %s.\n", __FUNCTION__, name);
    return false;
  }
