...
typedef struct {
  ...
  float c1[3], c2[3], c3[3], ic[3],
      tr[3]; /* ic = interior color, tr = transparency */
  float r1, r2, l1;
  float trans;
  int char_id;
  char type;
...
} CPrimitive; /* currently 172 bytes -> appoximately 6.5 million primitives per
                 gigabyte */
...
typedef struct {
  ...
  int NNormal;
  float
      LightNormal[3]; /* for lights - this is the direction of the light rays */
...
void BasisTrianglePrecomputePerspective(
    float* v1, float* v2, float* v3, float* pre);
