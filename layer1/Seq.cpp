    ...
    int* col_num_ptr, int fixed_row)
{
  ...
  if ((row_num >= 0) && (row_num < I->NRow)) {
    ...
    char_num =
        (x - I->rect.left - DIP2PIXEL(I->CharMargin)) / DIP2PIXEL(I->CharWidth);
    ...
    if (SeqFindRowCol(G, x, y, &row_num, &col_num, -1)) {
      ...
      case P_GLUT_RIGHT_BUTTON: {
        ...
        if (ExecutiveGetActiveSeleName(G, name, false, false)) {
          MenuActivate2Arg(
              G, x, y + DIP2PIXEL(20), x, y, false, "pick_sele", name, name);
        }
      } break;
...
void CSeq::draw(CGO* orthoCGO)
{
  ...
  if (G->HaveGUI && G->ValidContext) {
    ...
    int label_color_index =
        SettingGetGlobal_color(G, cSetting_seq_view_label_color);
    ...
    if (SettingGetGlobal_b(G, cSetting_bg_gradient)) {
      if (SettingGetGlobal_b(G, cSetting_seq_view_location)) {
        copy3f(ColorGet(G, SettingGetGlobal_color(G, cSetting_bg_rgb_bottom)),
            bg_color);
      } else {
        copy3f(ColorGet(G, SettingGetGlobal_color(G, cSetting_bg_rgb_top)),
            bg_color);
      }
    } else {
    ...
    if (I->ScrollBarActive) {
      I->m_ScrollBar.setBox(rect.bottom + DIP2PIXEL(I->ScrollBarWidth),
          rect.left + DIP2PIXEL(I->ScrollBarMargin), rect.bottom + DIP2PIXEL(2),
    ...
    if (I->NRow) { /* get current sequence sizes */
      ...
      int fill_color_index =
          SettingGetGlobal_color(G, cSetting_seq_view_fill_color);
      int unaligned_mode =
          SettingGetGlobal_i(G, cSetting_seq_view_unaligned_mode);
      ...
      for (a = I->NRow - 1; a >= 0; a--) {
        ...
        if ((row->label_flag || row->column_label_flag) && row->nCol) {
          row->title_width = col->offset + (col->stop - col->start);
          xx = x + DIP2PIXEL(I->CharMargin) +
               DIP2PIXEL(I->CharWidth) *
                   (col->offset + (max_title_width - row->title_width));
          ch_wid = (col->stop - col->start);
          pix_wid = DIP2PIXEL(I->CharWidth * ch_wid);
          tot_len = col->offset + ch_wid - I->NSkip;
          if (tot_len <= vis_size) {
            TextDrawSubStrFast(
                G, row->txt, xx, y1, col->start, ch_wid, orthoCGO);
          }
        }
      ...
      for (a = I->NRow - 1; a >= 0; a--) {
        ...
        for (b = 1; b < row->nCol; b++) {
          ...
          if (col->offset >= first_allowed) {
            xx = x + DIP2PIXEL(I->CharMargin) +
                 DIP2PIXEL(I->CharWidth) * (col->offset - I->NSkip);
            ch_wid = (col->stop - col->start);
            pix_wid = DIP2PIXEL(I->CharWidth * ch_wid);
            tot_len = col->offset + ch_wid - I->NSkip;
            if (tot_len <= vis_size) {
              ...
              if (col->inverse) {
                if (orthoCGO) {
                  ...
                  CGOVertex(
                      orthoCGO, xx, yy + DIP2PIXEL(I->LineHeight) - 1, 0.f);
                  CGOVertex(orthoCGO, xx + pix_wid, yy, 0.f);
                  CGOVertex(orthoCGO, xx + pix_wid,
                      yy + DIP2PIXEL(I->LineHeight) - 1, 0.f);
                  CGOEnd(orthoCGO);
                } else {
                  glBegin(GL_POLYGON);
                  glVertex2i(xx, yy);
        ...
        if (fill_char) {
          ...
          for (b = 0; b < row->nFill; b++) {
            ...
            if (col->offset >= first_allowed) {
              ...
              if (tot_len <= vis_size) {
                TextDrawCharRepeat(
                    G, fill_char, xx, y1, col->start, ch_wid, orthoCGO);
              }
            }
          }
        }
      ...
      if (I->Handler->box_active) {
        ...
        if ((box_row >= 0) && (box_row < I->NRow)) {
          ...
          if ((start_col >= 0) && (start_col < row->nCol) && (stop_col >= 0) &&
              (stop_col < row->nCol)) {
            int xx2;
            CSeqCol* col2;
            ...
            xx = x + DIP2PIXEL(I->CharMargin) +
                 DIP2PIXEL(I->CharWidth) * (col->offset - I->NSkip);
            xx2 = x + DIP2PIXEL(I->CharMargin) +
                  DIP2PIXEL(I->CharWidth) *
                      (col2->offset + (col2->stop - col2->start) - I->NSkip);
            if (orthoCGO) {
              CGOColorv(orthoCGO, overlay_color);
              CGOLineAsTriangleStrips(
                  orthoCGO, xx, yy, xx2, yy + DIP2PIXEL(I->LineHeight) - 2);
      ...
      if (I->ScrollBarActive) {
        ...
        float bot, top, cent;
        float height =
            (float) DIP2PIXEL(I->ScrollBarWidth - I->ScrollBarMargin);
        int last_color = -1;
        cur_color = blue;
        for (a = 0; a < I->NRow; a++) {
          row = I->Row.data() + a;
          if (!row->label_flag) {
            top = rect.bottom + DIP2PIXEL(I->ScrollBarMargin) +
                  (height * real_count) / n_real;
            real_count--;
            bot = rect.bottom + DIP2PIXEL(I->ScrollBarMargin) +
                  (height * real_count) / n_real;
            mode = 0;
            for (b = 0; b < row->nCol; b++) {
              ...
              if (col->inverse && (!mode)) {
                ...
                if (row->label_flag)
                  ...
                  cur_color =
                      ColorGet(G, col->color); /* is this safe? should be for
                                                  single-threading */
              } else if ((!col->inverse) && (mode)) {
                if (b) {
                  stop = (width *
                             (col[-1].offset + col[-1].stop - col[-1].start)) /
                         max_len;
                } else {
                ...
                if (last_color != col->color) {
                  if (b) {
                    stop = (width * (col[-1].offset + col[-1].stop -
                                        col[-1].start)) /
                           max_len;
                  } else {
                    stop = (width * col->offset) / max_len;
