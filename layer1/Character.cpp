...
int CharacterNewFromBitmap(PyMOLGlobals* G, int width, int height,
    unsigned char* bitmap, float x_orig, float y_orig, float advance,
    CharFngrprnt* fprnt, int sampling)
{
  ...
  if ((id > 0) && (id <= I->MaxAlloc)) {
    CharRec* rec = I->Char + id;
    PixmapInitFromBitmap(
        G, &rec->Pixmap, width, height, bitmap, fprnt->u.i.color, sampling);
...
int CharacterNewFromBytemap(PyMOLGlobals* G, int width, int height, int pitch,
    unsigned char* bytemap, float x_orig, float y_orig, float advance,
    CharFngrprnt* fprnt)
{
  CCharacter* I = G->Character;
  int id = CharacterGetNew(G);
...
void CharacterRenderOpenGLPrime(PyMOLGlobals* G, const RenderInfo* info)
{
  if (G->HaveGUI && G->ValidContext) {
    if ((info && !info->use_shaders) ||
        (!info && !SettingGetGlobal_b(G, cSetting_use_shaders))) {
...
short CharacterRenderOpenGL(PyMOLGlobals* G, const RenderInfo* info, int id,
    short isworldlabel, short relativeMode, CGO* shaderCGO)
      ...
      if (shaderCGO) {
        ...
        if (isworldlabel) {
          ...
          shaderCGO->add<cgo::draw::label>(glm::make_vec3(worldPos),
              glm::make_vec3(screenWorldOffset), glm::make_vec3(v0),
              glm::make_vec3(v1), glm::make_vec4(rec->extent), relativeMode,
...
int CharacterGetGeometry(PyMOLGlobals* G, int id, int* width, int* height,
