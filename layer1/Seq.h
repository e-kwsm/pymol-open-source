...
struct CSeqHandler {
  virtual CSeqRow* click(PyMOLGlobals* G, std::vector<CSeqRow>& rowVLA,
      int button, int row, int col, int mod, int x, int y) = 0;
  virtual CSeqRow* drag(PyMOLGlobals* G, std::vector<CSeqRow>& rowVLA, int row,
      int col, int mod) = 0;
  virtual CSeqRow* release(PyMOLGlobals* G, std::vector<CSeqRow>& rowVLA,
      int button, int row, int col, int mod) = 0;
...
void SeqDirty(
    PyMOLGlobals* G); /* sequence dirty -- need to update selections */
