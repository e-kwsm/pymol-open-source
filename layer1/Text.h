...
- Availability:
   ...
   * we don't want to slow down PyMOL launching any more than it already has
been
...
- Raytracing:
   ...
   * need a geometric stand-in for the font for rendering (filled rect + alpha
okay)
...
- Flexibility: want to be able to color fonts on the fly, but also support
multi-color fonts

- Threading:
   * font loading will require the Python interpreter lock, since some fonts are
defined
...
void TextDrawStrAt(
    PyMOLGlobals* G, const char* st, int x, int y, CGO* orthoCGO);
