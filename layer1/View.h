...
typedef struct CViewElem {
  ...
  int matrix_flag; /* homogenous transformation, column-major for OpenGL
                      compatibility */
  double matrix[16];
...
int ViewElemVLAFromPyList(
    PyMOLGlobals* G, PyObject* list, CViewElem** vla, int nFrame);
PyObject* ViewElemVLAAsPyList(
    PyMOLGlobals* G, const CViewElem* vla, int nFrame);
...
void ViewElemDraw(PyMOLGlobals* G, const CViewElem* src, const BlockRect* rect,
    int frames, const char* title, CGO* orthoCGO);

enum class ViewElemAction { Delete = -1, Insert = 1, Move = 2, Copy = 3 };
