...
static void GenerateCharFngrprnt(PyMOLGlobals* G, CharFngrprnt* fprnt,
    unsigned int c, int TextID, float size, int sampling, short no_flat,
    int flat)
{
  unsigned char* rgba;
...
static const char* FontTypeRenderOpenGLImpl(const RenderInfo* info,
    CFontType* I, const char* st, float size, int flat, const float* rpos,
    bool needSize, short relativeMode, bool shouldRender, CGO* shaderCGO)
{
  PyMOLGlobals* G = I->G;
  if (G->ValidContext) {
    unsigned int c;
    ...
    if (st && (*st)) {
      ...
      if (rpos) {
        TextSetIndentFactorX(G, rpos[0] < _m1   ? _m1
                                : rpos[0] > 1.f ? 1.f
                                                : rpos[0]);
        TextSetIndentFactorY(G, rpos[1] < _m1   ? _m1
                                : rpos[1] > 1.f ? 1.f
                                                : rpos[1]);

        if (needSize || rpos[0] < _1) { /* we need to measure the string width
          ...
          while ((c = *(sst++))) {
            ...
            if (!unicnt) {
              CharFngrprnt fprnt;
              GenerateCharFngrprnt(
                  G, &fprnt, c, I->TextID, size, sampling, 0, flat);
              {
                int id = CharacterFind(G, &fprnt);
                if (!id) {
                  id = TypeFaceCharacterNew(
                      I->TypeFace, &fprnt, size * sampling);
                }
                if (id) {
                  if (kern_flag) {
                    line_width +=
                        (TypeFaceGetKerning(I->TypeFace, last_c, c, size) /
                            sampling);
                  }
          ...
          tot_height = pymol_roundf(
              size * (nlines + (nlines - 1) * (text_spacing - 1.f)) +
              2.f * text_buffer[1]);
          TextSetHeight(G, tot_height);
          {
            float factor = rpos[0] / 2.0F - 0.5F;
            /* if -1. < rpos[0] < 1. , then we need to determine the label's
               width so that we can justify it appropriately */
        ...
        */
        ...
        if (rpos[1] < _1) {
          ...
          y_indent = pymol_roundf(
              (size * factor) * (1.f + text_spacing * max2(0.f, nlines - 1.f)));
        }
        {
          float factor = rpos[1];
        ...
        z_indent = (rpos[2] < _m1)   ? (rpos[2] + 1.f)
                   : (rpos[2] > 1.f) ? (rpos[2] - 1.f)
                                     : 0.f;
        if (!shaderCGO) {
          x_indent += screenWorldOffset[0] / v_scale;
          y_indent += pymol_roundf(screenWorldOffset[1] / v_scale);
        }
        screenWorldOffset[2] += z_indent; // need to take into account weird -1
                                          // to 1, and sub 1 from abs val
      }
      if (!pushed) {
      ...
      kern_flag = false;
      TextGetPos(G)[1] +=
          (int) pymol_roundf(size * (text_spacing * (nlines - 1) + descender));
      if (line_widths) {
      ...
      while (cont && (c = *(st++))) {
        if (c == '\n') {
          ...
          linenum++;
          TextGetPos(G)[1] += (int) pymol_roundf(
              size * (text_spacing * (nlines - 1 - linenum) + descender));
          if (line_widths)
            TextGetPos(G)[0] +=
                text_just * (text_width - line_widths[linenum]) / 2.f +
                text_buffer[0];
          kern_flag = false;
          continue;
        }
        ...
        if (!unicnt) {
          CharFngrprnt fprnt;
          GenerateCharFngrprnt(
              G, &fprnt, c, I->TextID, size, sampling, 1, flat);
          {
            int id = CharacterFind(G, &fprnt);
            if (!id) {
            ...
            if (id) {
              if (kern_flag) {
                TextAdvance(
                    G, TypeFaceGetKerning(I->TypeFace, last_c, c, size) /
                           sampling);
              }
              cont &= CharacterRenderOpenGL(G, info, id, true, relativeMode,
                  shaderCGO); /* handles advance */
            }
          }
          kern_flag = true;
          last_c = c;
  ...
  if (st && (*st)) {
    ...
    if (rpos) {
      if (needSize || rpos[0] < _1) { /* we need to measure the string width
        ...
        while ((c = *(sst++))) {
          ...
          if (!unicnt) {
            CharFngrprnt fprnt;
            GenerateCharFngrprnt(G, &fprnt, c, I->TextID, size, sampling, 1,
                0 /* flat is not set */);
            {
              int id = CharacterFind(G, &fprnt);
              if (!id) {
              ...
              if (id) {
                float adv;
                if (kern_flag) {
                  float kern = TypeFaceGetKerning(
                      I->TypeFace, last_c, c, size * sampling);
                  line_width += kern;
      ...
      tot_height = size * (nlines + (nlines - 1) * (text_spacing - 1.f)) +
                   2.f * text_buffer[1];
      TextSetHeight(G, tot_height);
      if (rpos[0] < _m1) {
      ...
      if (rpos[1] < _1) {
        float factor = -rpos[1] / 2.0F + 0.5F;
        factor = (factor > _1) ? _1 : (factor < _0) ? _0 : factor;
        y_indent = (sampling * size * factor) *
                   (1.f + text_spacing * max2(0.f, nlines - 1.f));
      }
      {
        float factor = rpos[1];
        factor = (factor > _1) ? _1 : (factor < _m1) ? _m1 : factor;
      ...
      scale3f(yn,
          y_indent -
              size * (sampling * text_spacing * (nlines - 1) + descender),
          y_adj);
    ...
    while ((c = *(st++))) {
      if (c == '\n') {
        ...
        kern_flag = false;
        linenum++;
        scale3f(yn, pymol_roundf(text_spacing * size * linenum * sampling),
            y_adj); // need to round to pixel in y
        subtract3f(TextGetPos(G), y_adj, TextGetPos(G));
        if (line_widths) {
          scale3f(xn, text_just * (line_widths[0] - line_widths[linenum]) / 2.f,
              x_adj);
      ...
      if (!unicnt) {
        ...
        GenerateCharFngrprnt(G, &fprnt, c, I->TextID, size, sampling, 0, 0);
        {
          ...
          if (id) {
            if (kern_flag) {
              float kern =
                  TypeFaceGetKerning(I->TypeFace, last_c, c, size * sampling);
              v = TextGetPos(I->G);
