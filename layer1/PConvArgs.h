...
inline void _PConvArgsToPyList_SetItem(
    PyObject* list, int i, const T& value, const Args&... rest)
{
...
inline bool _PConvArgsFromPyList_GetItem(
    PyMOLGlobals* G, PyObject* list, int len, int i)
{
  return (len == i);
}
...
template <typename T, typename... Args>
inline bool _PConvArgsFromPyList_GetItem(
    PyMOLGlobals* G, PyObject* list, int len, int i, T& out, Args&... rest)
{
  if (len <= i)
    return false;
