    ...
    float* rayView, float* angle, float shift)
{
    ...
    *angle = stAng;
    {
      float temp[16];
      identity44f(temp);
      MatrixRotateC44f(temp, (float) (-PI * stAng / 180), 0.0F, 1.0F,
          0.0F); /* y-axis rotation */
  ...
  if (Feedback(G, FB_Scene, FB_Debugging)) {
    ...
    fprintf(
        stderr, "SceneRay: %8.3f %8.3f %8.3f\n", camOri.x, camOri.y, camOri.z);
    fprintf(stderr, "SceneRay: %8.3f %8.3f %8.3f\n", rotMat[0], rotMat[1],
        rotMat[2]);
  }
}

bool SceneRay(PyMOLGlobals* G, int ray_width, int ray_height, int mode,
    char** headerVLA_ptr, char** charVLA_ptr, float angle, float shift,
    int quiet, G3dPrimitive** g3d, int show_timing, int antialias)
{
#ifdef _PYMOL_NO_RAY
  ...
  while (1) {
    ...
    for (slot = 0; slot <= I->grid.last_slot; slot++) {
      ...
      height =
          (float) (fabs(I->m_view.pos().z) * tan((fov / 2.0) * cPI / 180.0));
      width = height * aspRat;
...

      {
        float pixel_scale_value =
            SettingGetGlobal_f(G, cSetting_ray_pixel_scale);
        ...
        if (ortho) {
          ...
          RayPrepare(ray, -width, width, -height, height,
              ...
              fov, I->m_view.pos(), rayView, I->m_view.rotMatrix(), aspRat,
              ray_width, ray_height, pixel_scale_value, ortho, _1, _1,
          ...
          RayPrepare(ray, -back_width, back_width, -back_height, back_height,
              I->m_view.m_clipSafe().m_front, I->m_view.m_clipSafe().m_back,
              fov, I->m_view.pos(), rayView, I->m_view.rotMatrix(), aspRat,
              ray_width, ray_height, pixel_scale_value, ortho,
              height / back_height,
              I->m_view.m_clipSafe().m_front / I->m_view.m_clipSafe().m_back,
              ((float) ray_height) / I->Height);
        }
      }
      {
        ...
        if (SettingGetGlobal_b(G, cSetting_dynamic_width)) {
          ...
          info.dynamic_width_min =
              SettingGetGlobal_f(G, cSetting_dynamic_width_min);
          info.dynamic_width_max =
              SettingGetGlobal_f(G, cSetting_dynamic_width_max);
        }
        ...
        for (auto* obj : I->Obj) {
          ...
          if (obj->type != cObjectGroup) {
            if (SceneGetDrawFlag(&I->grid, slot_vla, obj->grid_slot)) {
              ...
              auto icx = SettingGetWD<int>(obj->Setting.get(),
                  cSetting_ray_interior_color, cColorDefault);
              ...
              if (!I->grid.active || I->grid.mode == GridMode::NoGrid ||
      ...
      if (mode != 2) { /* don't show pixel count for tests */
        if (!quiet) {
          ...
          " Ray: tracing %dx%d = %d rays against %d primitives.\n", ray_width,
              ray_height, ray_width *ray_height,
              RayGetNPrimitives(ray) ENDFB(G);
        }
      }
      switch (mode) {
      ...
      case 1: /* mode 1 is povray */
        ...
        if (!(charVLA_ptr && headerVLA_ptr)) { /* immediate mode */
          strcpy(
              prefix, SettingGet_s(G, nullptr, nullptr, cSetting_batch_prefix));
#ifndef _PYMOL_NOPY
      ...
      case 2: /* mode 2 is for testing of geometries */
        RayRenderTest(ray, ray_width, ray_height,
            I->m_view.m_clipSafe().m_front, I->m_view.m_clipSafe().m_back, fov);
        break;
      case 3: /* mode 3 is for Jmol */
      {
        G3dPrimitive* jp = RayRenderG3d(ray, ray_width, ray_height,
            I->m_view.m_clipSafe().m_front, I->m_view.m_clipSafe().m_back, fov,
            quiet);
        if (0) {
          ...
          for (a = 0; a < cnt; a++) {
            ...
            case 2:
              printf("triangle(%d,%d,%d,%d,%d,%d,%d,%d,%d);\n", jp[a].x1,
                  jp[a].y1, jp[a].z1, jp[a].x2, jp[a].y2, jp[a].z2, jp[a].x3,
                  jp[a].y3, jp[a].z3);
              break;
            case 3:
              printf(
                  "g3d.fillCylinder(gray,gray,(byte)3,%d,%d,%d,%d,%d,%d,%d);\n",
                  jp[a].r, jp[a].x1, jp[a].y1, jp[a].z1, jp[a].x2, jp[a].y2,
                  jp[a].z2);
              break;
            }
          }
    ...
    if ((mode == 0) && I->Image && !I->Image->empty()) {
      SceneApplyImageGamma(
          G, I->Image->pixels(), I->Image->getWidth(), I->Image->getHeight());
    }
    ...
    if (I->Image) {
      ...
      case cStereo_anaglyph: {
        ...
        }
        {
          ...
          int height, width;
          int a, b;
          float _r[3] = {0.F, 0.F, 0.F}, _l[3] = {0.F, 0.F, 0.F},
                _b[3] = {0.F, 0.F, 0.F};
          height = I->Image->getHeight();
          width = I->Image->getWidth();
  ...
  if (mode != 2) { /* don't show timings for tests */
    ...
    if (show_timing && !quiet) {
      if (!G->Interrupt) {
        ...
        " Ray: render time: %4.2f sec. = %3.1f frames/hour (%4.2f sec. "
        "accum.).\n",
...
void SceneRenderRayVolume(PyMOLGlobals* G, CScene* I)
{
  ...
  if (PIsGlutThread() && rayWidth == I->Width && rayHeight == I->Height)
    glDrawPixels(
        I->Width, I->Height, GL_DEPTH_COMPONENT, GL_FLOAT, rayDepthPixels);
#endif
  glDepthFunc(GL_LESS);
