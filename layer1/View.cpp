...
int ViewElemModify(PyMOLGlobals* G, CViewElem** handle, ViewElemAction action,
    int index, int count, int target)
{
  int ok = true;
  ...
  if (vla) {
    ...
    case ViewElemAction::Move:
      if ((index >= 0) && (target >= 0) && (index < n_frame) &&
          (target < n_frame)) {
...
void ViewElemDraw(PyMOLGlobals* G, const CViewElem* view_elem,
...
int ViewElemFromPyList(PyMOLGlobals* G, PyObject* list, CViewElem* view)
{
  ...
  if (ok && view->matrix_flag)
    ok = PConvPyListToDoubleArrayInPlace(
        PyList_GetItem(list, 1), view->matrix, 16);
  ...
  if (ok && view->post_flag)
    ok =
        PConvPyListToDoubleArrayInPlace(PyList_GetItem(list, 5), view->post, 3);
...
int ViewElemVLAFromPyList(
    PyMOLGlobals* G, PyObject* list, CViewElem** vla_ptr, int nFrame)
{
  int ok = true;
...
static void matrix_interpolate(Matrix53f imat, Matrix53f mat,
    float* pivot_point, float* bisect_dir, float* rot_axis, float rotate_angle,
    float* trans_axis, float translate_angle, float fxn, float linearity)
{
  int a;
  ...
  for (a = 0; a < 3; a++) {
    imat[4][a] =
        (float) ((((1.0 - fxn) * mat[3][a] + fxn * mat[4][a]) * linearity) +
                 (1.0 - linearity) * pos[a]);
  }
}
    ...
    for (a = 0; a < n; a++) {
      ...
      if (dst->specification_level) { /* has to be specified */
        ...
        if (dst->matrix_flag) {
          ...
          reorient44d(
              dst->matrix); /* convert those averages into a valid matrix */
        }
...
int ViewElemInterpolate(PyMOLGlobals* G, CViewElem* first, CViewElem* last,
    float power, float bias, int simple, float linearity, int hand, float cut)
{
  float first3x3[9];
  float last3x3[9];
  float inverse3x3[9];
  ...
  multiply33f33f(
      inverse3x3, last3x3, &rot[0][0]); /* [rot] = [first]^-1 [last] */
  ...
  if (hand) {
    if ((cPI - fabs(angle)) < 0.01F) { /* this a complete 180 degree motion */
      if (((rot_axis[0] * 0.7F + rot_axis[1] * 0.8F + rot_axis[2] * 0.9F) *
              hand * angle) > 0.0F) {
        invert3f(rot_axis);
        if (angle > 0) {
  ...
  if (!(simple || linear)) {
...

    {
      ...
      if ((fabs(translate_angle) > fabs(angle)) && (fabs(angle) > R_SMALL4)) {
        translate_angle =
            (float) (fabs(angle) * (translate_angle / fabs(angle)));
      }
      ...
      if ((translate_angle * angle) < 0.0F) {
        /* if motions are in opposing directions, then flip translation axis and
         * location */
        invert3f(bisect);
        invert3f(trans_axis);
      }
  ...
  for (a = 0; a < n; a++) {
    ...
    if (state_flag) { /* states are interpolated linearly by default */
      state =
          (int) (first->state * (1.0F - fxn) + (last->state * fxn) + 0.499F);
    }
    ...
    if (simple) {
      rotation_matrix3f(
          fxn * angle, rot_axis[0], rot_axis[1], rot_axis[2], &imat[0][0]);
      ...
      */
      ...
      matrix_interpolate(imat, rot, pivot, bisect, rot_axis, angle, trans_axis,
          translate_angle, fxn, linearity);
