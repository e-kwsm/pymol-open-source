...
int PLabelAtomAlt(PyMOLGlobals* G, AtomInfoType* at, const char* model,
    const char* expr, int index)
{
      ...
      if (ch == '\'') {
        ...
        if (label_next_token(tok, &expr)) {
          ...
          if (!strcmp(tok, "model")) {
            ...
            if (at->hetatm)
              label_len =
                  label_copy_text(label, "HETATM", label_len, label_max);
            else
              label_len = label_copy_text(label, "ATOM", label_len, label_max);
          } else if (!strcmp(tok, "name")) {
            label_len = label_copy_text(
                label, LexStr(G, at->name), label_len, label_max);
          } else if (!strcmp(tok, "resn")) {
            label_len = label_copy_text(
                label, LexStr(G, at->resn), label_len, label_max);
            ...
            label_len = label_copy_text(
                label, LexStr(G, at->chain), label_len, label_max);
          } else if (!strcmp(tok, "alt")) {
            label_len = label_copy_text(label, at->alt, label_len, label_max);
          } else if (!strcmp(tok, "segi")) {
            label_len = label_copy_text(
                label, LexStr(G, at->segi), label_len, label_max);
          } else if (!strcmp(tok, "ss")) {
            label_len =
                label_copy_text(label, at->ssType, label_len, label_max);
...
/* all of the following Python objects must be invariant & global for the
 * application */

/* these are module / module properties -- global and static for a given
 * interpreter */

/* local to this C code module */

static PyObject* P_pymol = nullptr;
static PyObject* P_pymol_dict =
    nullptr; /* must be refomed into globals and instance properties */
...
static PyObject* PImportModuleOrFatal(const char* name)
{
...
static PyObject* PGetAttrOrFatal(PyObject* o, const char* name)
{
...
static PyTypeObject Wrapper_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "wrapper.Wrapper", /* tp_name */
    0,                                                /* tp_basicsize */
};
static PyTypeObject settingWrapper_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "wrapper.SettingWrapper", /* tp_name */
    0,                                                       /* tp_basicsize */
};
 ...
 * Raise LookupError if `key` doesn't name a known setting
 */
static PyObject* SettingWrapperObjectSubScript(PyObject* obj, PyObject* key)
{
  ...
  if (!ret) {
    ...
    if (!ret) {
      ...
      ret = SettingGetPyObject(G, wobj->cs ? wobj->cs->Setting.get() : nullptr,
 ...
 */
static int SettingWrapperObjectAssignSubScript(
    PyObject* obj, PyObject* key, PyObject* val)
{
  auto& wobj = reinterpret_cast<SettingPropertyWrapperObject*>(obj)->wobj;
  ...
  if (wobj->read_only) {
    PyErr_SetString(
        PyExc_TypeError, "Use alter/alter_state to modify settings");
    return -1;
  }
  ...
  if (wobj->idx >= 0) {
    ...
    if (!SettingLevelCheck(G, setting_id, cSettingLevel_astate)) {
      ...
      return -1; // failure
    } else if (CoordSetSetSettingFromPyObject(
                   G, wobj->cs, wobj->idx, setting_id, val)) {
    }
  } else {
    // atom level
    if (!SettingLevelCheck(G, setting_id, cSettingLevel_atom)) {
      ...
      return -1; // failure
    } else if (AtomInfoSetSettingFromPyObject(
                   G, wobj->atomInfo, setting_id, val)) {
...
static PyObject* SettingWrapperObjectIter(PyObject* self)
{
  ...
  if (wobj->idx >= 0) {
    unique_id = wobj->cs->atom_state_setting_id
                    ? wobj->cs->atom_state_setting_id[wobj->idx]
                    : 0;
  }
 ...
 */
static int PyObject_GenericSetAttrAsItem(
 ...
 */
static PyObject* WrapperObjectSubScript(PyObject* obj, PyObject* key)
{
  ...
  if (ap) {
    ...
    case ATOM_PROP_STEREO:
      if (ObjectMoleculeUpdateMMStereoInfoForState(
              G, wobj->obj, wobj->state - 1) < 0) {
        PyErr_SetString(
            P_CmdException, "please install rdkit or set SCHRODINGER variable");
        return nullptr;
      }
      break;
    case ATOM_PROP_TEXT_TYPE:
#ifndef NO_MMLIBS
      ObjectMoleculeUpdateAtomTypeInfoForState(
          G, wobj->obj, wobj->state - 1, 1, 0);
#endif
      break;
    }
#endif
 ...
 */
static int WrapperObjectAssignSubScript(
    PyObject* obj, PyObject* key, PyObject* val)
{
  auto wobj = static_cast<WrapperObject*>(obj);
...
int PTruthCallStr4i(
    PyObject* object, const char* method, int a1, int a2, int a3, int a4)
{
  assert(PyGILState_Check());
    ...
    if (hash_code && entry) {
      ...
      for (i = 0; i < tuple_size; i++) {
        ...
        if (item != Py_None) {
           ...
           * hash tuples of ints & floats in exactly the same way (at least to
           * 31 bits of significance) */
          hash_long =
              0x7FFFFFFF &
              PyObject_Hash(
                  item); /* pos 32 bit # to preserve 32-bit/64-bit compat */
        } else {
          hash_long = 0; /* None doesn't hash consistently from Python version
                            to version */
        }
...
ov_status PCacheSet(PyMOLGlobals* G, PyObject* entry, PyObject* output)
{
  ...
  if (G->P_inst->cache && output) {
    ...
    PXDecRef(PYOBJECT_CALLMETHOD(G->P_inst->cmd, "_cache_set", "OiO", entry,
        SettingGetGlobal_i(G, cSetting_cache_max), G->P_inst->cmd));
...
ov_status PCacheGet(PyMOLGlobals* G, PyObject** result_output,
    PyObject** result_entry, PyObject* input)
{
  assert(PyGILState_Check());
  ...
  if (G->P_inst->cache) {
    ...
    if (OV_OK(CacheCreateEntry(&entry, input))) {
      output = PYOBJECT_CALLMETHOD(
          G->P_inst->cmd, "_cache_get", "OOO", entry, Py_None, G->P_inst->cmd);
      if (output == Py_None) {
        Py_DECREF(output);
        output = nullptr;
      } else {
...
static void my_interrupt(int a)
{
  exit(EXIT_FAILURE);
}
...
static WrapperObject* WrapperObjectNew()
{
...
int PAlterAtomState(PyMOLGlobals* G, PyObject* expr_co, int read_only,
    ObjectMolecule* obj, CoordSet* cs, int atm, int idx, int state,
    PyObject* space)
{
  assert(PyGILState_Check());
    ...
    PyObject* expr_co, int read_only, int atm, PyObject* space)
{
  ...
  return PAlterAtomState(
      G, expr_co, read_only, obj, cs, atm, /* idx */ -1, state, space);
}

/**
 * String conversion which takes "label_digits" setting into account.
 */
static int PLabelPyObjectToStrMaxLen(
    PyMOLGlobals* G, PyObject* obj, char* buffer, int maxlen)
{
  assert(PyGILState_Check());

  if (obj && PyFloat_Check(obj)) {
    snprintf(buffer, maxlen + 1, "%.*f",
        SettingGetGlobal_i(G, cSetting_label_digits), PyFloat_AsDouble(obj));
    return true;
  }
...
{ /* must call with unblocked interpreter */
  ...
  PBlock(G);
  PXDecRef(PYOBJECT_CALLFUNCTION(G->P_inst->unlock, "iO", 0,
      G->P_inst->cmd)); /* NOTE this may flush the command buffer! */
  PLockStatus(G);
...
{ /* must call with unblocked interpreter */
  ...
  PBlock(G);
  PXDecRef(PYOBJECT_CALLFUNCTION(G->P_inst->unlock, "iO", -1,
      G->P_inst->cmd)); /* prevents flushing of the buffer */
  PLockStatus(G);
  PyMOL_PopValidContext(G->PyMOL);
  PUnlockStatus(G);
  PUnlockGLUT(G);
...
int PLockAPIAsGlut(PyMOLGlobals* G, int block_if_busy)
{
  ...
  while (true) {
    if (!get_api_lock(G, block_if_busy)) {
      ...
      PUnlockGLUT(G);
      PUnblock(G);
      return false; /* busy -- so allow main to update busy status display (if
                       any) */
    }
    ...
    PXDecRef(PYOBJECT_CALLFUNCTION(G->P_inst->unlock, "iO", -1,
        G->P_inst->cmd)); /* prevent buffer flushing */
#ifndef WIN32
    {
      struct timeval tv;
...
void PConvertOptions(CPyMOLOptions* rec, PyObject* options)
{
  ...
  rec->internal_gui =
      PyInt_AsLong(PyObject_GetAttrString(options, "internal_gui"));
  rec->internal_feedback =
      PyInt_AsLong(PyObject_GetAttrString(options, "internal_feedback"));
  rec->show_splash =
      PyInt_AsLong(PyObject_GetAttrString(options, "show_splash"));
  ...
  rec->force_stereo =
      PyInt_AsLong(PyObject_GetAttrString(options, "force_stereo"));
  ...
  rec->external_gui =
      PyInt_AsLong(PyObject_GetAttrString(options, "external_gui"));
  rec->siginthand =
      PyInt_AsLong(PyObject_GetAttrString(options, "sigint_handler"));
  rec->reuse_helper =
      PyInt_AsLong(PyObject_GetAttrString(options, "reuse_helper"));
...
#endif
  rec->multisample =
      PyInt_AsLong(PyObject_GetAttrString(options, "multisample"));
  rec->window_visible =
      PyInt_AsLong(PyObject_GetAttrString(options, "window_visible"));
  rec->read_stdin = PyInt_AsLong(PyObject_GetAttrString(options, "read_stdin"));
  rec->presentation =
      PyInt_AsLong(PyObject_GetAttrString(options, "presentation"));
  rec->defer_builds_mode =
      PyInt_AsLong(PyObject_GetAttrString(options, "defer_builds_mode"));
  rec->full_screen =
      PyInt_AsLong(PyObject_GetAttrString(options, "full_screen"));
  load_str =
      PyString_AsString(PyObject_GetAttrString(options, "after_load_script"));
  rec->sphere_mode =
      PyInt_AsLong(PyObject_GetAttrString(options, "sphere_mode"));
  rec->stereo_capable =
      PyInt_AsLong(PyObject_GetAttrString(options, "stereo_capable"));
  rec->stereo_mode =
      PyInt_AsLong(PyObject_GetAttrString(options, "stereo_mode"));
  ...
  rec->launch_status =
      PyInt_AsLong(PyObject_GetAttrString(options, "launch_status"));
  rec->gldebug = PyInt_AsLong(PyObject_GetAttrString(options, "gldebug"));
  rec->openvr_stub =
      PyInt_AsLong(PyObject_GetAttrString(options, "openvr_stub"));
...
void PGetOptions(CPyMOLOptions* rec)
{
  ...
  invocation = PGetAttrOrFatal(
      pymol, "invocation"); /* get a handle to the invocation module */
...
#define _PYMOL_INIT_MODULES
...

  {
    ...
    G->P_inst->lock_api_status =
        PGetAttrOrFatal(G->P_inst->cmd, "lock_api_status");
    ...
    {
      PyObject* fn_closure =
          PyObject_GetAttrString(P_parser, "new_parse_closure");
    ...
    {
      PyObject* fn_closure =
          PyObject_GetAttrString(P_parser, "new_complete_closure");
      G->P_inst->complete =
          PYOBJECT_CALLFUNCTION(fn_closure, "O", G->P_inst->cmd);
      PXDecRef(fn_closure);
      if (!G->P_inst->complete)
    ...
    P_IncentiveOnlyException =
        PGetAttrOrFatal(P_pymol, "IncentiveOnlyException");
    ...
    if (!Wrapper_Type.tp_basicsize) {
      ...
      settingMappingMethods.mp_ass_subscript =
          &SettingWrapperObjectAssignSubScript;
      ...
      if (PyType_Ready(&Wrapper_Type) < 0 ||
        ...
        PRINTFB(G, FB_Python, FB_Errors)
        " PInit: Wrapper_Type, settingWrapper_Type, propertyWrapper_Type not "
        "ready\n" ENDFB(G);
        return;
      }
  ...
  PyRun_SimpleString("import pymol.importing;"
...
int PPovrayRender(PyMOLGlobals* G, const char* header, const char* inp,
    const char* file, int width, int height, int antialias)
{
  assert(!PyGILState_Check());
  ...
  int ok;
  PBlock(G);
  result = PYOBJECT_CALLMETHOD(P_povray, "render_from_string", "sssiii", header,
      inp, file, width, height, antialias);
  ...
  if (mode) {
    ...
    if (log && (log != Py_None)) {
      if (format == cPLog_no_flush) {
        PYOBJECT_CALLMETHOD(log, "write", "s",
            str); /* maximize responsiveness (for real-time) */
      } else {
        switch (mode) {
  ...
  if (PAutoBlock(G)) {
    if (!(PIsGlutThread() && G->P_inst->glut_thread_keep_out)) {
      ...
      while (!OrthoCommandIsEmpty(*ortho)) {
        ...
        PXDecRef(
            PYOBJECT_CALLFUNCTION(G->P_inst->parse, "si", buffer.c_str(), 0));
        PFLUSH_REPORT_UNCAUGHT_EXCEPTIONS(G);
...
static PyObject* PCatch_install(PyObject* self, PyObject* args)
{
  PyRun_SimpleString("import sys, pcatch\n"
...
static PyMethodDef PCatch_methods[] = {
    ...
    {"write", PCatchWrite, METH_VARARGS}, {"flush", PCatchFlush, METH_VARARGS},
    {"isatty", PCatchIsAtty,
        METH_VARARGS}, // called by pip.main(["install", "..."])
    {"_install", PCatch_install, METH_VARARGS}, {NULL, nullptr} /* sentinel */
};
...
void PCatchInit(void)
{
  ...
  static struct PyModuleDef moduledef = {
      PyModuleDef_HEAD_INIT, "pcatch", nullptr, -1, PCatch_methods};
