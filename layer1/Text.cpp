...
void TextSetLabelBkgrdInfo(
    PyMOLGlobals* G, float label_spacing, float label_just, const float* buff)
{
  CText* I = G->Text;
...
void TextDrawSubStrFast(PyMOLGlobals* G, const char* c, int x, int y, int start,
    int n, CGO* orthoCGO)
{
  c += start;
  TextSetPos2i(G, x, y);
...
void TextDrawCharRepeat(
    PyMOLGlobals* G, char c, int x, int y, int start, int n, CGO* orthoCGO)
{
  c += start;
  TextSetPos2i(G, x, y);
  while (n) {
...
void TextGetOutlineColor(PyMOLGlobals* G, unsigned char* red,
    unsigned char* green, unsigned char* blue, unsigned char* alpha)
{
  CText* I = G->Text;
...
const char* TextRenderOpenGL(PyMOLGlobals* G, const RenderInfo* info,
    int text_id, const char* st, float size, const float* rpos, short needSize,
    short relativeMode, short shouldRender, CGO* shaderCGO)
{
  CText* I = G->Text;
  ...
  if (st && (*st)) {
    ...
    if (font) {
      if (I->Flat) {
        ...
        return font->RenderOpenGL(info, st, size, rpos, needSize, relativeMode,
            shouldRender, shaderCGO);
      }
    }
...
void TextDrawStrAt(PyMOLGlobals* G, const char* st, int x, int y, CGO* orthoCGO)
{
  ...
  TextRenderOpenGL(G, nullptr, I->Default_ID, st, TEXT_DEFAULT_SIZE, nullptr,
      false, 0, 1, orthoCGO);
}
...
const char* TextRenderRay(PyMOLGlobals* G, CRay* ray, int text_id,
    const char* st, float size, const float* rpos, short needSize,
    short relativeMode)
{
  CText* I = G->Text;
...
int TextInit(PyMOLGlobals* G)
{
  ...
  I->addFont(
      7, FontTypeNew(G, TTF_DejaVuSans_Bold_dat, TTF_DejaVuSans_Bold_len));
  I->addFont(8, FontTypeNew(G, TTF_DejaVuSans_BoldOblique_dat,
                    TTF_DejaVuSans_BoldOblique_len));
  I->addFont(9, FontTypeNew(G, TTF_DejaVuSerif_dat, TTF_DejaVuSerif_len));
  I->addFont(
      10, FontTypeNew(G, TTF_DejaVuSerif_Bold_dat, TTF_DejaVuSerif_Bold_len));
  I->addFont(
      11, FontTypeNew(G, TTF_DejaVuSansMono_dat, TTF_DejaVuSansMono_len));
  I->addFont(12, FontTypeNew(G, TTF_DejaVuSansMono_Oblique_dat,
                     TTF_DejaVuSansMono_Oblique_len));
  I->addFont(13,
      FontTypeNew(G, TTF_DejaVuSansMono_Bold_dat, TTF_DejaVuSansMono_Bold_len));
  I->addFont(14, FontTypeNew(G, TTF_DejaVuSansMono_BoldOblique_dat,
  ...
  I->addFont(18, FontTypeNew(G, TTF_DejaVuSerif_BoldOblique_dat,
                     TTF_DejaVuSerif_BoldOblique_len));
#endif
#endif
...
bool TextStartsWithColorCode(const char* p)
{
  ...
  return (('0' <= p[1] && p[1] <= '9') && ('0' <= p[2] && p[2] <= '9') &&
          ('0' <= p[3] && p[3] <= '9'));
}
 ...
 */
bool TextSetColorFromCode(
    PyMOLGlobals* G, const char* p, const float* defaultcolor)
{
  ...
  if (p[1] == '-') {
    ...
    TextSetColor3f(
        G, (p[1] - '0') / 9.0F, (p[2] - '0') / 9.0F, (p[3] - '0') / 9.0F);
  }
