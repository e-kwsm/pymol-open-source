...
inline uchar CLIP_COLOR_VALUE(float cv)
{
  return ((cv > 1.f) ? 255 : (cv < 0.f) ? 0 : pymol_roundf(cv * 255));
}
inline float CONVERT_COLOR_VALUE(unsigned char cv)
{
  return ((cv > 255) ? 1.f : (cv < 0) ? 0.f : (cv / 255.f));
}

// normal values are mapped between { -1, 1 } to { 0, 255 },
// the values are mapped: { -1/128, -.5/192, 0./255, .5/63, 1./127 }
inline uchar CLIP_NORMAL_VALUE(float cv)
{
  return ((cv > 1.f)    ? 127
          : (cv < -1.f) ? 128
...
/* CGO_DRAW_ARRAYS : operation that calls glDrawArrays with all arrays in memory
   (i.e., stored in the CGO array). There can be up to 4 arrays (vertex, normal,
   color, and pick color array, where each are stored using GL_FLOAT array
   (except for the pick color array, which is GL_UNSIGNED_BYTE, because the 2nd
   2/3rds of the array is used to store atom/bond picking information. Also, the
   color array is stored using 4 floats, since glColorPointer() requires having
   4 as an argument for OpenGLES.
   - mode : GL Mode that is used
   - arrays : which arrays that are used (bitmask on CGO_<type>_ARRAY, where
   <type> can be VERTEX, NORMAL, COLOR, or PICK_COLOR)
...
/* CGO_DRAW_BUFFERS_INDEXED : operation that uses glDrawArrays with VBOs.  It
   also has a dynamic length since it holds the picking information (atom/bond)
   for each vertex, as well as space for picking to use when drawing colors
   (since colors are dynamically generated for each atom/bond value).
   - mode : GL Mode that is used
   - arrays : which arrays that are used (bitmask on CGO_<type>_ARRAY, where
   <type> can be VERTEX, NORMAL, COLOR, or PICK_COLOR)
   - narrays : number of arrays specified in arrays
   - nverts : number of total vertices specified in arrays
   - bufs[5] : each VBO id in order (if used, VERTEX, NORMAL, COLOR,
   PICK_COLOR), plus the vertex index array that specifies all vertices.
 */
#define CGO_DRAW_BUFFERS_INDEXED 0x21

/* CGO_BOUNDING_BOX : operation that allows the extent to be expanded.  Since
   the geometry data is not kept in the CGO object for VBO objects (only on the
   card), this object allows the bounding box of the object to be saved in the
   CGO.  This is used in the CGOGetExtent(), typically when the view is being
   automatically set, but can be used for other things.
 */
...
// CGOs are floating point arrays so we need to work with sizes in terms of
// floats
template <typename T> static size_t fsizeof()
{
...
struct trilines {
  ...
  trilines(unsigned nverts, unsigned buffer)
      : nverts(nverts)
      , buffer(buffer)
  {
  }
...
struct arrays : op_with_data {
      ...
      , nverts(_nverts)
  {
    ...
    if (arraybits & CGO_ACCESSIBILITY_ARRAY)
      narrays++;
    if (arraybits & CGO_COLOR_ARRAY)
      narrays++;
  };
  int mode;
  int arraybits;
...
struct buffers_indexed : op_with_data, op_with_draw_buffers {
  ...
  buffers_indexed(int _mode, short _arrays, int _nindices, int _nverts,
      size_t _vboid, size_t _iboid, int _n_data, size_t _pickvboid = 0)
      : mode(_mode)
      , arraybits(_arrays)
      , narrays(0)
      , nindices(_nindices)
...
struct sphere_buffers : op_with_data, op_with_draw_buffers {
  ...
  sphere_buffers(
      int _num_spheres, int _ub_flags, size_t _vboid, size_t _pickvboid)
...
struct cylinder_buffers : op_with_data, op_with_draw_buffers {
  ...
  cylinder_buffers(
      int _num_cyl, int _alpha, size_t _vboid, size_t _iboid, size_t _pickvboid)
      : num_cyl(_num_cyl)
      , alpha(_alpha)
      , vboid(_vboid)
      , iboid(_iboid)
...
struct textures : op_with_data, op_with_draw_buffers {
  ...
  textures(int _ntextures, size_t _vboid)
...
struct screen_textures : op_with_draw_buffers {
  ...
  screen_textures(int _nverts, size_t _vboid)
      : nverts(_nverts)
      , vboid(_vboid) {};
  int nverts;
  size_t vboid;
...
struct buffers_not_indexed : op_with_data, op_with_draw_buffers {
  ...
  buffers_not_indexed(
      int _mode, int _arrays, int _nverts, size_t _vboid, size_t _pickvboid = 0)
      : mode(_mode)
      , arraybits(_arrays)
      , narrays(0)
      , nverts(_nverts)
...
struct label {
  ...
  label(const vec3& _world_pos, const vec3& _screen_world_offset,
      const vec3& _screen_min, const vec3& _screen_max,
      const vec4& _text_extent, float _relative_mode, const vec3& _target_pos)
      : world_pos(_world_pos)
      , screen_world_offset(_screen_world_offset)
      , screen_min(_screen_min)
      , screen_max(_screen_max)
      , text_extent(_text_extent)
...
struct splitline {
  ...
  splitline(const float* v1, const float* v2, const float* color2arg,
      unsigned int index_2nd, int bond_2nd, bool isRamped, bool same_pick,
      bool eq_colors)
      : index(index_2nd)
      , bond(bond_2nd)
  {
...
struct custom : op_with_data, op_with_draw_buffers {
  ...
  custom(int _mode, int _nverts, size_t _vboid, size_t _pickvboid,
      ...
      , iboid(_iboid)
      , nindices(_nindices)
      , npickbufs(_npickbufs)
      , vertsperpickinfo(_vertsperpickinfo) {};
  int mode;
  int nverts;
  size_t vboid;
  size_t pickvboid;
  ...
  {
    return npickbufs * nverts * 2 / vertsperpickinfo;
  };
};
...
struct shadercylinder {
  ...
  shadercylinder(const float* _origin, const float* _axis,
      const float _tube_size, int _cap)
...
struct shadercylinder2ndcolor {
  ...
  shadercylinder2ndcolor(CGO* I, const float* _origin, const float* _axis,
      const float _radius, int _cap, const float* _color2,
      Pickable* pickcolor2 = nullptr, const float alpha = -1.f);
  float origin[3], axis[3], tube_size;
  int cap;
  float color2[3];
...
struct custom_cylinder {
  ...
  custom_cylinder(const float* _vertex1, const float* _vertex2,
      const float _radius, const float* _color1, const float* _color2,
...
struct custom_cylinder_alpha {
  ...
  custom_cylinder_alpha(const float* _vertex1, const float* _vertex2,
      const float _radius, const float* _color1, const float* _color2,
      const float _alpha1, const float _alpha2, const float _cap1,
      const float _cap2)
      : radius(_radius)
      , cap1(_cap1)
      , cap2(_cap2)
...
public:
  ...
  bool has_draw_buffers{false}, has_draw_cylinder_buffers{false},
      has_draw_sphere_buffers{false};
  bool use_shader{false}, cgo_shader_ub_color{false},
      cgo_shader_ub_normal{false};
  bool debug{false};
  bool no_pick{false};
  short render_alpha{0}; // 1 : render CGOSetZVector/CGORenderGLAlpha only
                         // 2 : render both CGOSetZVector/CGORenderGLAlpha and
                         // rest of object calcDepth=1 by default
  short sphere_quality{
      0}; // quality of spheres when simplified or rendered in immediate mode
  ...
  public:
    ...
    {
      return reinterpret_cast<const T*>(m_pc + 1);
    }
    ...
    bool is_stop() const { return m_pc == m_stop || op_code() == CGO_STOP; }
  };
...
CGO* CGOSimplifyNoCompress(const CGO* I, int est, short sphere_quality = -1,
    bool stick_round_nub = true);

// -1 - no lines, 0 - some no interpolation, 1 - all interpolation, 2 - all no
// interpolation
bool CGOCombineBeginEnd(CGO** I, bool do_not_split_lines = false);
CGO* CGOCombineBeginEnd(
    const CGO* I, int est = 0, bool do_not_split_lines = false);

void CGOFreeVBOs(CGO* I);

CGO* CGOOptimizeToVBOIndexed(const CGO* I, int est = 0,
    const float* color = nullptr, bool addshaders = true,
    bool embedTransparencyInfo = false);
#define CGOOptimizeToVBOIndexedWithColorEmbedTransparentInfo(                  \
    I, est, color, addshaders)                                                 \
  CGOOptimizeToVBOIndexed(I, est, color, addshaders, true)
#define CGOOptimizeToVBOIndexedWithColor CGOOptimizeToVBOIndexed
#define CGOOptimizeToVBOIndexedNoShader(I, est)                                \
  CGOOptimizeToVBOIndexed(I, est, nullptr, false)

bool CGOOptimizeToVBONotIndexed(CGO** I);
CGO* CGOOptimizeToVBONotIndexed(
    const CGO* I, int est = 0, bool addshaders = true);

#define CGOOptimizeToVBONotIndexedNoShader(I)                                  \
  CGOOptimizeToVBONotIndexed(I, 0, false)

CGO* CGOOptimizeSpheresToVBONonIndexed(const CGO* I, int est = 0,
    bool addshaders = false, CGO* leftOverCGO = nullptr);
#define CGOOptimizeSpheresToVBONonIndexedNoShader(I, est)                      \
  CGOOptimizeSpheresToVBONonIndexed(I, est, false, nullptr)
...
#define CGOCustomCylinderv(this, ...)                                          \
  (this)->add<cgo::draw::custom_cylinder>(__VA_ARGS__)
#define CGOShaderCylinder(this, ...)                                           \
  (this)->add<cgo::draw::shadercylinder>(__VA_ARGS__)
...
int CGODrawConnector(CGO* I, float* targetPt3d, float* labelCenterPt3d,
    float text_width, float text_height, float* screenOffset,
    float* screenWorldOffset, float* connectorColor, short relativeMode,
    int draw_bkgrd, float bkgrd_transp, float* bkgrd_color,
...
CGO* CGONewFromPyList(
    PyMOLGlobals* G, PyObject* list, int version, bool shouldCombine = true);
...
int CGOCountNumberOfOperationsOfTypeN(
    const CGO* I, const std::set<int>& optype);
int CGOCountNumberOfOperationsOfTypeN(
...
int CGOHasTransparency(
    const CGO* I, bool checkTransp = true, bool checkOpaque = false);
...
bool CGOHasAnyTriangleVerticesWithoutNormals(
    const CGO* I, bool checkTriangles = true);
#define CGOHasAnyLineVerticesWithoutNormals(I)                                 \
  CGOHasAnyTriangleVerticesWithoutNormals(I, false)
...
CGO* CGOConvertToTrilinesShader(
    const CGO* I, CGO* addTo, bool add_color = true);
CGO* CGOConvertToLinesShader(const CGO* I, CGO* addTo, bool add_color = true);

CGO* CGOConvertLinesToCylinderShader(
    const CGO* I, CGO* addTo, bool add_color = true);
CGO* CGOConvertCrossesToCylinderShader(
    const CGO* I, CGO* addTo, float cross_size);
