...
{
  float power =
      SettingGet_f(I->G, nullptr, I->Setting.get(), cSetting_motion_power);
  float bias =
      SettingGet_f(I->G, nullptr, I->Setting.get(), cSetting_motion_bias);
  int simple =
      SettingGet_i(I->G, nullptr, I->Setting.get(), cSetting_motion_simple);
  float linear =
      SettingGet_f(I->G, nullptr, I->Setting.get(), cSetting_motion_linear);
  int hand =
      SettingGet_i(I->G, nullptr, I->Setting.get(), cSetting_motion_hand);
  ...
  */
  ObjectMotion(I, MViewAction::Reinterpolate, -1, -1, power, bias, simple,
      linear, SettingGetGlobal_b(I->G, cSetting_movie_loop) ? 1 : 0, hand, 5, 1,
      -1, 1);
}
    ...
    int count, int target, int freeze, int localize)
{
  ...
  if (I->type == cObjectGroup) { /* propagate */
    ok = ExecutiveGroupMotionModify(
        I->G, I, action, index, count, target, freeze);
  } else {
    ...
    if (ok && I->ViewElem) {
      ...
      if (n_frame != size) {
        ...
        if ((!freeze) &&
            SettingGetGlobal_i(I->G, cSetting_movie_auto_interpolate)) {
...
void ObjectDrawViewElem(
    pymol::CObject* I, BlockRect* rect, int frames, CGO* orthoCGO)
{
  if (I->ViewElem) {
...
int ObjectMotion(pymol::CObject* I, MViewAction action, int first, int last,
    float power, float bias, int simple, float linear, int wrap, int hand,
    int window, int cycles, int state, int quiet)
{
  PyMOLGlobals* G = I->G;
  if (I->type == cObjectGroup) { /* propagate */
    return ExecutiveGroupMotion(G, I, action, first, last, power, bias, simple,
    ...
    if (action == MViewAction::Smooth) {
      ...
      if (SettingGet_b(I->G, nullptr, I->Setting.get(),
              cSetting_movie_auto_interpolate)) {
        action = MViewAction::Reinterpolate;
        last = save_last;
      }
    }
      ...
      if (I->ViewElem && I->TTTFlag) {
          ...
          last = first;
        {
          ...
          if (state >= 0) {
            state_tmp = state;
            state_flag = true;
          } else if (SettingGetIfDefined_i(
                         G, I->Setting.get(), cSetting_state, &state_tmp)) {
            state_flag = true;
            state_tmp--;
          }
...
void ObjectAdjustStateRebuildRange(pymol::CObject* I, int* start, int* stop)
{
  ...
  int async_builds =
      SettingGet_b(I->G, nullptr, I->Setting.get(), cSetting_async_builds);
  int max_threads =
      SettingGet_i(I->G, nullptr, I->Setting.get(), cSetting_max_threads);
  int all_states =
      SettingGet_i(I->G, nullptr, I->Setting.get(), cSetting_all_states);
  int dummy;
  if (all_states)
    return;
 ...
 * or strips them if they are terminal or sequential - if `name` equals a
 * reserved selection keyword, then also append an underscore.
 *
 * @param[in,out] name Object name to validate
...
/**
 * Like pymol::CObject::getCurrentState() but will return `-1` if the
 * `all_states` setting is set.
 *
 ...
 * should use pymol::CObject::getCurrentState() instead of setting
 * `ignore_all_states` to true.
 */
...
PyObject* ObjectAsPyList(const pymol::CObject* I)
{
  ...
  PyList_SetItem(
      result, 10, PyInt_FromLong(static_cast<long>(I->getRenderContext())));
...
int ObjectFromPyList(PyMOLGlobals* G, PyObject* list, pymol::CObject* I)
{
  ...
  if (ok)
    ll = PyList_Size(list);
  if (ok)
    ok = CPythonVal_PConvPyIntToInt_From_List(
        G, list, 0, reinterpret_cast<int*>(&I->type));
  if (ok)
  ...
  if (ok)
    ok = PConvPyListToFloatArrayInPlaceAutoZero(
        PyList_GetItem(list, 4), I->ExtentMin, 3);
  if (ok)
    ok = PConvPyListToFloatArrayInPlaceAutoZero(
        PyList_GetItem(list, 5), I->ExtentMax, 3);
  if (ok)
...
/*========================================================================*/
void ObjectCombineTTT(
    pymol::CObject* I, const float* ttt, int reverse_order, int store)
{
  if (I->type == cObjectGroup) {
    ...
    if (store < 0)
      store = SettingGet_i(
          I->G, I->Setting.get(), nullptr, cSetting_movie_auto_store);
 ...
 */
int ObjectGetTotalMatrix(
    pymol::CObject* I, int state, int history, double* matrix)
{
  int result = false;
  if (I->TTTFlag) {
  ...
  if (!history) {
    history = SettingGet<int>(
                  I->G, I->Setting.get(), nullptr, cSetting_matrix_mode) > 0;
  }
      ...
      if (I->Grabbed) {
        ...
        if (I->ViewElem[frame].state_flag) {
          ...
          if (I->Setting) {
            ...
            SettingSet_i(
                I->Setting.get(), cSetting_state, I->ViewElem[frame].state + 1);
          }
        }
      }
    }
...
/*========================================================================*/
pymol::CObject::CObject(PyMOLGlobals* G)
    : G(G)
{
