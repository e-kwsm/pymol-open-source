...
void MovieViewReinterpolate(PyMOLGlobals* G)
{
  ...
  MovieView(G, MViewAction::Reinterpolate, -1, -1, power, bias,
      1, /* note simple always = 1 for camera motion...*/
      linear, SettingGetGlobal_b(G, cSetting_movie_loop) ? 1 : 0, hand, 5, 1,
      nullptr, 0.5, -1, 1);
}

int MovieXtoFrame(
    PyMOLGlobals* G, BlockRect* rect, int frames, int x, int nearest)
{
    ...
    int count, int target, int freeze, int localize)
{
  ...
  if ((ok = ViewElemModify(G, &I->ViewElem, action, index, count, target))) {
    ...
    case ViewElemAction::Move:
      if ((index >= 0) && (target >= 0) && (index < I->NFrame) &&
          (target < I->NFrame)) {
        int i;
        for (i = 0; i < count; i++) {
...
void MovieCopyPrepare(PyMOLGlobals* G, int* width, int* height, int* length)
{
  ...
  SceneGetWidthHeight(G, width, height);
  {
    ...
    int a;
    /* make sure all the movie frames match the screen size or are pre-rendered
     * and are already the same size */
    for (a = 0; a < nFrame; a++) {
      ...
      if (image) {
        if ((image->getHeight() != *height) || (image->getWidth() != *width)) {
          ...
          if (uniform_height < 0) {
            ...
            if ((image->getHeight() != uniform_height) ||
                (image->getWidth() != uniform_width))
              uniform_flag = false;
          }
        }
      } else
...
int MovieCopyFrame(
    PyMOLGlobals* G, int frame, int width, int height, int rowbytes, void* ptr)
{
  CMovie* I = G->Movie;
  int result = false;
  int nFrame;
  ...
  if ((frame < nFrame) && (ptr)) {
    ...
    if (!I->Image[i]) {
      ...
      if ((I->Image[i]->getHeight() == height) &&
          (I->Image[i]->getWidth() == width)) {
...
int MovieFromPyList(PyMOLGlobals* G, PyObject* list, int* warning)
{
  ...
  if (ok && I->MatrixFlag)
    ok = PConvPyListToFloatArrayInPlace(
        PyList_GetItem(list, 2), I->Matrix, cSceneViewSize);
  if (ok)
  ...
  if (ok && I->NFrame) {
    ...
    if (ok)
      ok = PConvPyListToIntArrayInPlace(
          PyList_GetItem(list, 4), I->Sequence.data(), I->NFrame);
    if (ok)
...
static void MovieModalPNG(PyMOLGlobals* G, CMovie* I, CMovieModal* M)
{
  ...
  case 0: /* setup */
    ...
    {
      auto buffer =
          pymol::string_format("Creating movie (%d frames)...", M->nFrame);
  ...
  case 1: /* RENDER LOOP: advance a frame */
    if (M->frame < M->nFrame) {
      ...
      case cMyPNG_FormatPPM:
        M->fname =
            pymol::string_format("%s%04d.ppm", M->prefix.c_str(), M->frame + 1);
        break;
      case cMyPNG_FormatPNG:
      default:
        M->fname =
            pymol::string_format("%s%04d.png", M->prefix.c_str(), M->frame + 1);
        break;
      }
  ...
  case 2: /* IN RENDER LOOP: create the image */
    ...
    if ((M->frame >=
            M->start) && /* only render frames in the specified interval... */
        (M->frame <= M->stop) &&
        (M->file_missing)) { /* ...that don't already exist */
      if (!I->Image[M->image]) {
        SceneUpdate(G, false);
        if (SceneMakeMovieImage(
                G, false, M->modal, M->mode, M->width, M->height) ||
            !M->modal) {
          M->stage = 3;
        } else {
          /* didn't get an image... */
          PRINTFB(G, FB_Movie, FB_Errors)
          " MoviePNG-Error: unable to obtain a valid OpenGL image.  Trying "
          "again...\n" ENDFB(G);
        }
  ...
  case 3: /* IN RENDER LOOP: have image, so write to file */
    ...
    M->accumTiming += M->timing;
    {
      ...
      double est2 =
          ((M->nFrame - M->frame) / (float) (M->frame + 1)) * M->accumTiming;

      PRINTFB(G, FB_Movie, FB_Details)
      " Movie: frame %4d of %4d, %4.2f sec. (%d:%02d:%02d - %d:%02d:%02d to "
      "go).\n",
          M->frame + 1, M->nFrame, M->timing, (int) (est1 / 3600),
          ((int) (est1 / 60)) % 60, ((int) est1) % 60, (int) (est2 / 3600),
          ((int) (est2 / 60)) % 60, ((int) est2) % 60 ENDFB(G);
    }
    M->stage = 4;
    if (G->Interrupt) {
      M->stage = 5; /* abort */
...
int MoviePNG(PyMOLGlobals* G, const char* prefix, int save, int start, int stop,
    int missing_only, int modal, int format, int mode, int quiet, int width,
    int height)
{
  ...
  *M = CMovieModal();
  ...
  if (SettingGetGlobal_b(G, cSetting_seq_view)) {
    ...
    " MoviePNG-Warning: disabling seq_view, may conflict with movie "
    "export\n" ENDFB(G);
...
/*========================================================================*/
void MovieAppendSequence(
    PyMOLGlobals* G, const char* str, int start_from, bool freeze)
{
  CMovie* I = G->Movie;
  int c = 0;
  int i;
...
/*========================================================================*/
void MovieSetImage(
    PyMOLGlobals* G, int index, std::shared_ptr<pymol::Image> image)
{
  CMovie* I = G->Movie;
...
int MovieSeekScene(PyMOLGlobals* G, int loop)
{
  ...
  if (OVreturn_IS_OK(
          (ret = OVLexicon_BorrowFromCString(G->Lexicon, scene_name)))) {
    if (I->ViewElem) {
    ...
    if ((frame >= 0) && (frame < I->NFrame)) {
      ...
      if (I->ViewElem) {
        if (I->ViewElem[frame].scene_flag) {
          char* st =
              OVLexicon_FetchCString(G->Lexicon, I->ViewElem[frame].scene_name);
...
void MovieSetCommand(PyMOLGlobals* G, int frame, const char* command)
{
  ...
  if ((frame >= 0) && (frame < I->NFrame)) {
    ...
    PRINTFB(G, FB_Movie, FB_Errors)
    " Movie-Error: frame %d does not exist.  Use 'mset' to define movie "
    "first.\n",
        frame + 1 ENDFB(G);
  }
}
...
/*========================================================================*/
int MovieView(PyMOLGlobals* G, MViewAction action, int first, int last,
    float power, float bias, int simple, float linear, int wrap, int hand,
    int window, int cycles, const char* scene_name, float scene_cut, int state,
    int quiet)
{
  CMovie* I = G->Movie;
  int frame;
  ...
  case MViewAction::Reinterpolate:
    if (I->ViewElem) {
      ...
      for (frame = first; frame <= last; frame++) {
        if (!first_view) {
          ...
          if (I->ViewElem[frame].specification_level == 2) { /* specified */
            ...
            if (interpolate_flag) {
              ViewElemInterpolate(G, first_view, last_view, power, bias, simple,
                  linear, hand, scene_cut);
            }
      ...
      if ((!view_found) && (last >= first) && (first >= 0) &&
          (last <= I->NFrame)) {
        UtilZeroMem(
            I->ViewElem + first, sizeof(CViewElem) * (1 + (last - first)));
      }
...
int CMovie::click(int button, int x, int y, int mod)
{
  ...
  case P_GLUT_RIGHT_BUTTON: {
    ...
    if (mod == (cOrthoCTRL | cOrthoSHIFT))
      I->DragColumn = true;
    if (mod == (cOrthoSHIFT))
      ExecutiveMotionClick(
          G, &tmpRect, cMovieDragModeCopyKey, count, x, y, false);
    else
      ExecutiveMotionClick(
          G, &tmpRect, cMovieDragModeMoveKey, count, x, y, false);
    if (I->DragStartFrame < n_frame) {
      I->DragDraw = true;
      I->DragMenu = true;
      OrthoDirty(G);
    } else {
      ExecutiveMotionMenuActivate(
          G, &tmpRect, count, false, x, y, I->DragColumn);
    }
  } break;
  case P_GLUT_LEFT_BUTTON: {
    ...
    case (cOrthoSHIFT | cOrthoCTRL):
      I->DragColumn = true;
      ExecutiveMotionClick(
          G, &tmpRect, cMovieDragModeInsDel, count, x, y, true);
      I->DragDraw = true;
      OrthoDirty(G);
      break;
    case cOrthoCTRL:
  ...
  case P_GLUT_MIDDLE_BUTTON: {
    ...
    case cOrthoCTRL:
      I->DragDraw = true;
      ExecutiveMotionClick(
          G, &tmpRect, cMovieDragModeOblate, count, x, y, false);
      break;
    default:
      I->m_ScrollBar.click(button, x, y, mod);
      break;
...
int CMovie::drag(int x, int y, int mod)
{
  ...
  if (I->DragMode) {
    ...
    case cMovieDragModeCopyKey: {
      ...
      if (I->DragStartFrame < n_frame) {
        if ((abs(x - I->DragX) > 3) || (abs(y - I->DragY) > 5)) {
          I->DragMenu = false;
        }
        OrthoDirty(G);
      }
    ...
    case cMovieDragModeOblate:
      I->DragCurFrame =
          ViewElemXtoFrame(&I->DragRect, MovieGetLength(G), x, false);
      OrthoDirty(G);
      break;
    case cMovieDragModeInsDel:
      I->DragCurFrame =
          ViewElemXtoFrame(&I->DragRect, MovieGetLength(G), x, true);
      OrthoDirty(G);
      break;
    }
  }
  ...
  if (I->DragMode) {
    ...
    case cMovieDragModeMoveKey:
      if ((I->DragCurFrame == I->DragStartFrame) && (I->DragMenu)) {
        ...
        tmpRect.right -= I->LabelIndent;
        ExecutiveMotionMenuActivate(
            G, &tmpRect, count, true, x, y, I->DragColumn);
        I->DragMenu = false;
      } else if (I->DragDraw && (I->DragCurFrame != I->DragStartFrame) &&
                 (I->DragCurFrame >= 0) && (I->DragCurFrame < n_frame)) {
        buffer = pymol::string_format("cmd.mmove(%d,%d,%d%s)",
            1 + I->DragCurFrame, 1 + I->DragStartFrame, 1, extra);
      }
      break;
    case cMovieDragModeCopyKey:
      if ((I->DragCurFrame == I->DragStartFrame) && (I->DragMenu)) {
        ...
        I->DragMenu = false;
                 ...
                 (I->DragCurFrame >= 0) && (I->DragCurFrame < n_frame)) {
        buffer = pymol::string_format("cmd.mcopy(%d,%d,%d%s)",
            1 + I->DragCurFrame, 1 + I->DragStartFrame, 1, extra);
      }
      break;
    case cMovieDragModeOblate:
      if (I->DragDraw) {
        int min_frame = (I->DragStartFrame < I->DragCurFrame)
                            ? I->DragStartFrame
                            : I->DragCurFrame;
        int max_frame = (I->DragStartFrame > I->DragCurFrame)
                            ? I->DragStartFrame
                            : I->DragCurFrame;
        if (min_frame < 0)
          min_frame = 0;
    ...
    case cMovieDragModeInsDel:
      if (I->DragDraw) {
        ...
        if (I->DragCurFrame > I->DragStartFrame) {
          ...
          if (first < 0)
            first = 0;
          buffer = pymol::string_format("cmd.minsert(%d,%d%s)",
              I->DragCurFrame - I->DragStartFrame, first, extra);
        } else {
          int first = I->DragCurFrame;
          if (first < 0)
            first = 0;
          buffer = pymol::string_format("cmd.mdelete(%d,%d%s)",
              I->DragStartFrame - I->DragCurFrame, first + 1, extra);
        }
      }
      break;
    }
...
int MovieGetPanelHeight(PyMOLGlobals* G)
{
  ...
  if (movie_panel) {
    int row_height =
        DIP2PIXEL(SettingGetGlobal_i(G, cSetting_movie_panel_row_height));
    I->PanelActive = true;
...
void MovieDrawViewElem(
    PyMOLGlobals* G, BlockRect* rect, int frames, CGO* orthoCGO)
{
  CMovie* I = G->Movie;
  if (I->ViewElem) {
...
void CMovie::draw(CGO* orthoCGO)
{
  ...
  if (I->PanelActive) {
    ...
    if (count) {
      ...
      I->m_ScrollBar.setBox(
          tmpRect.top, tmpRect.left, tmpRect.bottom, tmpRect.right);
      {
      ...
      if (I->DragDraw) {
        ...
        case cMovieDragModeCopyKey: {
          ...
          if (I->DragStartFrame < n_frame)
            ViewElemDrawBox(G, &I->DragRect, I->DragStartFrame,
                I->DragStartFrame + 1, n_frame, white, false, orthoCGO);
          if ((I->DragCurFrame >= 0) && (I->DragCurFrame < n_frame)) {
            ViewElemDrawBox(G, &I->DragRect, I->DragCurFrame,
                I->DragCurFrame + 1, n_frame, grey, true, orthoCGO);
          }
        } break;
        case cMovieDragModeOblate: {
          float grey[4] = {0.75F, 0.75F, 0.75f, 0.5};
          ...
          if (max_frame >= n_frame)
            max_frame = n_frame - 1;
          ViewElemDrawBox(G, &I->DragRect, min_frame, max_frame + 1, n_frame,
              white, false, orthoCGO);
          ViewElemDrawBox(G, &I->DragRect, min_frame, max_frame + 1, n_frame,
              grey, true, orthoCGO);
        } break;
        case cMovieDragModeInsDel:
          if (I->DragCurFrame == I->DragStartFrame) {
            ViewElemDrawBox(G, &I->DragRect, I->DragStartFrame,
                I->DragStartFrame, n_frame, white, true, orthoCGO);
          } else if (I->DragCurFrame >= I->DragStartFrame) {
            float green[4] = {0.5F, 1.0F, 0.5F, 0.5F};
            ViewElemDrawBox(G, &I->DragRect, I->DragStartFrame, I->DragCurFrame,
                n_frame, green, true, orthoCGO);
          } else {
            float red[4] = {1.0F, 0.5F, 0.5F, 0.5F};
            ViewElemDrawBox(G, &I->DragRect, I->DragCurFrame, I->DragStartFrame,
                n_frame, red, true, orthoCGO);
          }
          break;
        }
      }
