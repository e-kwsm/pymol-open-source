...
static void RayRelease(CRay* I);

static void RayApplyMatrix33(
    unsigned int n, float3* q, const float m[16], float3* p);
static void RayApplyMatrixInverse33(
    unsigned int n, float3* q, const float m[16], float3* p);
static void RayTransformNormals33(
    unsigned int n, float3* q, const float m[16], float3* p);
static void RayTransformInverseNormals33(
    unsigned int n, float3* q, const float m[16], float3* p);
static void RayProjectTriangle(
    CRay* I, RayInfo* r, float* light, float* v0, float* n0, float scale);
...
float RayGetScreenVertexScale(CRay* I, float* v1)
{
  ...
  if (I->Ortho) {
    ratio = 2 * (float) (fabs(I->Pos.z) * tan((I->Fov / 2.0) * cPI / 180.0)) /
            (I->Height);
  } else {
    float front_size = 2 * I->Volume[4] *
                       ((float) tan((I->Fov / 2.0F) * PI / 180.0F)) /
                       (I->Height);
...
static void RayApplyContextToVertex(CRay* I, float* v)
{
  ...
  case pymol::RenderContext::UnitWindow: {
    ...
    if (!SettingGetGlobal_b(I->G, cSetting_ortho)) {
      ...
      v[0] -= 0.5F;
      v[1] -= 0.5F;
      v[0] = scale * v[0] * I->Range[0] / tw +
             (I->Volume[0] + I->Volume[1]) / 2.0F;
      v[1] = scale * v[1] * I->Range[1] / th +
             (I->Volume[2] + I->Volume[3]) / 2.0F;
...
static void compute_background_for_pixel(unsigned char* bkrd,
    short isOutsideInY, int bg_image_mode, const float* bg_image_tilesize,
    const float* bg_rgb, int bg_image_linear, unsigned char* bkgrd_data,
    int bkgrd_width, int bkgrd_height, float w, float wr, float hl,
    float hpixelx, short blend_bg)
{
  ...
  case 2: // isTiled
    wl = bkgrd_width *
         (fmodpos((float) w, bg_image_tilesize[0]) / bg_image_tilesize[0]);
    break;
  ...
  if (isBackground) {
    ...
    float valx[4] = {0.f, 0.f, 0.f, 0.f}, valy[4] = {0.f, 0.f, 0.f, 0.f},
          val[4] = {0.f, 0.f, 0.f, 0.f};
    int xd = (xp > 0.f) ? 1 : -1, yd = (yp > 0.f) ? 1 : -1;
    int wlfn = (wlf + xd + bkgrd_width) % bkgrd_width,
        hlfn = (hlf + yd + bkgrd_height) % bkgrd_height;
    ...
    copy4f(&bkgrd_data[(bkgrd_width * (int) floor(hl) + (int) floor(wl)) * 4],
        bkrd);
  }
...
static void fill_background_image(
    CRay* I, unsigned int* buffer, int width, int height, unsigned int cnt)
{
  int bkgrd_width = I->bkgrd_data->getWidth(),
      bkgrd_height = I->bkgrd_data->getHeight();
  ...
  auto bg_image_tilesize =
      SettingGet<const float*>(I->G, cSetting_bg_image_tilesize);
  ...
  tmpf =
      ColorGet(I->G, SettingGet_color(I->G, nullptr, nullptr, cSetting_bg_rgb));
  ...
  for (h = 0; h < height; h++) {
    ...
    case 2: // isTiled
      hl = bkgrd_height *
           (fmodpos((float) h, bg_image_tilesize[1]) / bg_image_tilesize[1]);
      break;
    case 3: // isCenteredRepeated
    ...
    for (w = 0; w < width; w++) {
      compute_background_for_pixel(bkrd, isOutsideInY, bg_image_mode,
          bg_image_tilesize, bg_rgb, bg_image_linear, bkgrd_data, bkgrd_width,
          bkgrd_height, w, wr, hl, hpixelx, opaque_back);
      if (I->BigEndian) {
        value = ((0xFF & bkrd[0]) << 24) | ((0xFF & bkrd[1]) << 16) |
                ((0xFF & bkrd[2]) << 8) | (0xFF & bkrd[3]);
      } else {
        value = ((0xFF & bkrd[3]) << 24) | ((0xFF & bkrd[2]) << 16) |
                ((0xFF & bkrd[1]) << 8) | (0xFF & bkrd[0]);
      }
      if (opaque_back) {
...
static void fill_gradient(CRay* I, int opaque_back, unsigned int* buffer,
    float* bkrd_bottom, float* bkrd_top, int width, int height,
    unsigned int cnt)
{
...
static void RayReflectAndTexture(CRay* I, RayInfo* r, int perspective)
{
  if (r->prim->wobble)
    ...
    case 4: {
      ...
      n[2] =
          I->Random[0xFF & (int) ((cos((v[2]) * tp[1]) * 256 * tp[2] + 148))];
      RayTransformNormals33(1, &n, I->ModelView, &n);
      scale3f(n, tp[0], n);
      add3f(n, r->surfnormal, r->surfnormal);
      normalize3f(r->surfnormal);
...
int RayExpandPrimitives(CRay* I)
{
  ...
  VLACacheSize(
      I->G, basis->Vert2Normal, int, nVert, 0, cCache_basis_vert2normal);
  ...
  for (a = 0; ok && a < I->NPrimitive; a++) {
    ...
    case cPrimCharacter:
      ...
      basis->Radius2[nVert] =
          I->Primitive[a].r1 * I->Primitive[a].r1; /*necessary?? */
    ...
    case cPrimSphere:
      ...
      basis->Radius[nVert] = I->Primitive[a].r1;
      basis->Radius2[nVert] =
          I->Primitive[a].r1 * I->Primitive[a].r1; /*precompute */
...
#define minmax(v, r)                                                           \
  ...
  float xmin = 0.0F, ymin = 0.0F, xmax = 0.0F, ymax = 0.0F, zmin = 0.0F,
        zmax = 0.0F;
...
int RayTransformFirst(CRay* I, int perspective, int identity)
{
  ...
  int two_sided_lighting =
      SettingGetGlobal_b(I->G, cSetting_two_sided_lighting);
  ...
  if (two_sided_lighting ||
      ...
      (SettingGetGlobal_i(I->G, cSetting_ray_interior_color) != -1) ||
      I->CheckInterior)
    backface_cull = 0;
  ...
  if (ok) {
    VLACacheSize(I->G, basis1->Vertex, float, 3 * basis0->NVertex, 1,
        cCache_basis_vertex);
    CHECKOK(ok, basis1->Vertex);
  }
  if (ok) {
    VLACacheSize(I->G, basis1->Normal, float, 3 * basis0->NNormal, 1,
        cCache_basis_normal);
  ...
  if (ok) {
    VLACacheSize(
        I->G, basis1->Radius, float, basis0->NVertex, 1, cCache_basis_radius);
  ...
  if (ok) {
    if (identity) {
      UtilCopyMem(
          basis1->Vertex, basis0->Vertex, basis0->NVertex * sizeof(float) * 3);
    } else {
      RayApplyMatrix33(basis0->NVertex, (float3*) basis1->Vertex, I->ModelView,
          (float3*) basis0->Vertex);
    }
  }
  ok &= !I->G->Interrupt;
  ...
  if (ok) {
    ...
    memcpy(basis1->Vert2Normal, basis0->Vert2Normal,
        basis0->NVertex * sizeof(int));
  }
  ok &= !I->G->Interrupt;
  if (ok) {
  ...
  if (ok) {
    if (identity) {
      UtilCopyMem(
          basis1->Normal, basis0->Normal, basis0->NNormal * sizeof(float) * 3);
    } else {
  ...
  if (perspective) {
    ...
    for (a = 0; ok && a < I->NPrimitive; a++) {
      ...
      case cPrimCharacter:
        ...
        prm->cull = (!identity) && backface_cull &&
                    ((v0[2] < 0.0F) && (v0[5] < 0.0F) && (v0[8] < 0.0F));
        break;
      case cPrimCone:
      case cPrimSausage:
      case cPrimCylinder:
        BasisCylinderSausagePrecompute(
            basis1->Normal + basis1->Vert2Normal[prm->vert] * 3,
            basis1->Precomp + basis1->Vert2Normal[prm->vert] * 3);
        break;
      }
      ok &= !I->G->Interrupt;
...
/*========================================================================*/
void RayRenderTest(
    CRay* I, int width, int height, float front, float back, float fov)
{
...
#define convert_col(c)                                                         \
  (0xFF000000 | (((int) (c[0] * 255.0)) << 16) |                               \
      (((int) (c[1] * 255.0)) << 8) | (((int) (c[2] * 255.0))))
...
void RayRenderVRML1(CRay* I, int width, int height, char** vla_ptr, float front,
    float back, float fov, float angle, float z_corr)
{
  char* vla = *vla_ptr;
...

  {
    ...
    sprintf(buffer, "    %8.6f %8.6f %8.6f 1.0\n",
        (I->Volume[0] + I->Volume[1]) / 2, (I->Volume[2] + I->Volume[3]) / 2,
        0.0F);
    UtilConcatVLA(&vla, &cc, buffer);
...
void RayRenderVRML2(CRay* I, int width, int height, char** vla_ptr, float front,
    float back, float fov, float angle, float z_corr)
{
  ...
  if (!identity) {
    sprintf(buffer,
        ...
        /* WLD: only write the viewpoint which matches PyMOL
           ...
           "}\n" */
        ,
        -z_corr, /* *0.96646  for some reason, PyMOL and C4D cameras differ by
                    about 3.5% ... */
        h_fov
        /*(wid[2] + wid[1]),
           (wid[0] + wid[1]),
    ...
    for (a = 0; a < I->NPrimitive; a++) {
      ...
      if (prim->type == cPrimTriangle) {
        ...
        for (b = mesh_start; b < a; b++) {
          ...
          sprintf(buffer,
              ...
              "%6.4f %6.4f %6.4f,\n"
              "%6.4f %6.4f %6.4f,\n",
              cprim->c1[0], cprim->c1[1], cprim->c1[2], cprim->c2[0],
              cprim->c2[1], cprim->c2[2], cprim->c3[0], cprim->c3[1],
              cprim->c3[2]);
          UtilConcatVLA(&vla, &cc, buffer);
        }
      ...
      case cPrimSphere:
        sprintf(buffer,
            ...
            " }\n"
            "}\n",
            vert[0] - mid[0], vert[1] - mid[1], vert[2] - mid[2], prim->r1,
            prim->c1[0], prim->c1[1], prim->c1[2]);
        UtilConcatVLA(&vla, &cc, buffer);
        break;
      case cPrimCone:
      ...
      case cPrimSausage: {
        ...
        if (prim->type == cPrimSausage) {
          ...
          sprintf(geometry,
              ...
              "                       shininess 0.8 }\n"
              "   }\n",
              prim->r1, prim->l1, (prim->c1[0] + prim->c2[0]) / 2,
        ...
        sprintf(buffer,
            ...
            " ]\n"
            "}\n",
            vert2[0] - mid[0], vert2[1] - mid[1], vert2[2] - mid[2], axis[0],
            axis[1], axis[2], angle, geometry);
        UtilConcatVLA(&vla, &cc, buffer);
      } break;
      case cPrimTriangle:
...
static int VectorHash_GetOrSetKeyValue(
    VectorHash* I, float* key, float* alpha, int* value)
{
  unsigned int hash;
...
static void unique_vector_add(VectorHash* vh, float* vector,
    float* vector_array, int* vector_count, int* index_array, int* index_count)
{
...
static void unique_color_add(VectorHash* vh, float* vector, float* vector_array,
    int* vector_count, int* index_array, int* index_count, float alpha)
{
  int index = *vector_count;
  ...
  case 1: {
    float* vector_slot =
        vector_array + 4 * (*vector_count); /* NOTE 4x spacing */
    copy3f(vector, vector_slot);
    vector_slot[3] = alpha;
    (*vector_count)++;
  }
...
static ov_size idtf_dump_model_nodes(
    char** vla, ov_size cnt, IdtfResourceMesh* mesh_vla, int n_mesh)
{
  int a;
...
static ov_size idtf_dump_resource_header(char** vla, ov_size cnt)
{
  ...
  UtilConcatVLA(
      vla, &cnt, "\t\tLIGHT_ATTENUATION 1.000000 0.000000 0.000000\n");
...
static ov_size idtf_dump_resources(char** vla, ov_size cnt,
    IdtfResourceMesh* mesh_vla, int n_mesh, IdtfMaterial* material)
{
  {
    OrthoLineType buffer;
...

  {
...

    {
      ...
      for (c = 0; c < n_color; c++) {
        ...
        UtilConcatVLA(
            vla, &cnt, "\t\tMATERIAL_SPECULAR 0.750000 0.750000 0.750000\n");
...
#ifdef IDTF_COLOR
        sprintf(
            buffer, "\t\t\tMODEL_DIFFUSE_COLOR_COUNT %d\n", mesh->color_count);
        UtilConcatVLA(vla, &cnt, buffer);

        sprintf(
            buffer, "\t\t\tMODEL_SPECULAR_COLOR_COUNT %d\n", mesh->color_count);
        UtilConcatVLA(vla, &cnt, buffer);
#else
    ...
    if (mesh_vla) {
      ...
      for (a = 0; a < I->NPrimitive; a++) {
        ...
        case cPrimSphere:
          if (!mesh) {
            ...
            if (VLACheck(mesh_vla, IdtfResourceMesh, mesh_cnt)) {
              ...
              if ((mesh->face_position_list = VLACalloc(int, 3)) &&
                  (mesh->face_normal_list = VLACalloc(int, 3)) &&
                  (mesh->face_shading_list =
                          VLACalloc(int, 1)) && /* defaults to zero */
        ...
        case cPrimTriangle:
          if (mesh) {
            if (VLACheck(
                    mesh->face_position_list, int, mesh->face_count * 3 + 2) &&
                VLACheck(
                    mesh->face_normal_list, int, mesh->face_count * 3 + 2) &&
                VLACheck(mesh->face_shading_list, int, mesh->face_count) &&
                VLACheck(mesh->model_position_list, float,
                    (mesh->position_count + 3) * 3) &&
                VLACheck(
                    mesh->face_color_list, int, mesh->face_count * 3 + 2) &&
                VLACheck(mesh->model_diffuse_color_list, float,
                    (mesh->color_count + 3) * 4) &&
                VLACheck(mesh->model_normal_list, float,
                    (mesh->normal_count + 3) * 3)) {

              float* vert = base->Vertex + 3 * (prim->vert);
              float* norm =
                  base->Normal + 3 * base->Vert2Normal[prim->vert] + 3;
              ...
              unique_color_add(mesh->normal_hash, prim->c1,
                  ...
                  mesh->face_color_list, &face_color_count, 1.0F - prim->trans);
              if (reverse) {
                vert += 6;
                norm += 6;
                unique_vector_add(mesh->position_hash, vert,
...

      {
        ...
        if (material && (material->color_list = VLAlloc(float, 4)) &&
          ...
          for (a = 0; a < mesh_cnt; a++) {
            ...
            for (b = 0; b < mesh->face_count; b++) {
              ...
              if (VLACheck(material->color_list, float,
                      material->color_count * 4 + 3)) {
          ...
          }
          {
            ...
            cnt = idtf_dump_resources(
                rsrc_vla, cnt, mesh_vla, mesh_cnt, material);
...
void RayRenderObjMtl(CRay* I, int width, int height, char** objVLA_ptr,
    char** mtlVLA_ptr, float front, float back, float fov, float angle,
    float z_corr)
{
...
void RayRenderPOV(CRay* I, int width, int height, char** headerVLA_ptr,
    char** charVLA_ptr, float front, float back, float fov, float angle,
    int antialias)
{
  float fog;
  const float* bkrd;
  ...
  smooth_color_triangle =
      SettingGetGlobal_b(I->G, cSetting_smooth_color_triangle);
  PRINTFB(I->G, FB_Ray, FB_Blather)
...

  {
    ...
    if (!identity) {
      if (!ortho) {
        sprintf(buffer,
            "camera {direction<0.0,0.0,%8.3f>\n location <0.0 , 0.0 , 0.0>\n "
            "right %12.10f*x up y \n }\n",
            -57.3F * cos(fov * cPI / (180 * 2.4)) /
                fov, /* by trial and error */
            I->Range[0] / I->Range[1]);
      } else {
        sprintf(buffer,
            "camera {orthographic location <0.0 , 0.0 , %12.10f>\nlook_at  "
            "<0.0 , 0.0 , -1.0> right %12.10f*x up %12.10f*y}\n",
      ...
      if (!ortho) {
        sprintf(buffer,
            "camera {angle %12.10f "
            "sky<%12.10f,%12.10f,%12.10f>\nlocation<%12.10f,%12.10f,%12.10f>"
            "\nlook_at<%12.10f,%12.10f,%12.10f> right %12.10f*x up y }\n",
            fov * I->Range[0] / I->Range[1], view[1], view[5], view[9], loc[0],
            loc[1], loc[2], look[0], look[1], look[2],
            -I->Range[0] / I->Range[1]);
      } else {
        sprintf(buffer,
            "camera {orthographic "
            "sky<%12.10f,%12.10f,%12.10f>\nlocation<%12.10f,%12.10f,%12.10f>"
            "\nlook_at<%12.10f,%12.10f,%12.10f> right %12.10f*x up "
            "%12.10f*y}\n",
            view[1], view[5], view[9], loc[0], loc[1], loc[2], look[0], look[1],
            look[2], -I->Range[0], I->Range[1]);
      }
    }
...

  {
    float ambient = SettingGetGlobal_f(I->G, cSetting_ambient) +
                    SettingGetGlobal_f(I->G, cSetting_direct);
    ...
    sprintf(buffer,
        "#default { finish{phong %8.3f ambient %8.3f diffuse %8.3f phong_size "
        "%8.6f}}\n",
        SettingGetGlobal_f(I->G, cSetting_spec_reflect), ambient, reflect,
        spec_power);
    UtilConcatVLA(&headerVLA, &hc, buffer);
  }
  ...
  if (!identity) {
    ...
    if (opaque_back) { /* drop a plane into the background for the background
                          color */
      sprintf(buffer,
  ...
  for (a = 0; a < I->NPrimitive; a++) {
    ...
    case cPrimSausage:
      ...
      sprintf(buffer, "pigment{color rgb<%6.4f1,%6.4f,%6.4f>}}\n",
          (prim->c1[0] + prim->c2[0]) / 2, (prim->c1[1] + prim->c2[1]) / 2,
    ...
    case cPrimTriangle:
      norm = base->Normal + 3 * base->Vert2Normal[prim->vert] +
             3; /* first normal is the average */

      if (!TriangleDegenerate(
              vert, norm, vert + 3, norm + 3, vert + 6, norm + 6)) {
        ...
        if (smooth_color_triangle) {
          sprintf(buffer,
              ...
              prim->c1[1], prim->c1[2], vert[3], vert[4], vert[5], norm[3],
              norm[4], norm[5], prim->c2[0], prim->c2[1], prim->c2[2], vert[6],
              vert[7], vert[8], norm[6], norm[7], norm[8], prim->c3[0],
              prim->c3[1], prim->c3[2]);
          UtilConcatVLA(&charVLA, &cc, buffer);
        } else {
          ...
          sprintf(buffer,
              ...
              "3,\n<%12.10f,%12.10f,%12.10f>,\n<%12.10f,%12.10f,%12.10f>,\n<%"
              "12.10f,%12.10f,%12.10f>}\n",
              vert[0], vert[1], vert[2], vert[3], vert[4], vert[5], vert[6],
              vert[7], vert[8], norm[0], norm[1], norm[2], norm[3], norm[4],
          ...
          sprintf(buffer,
              "texture { pigment{color rgb<%6.4f1,%6.4f,%6.4f> %s}}\n",
...
int RayHashThread(CRayHashThreadInfo* T)
{
  ...
  /* utilize a little extra wasted CPU time in thread 0 which computes the
   * smaller map... */
  if (!T->phase) {
    if (T->ray->bkgrd_data) {
      fill_background_image(T->ray, T->image, T->width, T->height,
          T->width * (unsigned int) T->height);
    } else if (T->bkrd_is_gradient) {
      fill_gradient(T->ray, T->opaque_back, T->image, T->bkrd_top,
          T->bkrd_bottom, T->width, T->height,
          T->width * (unsigned int) T->height);
    } else {
...
{   /* can only be called for a pixel NOT on the edge */
  { /* color testing */
        ...
        compare7, compare8;
    {
      ...
      if (back_two)
        threshold =
            (threshold >> 1); // halve threshold for pixels that hit background
    }

    {
      int current;
      unsigned int shift = 0;
      int sum1 = 0, sum2 = 3, sum3 = 0, sum4 = 0, sum5 = 0, sum6 = 0, sum7 = 0,
          sum8 = 0;
      int a;
      for (a = 0; a < 4; a++) {
...
static void RayPrimGetColorRamped(
    PyMOLGlobals* G, float* matrix, RayInfo* r, float* fc)
{
  ...
  case cPrimSausage:
    ...
    /* TODO : here is where we would set the correct color if we implemented
       ...
       parameter/variable to the primitive to tell this function what
       interpolation to do */
...
int RayTraceThread(CRayThreadInfo* T)
{
  ...
  const float shadow_decay =
      SettingGetGlobal_f(I->G, cSetting_ray_shadow_decay_factor);
  const float shadow_range =
      SettingGetGlobal_f(I->G, cSetting_ray_shadow_decay_range);
  ...
  float hl;
  float wr = T->bgWidth / (float) T->width,
        hr = T->bgHeight / (float) T->height;
  float bg_rgb[3];
  const float* tmpf;
  int chromadepth;
  ...
  interior_reflect =
      1.0F - SettingGetGlobal_f(I->G, cSetting_ray_interior_reflect);
  ...
  oblique_power =
      SettingGetGlobal_f(I->G, cSetting_ray_transparency_oblique_power);
  ...
  /* divide up the reflected light component over all lights */
  {
    ...
    lreflect = reflect_scale *
               (SettingGetGlobal_f(I->G, cSetting_reflect) - ray_scatter);
    if (lreflect < _0)
      lreflect = _0;
  ...
  SceneGetAdjustedLightValues(I->G, &settingSpecReflect, &settingSpecPower,
      &settingSpecDirect, &settingSpecDirectPower, 10);
  ...
  if (perspective) {
    ...
    height_range =
        (T->front) * 2 * ((float) tan((T->fov / 2.0F) * PI / 180.0F));
  ...
  MapCacheInit(
      &BasisCall[0].cache, I->Basis[1].Map, T->phase, cCache_map_scene_cache);
  ...
  for (yy = T->y_start; (yy < T->y_stop); yy++) {
    ...
    y = T->y_start +
        ((yy - T->y_start) + offset) %
            (render_height); /* make sure threads write to different pages */
    if (T->bkrd_data) {
      switch (bg_image_mode) {
      case 1: // isCentered
      {
      ...
      case 2: // isTiled
        hl = (float) T->bgHeight *
             (fmodpos((float) y, bg_image_tilesize[1]) / bg_image_tilesize[1]);
        break;
      case 3: // isCenteredRepeated
    ...
    if ((!T->phase) && !(yy & 0xF)) { /* don't slow down rendering too much */
      if (T->edging_cutoff) {
        if (T->edging) {
          OrthoBusyFast(
              I->G, (int) (2.5F * T->height / 3 + 0.5F * y), 4 * T->height / 3);
        } else {
          OrthoBusyFast(
              I->G, (int) (T->height / 3 + 0.5F * y), 4 * T->height / 3);
        }
      } else {
    ...
    if ((y % T->n_thread) == T->phase) { /* this is my scan line */
      ...
      for (x = T->x_start; (x < T->x_stop); x++) {
        if (T->bkrd_data) {
          ...
          compute_background_for_pixel(bkrd_uc, isOutsideInY, bg_image_mode,
              bg_image_tilesize, bg_rgb, bg_image_linear,
              (unsigned char*) T->bkrd_data, T->bgWidth, T->bgHeight, x, wr, hl,
              hpixelx, orig_opaque_back);
        ...
        while (1) {
          if (T->edging) {
            if (!edge_sampling) {
              if (x && y && (x < (T->width - 1)) &&
                  (y < (T->height - 1))) { /* not on the edge... */
                if (find_edge(T->edging + (pixel - T->image),
                        depth + (pixel - T->image), T->width, T->edging_cutoff,
                        bkrd_value)) {
            ...
            if (edge_sampling) {
              if (edge_cnt == 5) {
                ...
                if (orig_opaque_back && c4 < 255) {
                  ...
                  pixel_c[0] =
                      pymol_roundf(bkpart * bkrd[0] * 255 + ppart * pixel_c[0]);
                  pixel_c[1] =
                      pymol_roundf(bkpart * bkrd[1] * 255 + ppart * pixel_c[1]);
                  pixel_c[2] =
                      pymol_roundf(bkpart * bkrd[2] * 255 + ppart * pixel_c[2]);
                  pixel_c[3] = 255;
                }
          ...
          while ((persist > _persistLimit) && (pass <= max_pass)) {
            ...
            if (((i >= 0) || interior_flag) && (pass < max_pass)) {
              ...
              if (interior_flag) {
                ...
                if (interior_wobble >= 0) {
                  wobble_save =
                      r1.prim->wobble; /* This is a no-no for multithreading! */
                ...
                default: /* sphere, cylinder, sausage, etc. */
                  ...
                  if (r1.prim->ramped) {
                    ...
                    case cPrimCone:
                      ft = r1.tri1;
                      fc[0] =
                          (r1.prim->c1[0] * (_1 - ft)) + (r1.prim->c2[0] * ft);
                      fc[1] =
                          (r1.prim->c1[1] * (_1 - ft)) + (r1.prim->c2[1] * ft);
                      fc[2] =
                          (r1.prim->c1[2] * (_1 - ft)) + (r1.prim->c2[2] * ft);
                      break;
                    default:
                      fc[0] = r1.prim->c1[0];
                      fc[1] = r1.prim->c1[1];
                  ...
                  if ((r1.surfnormal[2] > _0) || two_sided_lighting) {
                    ...
                    if (oblique_factor != _1) {
                      if (oblique_factor > _p5) {
                        oblique_factor =
                            (float) (_p5 +
                                     _p5 * (_1 - pow((_1 - oblique_factor) * _2,
                                                     oblique_power)));
                      } else {
                        oblique_factor = (float) (_p5 * pow(oblique_factor * _2,
                                                            oblique_power));
                      }
                    }
                    r1.trans *= (trans_oblique * oblique_factor) +
                                (1.0F - trans_oblique);
                    if (r1.trans < 0.06F)
                      r1.trans =
                          0.06F; /* don't allow transparent to become opaque */
                  }
                }
...

              {
                ...
                direct_cmp = legacy_1m * pow_surfnormal2 + /* new model */
                             legacy * ((float) (dotgle + pow_dotgle) *
                                          _p5); /* legacy model */
              }
                  ...
                  if (lit > _0) {
                    {
                      ...
                      reflect_cmp +=
                          legacy_1m *
                              ((float) (lit * pow_dotgle)) + /* new model */
                          legacy * ((float) (lit * (dotgle + pow_dotgle) *
                                             _p5)); /* legacy model */
                    }
                    ...
                    if (bc < (spec_count + 2)) {
                      ...
                      if (spec_local && perspective) {
                        ...
                        dotgle = -dot_product3f(r1.surfnormal,
                            bp->SpecNormal); /* fast OpenGL-like global specular
                                              */
                      }
                      if (dotgle < _0)
                        dotgle = _0;
                      if (r1.prim->type != cPrimCharacter) {
                        excess += (float) (pow(dotgle, settingSpecPower) *
                                           settingSpecReflect * lit);
                      } else {
                        excess +=
                            (float) (pow(dotgle, settingSpecPower) *
                                     settingSpecReflect * lit * ray_lab_spec);
                      }
                    }
                  }
                }
              ...
              bright =
                  ambient +
                  (((_1 - direct_shade) + direct_shade * lit) * direct *
                          direct_cmp +
                      lreflect * reflect_cmp *
                          (legacy_1m + legacy * direct_cmp)); /* blend legacy */
              if (excess > _1)
                excess = _1;
              if (bright > _1)
                bright = _1;
              ...
              if (n_basis_tmp && fogFlagTmp) {
                ...
                if (!pass) {
                  if (r1.trans < trans_spec_cut) {
                    ...
                    first_excess = excess * ray_trans_spec * trans_spec_scale *
                                   (_1 - r1.trans);
                  }
                } else {
                  fc[0] += first_excess;
            ...
            if (pixel_flag) {
              ...
              if (orig_opaque_back) {
                if (I->BigEndian)
                  *pixel =
                      T->fore_mask | (cc0 << 24) | (cc1 << 16) | (cc2 << 8);
                else
            ...
            if (pass && persist < 1.f) { /* average all four channels */
              ...
              if (!orig_opaque_back) {
                ...
                } else { /* hit something -- so keep blend and compute
                            cumulative alpha */

                  fc[0] = (0xFF & ((*pixel) >> 24)) * mix_in +
                          (0xFF & (last_pixel >> 24)) * persist_inv;
                  fc[1] = (0xFF & ((*pixel) >> 16)) * mix_in +
                          (0xFF & (last_pixel >> 16)) * persist_inv;
                  fc[2] = (0xFF & ((*pixel) >> 8)) * mix_in +
                          (0xFF & (last_pixel >> 8)) * persist_inv;
                  fc[3] = (0xFF & ((*pixel))) * mix_in +
            ...
            if (depth && (i >= 0) && (r1.trans < trans_cutoff) &&
            ...
            if (i >= 0) {
              if (r1.prim->type ==
                  cPrimSausage) { /* carry ray through the stick */
                if (perspective)
              ...
              if ((!backface_cull) && (trans_mode != 2))
                ...
                if (!r1.prim->no_lighting && (persist < 0.9999F) &&
                    (r1.trans > 0.05F)) {
...

          {
            ...
            if (pixel_c[3] < 255) {
              ...
              if (T->bkrd_data || T->bkrd_is_gradient) {
                pixel_c[0] = (unsigned char) pymol_roundf(
                    pixel_c[0] * pixa + bkrd[0] * pixam1255);
                pixel_c[1] = (unsigned char) pymol_roundf(
                    pixel_c[1] * pixa + bkrd[1] * pixam1255);
                pixel_c[2] = (unsigned char) pymol_roundf(
                    pixel_c[2] * pixa + bkrd[2] * pixam1255);
                pixel_c[3] = (unsigned char) pymol_roundf(
                    pixel_c[3] * pixa + bkrd[3] * pixam1255);
              }
            }
          }
...
/*========================================================================*/
void RayRender(CRay* I, unsigned int* image, double timing, float angle,
    int antialias, unsigned int* return_bg)
{
  int a, x, y;
  ...
  VLACacheSize(
      I->G, I->Primitive, CPrimitive, I->NPrimitive, 0, cCache_ray_primitive);
#ifdef PROFILE_BASIS
  n_cells = 0;
  n_prims = 0;
  n_triangles = 0;
  ...
  if (antialias > 1) {
    ...
    image = CacheAlloc(
        I->G, unsigned int, buffer_size, 0, cCache_ray_antialias_buffer);
    ErrChkPtr(I->G, image);
  } else {
    buffer_size = width * height;
  }
  ...
  bg_image_filename =
      SettingGet_s(I->G, nullptr, nullptr, cSetting_bg_image_filename);
  ...
  if (bkrd_is_gradient) {
    bkrd_ptr = ColorGet(
        I->G, SettingGet_color(I->G, nullptr, nullptr, cSetting_bg_rgb_top));
    copy3f(bkrd_ptr, bkrd_top);
    bkrd_ptr = ColorGet(
        I->G, SettingGet_color(I->G, nullptr, nullptr, cSetting_bg_rgb_bottom));
    copy3f(bkrd_ptr, bkrd_bottom);
  } else {
    bkrd_ptr = ColorGet(
        I->G, SettingGet_color(I->G, nullptr, nullptr, cSetting_bg_rgb));
    copy3f(bkrd_ptr, bkrd_top);
    copy3f(bkrd_ptr, bkrd_bottom);
  }
  ...
  if (!bkrd_is_gradient) {
    ...
    " RayNew: Background = %x %d %d %d\n", background,
        (int) (bkrd_top[0] * 255), (int) (bkrd_top[1] * 255),
        (int) (bkrd_top[2] * 255) ENDFB(I->G);
  }
  if (return_bg)
  ...
  if (!I->NPrimitive) { /* nothing to render! */
    if (I->bkgrd_data) {
      fill_background_image(
          I, image, width, height, width * (unsigned int) height);
    } else if (bkrd_is_gradient) {
      fill_gradient(I, opaque_back, image, bkrd_top, bkrd_bottom, width, height,
    ...
    if (shadows && (n_thread > 1)) { /* parallel execution */
      ...
      CRayHashThreadInfo* thread_info =
          pymol::calloc<CRayHashThreadInfo>(I->NBasis);
...

      {
        ...
        for (bc = 2; bc < I->NBasis; bc++) {
          int* vert2prim_ptr =
              I->Vert2Prim.empty() ? nullptr : I->Vert2Prim.data();
      ...
      int* vert2prim_ptr = I->Vert2Prim.empty() ? nullptr : I->Vert2Prim.data();
      ok &=
          BasisMakeMap(I->Basis + 1, vert2prim_ptr, I->Primitive, I->NPrimitive,
              I->Volume, 0, cCache_ray_map, perspective, front, I->PrimSize);
      if (ok && shadows) {
        int bc;
        float factor = SettingGetGlobal_f(I->G, cSetting_ray_hint_shadow);
        for (bc = 2; ok && bc < I->NBasis; bc++) {
          ok &= BasisMakeMap(I->Basis + bc, vert2prim_ptr, I->Primitive,
              I->NPrimitive, nullptr, bc - 1, cCache_ray_map, false, _0,
              I->PrimSize * factor);
        }
      }

      /* serial tasks which RayHashThread does in parallel mode using the first
       * thread */
      ...
      if (ok) {
        if (I->bkgrd_data) {
          ...
          fill_gradient(I, opaque_back, image, bkrd_top, bkrd_bottom, width,
              height, width * (unsigned int) height);
        } else {
          fill(image, background, width * (unsigned int) height);
        }
        RayComputeBox(I);
    ...
    if (ok) {
      if (shadows) {
        ...
        " Ray: voxels: [%4.2f:%dx%dx%d], %4.2f sec.\n", I->Basis[1].Map->Div,
            I->Basis[1].Map->Dim[0], I->Basis[1].Map->Dim[1],
            I->Basis[1].Map->Dim[2], now ENDFB(I->G);
      }
    }
    /* IMAGING */
    ...
    if (ok) {
      ...
      if (perspective) {
        ...
        for (c = 0; c < 4; c++) {
          ...
          x_pixel = (int) (width * (((x_test * I->Volume[5]) - I->Volume[0]) /
                                       I->Range[0]));
          y_pixel = (int) (height * (((y_test * I->Volume[5]) - I->Volume[2]) /
                                        I->Range[1]));
        ...
        x_start =
            (int) ((width * (I->min_box[0] - I->Volume[0])) / I->Range[0]) - 2;
        x_stop =
            (int) ((width * (I->max_box[0] - I->Volume[0])) / I->Range[0]) + 2;

        y_stop =
            (int) ((height * (I->max_box[1] - I->Volume[2])) / I->Range[1]) + 2;
        y_start =
            (int) ((height * (I->min_box[1] - I->Volume[2])) / I->Range[1]) - 2;
      }
      if (x_start < 0)
        x_start = 0;
      if (y_start < 0)
      ...
      for (a = 0; a < n_thread; a++) {
        ...
        rt[a].edging = nullptr;
        rt[a].edging_cutoff =
            oversample_cutoff; /* info needed for busy indicator */
      ...
      if (oversample_cutoff) { /* perform edge oversampling, if requested */
        ...
        edging = CacheAlloc(
            I->G, unsigned int, buffer_size, 0, cCache_ray_edging_buffer);
  ...
  if (ok && depth && ray_trace_mode) {
    ...
    if (ok) {
      ...
      for (y = 0; y < height; y++)
        for (x = 0; x < width; x++) {
          ...
          *(q++) = dzdy;
          /*
             if(((x == y )||(x==width/2)||(y==height/2))&&((dzdx!=0.0F) ||
             (dzdy!=0.0F))) { printf("%5d %5d : %8.3f %8.3f\n",y,x,dzdx,dzdy);
             } */
    ...
    if (ok) {
      int i;
      {
        ...
        if (fogFlag) { /* make sure we have depth values at every potentially
                          drawn pixel */
        ...
        }
        {
          ...
          float slope_f =
              SettingGetGlobal_f(I->G, cSetting_ray_trace_slope_factor);
          float depth_f =
              SettingGetGlobal_f(I->G, cSetting_ray_trace_depth_factor);
          float disco_f =
              SettingGetGlobal_f(I->G, cSetting_ray_trace_disco_factor);
          ...
          for (y = 0; y < height; y++) {
            ...
            for (x = 0; x < width; x++) {
              ...
              if ((max_slope > (slope_f)) /* depth */
                  || (max_depth > (depth_f))
                  /* slope */
                  /* gradient discontinuities -- could probably use more
                     tuning... */
                  || ((min_dot < _8) &&
                         ((max_dz > disco_f) || (max_pz > disco_f))) ||
                  ((min_dot < _4) &&
                      ((max_dz > disco_f_625) || (max_pz > disco_f_625))) ||
                  ((min_dot < _25) &&
                      ((max_dz > disco_f_5) || (max_pz > disco_f_5))) ||
                  ((min_dot < _m25) &&
                      ((max_dz > disco_f_45) && (max_pz > disco_f_45)))) {
                if (fogFlag) {
                  ...
                  if (orig_opaque_back) {
                    fc[0] = (0xFF & (background >> 24)) * ffact +
                            (0xFF & (trace_word >> 24)) * ffact1m;
                    fc[1] = (0xFF & (background >> 16)) * ffact +
                            (0xFF & (trace_word >> 16)) * ffact1m;
                    fc[2] = (0xFF & (background >> 8)) * ffact +
                            (0xFF & (trace_word >> 8)) * ffact1m;
                    fc[3] = (0xFF & (background)) * ffact +
                            (0xFF & (trace_word)) * ffact1m;
  ...
  printf("int n_cells = %d;\nint n_prims = %d;\nint n_triangles = %8.3f;\nint "
         ...
         "%8.3f;\nint n_skipped = %8.3f;\n",
      n_cells, n_prims, n_triangles / ((float) n_cells),
      n_spheres / ((float) n_cells), n_cylinders / ((float) n_cells),
      n_sausages / ((float) n_cells), n_skipped / ((float) n_cells));
#endif
...
void RayGetScaledAxes(CRay* I, float* xn, float* yn)
{
  ...
  RayApplyMatrixInverse33(
      1, (float3*) xn0, glm::value_ptr(I->Rotation), (float3*) xn0);
  RayApplyMatrixInverse33(
      1, (float3*) yn0, glm::value_ptr(I->Rotation), (float3*) yn0);
...
int CRay::character(int char_id)
{
...

  {
    ...
    RayApplyMatrixInverse33(
        1, (float3*) xn, glm::value_ptr(I->Rotation), (float3*) xn);
    RayApplyMatrixInverse33(
        1, (float3*) yn, glm::value_ptr(I->Rotation), (float3*) yn);
    RayApplyMatrixInverse33(
        1, (float3*) zn, glm::value_ptr(I->Rotation), (float3*) zn);

    CharacterGetGeometry(
        I->G, char_id, &width_i, &height_i, &xorig, &yorig, &advance);
    width = (float) width_i;
    ...
    *(pp) = (*p);
    ...
    I->PrimSize += 2 * (diff3f(p->v1, p->v2) + diff3f(p->v1, p->v3) +
                           diff3f(p->v2, p->v3));
    I->PrimSizeCnt += 6;
...
{
  return cylinder3fv(cyl.vertex1, cyl.vertex2, cyl.radius, cyl.color1,
      cyl.color2, 1.0f - Trans, 1.0f - Trans);
}
int CRay::cylinder3fv(
    const cgo::draw::cylinder& cyl, const float alpha1, const float alpha2)
{
  return cylinder3fv(cyl.vertex1, cyl.vertex2, cyl.radius, cyl.color1,
      cyl.color2, alpha1, alpha2);
}

int CRay::cylinder3fv(const float* v1, const float* v2, float r,
    const float* c1, const float* c2, const float alpha1, const float alpha2)
{
  CRay* I = this;
  CPrimitive* p;
  int ok = true;
...
/*========================================================================*/
int CRay::sausage3fv(
    const float* v1, const float* v2, float r, const float* c1, const float* c2)
{
  CRay* I = this;
  CPrimitive* p;
  int ok = true;
...
int CRay::ellipsoid3fv(
    const float* v, float r, const float* n1, const float* n2, const float* n3)
{
  CRay* I = this;
  CPrimitive* p;
  int ok = true;
...
/*========================================================================*/
int CRay::triangle3fv(const float* v1, const float* v2, const float* v3,
    ...
    const float* c2, const float* c3)
{
  ...
  cross_product3f(s1, s2, n0);
  if (normals_exist) {
    if ((fabs(n0[0]) < RAY_SMALL) && (fabs(n0[1]) < RAY_SMALL) &&
        (fabs(n0[2]) < RAY_SMALL)) {
      copy3f(nx, n0);
    } /* fall-back */
  ...
  I->PrimSize +=
      diff3f(p->v1, p->v2) + diff3f(p->v1, p->v3) + diff3f(p->v2, p->v3);
  I->PrimSizeCnt += 3;
...
int CRay::triangleTrans3fv(const float* v1, const float* v2, const float* v3,
    const float* n1, const float* n2, const float* n3, const float* c1,
    const float* c2, const float* c3, float t1, float t2, float t3)
{
  CRay* I = this;
  CPrimitive* p;
  int ok = true;
...
void RayPrepare(CRay* I, float v0, float v1, float v2, float v3, float v4,
    float v5, float fov, glm::vec3 pos, float* mat, const glm::mat4& rotMat,
    float aspRat, int width, int height, float pixel_scale, int ortho,
...
/*prepare for vertex calls */
{
  ...
  if (!I->Primitive)
    I->Primitive =
        VLACacheAlloc(I->G, CPrimitive, 10000, 3, cCache_ray_primitive);
  I->Volume[0] = v0;
  I->Volume[1] = v1;
  I->Volume[2] = v2;
  I->Volume[3] = v3;
  ...
  if (ortho) {
    ...
    I->PixelRadius =
        (((float) I->Range[0]) / width) * pixel_scale * pixel_ratio;
  }
    ...
    unsigned int n, float3* q, const float m[16], float3* p)
{
  ...
  for (i = 0; i < n;
       i++) { /* renormalize - can we do this to the matrix instead? */
    normalize3f(q[i]);
  }
}
...
float RayGetScaledAllAxesAtPoint(
    CRay* I, float* pt, float* xn, float* yn, float* zn)
{
  float xn0[3] = {1.0F, 0.0F, 0.0F};
  float yn0[3] = {0.0F, 1.0F, 0.0F};
  ...
  RayApplyMatrixInverse33(
      1, (float3*) yn0, glm::value_ptr(I->Rotation), (float3*) yn0);
  RayApplyMatrixInverse33(
      1, (float3*) zn0, glm::value_ptr(I->Rotation), (float3*) zn0);
