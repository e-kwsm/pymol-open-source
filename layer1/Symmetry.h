...
public:
  ...
  const float* getSymMat(int i) const { return SymMatVLA + i * 16; }
  CSymmetry(PyMOLGlobals* G)
      : G(G)
      , Crystal(G) {};
};
...
void SymmetrySpaceGroupRegister(
    PyMOLGlobals* G, const char* sg, const std::vector<std::string>& sym_op);
