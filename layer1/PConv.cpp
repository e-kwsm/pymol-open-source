...
int PConvPyListToBitmask(PyObject* obj, int* bitmask, ov_size ll)
{
  ...
  if (ll > 0)
    ok_assert(
        1, PConvPyListToSCharArrayInPlaceAutoZero(obj, &visRepArr[0], ll));
...
int PConvAttrToIntArrayInPlace(
    PyObject* obj, const char* attr, int* f, ov_size ll)
{
  int ok = true;
  PyObject* tmp;
  if (!obj) {
...
int PConvAttrToFloatArrayInPlace(
    PyObject* obj, const char* attr, float* f, ov_size ll)
{
  int ok = true;
  PyObject* tmp;
  if (!obj) {
...
PyObject* PConvFloatArrayToPyList(const float* f, int l, bool dump_binary)
{
  ...
  if (dump_binary) {
    return PyBytes_FromStringAndSize(
        reinterpret_cast<const char*>(f), l * sizeof(float));
  }
#endif
  int a;
...
PyObject* PConvFloatVLAToPyList(const float* f)
{
  ...
  result = PyList_New(l);
  for (a = 0; a < l; a++) {
    PyList_SetItem(result, a,
        PyFloat_FromDouble((double) *(f++))); /* set item steals ref */
  }
  return (PConvAutoNone(result));
}
    ...
    if (result) {
      ...
      for (i = 0; i < size; i++) {
        PyTuple_SetItem(result, i,
            PyFloat_FromDouble((double) *(vla++))); /* set item steals ref */
      }
    }
  }
  return (PConvAutoNone(result));
...
PyObject* PConvIntVLAToPyTuple(int* vla)
{
  ...
  if (vla) {
    ...
    if (result) {
      ...
      for (i = 0; i < size; i++) {
        PyTuple_SetItem(
            result, i, PyInt_FromLong(*(vla++))); /* set item steals ref */
      }
    }
  }
  return (PConvAutoNone(result));
...
PyObject* PConvIntArrayToPyList(const int* f, int l, bool dump_binary)
{
  ...
  if (dump_binary) {
    return PyBytes_FromStringAndSize(
        reinterpret_cast<const char*>(f), l * sizeof(int));
  }
#endif
  int a;
  PyObject* result = PyList_New(l);
