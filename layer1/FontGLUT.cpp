...
{
  return masked_byte_equals(
      c, 0xc0 /* mask  0b11000000 */, 0x80 /* value 0b10000000 */);
}
...
static unsigned int next_utf8_character(const char*& st, bool& error)
{
  ...
  if (!error) {
    ...
    } else if (masked_byte_equals(
                   c, 0xf0 /* 0b11110000 */, 0xe0 /* 0b11100000 */)) {
    ...
    bool shouldRender, CGO* shaderCGO)
{
  ...
  if (G->ValidContext) {
    ...
    if (st && (*st)) {
      ...
      if (rpos) {
        if (rpos[0] < _1) { /* we need to measure the string width before
                               starting to draw */
        ...
        }
        {
          ...
          if (ch) {
            if (!textured) {
              ...
              glBitmap(ch->width, ch->height, ch->xorig, ch->yorig, ch->advance,
                  0, ch->bitmap);
#endif
              ...
              fprnt.u.i.ch = (unsigned int) c;
              {
                ...
                if (!id) {
                  id = CharacterNewFromBitmap(G, ch->width, ch->height,
                      (unsigned char*) ch->bitmap, (float) ch->xorig,
                      (float) ch->yorig, (float) ch->advance, &fprnt, sampling);
                }
                if (id) {
                  CharacterRenderOpenGL(G, info, id, false, relativeMode,
      ...
      if (!textured) {
        FontGLUTRestore(I);
        glFlush(); /* workaround for screen flashes on late-model nVidia
                      hardware */
      }
    }
  }
    ...
    const float* rpos, bool needSize, short relativeMode)
{
  ...
  if (st && (*st)) {
    ...
    if (rpos) {
      ...
      if (rpos[0] < _1) { /* we need to measure the string width before starting
                             to draw */
        float factor = rpos[0] / 2.0F - 0.5F;
        const char* sst = st;
        if (factor < _m1)
