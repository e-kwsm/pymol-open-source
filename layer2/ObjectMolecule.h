...
struct ObjectMolecule : public pymol::CObject {
  ...
  int SeleBase = 0; /* for internal usage by  selector & only valid during
                       selection process */
...
public:
  ...
  // for reporting available assembly ids after mmCIF loading - SUBJECT TO
  // CHANGE
...
/* this is a record that holds information for specific types of Operatations on
 * Molecules, eg. translation/rotation/etc */
...
struct HBondCriteria {
  ...
  float factor_a,
      factor_b; /* 0.5/(maxAngle^power_a), 0.5/(maxAngle^power_b)) */
  float cone_dangle;
};
...
struct PDBInfoRec {
  ...
  int multi_object_status; /* 0 = unknown, 1 = is multi_object, -1 is not
                              multi_object */
  int multiplex;

  inline bool is_pqr_file() const { return variant == PDB_VARIANT_PQR; }
};
...
int ObjectMoleculeGetNearestAtomIndex(ObjectMolecule* I, const float* point,
...
int* ObjectMoleculeGetPrioritizedOtherIndexList(
    ObjectMolecule* I, struct CoordSet* cs);
...
int ObjectMoleculeIsAtomBondedToName(
    ObjectMolecule* obj, int a0, const char* name, int);
...
pymol::Result<> ObjectMoleculeDeleteStates(
    ObjectMolecule* I, const std::vector<int>& state);
...
int ObjectMoleculeAddPseudoatom(ObjectMolecule* I, int sele_index,
    ...
    const char* label, const float* pos, int color, int state, int more,
    int quiet);
...
ObjectMolecule* ObjectMoleculeReadPDBStr(PyMOLGlobals* G, ObjectMolecule* obj,
    const char* molstr, int frame, int discrete, char* pdb_name,
...
/**
 * Renders the positions of the indicators based for the selection in this
 * object
...
int ObjectMoleculeMerge(ObjectMolecule* I, pymol::vla<AtomInfoType>&& ai,
    struct CoordSet* cs, int bondSearchFlag, int aic_mask, int invalidate);
...
int ObjectMoleculeGetAtomVertex(
    const ObjectMolecule*, int state, int index, float* v);
...
int ObjectMoleculeTransformSelection(ObjectMolecule* I, int state, int sele,
    const float* TTT, int log, const char* sname, int homogenous, int global);
...
int ObjectMoleculePrepareAtom(
    ObjectMolecule* I, int index, AtomInfoType* ai, bool uniquefy = true);
void ObjectMoleculeReplaceAtom(ObjectMolecule* I, int index, AtomInfoType&& ai);
int ObjectMoleculePreposReplAtom(
    ObjectMolecule* I, int index, AtomInfoType* ai);
...
void ObjectMoleculeGetAtomSele(
    const ObjectMolecule* I, int index, char* buffer);
...
bool ObjectMoleculeConnect(ObjectMolecule* I, int& nbond,
    pymol::vla<BondType>& bond, struct CoordSet* cs, int searchFlag,
    int connectModeOverride, bool pbc = false);
/**
 * Connects bonds for a discrete object
 * (https://pymolwiki.org/index.php/Discrete_objects)
...
struct CoordSet* ObjectMoleculePDBStr2CoordSet(PyMOLGlobals* G,
    const char* buffer, AtomInfoType** atInfoPtr, const char** restart_model,
    char* segi_override, char* pdb_name, const char** next_pdb,
    PDBInfoRec* pdb_info, int quiet, int* model_number);

#ifndef NO_MMLIBS
int ObjectMoleculeUpdateAtomTypeInfoForState(PyMOLGlobals* G,
    ObjectMolecule* obj, int state, int initialize, int format);
#endif

#ifdef _PYMOL_IP_EXTRAS
int ObjectMoleculeUpdateMMStereoInfoForState(
    PyMOLGlobals* G, ObjectMolecule* obj, int state, int initialize = 1);
#endif
