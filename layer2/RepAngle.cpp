...
static int RepAngleCGOGenerate(RepAngle* I, RenderInfo* info)
{
  ...
  int color = SettingGet_color(
      G, nullptr, I->ds->Obj->Setting.get(), cSetting_angle_color);
...
void RepAngle::render(RenderInfo* info)
{
  ...
  line_width = SceneGetDynamicLineWidth(info, line_width);
  dash_transparency = SettingGet_f(
  ...
  if (ray) {
    ...
    if (pick) {
      ...
      if (!generate_shader_cgo) {
        RepAngleRenderImmediate(
            I, info, color, dash_transparency_enabled, dash_transparency);
      } else {
        CGORender(I->shaderCGO, nullptr, nullptr, nullptr, info, I);
      }
    }
...
Rep* RepAngleNew(DistSet* ds, int state)
{
  ...
  int a;
  int n = 0;
  float *v, *v1, *v2, *v3, *v4, d1[3], d2[3], d3[3], n1[3], n3[3], l1, l2, x[3],
      y[3];
  ...
  float dash_transparency;
  dash_transparency = SettingGet_f(
      G, nullptr, ds->Obj->Setting.get(), cSetting_dash_transparency);
  dash_transparency =
      (dash_transparency < 0.f
              ? 0.f
              : (dash_transparency > 1.f ? 1.f : dash_transparency));
  ...
  if (ds->NAngleIndex) {
    ...
    for (a = 0; ok && a < ds->NAngleIndex; a = a + 5) {
      ...
      if (l1 > l2)
        ...
        radius = l1;
      radius *=
          SettingGet_f(G, nullptr, ds->Obj->Setting.get(), cSetting_angle_size);
      ...
      /* figure out dash/gap phasing that will lead to nicely spaced dashes and
       * gaps */
