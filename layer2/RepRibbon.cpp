...
static bool RepRibbonCGOGenerate(RepRibbon* I)
{
  ...
  bool ribbon_as_cylinders =
      SettingGet<bool>(G, cSetting_render_as_cylinders) &&
      SettingGet<bool>(G, I->cs->Setting.get(), I->obj->Setting.get(),
...
Rep* RepRibbonNew(CoordSet* cs, int state)
{
  ...
  power_a = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_power);
  power_b = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_power_b);
  throw_ = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_throw);
  int trace_ostate = SettingGet_i(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_trace_atoms);
  ...
  ribbon_color = SettingGet_color(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_color);

  sampling = SettingGet_i(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_sampling);
  if (sampling < 1)
    sampling = 1;
  I->radius = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_radius);
  I->ribbon_width = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_width);
  ...
  for (a1 = 0; a1 < obj->NAtom; ++a1) {
    ...
    if (a >= 0) {
      ...
      if (GET_BIT(obj->AtomInfo[a1].visRep, cRepRibbon)) {
        trace =
            AtomSettingGetWD(G, ai, cSetting_ribbon_trace_atoms, trace_ostate);

        if (trace || ((obj->AtomInfo[a1].protons == cAN_C) &&
                         (WordMatchExact(G, G->lex_const.CA,
                             obj->AtomInfo[a1].name, true)) &&
                         !AtomInfoSameResidueP(G, last_ai, ai))) {
          PRINTFD(G, FB_RepRibbon)
          " RepRibbon: found atom in %d; a1 %d a2 %d\n", obj->AtomInfo[a1].resv,
              a1, a2 ENDFD;
          ...
          if (a2 >= 0) {
            if (trace) {
              ...
              if (!ObjectMoleculeCheckBondSep(
                      obj, a1, a2, 3)) /* CA->N->C->CA = 3 bonds */
                a2 = -1;
            }
          }
          PRINTFD(G, FB_RepRibbon)
          ...
          a2 = a1;
                       ...
                       ((na_mode == 1) && (ai->protons == cAN_C) &&
                           (WordMatchExact(G, "C4*", LexStr(G, ai->name), 1) ||
                               WordMatchExact(
                                   G, "C4'", LexStr(G, ai->name), 1)))) &&
                   !AtomInfoSameResidueP(G, last_ai, ai)) {
          if (a2 >= 0) {
            if (!ObjectMoleculeCheckBondSep(
                    obj, a1, a2, 6)) { /* six bonds between phosphates */
          ...
          *(v++) = *(v1++);
          ...
          a2 = a1;
        } else if ((a2 >= 0) && last_ai && (ai->protons == cAN_O) &&
  ...
  if (nAt) {
    ...
    float alpha = 1.f - SettingGet_f(G, nullptr, I->obj->Setting.get(),
                            cSetting_ribbon_transparency);
    if (fabs(alpha - 1.0) < R_SMALL4)
      alpha = 1.0F;
    CGOAlpha(I->primitiveCGO,
        alpha); // would be good to set these at render time instead
...
void RepRibbonRenderImmediate(CoordSet* cs, RenderInfo* info)
{
  ...
  if (info->ray || info->pick || (!(G->HaveGUI && G->ValidContext)))
    ...
    int trace, trace_ostate = SettingGet_i(G, cs->Setting.get(),
                   obj->Setting.get(), cSetting_ribbon_trace_atoms);
    int trace_mode = SettingGet_i(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_trace_atoms_mode);
    int na_mode = SettingGet_i(G, cs->Setting.get(), obj->Setting.get(),
        cSetting_ribbon_nucleic_acid_mode);
    float ribbon_width = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_ribbon_width);
    int a1, a2 = -1;
    ...
    for (a1 = 0; a1 < nAtIndex; a1++) {
      ...
      if (a >= 0) {
        ...
        if (GET_BIT(ai->visRep, cRepRibbon)) {
          trace = AtomSettingGetWD(
              G, ai, cSetting_ribbon_trace_atoms, trace_ostate);
