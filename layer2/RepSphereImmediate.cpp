    ...
    ObjectMolecule* obj, int* repActive, float sphere_scale)
{
  ...
  int ds = SettingGet_i(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_sphere_quality);
  if (ds < 0)
    ds = 0;
  if (ds > 4)
    ds = 4;
...
static void RenderImmediate_DoPreGL(PyMOLGlobals* G, int sphere_mode,
    float* pixel_scale, CoordSet* cs, ObjectMolecule* obj, float sphere_scale)
{
  switch (sphere_mode) {
  case 2:
  case 7:
  ...
  default:
    ...
    glPointSize(SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sphere_point_size));
    break;
  }
}
#endif
...
void RepSphereRenderImmediate(CoordSet* cs, RenderInfo* info)
{
  ...
  if (info->ray || info->pick || (!(G->HaveGUI && G->ValidContext)))
    ...
    int sphere_mode = SettingGet_i(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sphere_mode);
    float sphere_scale = SettingGet_f(
    ...
    if (sphere_mode > 0) { /* point-based modees */
      ...
      RenderImmediate_DoPreGL(
          G, sphere_mode, &pixel_scale, cs, obj, sphere_scale);
      switch (sphere_mode) {
      case 5:
      ...
      case 4:
        ...
        break;
      default:
        RenderSphereMode_Immediate_1_2_3(
            G, info, cs, obj, &repActive, pixel_scale, sphere_mode);
      }
    } else {
      RenderSphereMode_Immediate_Triangles(
          G, cs, obj, &repActive, sphere_scale);
    }
