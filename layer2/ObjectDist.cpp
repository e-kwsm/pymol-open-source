...
int ObjectDistGetLabelTxfVertex(ObjectDist* I, int state, int index, float* v)
{
  ...
  if (!I->DSet.empty()) {
    ...
    state = state % I->DSet.size();
    {
      ...
      if ((!ds) && (SettingGet_b(
                       I->G, I->Setting.get(), nullptr, cSetting_all_states))) {
        state = 0;
        ds = I->DSet[state].get();
      }
      if (ds) {
...
int ObjectDistMoveLabel(
    ObjectDist* I, int state, int index, float* v, int mode, int log)
{
  int result = 0;
...
/* ObjectDistMoveWithObject -- updates the vertex positions of a distance
 * measure
 *
 * PARAMS
 *   (ObjectDist*) I
...
int ObjectDistNewFromPyList(
    PyMOLGlobals* G, PyObject* list, ObjectDist** result)
{
  int ok = true;
...
/*========================================================================*/
ObjectDist::ObjectDist(PyMOLGlobals* G)
    : pymol::CObject(G)
{
  auto I = this;
...
ObjectDist* ObjectDistNewFromSele(PyMOLGlobals* G, ObjectDist* oldObj,
    int sele1, int sele2, int mode, float cutoff, int labels, int reset,
    float* result, int state, int state1, int state2)
{
  int a, mn;
  ...
  *result = 0.0;
  ...
  /* here we determine the highest number of states with which we need to
   * concern ourselves */
  ...
  if (mn) {
    ...
    for (a = 0; a < mn; a++) {
      ...
      /* I->DSet[a] = new DistSet(G, selections, states, etc) -- created this
       * new DistSet */
      if (5 <= mode && mode <= 7) {
#ifdef _PYMOL_INCENTIVE
#else
        PRINTFB(G, FB_ObjectDist, FB_Errors)
        " ObjectDist-Error: modes 5-7 only available in Incentive "
        "PyMOL\n" ENDFB(G);
        I->DSet[a] = nullptr;
#endif
        ...
        I->DSet[a].reset(SelectorGetDistSet(G, I->DSet[a].release(), sele1,
            state1, sele2, state2, mode, cutoff, &dist));
      }

      /* if the distances are valid, then tally the total and set the
       * ObjectMolecule pointer as necessary */
      if (I->DSet[a]) {
        dist_sum += dist; /* average distance over N states */
        dist_cnt++;
        I->DSet[a]->Obj =
            I; /* point to the ObjectMolecule for this state's DistanceSet */
      }
...
ObjectDist* ObjectDistNewFromAngleSele(PyMOLGlobals* G, ObjectDist* oldObj,
    int sele1, int sele2, int sele3, int mode, int labels, float* result,
    int reset, int state, int state1, int state2, int state3)
{
  int a, mn;
  float angle_sum = 0.0;
  int angle_cnt = 0;
  ...
  *result = 0.0;
  ...
  if (mn) {
    for (a = 0; a < mn; a++) {
      ...
      VecCheck(I->DSet, a);
      I->DSet[a].reset(SelectorGetAngleSet(G, I->DSet[a].release(), sele1,
          state1, sele2, state2, sele3, state3, mode, &angle_sum, &angle_cnt));
...
ObjectDist* ObjectDistNewFromDihedralSele(PyMOLGlobals* G, ObjectDist* oldObj,
    int sele1, int sele2, int sele3, int sele4, int mode, int labels,
    float* result, int reset, int state)
{
  int a, mn;
  float angle_sum = 0.0;
  int angle_cnt = 0;
  ...
  *result = 0.0;
  ...
  if (mn) {
    for (a = 0; a < mn; a++) {
      ...
      VecCheck(I->DSet, a);
      I->DSet[a].reset(SelectorGetDihedralSet(G, I->DSet[a].release(), sele1,
          state1, sele2, state2, sele3, state3, sele4, state4, mode, &angle_sum,
          &angle_cnt));
