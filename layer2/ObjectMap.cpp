...
/* MapState::ValidXtal -- determines whether the MapState's Xtal type passed in
 * is valid PARAMS ms, MapState RETURNS true/false
 */
 ...
 */
int ObjectMapStateGetExcludedStats(PyMOLGlobals* G, ObjectMapState* ms,
    float* vert_vla, float beyond, float within, float* level)
{
    ...
    for (c = 0; c < fdim[2]; c++) {
      for (b = 0; b < fdim[1]; b++) {
        for (a = 0; a < fdim[0]; a++) {
          ...
          if (within_flag &&
              beyond_flag) { /* point isn't too close to any vertex */
 ...
 */
int ObjectMapStateGetDataRange(
    PyMOLGlobals* G, ObjectMapState* ms, float* min, float* max)
{
...
int ObjectMapStateGetHistogram(PyMOLGlobals* G, ObjectMapState* ms,
    int n_points, float limit, float* histogram, float min_arg, float max_arg)
{
  float max_val = 0.0f, min_val = 0.0f;
...
static void ObjectMapStateTrim(
    PyMOLGlobals* G, ObjectMapState* ms, float* mn, float* mx, int quiet)
{
  int div[3];
  int min[3];
  int fdim[4];
      ...
      for (b = 0; b < fdim[1]; b++) {
        ...
        for (a = 0; a < fdim[0]; a++) {
          ...
          if ((a & 0x1) || (b & 0x1) || (c & 0x1)) {
            F3(field->data, a, b, c) = FieldInterpolatef(
                ms->Field->data.get(), a / 2, b / 2, c / 2, x, y, z);
          } else {
    ...
    for (c = 0; c < fdim[2]; c++) {
      ...
      for (b = 0; b < fdim[1]; b++) {
        ...
        for (a = 0; a < fdim[0]; a++) {
          ...
          copy3f(vr, vt);
          F3(field->data, a, b, c) =
              FieldInterpolatef(ms->Field->data.get(), a_2, b_2, c_2, x, y, z);
        }
      }
    }
    for (a = 0; a < 3; a++) {
    ...
    ObjectMap* I, int state, float* mn, float* mx, int quiet)
{
  ...
  /* TO DO: convert mn and mx into map local coordinates if map itself is
   * transformed...  */
  ...
  } else if ((state >= 0) && (state < I->State.size()) &&
...
int ObjectMapStateContainsPoint(ObjectMapState* ms, float* point)
{
  ...
  if (ObjectMapStateValidXtal(ms)) {
    ...
    if ((x >= ms->Min[0]) && (x <= ms->Max[0]) && (y >= ms->Min[1]) &&
        (y <= ms->Max[1]) && (z >= ms->Min[2]) && (z <= ms->Max[2]))
      result = true;
  }
  return (result);
}
...
int ObjectMapStateInterpolate(
    ObjectMapState* ms, const float* array, float* result, int* flag, int n)
{
  int ok = true;
  const float* inp;
  int a, b, c;
  ...
  if (ObjectMapStateValidXtal(ms)) {
    ...
    while (n--) {
      ...
      *(result++) = FieldInterpolatef(ms->Field->data.get(), a - ms->Min[0],
...
#ifndef _PYMOL_NOPY
static int ObjectMapNumPyArrayToMapState(
    PyMOLGlobals* G, ObjectMapState* I, PyObject* ary, int quiet);
#endif
...
ObjectMapState::ObjectMapState(const ObjectMapState& src)
    : CObjectState(src)
{
...
static int ObjectMapStateFromPyList(
    PyMOLGlobals* G, ObjectMapState* I, PyObject* list)
{
  int ok = true;
  int ll = 0;
  PyObject* tmp;
    ...
    if (!PyList_Check(list))
      ...
      if (ok)
        ok = PConvPyListToFloatArrayInPlace(
            PyList_GetItem(list, 6), I->Corner, 24);
      if (ok)
        ok = PConvPyListToFloatArrayInPlace(
            PyList_GetItem(list, 7), I->ExtentMin, 3);
      if (ok)
        ok = PConvPyListToFloatArrayInPlace(
            PyList_GetItem(list, 8), I->ExtentMax, 3);
      if (ok)
      ...
      if (ok)
        ok = CPythonVal_PConvPyListToIntArrayInPlace_From_List(
            G, list, 13, I->FDim, 4);
      if (ok) {
...
void ObjectMapStateClamp(
    ObjectMapState* I, float clamp_floor, float clamp_ceiling)
{
  int a, b, c;
  float* fp;
  ...
  for (StateIterator iter(G, I->Setting.get(), state, I->State.size());
    ...
    if (ms) {
          ...
          if (pick) {
            ...
            if (ms->shaderCGO) {
              CShaderPrg* shaderPrg =
                  G->ShaderMgr->Enable_DefaultShader(info->pass);
              if (shaderPrg) {
                shaderPrg->SetLightingEnabled(0);

                CGORender(ms->shaderCGO.get(), ColorGet(G, I->Color), nullptr,
      ...
      if ((I->visRep & cRepDotBit)) {
        ...
        if (ms->have_range &&
            SettingGet_b(G, nullptr, I->Setting.get(), cSetting_dot_normals)) {
        ...
        if (ms->have_range) {
          ...
          if (SettingGet_b(
                  G, nullptr, I->Setting.get(), cSetting_dot_normals)) {
...
bool ObjectMap::setSymmetry(CSymmetry const& symmetry, int state)
{
  ...
  for (StateIterator iter(G, Setting.get(), state, State.size());
       iter.next();) {
...
/*========================================================================*/
ObjectMap::ObjectMap(PyMOLGlobals* G)
    : pymol::CObject(G)
{
  auto I = this;
  I->type = cObjectMap;
...
/*========================================================================*/
ObjectMapState* ObjectMapNewStateFromDesc(
    PyMOLGlobals* G, ObjectMap* I, ObjectMapDesc* inp_md, int state, int quiet)
{
  int ok = true;
  float v[3];
  int a, b, c, d;
  ...
  *(md) = *(inp_md);
  ...
  switch (md->mode) {
  case cObjectMap_OrthoMinMaxGrid: /* Orthorhombic: min, max, spacing, centered
                                      over range  */
...
static int ObjectMapCCP4StrToMap(
    ObjectMap* I, char* CCP4Str, int bytes, int state, int quiet, int format)
{
  auto G = I->G;
  char* p;
  int* i;
  ...
  default:
    PRINTFB(I->G, FB_ObjectMap, FB_Errors)
    "ObjectMapCCP4-ERR: Only map mode 0-2 currently supported (this map is "
    "mode %d)",
        map_mode ENDFB(I->G);
    return (0);
  }
  ...
  // 38-52 future use "Storage space sometimes used by specific programs
  // (default = 0)"
  ...
  if (format != cLoadTypeCCP4Str && lengthsq3f(mrc2000origin) > R_SMALL4) {
    double matrix[] = {1., 0., 0., mrc2000origin[0], 0., 1., 0.,
        mrc2000origin[1], 0., 0., 1., mrc2000origin[2], 0., 0., 0., 1.};
    ...
    if (!quiet) {
      ...
      " ObjectMapCCP4: Applied MRC 2000 ORIGIN %.2f %.2f %.2f\n",
          mrc2000origin[0], mrc2000origin[1], mrc2000origin[2] ENDFB(I->G);
  ...
  if ((nx == ny) && (!nz)) {
    ...
    if (!quiet) {
      ...
      " ObjectMapCCP4: NZ value is zero, but NX = NY, so we'll guess NZ = NX = "
      "NY.\n" ENDFB(I->G);
    }
  }
  ...
  if (bytes < expectation) {
    if (bytes == (expectation - sym_skip)) {
      ...
      if (!quiet) {
        PRINTFB(I->G, FB_ObjectMap, FB_Blather)
        " ObjectMapCCP4: Map has invalid symmetry length -- working "
        "around.\n" ENDFB(I->G);
      }
 ...
 *
 * CCP4:
 * - can have SKWMAT/SKWTRN, but almost no other progam supports them (Maestro
 * does, to some extent)
 * - can't have ORIGIN
 *
 * MRC:
 ...
 * - can't have non-orthogonal axes (or shouldn't, because others might ignore
 * it)
 */
std::vector<char> ObjectMapStateToCCP4Str(
    const ObjectMapState* ms, int quiet, int format)
{
  ...
  if (field->type != cFieldFloat || field->base_size != 4) {
  ...
  if (lengthsq3f(b_origin) > R_SMALL4 || format == cLoadTypeMRC) {
    ...
    transform33f3f(
        ms->Symmetry->Crystal.fracToReal(), n_start_frac, n_start_real);
...
/*========================================================================*/
static int ObjectMapPHIStrToMap(
    ObjectMap* I, char* PHIStr, int bytes, int state, int quiet)
{
  ...
  if (little_endian != map_endian) {
    ...
    rev[0] =
        p[0]; /* gotta go char by char because of memory alignment issues ... */
    rev[1] = p[1];
    rev[2] = p[2];
    rev[3] = p[3];
  }
  ...
  for (c = 0; c < ms->FDim[2];
       c++) { /* z y x ordering into c b a  so that x = a, etc. */
    for (b = 0; b < ms->FDim[1]; b++) {
      for (a = 0; a < ms->FDim[0]; a++) {
        ...
        if (little_endian != map_endian) {
          ...
          rev[0] = p[0]; /* gotta go char by char because of memory alignment
                            issues ... */
          rev[1] = p[1];
          rev[2] = p[2];
          rev[3] = p[3];
...
/*========================================================================*/
static int ObjectMapXPLORStrToMap(
    ObjectMap* I, char* XPLORStr, int state, int quiet)
{
...
/*========================================================================*/
static int ObjectMapFLDStrToMap(
    ObjectMap* I, char* PHIStr, int bytes, int state, int quiet)
{
  char* p;
  float dens, dens_rev;
  ...
  while ((*p) && (!(got_ndim && got_dim1 && got_dim2 && got_dim3 &&
                     got_veclen && got_min_ext && got_max_ext && got_field &&
                     got_nspace && got_data))) {
  ...
  if (got_ndim && got_dim1 && got_dim2 && got_dim3 && got_veclen &&
        ...
        for (b = 0; b < ms->FDim[1]; b++) {
          for (a = 0; a < ms->FDim[0]; a++) {
            ...
            if (little_endian != map_endian) {
              ...
              rev[0] = p[0]; /* gotta go char by char because of memory
                                alignment issues ... */
              rev[1] = p[1];
              rev[2] = p[2];
              rev[3] = p[3];
...
static int ObjectMapBRIXStrToMap(
    ObjectMap* I, char* BRIXStr, int bytes, int state, int quiet)
{
  char *p, *pp;
  float dens;
  int a, b, c, d, e;
  ...
  if (got_smiley) {
    ...
    while ((*p) && (!(got_origin && got_extent && got_grid && got_cell &&
                       got_prod && got_plus && got_sigma))) {
    ...
    if (!passed_endian_check) {
      PRINTFB(I->G, FB_ObjectMap, FB_Errors)
      " Error: This looks like a DSN6 map file, but I can't match "
      "endianness.\n" ENDFB(I->G);
    } else {
  ...
  if (got_origin && got_extent && got_grid && got_cell && got_plus &&
      got_prod) {
    ...
    if (!(ms->FDim[0] && ms->FDim[1] && ms->FDim[2]))
...

      {
              ...
              for (c = 0; c < block_size; c++) {
                ...
                for (b = 0; b < block_size; b++) {
                  ...
                  for (a = 0; a < block_size; a++) {
                    ...
                    dens =
                        (((float) (*((unsigned char*) (p++)))) - plus) / prod;
                    if ((ia <= ms->Max[0]) && (ib <= ms->Max[1]) &&
                        (ic <= ms->Max[2])) {
        ...
        if (n_pts > 1) {
          ...
          calc_sigma =
              (float) sqrt1d((sumsq - (sum * sum / n_pts)) / (n_pts - 1));
        }
      }
...
static int ObjectMapGRDStrToMap(
    ObjectMap* I, char* GRDStr, int bytes, int state, int quiet)
{
  ...
  if (ascii) {
    ...
    However, that doesn't make sense...in the files I see, scale and oldmid
    don't seem to exist!
    ...
    write(14)toplbl write(14)ivary, nbyte, intdat, extent, extent, extent, xang,
    yang, zang, xstart, xend, ystart, yend, zstart, zend, intx, inty, intz do k
    = 1,igrid do j = 1,igrid write(14)(phimap(i,j,k),i=1,igrid) end do end d
    ...
    */
    ...
    if (little_endian != map_endian) {
      ...
      rev_union.rev[0] = p[0]; /* gotta go char by char because of memory
                                  alignment issues ... */
      rev_union.rev[1] = p[1];
      rev_union.rev[2] = p[2];
      rev_union.rev[3] = p[3];
  ...
  if (ascii)
  ...
  else {
    {
      ...
      if (rev_union.block_len != block_len_check) {
        PRINTFB(I->G, FB_ObjectMap, FB_Warnings)
        " ObjectMapGRD-Warning: block length not matched -- not a true GRD "
        "binary?\n" ENDFB(I->G);
      }
      ...
      if ((ivary) || (nbyte != 4) || (intdat)) {
        if (!quiet) {
          PRINTFB(I->G, FB_ObjectMap, FB_Warnings)
          " ObjectMapGRD-Warning: funky ivary, nbyte, intdat -- not a true GRD "
          "binary?\n" ENDFB(I->G);
        }
      }
    }
...
/*========================================================================*/
static ObjectMap* ObjectMapReadXPLORStr(
    PyMOLGlobals* G, ObjectMap* I, char* XPLORStr, int state, int quiet)
{
  int ok = true;
  int isNew = true;
...
/*========================================================================*/
static ObjectMap* ObjectMapReadCCP4Str(PyMOLGlobals* G, ObjectMap* I,
    char* XPLORStr, int bytes, int state, int quiet, int format)
{
  int ok = true;
  int isNew = true;
...
ObjectMap* ObjectMapLoadCCP4(PyMOLGlobals* G, ObjectMap* obj, const char* fname,
    int state, int is_string, int bytes, int quiet, int format)
{
  ObjectMap* I = nullptr;
  char* buffer;
  long size;
...
/*========================================================================*/
static ObjectMap* ObjectMapReadFLDStr(PyMOLGlobals* G, ObjectMap* I,
...
/*========================================================================*/
static ObjectMap* ObjectMapReadBRIXStr(PyMOLGlobals* G, ObjectMap* I,
    char* MapStr, int bytes, int state, int quiet)
{
  int ok = true;
  int isNew = true;
...
/*========================================================================*/
static ObjectMap* ObjectMapReadGRDStr(PyMOLGlobals* G, ObjectMap* I,
    char* MapStr, int bytes, int state, int quiet)
{
  int ok = true;
  int isNew = true;
...
/*========================================================================*/
static ObjectMap* ObjectMapReadPHIStr(PyMOLGlobals* G, ObjectMap* I,
    char* MapStr, int bytes, int state, int quiet)
{
  int ok = true;
  int isNew = true;
...
/*========================================================================*/
ObjectMap* ObjectMapLoadPHI(PyMOLGlobals* G, ObjectMap* obj, const char* fname,
    ...
    PyMOLGlobals* G, const char* DXStr, int bytes, bool quiet)
{
  ...
  while ((*p) && stage == 2) {
    ...
    if (3 != sscanf(p, " %f %f %f", delta + delta_i, delta + delta_i + 1,
                 delta + delta_i + 2)) {
...
/*========================================================================*/
ObjectMap* ObjectMapLoadDXFile(
    PyMOLGlobals* G, ObjectMap* obj, const char* fname, int state, int quiet)
{
  ObjectMap* I = nullptr;
  long size;
  char* buffer;
...
/*========================================================================*/
static int ObjectMapACNTStrToMap(
    ObjectMap* I, char* ACNTStr, int bytes, int state, int quiet)
{
...
ObjectMap* ObjectMapLoadACNTFile(
    PyMOLGlobals* G, ObjectMap* obj, const char* fname, int state, int quiet)
{
  ObjectMap* I = nullptr;
  long size;
...
/*========================================================================*/
ObjectMap* ObjectMapLoadFLDFile(
    PyMOLGlobals* G, ObjectMap* obj, const char* fname, int state, int quiet)
{
  ObjectMap* I = nullptr;
  long size;
...
/*========================================================================*/
ObjectMap* ObjectMapLoadBRIXFile(
    PyMOLGlobals* G, ObjectMap* obj, const char* fname, int state, int quiet)
{
  ObjectMap* I = nullptr;
  long size;
...
/*========================================================================*/
ObjectMap* ObjectMapLoadGRDFile(
    PyMOLGlobals* G, ObjectMap* obj, const char* fname, int state, int quiet)
{
  ObjectMap* I = nullptr;
  long size;
...
/*========================================================================*/
ObjectMap* ObjectMapLoadXPLOR(PyMOLGlobals* G, ObjectMap* obj,
    const char* fname, int state, int is_file, int quiet)
{
  ObjectMap* I = nullptr;
  long size;
  char* buffer;
...
#ifndef _PYMOL_NOPY
static int ObjectMapNumPyArrayToMapState(
    PyMOLGlobals* G, ObjectMapState* ms, PyObject* ary, int quiet)
{
...
/*========================================================================*/
ObjectMap* ObjectMapLoadChemPyBrick(PyMOLGlobals* G, ObjectMap* I,
    PyObject* Map, int state, int discrete, int quiet)
{
#ifndef _PYMOL_NUMPY
  ErrMessage(
      G, "ObjectMap", "missing numpy support, required for chempy.brick");
  return nullptr;
#endif
