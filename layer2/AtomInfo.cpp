...
#ifndef _PYMOL_NOPY
int AtomInfoSetSettingFromPyObject(
    PyMOLGlobals* G, AtomInfoType* ai, int setting_id, PyObject* val)
{
  if (val == Py_None)
    val = nullptr;
...
PyObject* SettingGetIfDefinedPyObject(
...
/*========================================================================*/
void AtomInfoGetPDB3LetHydroName(
    PyMOLGlobals* G, const char* resn, const char* iname, char* oname)
{
  oname[0] = ' ';
  ...
  case 'G':
    ...
    case 'L':
      switch (resn[2]) {
      case 'N':
        if ((iname[0] == 'H') && ((iname[1] == 'B') || (iname[1] == 'G')) &&
            ((iname[2] >= '0') && (iname[2] <= '9'))) {
          oname[0] = iname[2];
          oname[1] = iname[0];
          oname[2] = iname[1];
...
{
  return AtomInfoKnownProteinResName(resn) || AtomInfoKnownNucleicResName(resn);
}
...
PyObject* AtomInfoAsPyList(PyMOLGlobals* G, const AtomInfoType* I)
{
  ...
  PyList_SetItem(
      result, 11, PyInt_FromLong((int) I->isHydrogen())); // TODO redundant
...
#define PCONVPYSTRTOLEXIDX(i, n)                                               \
  {                                                                            \
    ok = CPythonVal_PConvPyStrToStr_From_List(                                 \
        G, list, i, temp, sizeof(OrthoLineType));                              \
    n = LexIdx(G, temp);                                                       \
  }
  ...
  if (ok)
    ok =
        CPythonVal_PConvPyStrToStr_From_List(G, list, 2, I->alt, sizeof(Chain));
  if (ok) {
  ...
  if (ok)
    PCONVPYSTRTOLEXIDX(4, I->segi);
  if (ok)
    PCONVPYSTRTOLEXIDX(5, I->resn);
  if (ok)
    PCONVPYSTRTOLEXIDX(6, I->name);
  if (ok)
    ok = CPythonVal_PConvPyStrToStr_From_List(
        G, list, 7, I->elem, sizeof(ElemName));
  if (ok)
    PCONVPYSTRTOLEXIDX(8, I->textType);
  if (ok)
    PCONVPYSTRTOLEXIDX(9, I->label);
  if (ok)
...
void AtomInfoCopy(PyMOLGlobals* G, const AtomInfoType* src, AtomInfoType* dst,
    int copy_properties)
{
 ...
 */
void AtomInfoCombine(
    PyMOLGlobals* G, AtomInfoType* dst, AtomInfoType&& src_, int mask)
{
...
/*========================================================================*/
/**
 * Make atom names in `atInfo1` unique w.r.t.\ `atInfo0` (and to `atInfo1`
 * itself).
...
int AtomInfoUniquefyNames(PyMOLGlobals* G, const AtomInfoType* atInfo0, int n0,
    AtomInfoType* atInfo1, int* flag1, int n1, const ObjectMolecule* mol)
{
...
{
  /* inefficient but reliable way to find where residue atoms are located in an
   * object for purpose of residue-based operations */
  int a;
...
/*========================================================================*/
void AtomInfoBracketResidueFast(
    PyMOLGlobals* G, const AtomInfoType* ai0, int n0, int cur, int* st, int* nd)
{
  /* efficient but unreliable way to find where residue atoms are located in an
   * object for purpose of residue-based operations. This function finds all
   * atoms in the AtomInfoType array in both directions that belong to the same
   * residue. It starts the search from the "cur" offset, and returns the
   * beginning offset (st) and the ending offset (nd).*/
  int a;
  const AtomInfoType* ai1;
...
float AtomInfoGetBondLength(
    PyMOLGlobals* G, const AtomInfoType* ai1, const AtomInfoType* ai2)
{
  float result = 1.6F;
 ...
 */
const ElementTableItemType ElementTable[] = {{"lonepair", "LP", 0.50, 0.000000},
    {"hydrogen", "H", 1.20, 1.007940}, {"helium", "He", 1.40, 4.002602},
    {"lithium", "Li", 1.82, 6.941000}, {"beryllium", "Be", 1.80, 9.012182},
    {"boron", "B", 1.85, 10.811000}, {"carbon", "C", 1.70, 12.010700},
    {"nitrogen", "N", 1.55, 14.006700}, {"oxygen", "O", 1.52, 15.999400},
    {"fluorine", "F", 1.47, 18.998403}, {"neon", "Ne", 1.54, 20.179700},
    {"sodium", "Na", 2.27, 22.989770}, {"magnesium", "Mg", 1.73, 24.305000},
    {"aluminum", "Al", 2.00, 26.981538}, {"silicon", "Si", 2.10, 28.085500},
    {"phosphorus", "P", 1.80, 30.973761}, {"sulfur", "S", 1.80, 32.065000},
    {"chlorine", "Cl", 1.75, 35.453000}, {"argon", "Ar", 1.88, 39.948000},
    {"potassium", "K", 2.75, 39.098300}, {"calcium", "Ca", 1.80, 40.078000},
    {"scandium", "Sc", 1.80, 44.955910}, {"titanium", "Ti", 1.80, 47.867000},
    {"vanadium", "V", 1.80, 50.941500}, {"chromium", "Cr", 1.80, 51.996100},
    {"manganese", "Mn", 1.73, 54.938049}, {"iron", "Fe", 1.80, 55.845000},
    {"cobalt", "Co", 1.80, 58.933200}, {"nickel", "Ni", 1.63, 58.693400},
    {"copper", "Cu", 1.40, 63.546000}, {"zinc", "Zn", 1.39, 65.390000},
    {"gallium", "Ga", 1.87, 69.723000}, {"germanium", "Ge", 1.80, 72.640000},
    {"arsenic", "As", 1.85, 74.921600}, {"selenium", "Se", 1.90, 78.960000},
    {"bromine", "Br", 1.85, 79.904000}, {"krypton", "Kr", 2.02, 83.800000},
    {"rubidium", "Rb", 1.80, 85.467800}, {"strontium", "Sr", 1.80, 87.620000},
    {"yttrium", "Y", 1.80, 88.905850}, {"zirconium", "Zr", 1.80, 91.224000},
    {"niobium", "Nb", 1.80, 92.906380}, {"molybdenum", "Mo", 1.80, 95.940000},
    {"technetium", "Tc", 1.80, 98.000000},
    {"ruthenium", "Ru", 1.80, 101.070000}, {"rhodium", "Rh", 1.80, 102.905500},
    {"palladium", "Pd", 1.63, 106.420000}, {"silver", "Ag", 1.72, 107.868200},
    {"cadmium", "Cd", 1.58, 112.411000}, {"indium", "In", 1.93, 114.818000},
    {"tin", "Sn", 2.17, 118.710000}, {"antimony", "Sb", 1.80, 121.760000},
    {"tellurium", "Te", 2.06, 127.600000}, {"iodine", "I", 1.98, 126.904470},
    {"xenon", "Xe", 2.16, 131.293000}, {"cesium", "Cs", 1.80, 132.905450},
    {"barium", "Ba", 1.80, 137.327000}, {"lanthanum", "La", 1.80, 138.905500},
    {"cerium", "Ce", 1.80, 140.116000},
    {"praseodymium", "Pr", 1.80, 140.907650},
    {"neodymium", "Nd", 1.80, 144.240000},
    {"promethium", "Pm", 1.80, 145.000000},
    {"samarium", "Sm", 1.80, 150.360000}, {"europium", "Eu", 1.80, 151.964000},
    {"gadolinium", "Gd", 1.80, 157.250000}, {"terbium", "Tb", 1.80, 158.925340},
    {"dysprosium", "Dy", 1.80, 162.500000}, {"holmium", "Ho", 1.80, 164.930320},
    {"erbium", "Er", 1.80, 167.259000}, {"thulium", "Tm", 1.80, 168.934210},
    {"ytterbium", "Yb", 1.80, 173.040000}, {"lutetium", "Lu", 1.80, 174.967000},
    {"hafnium", "Hf", 1.80, 178.490000}, {"tantalum", "Ta", 1.80, 180.947900},
    {"tungsten", "W", 1.80, 183.840000}, {"rhenium", "Re", 1.80, 186.207000},
    {"osmium", "Os", 1.80, 190.230000}, {"iridium", "Ir", 1.80, 192.217000},
    {"platinum", "Pt", 1.75, 195.078000}, {"gold", "Au", 1.66, 196.966550},
    {"mercury", "Hg", 1.55, 200.590000}, {"thallium", "Tl", 1.96, 204.383300},
    {"lead", "Pb", 2.02, 207.200000}, {"bismuth", "Bi", 1.80, 208.980380},
    {"polonium", "Po", 1.80, 208.980000}, {"astatine", "At", 1.80, 209.990000},
    {"radon", "Rn", 1.80, 222.020000}, {"francium", "Fr", 1.80, 223.020000},
    {"radium", "Ra", 1.80, 226.030000}, {"actinium", "Ac", 1.80, 227.030000},
    {"thorium", "Th", 1.80, 232.038100},
    {"protactinium", "Pa", 1.80, 231.035880},
    {"uranium", "U", 1.86, 238.028910}, {"neptunium", "Np", 1.80, 237.050000},
    {"plutonium", "Pu", 1.80, 244.060000},
    {"americium", "Am", 1.80, 243.060000}, {"curium", "Cm", 1.80, 247.070000},
    ...
    {"dubnium", "Db", 1.80, 262.110000}, {"seaborgium", "Sg", 1.80, 266.120000},
    {"bohrium", "Bh", 1.80, 264.120000}, {"hassium", "Hs", 1.80, 269.130000},
    {"meitnerium", "Mt", 1.80, 268.140000},
    {"darmstadtium", "Ds", 1.80, 281.000000},
    {"roentgenium", "Rg", 1.80, 281.000000},
    {"copernicium", "Cn", 1.80, 285.000000},
    {"nihonium", "Nh", 1.80, 286.000000}, {"flerovium", "Fl", 1.80, 289.000000},
    {"moscovium", "Mc", 1.80, 290.000000},
    {"livermorium", "Lv", 1.80, 293.000000},
    {"tennessine", "Ts", 1.80, 294.000000},
    {"oganesson", "Og", 1.80, 294.000000}, {NULL, nullptr, 0.00, 0.000000}};
...
int AtomInfoGetColor(PyMOLGlobals* G, const AtomInfoType* at1)
{
  ...
  case cAN_N:
    return G->AtomInfo->NColor;
  case cAN_C:
    return G->AtomInfo->CColor;
  case cAN_O:
    return G->AtomInfo->OColor;
  case cAN_P:
    return G->AtomInfo->PColor;
  }
...
static int AtomInfoNameCompare(
    PyMOLGlobals* G, const char* name1, const char* name2)
{
  const char *n1, *n2;
  int cmp;
...
static int AtomInfoNameCompare(
    PyMOLGlobals* G, const lexidx_t& name1, const lexidx_t& name2)
{
  if (name1 == name2)
    return 0;
 ...
 *   1: at1 > at2
 */
static int AtomInfoCompare(PyMOLGlobals* G, const AtomInfoType* at1,
    const AtomInfoType* at2, bool ignore_hetatm, bool ignore_rank)
{
  int wc;

  if ((wc = WordCompare(G, at1->segi, at2->segi, false)))
    return wc;
  if ((wc = WordCompare(G, at1->chain, at2->chain, false)))
    return wc;
  if (!ignore_hetatm && at1->hetatm != at2->hetatm)
    return (at2->hetatm) ? -1 : 1;
  if (at1->resv != at2->resv)
    return (at1->resv < at2->resv) ? -1 : 1;
  ...
  if ((wc = WordCompare(G, at1->resn, at2->resn, true)))
    return wc;
  if (at1->discrete_state != at2->discrete_state)
    return (at1->discrete_state < at2->discrete_state) ? -1 : 1;
  ...
  if (at1->priority != at2->priority)
    return (at1->priority < at2->priority) ? -1 : 1;

  // Changed (PyMOL 2.1): name before alt
  if ((wc = AtomInfoNameCompare(G, at1->name, at2->name)))
    return wc;
...
rank_compare:
  if (!ignore_rank && at1->rank != at2->rank)
    return (at1->rank < at2->rank) ? -1 : 1;
...
int AtomInfoCompare(
    PyMOLGlobals* G, const AtomInfoType* at1, const AtomInfoType* at2)
{
  return AtomInfoCompare(G, at1, at2, false, false);
}

int AtomInfoCompareIgnoreRankHet(
    PyMOLGlobals* G, const AtomInfoType* at1, const AtomInfoType* at2)
{
  return AtomInfoCompare(G, at1, at2, true, true);
}

int AtomInfoCompareIgnoreRank(
    PyMOLGlobals* G, const AtomInfoType* at1, const AtomInfoType* at2)
{
  return AtomInfoCompare(G, at1, at2, false, true);
}

int AtomInfoCompareIgnoreHet(
    PyMOLGlobals* G, const AtomInfoType* at1, const AtomInfoType* at2)
{
...
int AtomInfoSameResidue(
    PyMOLGlobals* G, const AtomInfoType* at1, const AtomInfoType* at2)
{
  return (at1->resv == at2->resv && at1->chain == at2->chain &&
          at1->hetatm == at2->hetatm &&
          at1->discrete_state == at2->discrete_state &&
          at1->inscode == at2->inscode && at1->segi == at2->segi &&
          WordMatchExact(G, at1->resn, at2->resn,
              SettingGet<bool>(G, cSetting_ignore_case)));
}

int AtomInfoSameResidueP(
    PyMOLGlobals* G, const AtomInfoType* at1, const AtomInfoType* at2)
{
  if (at1 && at2)
...
int AtomInfoSequential(
    PyMOLGlobals* G, const AtomInfoType* at1, const AtomInfoType* at2, int mode)
{
  if (mode > 0) {
 ...
 */
int AtomInfoMatch(PyMOLGlobals* G, const AtomInfoType* at1,
    const AtomInfoType* at2, bool ignore_case, bool ignore_case_chain)
{
  if (at1->resv == at2->resv)
  ...
  if (!*e) { /* try to guess atomic type from name */
      ...
      if (*(e + 1) == 'A') {
        if (!WordMatchExact(G, G->lex_const.CA, I->resn, true) &&
          ...
          *(e + 1) = 0;
                       ...
                       'a') || /* CA intpreted as carbon, not calcium */
                     (*(e + 1) == 'l') ||
                     (*(e + 1) == 'L') || (*(e + 1) == 'u') ||
                     (*(e + 1) == 'U') || (*(e + 1) == 'o') ||
                     (*(e + 1) == 'O') || (*(e + 1) == 's') ||
    ...
    case 'D': /* take deuterium to hydrogen */
      *(e + 1) = 0;
      break;
    case 'N':
      if (!((*(e + 1) == 'i') || (*(e + 1) == 'I') || (*(e + 1) == 'a') ||
              (*(e + 1) == 'A') || (*(e + 1) == 'b') || (*(e + 1) == 'B')))
        *(e + 1) = 0;
      break;
    case 'S':
      if (!((*(e + 1) == 'e') || (*(e + 1) == 'E') || (*(e + 1) == 'r') ||
              (*(e + 1) == 'R') || (*(e + 1) == 'c') || (*(e + 1) == 'C') ||
              (*(e + 1) == 'b') || (*(e + 1) == 'B')))
        *(e + 1) = 0;
...
{
  switch (ai->mmstereo) {
  case 1 /* MMSTEREO_CHIRALITY_R */:
    return "R";
  case 2 /* MMSTEREO_CHIRALITY_S */:
    return "S";
  }

  switch (ai->stereo) {
  case SDF_CHIRALITY_ODD:
    return "odd";
  case SDF_CHIRALITY_EVEN:
    return "even";
  }
...
void AtomInfoSetStereo(AtomInfoType* ai, const char* stereo)
{
  ...
  case 'R':
    ai->mmstereo = 1;
    ai->stereo = 0;
    break; // MMSTEREO_CHIRALITY_R
  case 'S':
    ai->mmstereo = 2;
    ai->stereo = 0;
    break; // MMSTEREO_CHIRALITY_S
  case 'E':
    ai->mmstereo = 0;
    ai->stereo = SDF_CHIRALITY_EVEN;
    break;
  case 'O':
    ai->mmstereo = 0;
    ai->stereo = SDF_CHIRALITY_ODD;
    break;
  case 'A': // ANS (s), ANR (r) pseudochirality
  case 'N':
  case 0:
    ai->mmstereo = ai->stereo = 0;
    break;
  default:
    ai->mmstereo = ai->stereo = 3;
    break;
  }
}
 ...
 */
void AtomInfoGetAlignedPDBResidueName(
    PyMOLGlobals* G, const AtomInfoType* ai, ResName& resn)
{
 ...
 */
void AtomInfoGetAlignedPDBAtomName(PyMOLGlobals* G, const AtomInfoType* ai,
    const ResName& resn, AtomName& name)
{
  ...
  if (!ai->name) {
    ...
    if (ai_name_len < 4) {     /* atom name less than length 4 */
      if (!isdigit(name[0])) { /* doesn't start with a number */
        if ((toupper(ai->elem[0]) == toupper(name[0])) &&
            ((!ai->elem[1]) || /* symbol len = 1 */
                (toupper(ai->elem[1]) ==
                    toupper(name[1])))) { /* matched len 2 */
      ...
      if ((ai->elem[0] == name[0]) &&
        ...
        if ((!ai->elem[1]) &&
            (ai->elem[0])) { /* but if element is one letter... */
          switch (reformat) {
        ...
        if (reformat == 2) { /* AMBER compliance mode */
          if (isdigit(name[0])) {
            if ((ai->elem[0] == name[1]) &&
                ((!(ai->elem[1])) ||
                    (toupper(ai->elem[1]) == toupper(name[2])))) {
              /* rotate the name to place atom symbol in column 0 to comply with
               * Amber PDB format */
              name[0] = ai_name[1];
              name[1] = ai_name[2];
  ...
  } else { /* LITERAL mode: preserve what was in the original PDB as best PyMOL
              can this should enable people to open and save amber pdb files
              without issues */
