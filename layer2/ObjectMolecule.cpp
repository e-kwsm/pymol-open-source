...
static CoordSet* ObjectMoleculeMMDStr2CoordSet(PyMOLGlobals* G,
    const char* buffer, AtomInfoType** atInfoPtr, const char** restart);

static void ObjectMoleculeTransformTTTf(
    ObjectMolecule* I, float* ttt, int state);

static int ObjectMoleculeGetAtomGeometry(
    const ObjectMolecule* I, int state, int at);

static void ObjectMoleculeInferHBondFromChem(ObjectMolecule* I);
 ...
 */
static int BondTypeInOrder(
    PyMOLGlobals* G, const BondType* bonds, int i1, int i2)
{
 ...
 */
static void ObjectMoleculeRemoveDuplicateBonds(
    PyMOLGlobals* G, ObjectMolecule* I)
{
  ...
  for (int i = 0, j = -1; i < I->NBond; ++i) {
    ...
    if (j != -1) {
      ...
      if (ptr->index[0] == other.index[0] && ptr->index[1] == other.index[1])
        equal = true;
    }
 ...
 * on the merged atoms.
 */
static bool ObjectMoleculeSetNotDiscrete(PyMOLGlobals* G, ObjectMolecule* I)
{
  if (!I->DiscreteFlag)
    return true;
  ...
  for (state = 0; state < I->NCSet; state++) {
    ...
    for (idx = 0; idx < cs->NIndex; idx++) {
      ...
      if (I->DiscreteCSet[ao]) {
        ...
        AtomInfoCopy(G, I->AtomInfo + ao, I->AtomInfo + an);
        cs->IdxToAtm[idx] = an;
      }
...
int ObjectMoleculeCheckFullStateSelection(
...
char* ObjectMolecule::getCaption(char* ch, int len) const
{
  ...
  int counter_mode = SettingGet_i(
      I->G, I->Setting.get(), nullptr, cSetting_state_counter_mode);
  int frozen =
      SettingGetIfDefined_i(I->G, I->Setting.get(), cSetting_state, &objState);
  ...
  if (state >= 0) {
        ...
        if (show_state) {
          if (show_as_fraction) {
            if (strlen(cs->Name)) { /* NAME */
              n = snprintf(ch, len, "%s %s%d/%d", cs->Name, frozen_str,
                  state + 1, I->NCSet);
            } else { /* no name */
    ...
    } else { /* state > NCSet, out of range due to other object or global
                setting */
      if (show_state) {
        if (show_as_fraction) {
...
static int recursive_match(int a, int b, int aa, int bb, match_info* mi)
{
  if (mi->atom_mark_a[a] && mi->atom_mark_b[b]) {
    ...
    if ((num_a != num_b) ||
        (num_a > 4)) /* must have the same number of neighbors (four or less) */
      return false;
    else {
      ...
      if (n_u_a == n_u_b) {
        if (!n_u_a)
          ...
          match_flag = recmat3(0, 1, 2) || recmat3(0, 2, 1) ||
                       recmat3(1, 0, 2) || recmat3(1, 2, 0) ||
                       recmat3(2, 0, 1) || recmat3(2, 1, 0);
        } else if (n_u_a == 4) {
          match_flag = recmat4(0, 1, 2, 3) || recmat4(0, 2, 1, 3) ||
                       recmat4(1, 0, 2, 3) || recmat4(1, 2, 0, 3) ||
                       recmat4(2, 0, 1, 3) || recmat4(2, 1, 0, 3);
          if (!match_flag) {
            match_flag = recmat4(0, 1, 3, 2) || recmat4(0, 2, 3, 1) ||
                         recmat4(1, 0, 3, 2) || recmat4(1, 2, 3, 0) ||
                         recmat4(2, 0, 3, 1) || recmat4(2, 1, 3, 0);
            if (!match_flag) {
              match_flag = recmat4(0, 3, 1, 2) || recmat4(0, 3, 2, 1) ||
                           recmat4(1, 3, 0, 2) || recmat4(1, 3, 2, 0) ||
                           recmat4(2, 3, 0, 1) || recmat4(2, 3, 1, 0);
              if (!match_flag) {
                match_flag = recmat4(3, 0, 1, 2) || recmat4(3, 0, 2, 1) ||
                             recmat4(3, 1, 0, 2) || recmat4(3, 1, 2, 0) ||
                             recmat4(3, 2, 0, 1) || recmat4(3, 2, 1, 0);
              }
            }
          }
        }
...
int ObjectMoleculeXferValences(ObjectMolecule* Ia, int sele1, int sele2,
    int target_state, ObjectMolecule* Ib, int sele3, int source_state,
    int quiet)
{
  int* matched = nullptr;
                ...
                while (match > matched) {
                  ...
                  if ((bd_a >= 0) && (bd_a >= 0)) {
                    ...
                    if ((SelectorIsMember(G, a1_entry, sele1) &&
                      ...
                      /* only update bonds which actually sit between the two
                       * selections */
                  ...
                  mi.atom_mark_b[at_b] =
                      false; /* release source for future matching */
                  if (bd_b >= 0)
...
void ObjectMoleculeTransformState44f(ObjectMolecule* I, int state,
    const float* matrix, int log_trans, int homogenous, int transformed)
{
  int a;
  float tmp_matrix[16];
  CoordSet* cs;
  ...
  if (!use_matrices) {
    ObjectMoleculeTransformSelection(
        I, state, -1, matrix, log_trans, I->Name, homogenous, true);
  } else {
    ...
    if (state < 0) { /* all states */
      ...
      cs = I->CSet[0];
      if (cs && SettingGet_b(I->G, I->Setting.get(), nullptr,
...
/*========================================================================*/
static int ObjectMoleculeFixSeleHydrogens(
    ObjectMolecule* I, int sele, int state)
{
  int a;
  int seleFlag = false;
  ...
  if (seleFlag) {
    ...
    if (!ObjectMoleculeVerifyChemistry(I, state)) {
      ErrMessage(
          I->G, " AddHydrogens", "missing chemical geometry information.");
    } else {
      ai0 = I->AtomInfo;
      for (a = 0; a < I->NAtom; a++) {
        if (!ai0->isHydrogen()) { /* only do heavies */
          if (SelectorIsMember(I->G, ai0->selEntry, sele)) {
            for (StateIterator iter(I->G, I->Setting.get(), state, I->NCSet);
              ...
              seleFlag |=
                  ObjectMoleculeSetMissingNeighborCoords(I, cs, a, true);
            }
          }
        }
        ai0++;
 ...
 * before OG2, CG, HB1)
 */
int ObjectMoleculeGetTopNeighbor(
    PyMOLGlobals* G, ObjectMolecule* I, int start, int excluded)
{
  ...
  for (auto const& neighbor : AtomNeighbors(I, start)) {
    ...
    if ((highest_at < 0) && (at != excluded)) {
      ...
      highest_at = at;
                   ...
                   ((ai->protons == highest_prot) &&
                       (ai->priority < lowest_pri))) &&
               (at != excluded)) {
      highest_prot = ai->protons;
      highest_at = at;
      lowest_pri = ai->priority;
...
ObjectMolecule* ObjectMoleculeLoadTRJFile(PyMOLGlobals* G, ObjectMolecule* I,
    const char* fname, int frame, int interval, int average, int start,
    int stop, int max, const char* sele, int image, const float* shift,
    int quiet)
{
  FILE* f;
  char* buffer;
...
#define GETTING_LOW 10000
                    ...
                    while (r_act != 3) {
                      ...
                      if (r_act == 2) { /* time to image */
                        if (r_cnt) {
                          ...
                          transform33f3f(
                              periodicbox.realToFrac(), r_cent, r_cent);
                          r_trans[0] =
                              fmodf(1000.0F + shift[0] + r_cent[0], 1.0F);
                          r_trans[1] =
                              fmodf(1000.0F + shift[1] + r_cent[1], 1.0F);
                          r_trans[2] =
                              fmodf(1000.0F + shift[2] + r_cent[2], 1.0F);
                          r_trans[0] -= r_cent[0];
                          r_trans[1] -= r_cent[1];
                          r_trans[2] -= r_cent[2];
                          transform33f3f(
                              periodicbox.fracToReal(), r_trans, r_trans);
                          fp = r_fp_start;
                          while (fp < r_fp_stop) {
                            *(fp++) += r_trans[0];
                            *(fp++) += r_trans[1];
  ...
  if (zoom_flag)
    if (SettingGetGlobal_i(G, cSetting_auto_zoom)) {
      ExecutiveWindowZoom(
          G, I->Name, 0.0, -1, 0, 0, quiet); /* auto zoom (all states) */
    }
...
static const char* findflag(
    PyMOLGlobals* G, const char* p, const char* flag, const char* format)
{
...
/*========================================================================*/
static CoordSet* ObjectMoleculeTOPStr2CoordSet(
    PyMOLGlobals* G, const char* buffer, AtomInfoType** atInfoPtr)
{
  const char* p;
  int nAtom;
  ...
  case 2:
    ...
    PRINTFB(G, FB_ObjectMolecule, FB_Details)
    " TOPStrToCoordSet: Warning: can't currently image a truncated "
    "octahedron...\n" ENDFB(G);
    break;
  case 1:
...
/*========================================================================*/
static ObjectMolecule* ObjectMoleculeReadTOPStr(
    PyMOLGlobals* G, ObjectMolecule* I, char* TOPStr, int frame, int discrete)
{
  CoordSet* cset = nullptr;
  ...
  if (ok) {
    ...
    if (isNew) {
      ...
      ok &= ObjectMoleculeMerge(
          I, std::move(atInfo), cset, false, cAIC_AllMask, true);
    }
    if (isNew)
      I->NAtom = nAtom;
    /*
...
void ObjectMoleculeSculptImprint(
    ObjectMolecule* I, int state, int match_state, int match_by_segment)
{
  PRINTFD(I->G, FB_ObjectMolecule)
  " %s: entered.\n", __func__ ENDFD;
...
float ObjectMoleculeSculptIterate(
    ObjectMolecule* I, int state, int n_cycle, float* center)
{
  PRINTFD(I->G, FB_ObjectMolecule)
  " %s: entered.\n", __func__ ENDFD;
  if (I->Sculpt) {
...
/*========================================================================*/
int ObjectMoleculeGetPhiPsi(
    ObjectMolecule* I, int ca, float* phi, float* psi, int state)
{
  int np = -1;
  int cm = -1;
  int c = -1;
...
int ObjectMoleculeCheckBondSep(ObjectMolecule* I, int a0, int a1, int dist)
{
  ...
  while (depth) {                  /* keep going until we've traversed tree */
    while (Neighbor[stack[depth]] >=
           0) {                    /* end of branches? go back up one bond */
      n0 = Neighbor[stack[depth]]; /* get current neighbor index */
      stack[depth] += 2;           /* set up next neighbor */
      distinct = true; /* check to see if current candidate is distinct from
                          ancestors */
      for (a = 1; a < depth; a++) {
        if (history[a] == n0)
          distinct = false;
...
/*========================================================================*/
pymol::Result<> ObjectMoleculeSetStateTitle(
    ObjectMolecule* I, int state, const char* text)
{
    ...
    bool vis_only, CGO& selIndicatorsCGO)
{
  ...
  if (SettingGetIfDefined_i(G, I->Setting.get(), cSetting_all_states, &a)) {
    curState =
        a ? -1 : SettingGet_i(G, I->Setting.get(), nullptr, cSetting_state);
          ...
          if (SelectorIsMember(G, atInfo[*(idx2atm++)].selEntry, sele)) {
            if (all_vis)
              ...
              if (visRep & (cRepCylBit | cRepSphereBit | cRepSurfaceBit |
                               cRepLabelBit | cRepNonbondedSphereBit |
                               cRepCartoonBit | cRepRibbonBit | cRepLineBit |
                               cRepMeshBit | cRepDotBit | cRepNonbondedBit)) {
                flag = true;
              }
            }
            if (flag) {
...
/*========================================================================*/
static CoordSet* ObjectMoleculeXYZStr2CoordSet(PyMOLGlobals* G,
    const char* buffer, AtomInfoType** atInfoPtr, const char** restart)
{
  ...
  while (*p) {
    ...
    if (valid_atom) {
      ...
      " ObjectMolecule-DEBUG: %s %s %d %s %8.3f %8.3f %8.3f %6.2f %6.2f %s\n",
          LexStr(G, ai->name), LexStr(G, ai->resn), ai->resv,
          LexStr(G, ai->chain), *(coord + a), *(coord + a + 1),
          *(coord + a + 2), ai->b, ai->q, LexStr(G, ai->segi) ENDFD;
...
int ObjectMoleculeRenameAtoms(ObjectMolecule* I, int* flag, int force)
{
  ...
  result = AtomInfoUniquefyNames(
      I->G, nullptr, 0, I->AtomInfo.data(), flag, I->NAtom);
  return result;
}
 ...
 * @param valence_vec Valence vector. If nullptr, don't move. If null-vector,
 * don't merge.
 */
    ...
    bool move_flag)
{
  ...
  for (StateIterator iter(G, nullptr, state0, I->NCSet); iter.next();) {
    ...
    ok = AddCoordinateIntoCoordSet(
        I->CSet[iter.state], cs.get(), coord_orig.data(), at0, mat1_inv, vec);
    p_return_val_if_fail(ok, pymol::Error::MEMORY);
  }
...
/*========================================================================*/
int ObjectMoleculeAttach(
    ObjectMolecule* I, int index, pymol::vla<AtomInfoType>&& nai)
{
  int a;
  AtomInfoType* ai;
  BondType* bond;
  ...
  ok_assert(1, ObjectMoleculeMerge(I, std::move(nai), cs, false, cAIC_AllMask,
                   true)); // will free nai and cs->TmpLinkBond
  ...
  if ((index >= 0) && (index <= I->NAtom)) {
    while (ok) {
      ...
      if (ok) {
        ...
        d = AtomInfoGetBondLength(I->G, ai, nai);
        if (ok)
          ok &= ObjectMoleculeMerge(I, std::move(atInfo), cs, false,
              cAIC_AllMask, true); /* will free nai and cs->TmpLinkBond  */
      }
      if (ok)
        ok &= ObjectMoleculeExtendIndices(I, -1);
      for (a = 0; ok && a < I->NCSet;
           a++) { /* add atom to each coordinate set */
...
/*========================================================================*/
int ObjectMoleculePrepareAtom(
    ObjectMolecule* I, int index, AtomInfoType* ai, bool uniquefy)
{
...
void ObjectMoleculeSaveUndo(ObjectMolecule* I, int state, int log)
{
  ...
  if (cs) {
    ...
    memcpy(
        I->UndoCoord[I->UndoIter], cs->Coord, sizeof(float) * cs->NIndex * 3);
  ...
  if (I->UndoState[I->UndoIter] >= 0) {
    ...
    if (cs) {
      if (cs->NIndex == I->UndoNIndex[I->UndoIter]) {
        memcpy(cs->Coord.data(), I->UndoCoord[I->UndoIter],
            sizeof(float) * cs->NIndex * 3);
        I->UndoState[I->UndoIter] = -1;
        FreeP(I->UndoCoord[I->UndoIter]);
  ...
  if (I->Bond) {
    ...
    for (a = 0; a < I->NBond; a++) {
      ...
      if (both == 2) {
        ...
        switch (mode) {
        case 0: /* cycle */
          switch (SettingGet_i(I->G, I->Setting.get(), nullptr,
              cSetting_editor_bond_cycle_mode)) {
          case 1: /* 1 arom 2 3 */
            switch (b0->order) {
            case 1:
              b0->order = 4;
...
void ObjectMoleculePurge(ObjectMolecule* I)
{
  ...
  // note that we don't delete atom selection members -- those may be needed in
  // the new object.
...
int ObjectMoleculeGetAtomGeometry(const ObjectMolecule* I, int state, int at)
{
  ...
  if (nn == 4)
    ...
    normalize3f(cp2);
    normalize3f(cp3);
    avg = (dot_product3f(cp1, cp2) + dot_product3f(cp2, cp3) +
              dot_product3f(cp3, cp1)) /
          3.0F;
    if (avg > 0.75)
...
/*========================================================================*/
static float compute_avg_center_dot_cross_fn(
    ObjectMolecule* I, CoordSet* cs, int n_atom, int* atix)
{
  float result = 0.0F;
  float* v[9];
...
static float compute_avg_ring_dot_cross_fn(
    ObjectMolecule* I, CoordSet* cs, int n_atom, int* atix, float* dir)
{
  float result = 0.0F;
  float* v[9];
  int missing_flag = false;
...
void ObjectMoleculeGuessValences(
    ObjectMolecule* I, int state, int* flag1, int* flag2, int reset)
{
  ...
  float (*compute_avg_ring_dot_cross)(
      ObjectMolecule*, CoordSet*, int, int*, float*);
  ...
  if (cs && obs_bond && obs_atom && flag && flag1 && flag2) {
    ...
    for (a = 0; a < I->NAtom; a++) {
      ...
      if ((!ai->chemFlag) && (flag[a])) {
        /*  determine whether or not atom participates in a planer system with 5
         * or 6 atoms */

        {
          ...
          int escape_count =
              ESCAPE_MAX; /* don't get bogged down with structures
                        ...
                        while (((mem[5] = neighbor[nbr[4]]) >= 0) &&
                          ...
                          if ((mem[5] != mem[3]) && (mem[5] != mem[2]) &&
                            ...
                            if (mem[5] == mem[0]) { /* five-cycle */
                              ...
                              float avg_dot_cross = compute_avg_ring_dot_cross(
                                  I, cs, 5, mem, dir);
                              for (i = 0; i < 5; i++) {
                              ...
                              if (avg_dot_cross > planer_cutoff) {
                                if (verify_planer_bonds(
                                  ...
                                  for (i = 0; i < 5; i++) {
                                    ...
                                    obs_bond[neighbor[nbr[i] + 1]].planer =
                                        true;
                                  }
                                }
                              }
                            ...
                            while (((mem[6] = neighbor[nbr[5]]) >= 0) &&
                              ...
                              if ((mem[6] != mem[4]) && (mem[6] != mem[3]) &&
                                ...
                                if (mem[6] == mem[0]) { /* six-cycle */
                                  ...
                                  float avg_dot_cross =
                                      compute_avg_ring_dot_cross(
                                          I, cs, 6, mem, dir);
                                  for (i = 0; i < 6; i++) {
                                    obs_atom[mem[i]].cyclic = true;
                                    obs_bond[neighbor[nbr[i] + 1]].cyclic =
                                        true;
                                  }
                                  if (avg_dot_cross > planer_cutoff) {
                                    if (verify_planer_bonds(I, cs, 6, mem,
                                            neighbor, dir, 0.35F)) {
                                      for (i = 0; i < 6; i++) {
                                        obs_atom[mem[i]].planer = true;
                                        obs_bond[neighbor[nbr[i] + 1]].planer =
              ...
              }
              {
                ...
                case cAN_C: /* planer carbons */
                    ...
                    if (avg_dot_cross > planer_cutoff) {
                      ...
                      if ((n1_at >= 0) && (o1_at >= 0) && (o2_at < 0)) {
                        ...
                        if ((o1_len < 1.38F) &&
                          ...
                          if ((flag1[a] && flag2[o2_at]) ||
                              (flag2[a] && flag1[o2_at]))
                            bondInfo[o2_bd].order = 4;
                        } else if ((o1_len < 1.38F) &&
                                   (neighbor[neighbor[o1_at]] == 1)) {
                          if ((flag1[a] && flag2[o1_at]) ||
                              (flag2[a] && flag1[o1_at]))
                            bondInfo[o1_bd].order = 2;
                        } else if ((o2_len < 1.38F) &&
                                   (neighbor[neighbor[o2_at]] == 1))
                          if ((flag1[a] && flag2[o2_at]) ||
                              (flag2[a] && flag1[o2_at]))
                        ...
                        if ((o1_len < 1.38F) && (o2_len < 1.38F) &&
                          ...
                          if ((flag1[a] && flag2[o2_at]) ||
                            ...
                            bondInfo[o2_bd].order = 2;
                                 ...
                                 (c1_at >= 0) && (n1_len < 1.43F) &&
                                 (n2_len < 1.43F) && obs_atom[c1_at].planer &&
                                 obs_atom[c1_at].cyclic && (!ob_at->cyclic) &&
                                 (!obs_atom[n1_at].cyclic) &&
                                 (!obs_atom[n2_at].cyclic)) {
                        if ((flag1[a] && flag2[n1_at]) ||
                            (flag2[a] && flag1[n1_at]))
                          bondInfo[n1_bd].order = 4;
                        atomInfo[n1_at].valence = 3;
                        atomInfo[n1_at].geom = cAtomInfoPlanar;
                        atomInfo[n1_at].chemFlag = 2;
                        if ((flag1[a] && flag2[n2_at]) ||
                            (flag2[a] && flag1[n2_at]))
                  ...
                  if ((nn >= 2) && (nn <= 3) && (n1_at >= 0) && (o1_at < 0) &&
                      (n2_at < 0) && (n1_len < 1.36F) && (!ob_at->cyclic) &&
                      (!obs_bond[n1_bd].cyclic) && (!obs_atom[n1_at].planer) &&
                      ((nn == 2) ||
                          ((nn == 3) && (avg_dot_cross > planer_cutoff)))) {
                    ...
                    { /* check nitrogen planarity */
                      ...
                      if (nn2 > 2) {
                        ...
                        n1_dot_cross =
                            compute_avg_center_dot_cross(I, cs, 4, atm2);
                      }
                    }
    ...
    for (a = 0; a < I->NAtom; a++) {
      ...
      if ((!ai->chemFlag) && (ai->protons == cAN_N) &&
          (ai->formalCharge == 0) && flag[a] && obs_atom[a].cyclic &&
          obs_atom[a].aromatic) {
                    ...
                    while (((mem[4] = neighbor[nbr[3]]) >= 0) &&
                      ...
                      if ((mem[4] != mem[2]) && (mem[4] != mem[1]) &&
                        ...
                        while (((mem[5] = neighbor[nbr[4]]) >= 0) &&
                               ((!atmToIdx) || (atmToIdx[mem[4]] >= 0))) {
                          if (!(escape_count--))
                            goto escape2; /* BUG FIX: need a new escape2,
                                             instead of mistakenly going back to
                                             the first escape, which is in the
                                             loop above */
                          if ((mem[5] != mem[3]) && (mem[5] != mem[2]) &&
                              (mem[5] != mem[1])) {
                            if (mem[5] == mem[0] && (!ai->chemFlag)) {

                              /* unassigned aromatic nitrogen-containing
                               * five-cycle */
                              ...
                              if ((atomInfo[mem[1]].protons == cAN_C) &&
                                ...
                                int nn2 = neighbor[n2++];
                                if (nn2 ==
                                    2) { /* second nitrogen also ambiguous */
                                  ai->valence = 3;
                                  ai->chemFlag = 2;
                                  ai->geom = cAtomInfoPlanar;
                                }
  ...
  if (warning1 || warning2) {
    ...
    "guessing valences.\n",
        __func__, warning1, warning2 ENDFB(I->G);
  }
  FreeP(obs_bond);
  FreeP(obs_atom);
  FreeP(flag);
          ...
          if (geom >= 0) {
            ...
            if (nn == 1) { /* only one neighbor */
              ...
              if (ai2->chemFlag && (ai2->geom == cAtomInfoTetrahedral)) {
                ai->chemFlag =
                    true; /* singleton carbon bonded to tetC must be tetC */
                ai->geom = cAtomInfoTetrahedral;
                ai->valence = 4;
              }
            }
    ...
    case cAN_N:
      if (has_hydro)
        ...
        if (delocalized && (neighbor_has_double_bond) && (!has_double_bond) &&
            (ai->geom == cAtomInfoPlanar) && (nn == 2) &&
            (ai->formalCharge >= 0)) {
          /* there's a fair chance of a resonance structure with this nitrogen
           * as a donor */
          ai->hb_donor = true;
        }
        if ((ai->geom != cAtomInfoPlanar) && (nn == 3) &&
            (ai->formalCharge >= 0) && (!delocalized)) {
...
void ObjectMoleculeInferChemFromBonds(ObjectMolecule* I, int state)
{
  ...
  for (b = 0; b < I->NBond; b++) {
    ...
    if (order == 3) {
      /* override existing chemistry * this is a temp fix to a pressing
         problem... we need to rethink the chemisty assignment ordering (should
         bond information come first? */
      ...
      if (ai0->chemFlag != 2) {
        ...
        case cAN_N:
          ai0->valence =
              (ai0->formalCharge == 1) ? 3 : 2; // TODO wrong for neutral -NH-
          break;
        case cAN_C:
          ai0->valence = 3;
          break;
      ...
      if (ai1->chemFlag != 2) {
        ...
        case cAN_N:
          ai1->valence =
              (ai1->formalCharge == 1) ? 3 : 2; // TODO wrong for neutral -NH-
          break;
        case cAN_C:
          ai1->valence = 3;
          break;
            ...
            if (ai->geom == cAtomInfoTetrahedral) {
              ...
              for (auto const& neighbor : AtomNeighbors(I, a)) {
                ...
                if ((ai0->chemFlag) && (ai0->geom == cAtomInfoPlanar) &&
                  ...
                  ai->geom =
                      cAtomInfoPlanar; /* found probable delocalization */
                  if (ai->formalCharge == 0)
...
/*========================================================================*/
int ObjectMoleculeTransformSelection(ObjectMolecule* I, int state, int sele,
    const float* matrix, int log, const char* sname, int homogenous, int global)
{
  ...
  while (1) {
    ...
    if (state < I->NCSet) {
      ...
      if (cs) {
        int use_matrices =
            SettingGet_i(G, I->Setting.get(), nullptr, cSetting_matrix_mode);
        if (use_matrices < 0)
          use_matrices = 0;
        ...
        if (sele >= 0) { /* transforming select atoms */
          ...
          if (!use_matrices) {
            ...
            ObjectMoleculeTransformState44f(
                I, state, matrix, false, homogenous, false);
          }
        }
        if (flag) {
          cs->invalidateRep(cRepAll, cRepInvCoord);
 ...
 * @verbatim
     ...
     NOTE: all atoms have an offset and a terminator whether or not they have
 any bonds:

     Here's an example of an ALA
     [ offet for atom1, offset for atom2, ..., offset for atom9, \
     {10, 16, 26, 32, 36, 46, 50, 54, 58, 62, \
     (atom1)  nbonds=2, neighbor index1=5, bond index=1, neighbor index2=1, bond
 index=0, null terminator=-1 \ 2, 5, 1, 1, 0, -1,
     (atom2) nbonds=4, nbr index1=6, bond id=4; nbr idx2=4, bond idx=3; nbr
 index3=2, bond idx=2; nbr index=0, bond index=0, null=-1 \ 4, 6, 4, 4, 3, 2, 2,
 0, 0, -1, (atom3) ... 2, 3, 5, 1, 2, -1, 1, 2, 5, -1, 4, 9, 8, 8, 7, 7, 6, 1,
 3, -1, 1, 0, 1, -1, 1, 1, 4, -1, 1, 4, 6, -1, 1, 4, 7, -1, 1, 4, 8, -1 }
   @endverbatim
...
int const* ObjectMolecule::getNeighborArray() const
{
  ...
  if (!I->Neighbor) {
    ...
    for (int a = 0; a < I->NAtom; a++) {
      ...
      neighbor[a] =
          c + d + d +
          1; /* set initial position to end of list, we'll fill backwards */
    ...
    for (int b = 0; b < I->NBond; b++, ++bnd) {
      ...
      neighbor[l0]--;
      neighbor[neighbor[l0]] =
          l1; /* store neighbor references (I->AtomInfo, etc.) */

      neighbor[l1]--;
      neighbor[neighbor[l1]] = b; /* store bond indices (for I->Bond) */
      neighbor[l1]--;
      neighbor[neighbor[l1]] =
          l0; /* store neighbor references (I->AtomInfo, etc.) */
    }
...
/*========================================================================*/
#ifndef _PYMOL_NOPY
static CoordSet* ObjectMoleculeChemPyModel2CoordSet(
    PyMOLGlobals* G, PyObject* model, AtomInfoType** atInfoPtr)
{
  int nAtom, nBond;
  int a, c;
  float* coord = nullptr;
    ...
    for (a = 0; a < nAtom; a++) {
      ...
      if (!ignore_ids) {
        if (ok) { /* get chempy atom id if extant */
          if (PTruthCallStr(atom, "has", "id")) {
            ...
            if (!ok)
              ErrMessage(G, __func__, "can't read atom identifier");
            Py_XDECREF(tmp);
          } else {
            ai->id = -1;
          }
      ...
      if (ok) {
        ...
        if (PTruthCallStr(atom, "has", "text_type")) {
          ...
          if (tmp) {
            OrthoLineType temp;
            ok = PConvPyObjectToStrMaxClean(
                tmp, temp, sizeof(OrthoLineType) - 1);
            ai->textType = LexIdx(G, temp);
          }
          if (!ok)
      ...
      if (ok) {
        if (PTruthCallStr(atom, "has", "bohr")) {
          ...
          if (!ok)
            ErrMessage(G, __func__, "can't read elec. radius");
          Py_XDECREF(tmp);
        } else {
          ai->elec_radius = 0.0F;
        }
      ...
      if (ok) {
        if (PTruthCallStr(atom, "has", "numeric_type")) {
          ...
          if (!ok)
            ErrMessage(G, __func__, "can't read numeric_type");
          Py_XDECREF(tmp);
        } else {
      ...
      if (ok) {
        if (PTruthCallStr(atom, "has", "formal_charge")) {
          ...
          if (!ok)
            ErrMessage(G, __func__, "can't read formal_charge");
          Py_XDECREF(tmp);
        } else {
          ai->formalCharge = 0;
        }
      ...
      if (ok) {
        if (PTruthCallStr(atom, "has", "partial_charge")) {
          ...
          if (!ok)
            ErrMessage(G, __func__, "can't read partial_charge");
          Py_XDECREF(tmp);
        } else {
      ...
      if (ok) {
        ...
        if (tmp) {
          ...
          ok = PConvPyObjectToStrMaxClean(
              tmp, tmp_ins_code, sizeof(ResIdent) - 1);
          if (!ok)
      ...
      if (ok) {
        ...
        if (!ok)
          ErrMessage(G, __func__, "can't read alternate conformation");
        Py_XDECREF(tmp);
      }
      ...
      if (ok) {
        ...
        if (!ok)
          ErrMessage(G, __func__, "can't read secondary structure");
        Py_XDECREF(tmp);
      }
      ...
      if (ok && PyObject_HasAttrString(atom, "cartoon_trgb")) {
        ...
        if (tmp) {
          ...
          if (!ok)
            ...
            SettingSet(
                G, cSetting_cartoon_color, ColorGetIndex(G, color_name), ai);
          }
        }
        Py_XDECREF(tmp);
      }
      if (ok && PyObject_HasAttrString(atom, "label_trgb")) {
        ...
        if (tmp) {
          ...
          if (!ok)
            ...
            sprintf(color_name, "0x%08x", trgb);
            SettingSet(
                G, cSetting_label_color, ColorGetIndex(G, color_name), ai);
          }
        }
        Py_XDECREF(tmp);
      }
      ...
      if (ok && PyObject_HasAttrString(atom, "ribbon_trgb")) {
        ...
        if (tmp) {
          ...
          if (!ok)
            ...
            sprintf(color_name, "0x%08x", trgb);
            SettingSet(
                G, cSetting_ribbon_color, ColorGetIndex(G, color_name), ai);
          }
        }
        Py_XDECREF(tmp);
      }
  ...
  if (nAtom) {
    ...
    if (ok) {
      ...
      for (a = 0; a < nBond; a++) {
        ...
        if (!index)
          ok = ErrMessage(G, __func__, "can't get bond indices");
        else {
          for (c = 0; c < 2; c++) {
...
ObjectMolecule* ObjectMoleculeLoadChemPyModel(PyMOLGlobals* G,
    ObjectMolecule* I, PyObject* model, int frame, int discrete)
{
#ifdef _PYMOL_NOPY
  return nullptr;
#else
    ...
    const float* coords, int coords_len, int frame)
{
  ...
  return ObjectMoleculeLoadCoords(
      G, (ObjectMolecule*) cobj, coords, coords_len, frame);
}
 ...
 */
ObjectMolecule* ObjectMoleculeLoadCoords(
    PyMOLGlobals* G, ObjectMolecule* I, PyObject* coords, int frame)
{
#ifdef _PYMOL_NOPY
  return nullptr;
#else
...
static CoordSet* ObjectMoleculeMOLStr2CoordSet(PyMOLGlobals* G,
    const char* buffer, AtomInfoType** atInfoPtr, const char** restart)
{
  const char* p;
  int nAtom, nBond;
...
static CoordSet* ObjectMoleculeSDF2Str2CoordSet(PyMOLGlobals* G,
    const char* buffer, AtomInfoType** atInfoPtr, const char** next_mol)
{
  char cc[MAXLINELEN];
  const char* p;
  ...
  p = *next_mol;
  if (p) {
    while (*p) { /* we simply need to skip until we've read past the end of the
                    SDF record */
      p = ncopy(cc, p, 4);
      p = nextline(p);
 ...
 */
static std::vector<signed char> get_bond_order_sums(ObjectMolecule* obj)
{
  ...
  for (int secondpass = 0; secondpass < 2; ++secondpass) {
    for (int b = 0; b < obj->NBond; ++b) {
      if (orders[b] == 4) {
        ...
        if (secondpass ? (freevalences[atm1] >= 1 && freevalences[atm2] >= 1)
 ...
 */
static void ObjectMoleculeMOL2SetFormalCharges(
    ...
    const char* buffer, AtomInfoType** atInfoPtr, const char** next_mol)
{
  ...
  while ((*p) && ok) {
    ...
    last_p = p;
    /* top level -- looking for an RTI, assuming p points to the beginning of a
     * line */
    ...
    if (cc[0] == '@') {
      if (WordMatchExact(G, cc, "@<TRIPOS>MOLECULE", true) ||
        ...
        for (a = 0; a < nAtom; a++) {
          ...
          if (ok) {
            ...
            if (cc_resi[0]) { /* subst_id is residue identifier */
              ...
              if (cc[0]) {
                if (nSubst == 0) {
                  // without substructure information (e.g. OpenBabel exported
                  // MOL2): if subst_name includes the subst_id (e.g. 5 ALA5)
        ...
        if (!have_molecule) {
          ok =
              ErrMessage(G, "ReadMOL2File", "@SUBSTSRUCTURE before @MOLECULE!");
          break;
        }
        p = ParseNextLine(p);
        if (ok) {
          ...
          for (a = 0; a < nSubst; a++) {
            ...
            if (ok && (!end_line)) {
              ...
              if (sscanf(cc, "%s", segment) != 1) {
                ...
                seg_flag = true;
                if (!segment[1]) { /* if segment is single letter, then also
                                      assign to chain field */
            ...
            if (ok && (root >= 0) && (root < nAtom)) {
              ...
              if ((strlen(subst_name) == 4) &&
                ...
                /* if subst_name looks like a PDB code, then it isn't a residue
                   identifier so do nothing... */
              } else {
                ...
                if (!subst_flag) {
                  ...
                  if (sscanf(cc, "%d", &resv) == 1) {
                    ...
                    UtilNCopy(resi, pp,
                        cResiLen); /* now get the textual residue identifier */
                  ...
                  /* normal mode: assume that the substructure ID is a vanilla
                   * residue identifier */

                  char* pp = subst_name;

                  if (resn_flag) { /* if we have a resn, then remove it from the
                                      subst_name */
                    char* qq = resn;
                    while ((*pp) && (*qq)) {
                      if (*pp == *qq) {
...
/**
 * Read one molecule in MOL2, MOL, SDF, MMD or XYZ format from the string
 * pointed to by (*next_entry). All these formats (except MOL) support multiple
...
ObjectMolecule* ObjectMoleculeReadStr(PyMOLGlobals* G, ObjectMolecule* I,
    const char** next_entry, cLoadType_t content_format, int frame,
    int discrete, int quiet, int multiplex, char* new_name,
  ...
  *next_entry = nullptr;
  ...
  while (repeatFlag) {
    ...
    if (ok && !skip_out) {
      ...
      if ((discrete > 0 && !I->DiscreteFlag) || // should set to discrete if
                                                // explicitly defined
  ...
  if (deferred_tasks &&
      I) { /* defer time-consuming tasks until all states have been loaded */
    ...
    CoordSet* cs, int bondSearchFlag, int aic_mask, int invalidate)
{
  ...
  if (ok) {
    for (a = 0; a < n_index; a++)
      ai2[a] = std::move(
          ai[index[a]]); /* creates a sorted list of atom info records */
  }
  ai = std::move(ai2);
      ...
      if (!I->DiscreteFlag) { /* don't even try matching for discrete objects */
        while (b < n_atom) {
          ...
          if (!ac) {
            found = true;
            break;
          } else if (ac < 0) { /* atom is before current, so try going the other
                                  way */
            reverse = true;
            break;
          } else if (!b) {
            ...
            if (ac > 0) { /* atom is after all atoms in list, so don't even
                             bother searching */
              break;
            }
  ...
  VLAFreeP(
      ai); /* note that we're trusting AtomInfoCombine to have
  ...
  if (ok && expansionFlag) { /* expansion flag means we have introduced at least
                                1 new atom */
    ...
    if (nBond) {
      ...
      if (ok) {
        for (a = 0; a < nBond; a++) { /* iterate over new bonds */
          found = false;
          if (!I->DiscreteFlag) { /* don't even try matching for discrete
                                     objects */
            ...
            while (b < I->NBond) {
              ...
              if (!ac) { /* zero is a match */
                ...
                if (ac > 0) /* bond is after all bonds in list, so don't even
                               bother searching */
                  break;
                /* next, try to jump to an appropriate position in the list */
...
void ObjectMoleculeTransformTTTf(ObjectMolecule* I, float* ttt, int frame)
{
  ...
  for (b = 0; b < I->NCSet; b++) {
    if ((frame < 0) || (frame == b)) {
      ...
      if (cs) {
        cs->invalidateRep(cRepAll, cRepInvCoord);
        MatrixTransformTTTfN3f(
            cs->NIndex, cs->Coord.data(), ttt, cs->Coord.data());
...
bool ObjectMoleculeSeleOp(ObjectMolecule* I, int sele, ObjectMoleculeOpRec* op)
{
  ...
  if (sele >= 0) {
    ...
    case OMOP_SFIT: /* state fitting within a single object */
      vt = pymol::malloc<float>(
          3 * op->nvv2); /* temporary (matching) target vertex pointers */
      cnt = 0;
      for (a = 0; a < I->NAtom; a++) {
        s = I->AtomInfo[a].selEntry;
        if (SelectorIsMember(G, s, sele)) {
      ...
      if (cnt) { /* only perform action for selected object */
        ...
        for (b = 0; b < I->NCSet; b++) {
          ...
          if (I->CSet[b] && (b != op->i2)) {
            ...
            if (op->nvv1 != op->nvv2) {
              ...
              "Executive-Warning: Missing atoms in state %d (%d instead of "
              "%d).\n",
            ...
            if (op->nvv1) {
              if (op->i1 != 0) /* fitting flag */
                rms = MatrixFitRMSTTTf(
                    G, op->nvv1, op->vv1, vt, nullptr, op->ttt);
              else
                    ...
                    if (SelectorIsMember(G, s, sele)) {
                      ...
                      if (a1 >= 0) {
                        ...
                        if (match_flag) {
                          ...
                          *(vt2 + 1) =
                              ((premult * (*(vt2 + 1))) + *(vv2++)) / divisor;
                          *(vt2 + 2) =
                              ((premult * (*(vt2 + 2))) + *(vv2++)) / divisor;
                        }
                      }
                    }
                  }
        ...
        VLASize(op->f1VLA, float,
            I->NCSet); /* NOTE this action is object-specific! */
      }
      FreeP(vt);
      break;
    case OMOP_OnOff:
            ...
            if (b < I->NCSet) {
              if (I->CSet[b]) {
                ...
                if (a1 >= 0) {
                  ...
                  vv1 = op->vv1 +
                        3 * (op->i1 * offset + op->i2); /* atom-based offset */
                  vv2 = I->CSet[b]->coordPtr(a1);
                  *(vv1++) = *(vv2++);
                  *(vv1++) = *(vv2++);
                  *(vv1++) = *(vv2++);
              ...
              if (I->CSet[b]) {
                ...
                if (a1 >= 0) {
                  if (op->ii1[op->i1 * offset + op->i2]) { /* copy flag */
                    vv1 = op->vv1 + 3 * (op->i1 * offset +
                                            op->i2); /* atom-based offset */
                    vv2 = I->CSet[b]->coordPtr(a1);
                    *(vv2++) = *(vv1++);
                    *(vv2++) = *(vv1++);
                    *(vv2++) = *(vv1++);
    ...
    default: {
      ...
      // only do this for "label2" command (better logic in
      // WrapperObjectSubScript)
      ...
      if (use_stereo) {
        ...
        " NO_MMLIBS-Warning: stereochemistry not supported in this PyMOL "
        "build.\n" ENDFB(G);
      }
      if (use_text_type) {
        PRINTFB(G, FB_ObjectMolecule, FB_Warnings)
        " NO_MMLIBS-Warning: automatic 'text_type' assignment not supported in "
        "this PyMOL build.\n" ENDFB(G);
      }
#endif
#endif
      ...
      for (a = 0; a < I->NAtom; a++) {
                ...
                if (!op->s1[0]) {
                  ...
                  case cExecutiveLabelEvalOn: {
                    ...
                    if (PLabelAtom(I->G, I, cs, expr_co, a)) {
                      if (ai->label) {
                        op->i1++; /* only if the string has been set, report
                                     labelled */
                      }
                      ai->visRep |= cRepLabelBit;
                      hit_flag = true;
                  ...
                  case cExecutiveLabelEvalAlt: {
                    if (PLabelAtomAlt(
                            I->G, &I->AtomInfo[a], I->Name, op->s1, a)) {
                      if (ai->label) {
                        op->i1++; /* only if the string has been set, report
                                     labelled */
                      }
...
#ifndef _PYMOL_NOPY
                if (PAlterAtom(I->G, I, cs, expr_co, op->i2, a, op->py_ob1))
                  op->i1++;
                else
#endif
#ifdef _WEBGL
              ...
              if (ok) {
                if (op->i2 < I->NCSet) {
                  ...
                  if (cs) {
                    ...
                    if (a1 >= 0) {
#ifndef _PYMOL_NOPY
                      if (PAlterAtomState(I->G, expr_co, op->i3, I, cs, a, a1,
                              op->i2, op->py_ob1)) {
                        op->i1++;
                        hit_flag = true;
                      } else
        ...
        case OMOP_CSetMoment:
          ...
          if ((op->cs1 >= 0) && (op->cs1 < I->NCSet)) {
            ...
            if ((I->NCSet == 1) && (SettingGet_b(G, nullptr, I->Setting.get(),
                                       cSetting_static_singletons))) {
              cs = I->CSet[0]; /*treat static singletons as present in each
                                  state */
            }
          }
            ...
            if (cs) {
              ...
              if (SelectorIsMember(G, s, sele)) {
                ...
                case OMOP_VERT:
                  ...
                  a1 = cs->atmToIdx(a);
                  if (a1 >= 0) {
                    /* if a1 is a valid atom index, then copy it's xyz
                     * coordinates into vv1; increment the counter, nvv1 */
                    VLACheck(op->vv1, float, (op->nvv1 * 3) + 2);
                    vv2 = cs->coordPtr(a1);
                    vv1 = op->vv1 + (op->nvv1 * 3);
                    *(vv1++) = *(vv2++);
                ...
                case OMOP_SVRT:
                  ...
                  if (b == op->i1) {
                    ...
                    if (a1 >= 0) {
                      ...
                      op->i1VLA[op->nvv1] =
                          a; /* save atom index for later comparisons */
                      vv2 = cs->coordPtr(a1);
                      vv1 = op->vv1 + (op->nvv1 * 3);
                      *(vv1++) = *(vv2++);
                      *(vv1++) = *(vv2++);
                  ...
                  break;
                case OMOP_MOME:
                  /* Moment of inertia tensor - unweighted - assumes v1 is
                   * center of molecule */
                  a1 = cs->atmToIdx(a);
                  if (a1 >= 0) {
                    subtract3f(cs->coordPtr(a1), op->v1, v1);
              ...
              case OMOP_INVA:
                /* shouldn't this be calling the object invalidation routine
                 * instead? */
                if (inv_flag) {
    ...
    if (hit_flag) {
      ...
      case OMOP_OnOff:
        ...
        break;
      case OMOP_RevalenceFromSource:
        if (ObjectMoleculeXferValences(
                I, op->i1, op->i2, op->i3, op->obj3, op->i4, op->i5, op->i6)) {
...
void ObjectMoleculeGetAtomSele(const ObjectMolecule* I, int index, char* buffer)
{
  ...
  snprintf(buffer, OrthoLineLength, "/%s/%s/%s/%s`%d%s/%s`%s", I->Name,
      ...
      inscode_str, LexStr(G, ai->name), ai->alt);
}
...
std::string ObjectMoleculeGetAtomSeleLog(
    const ObjectMolecule* I, int index, int quote)
{
...
void ObjectMoleculeGetAtomSeleLog(
    const ObjectMolecule* I, int index, char* buffer, int quote)
{
...
static void ObjMolCoordSetUpdateSpawn(PyMOLGlobals* G,
    CCoordSetUpdateThreadInfo* Thread, int n_thread, int n_total)
{
  if (n_total == 1) {
...
void ObjectMolecule::update()
{
  ...
  }
  {
    ...
    if ((I->NCSet == 1) && (SettingGet_b(G, I->Setting.get(), nullptr,
                               cSetting_static_singletons))) {
      start = 0;
      stop = 1;
    }
      ...
      if (multithread && (n_thread) && (stop - start) > 1) {
        ...
        for (a = start; a < stop; a++)
          if ((a < I->NCSet) && I->CSet[a])
            cnt++;
        {
          CCoordSetUpdateThreadInfo* thread_info =
              pymol::malloc<CCoordSetUpdateThreadInfo>(cnt);
          if (thread_info) {
            cnt = 0;
            for (a = start; a < stop; a++) {
              if ((a < I->NCSet) && I->CSet[a]) {
      ...
      { /* single thread */
        for (a = start; a < stop; a++) {
          if ((a < I->NCSet) && I->CSet[a] && (!G->Interrupt)) {
            ...
            PRINTFB(G, FB_ObjectMolecule, FB_Blather)
            " ObjectMolecule-DEBUG: updating representations for state %d of "
            "\"%s\".\n",
                a + 1, I->Name ENDFB(G);
            I->CSet[a]->update(a);
          }
...
void ObjectMolecule::invalidate(cRep_t rep, cRepInv_t level, int state)
{
  ...
  if (level >= cRepInvColor) {
    /* after label, this gets called, so we shouldn't invalidate types b/c
       PYMOL-317 not sure if that is the exact level we should cut this off at
       1/28/13 BB: changed from >cRepInvText to >=cRepInvColor for invalidating
       surface colors during gadget invalidation */
...
/*========================================================================*/
int ObjectMoleculeMoveAtom(
    ObjectMolecule* I, int state, int index, const float* v, int mode, int log)
{
  int result = 0;
  PyMOLGlobals* G = I->G;
  CoordSet* cs;
  if (!(I->AtomInfo[index].protekted == cAtomProtected_explicit)) {
    ...
    state = state % I->NCSet;
    if ((!I->CSet[state]) &&
        (SettingGet_b(G, I->Setting.get(), nullptr, cSetting_all_states)))
      state = 0;
    cs = I->CSet[state];
    if (cs) {
  ...
  if (log) {
    ...
    if (SettingGetGlobal_i(G, cSetting_logging)) {
      ...
      sprintf(line,
          "cmd.translate_atom(\"%s\",%15.9f,%15.9f,%15.9f,%d,%d,%d)\n", buffer,
          v[0], v[1], v[2], state + 1, mode, 0);
      PLog(G, line, cPLog_no_flush);
    }
  }
  return (result);
...
/*========================================================================*/
int ObjectMoleculeMoveAtomLabel(
    ObjectMolecule* I, int state, int index, float* v, int log, float* diff)
{
  int result = 0;
  CoordSet* cs;
  if (!(I->AtomInfo[index].protekted == cAtomProtected_explicit)) {
...
/*========================================================================*/
int ObjectMoleculeGetBondPaths(
    ObjectMolecule* I, int atom, int max, ObjectMoleculeBPRec* bp)
{
...
/*========================================================================*/
int*** ObjectMoleculeGetBondPrint(
    ObjectMolecule* I, int max_bond, int max_type, int* dim)
{
  int a, b, i, c;
  int at1, at2;
  int*** result = nullptr;
...
/*========================================================================*/
float ObjectMoleculeGetAvgHBondVector(
    ObjectMolecule* I, int atom, int state, float* v, float* incoming)
...
/* computes average / optima hydrogen bonding vector for an atom */
{
  ...
  auto const* cs = I->getCoordSet(state);
  if (cs) {
    if (CoordSetGetAtomVertex(
            cs, atom, v_atom)) { /* atom exists in this C-set */
      for (auto const& neighbor : AtomNeighbors(I, atom)) {
        auto const a2 = neighbor.atm;
        ...
        if (!isH ||
            (ai.protons == cAN_O && isH)) { /* ignore hydrogens unless water*/
...
/*========================================================================*/
int ObjectMoleculeGetAtomVertex(
    const ObjectMolecule* I, int state, int index, float* v)
{
  int result = 0;
  if (state < 0)
...
/*========================================================================*/
int ObjectMoleculeGetAtomTxfVertex(
    const ObjectMolecule* I, int state, int index, float* v)
{
  int result = 0;
...
/*========================================================================*/
int ObjectMoleculeSetAtomVertex(
    ObjectMolecule* I, int state, int index, float* v)
{
  int result = 0;
  if (state < 0)
    state = SettingGet_i(I->G, nullptr, I->Setting.get(), cSetting_state) - 1;
...
ObjectMolecule::ObjectMolecule(PyMOLGlobals* G, int discreteFlag)
    : pymol::CObject(G)
{
  auto I = this;
  int a;
...
/*========================================================================*/
pymol::Result<> ObjectMoleculeDeleteStates(
    ObjectMolecule* I, const std::vector<int>& states)
{
  auto& CSet = I->CSet;
...
ObjectMolecule* ObjectMoleculeReadPDBStr(PyMOLGlobals* G, ObjectMolecule* I,
    const char* PDBStr, int state, int discrete, char* pdb_name,
    const char** next_pdb, PDBInfoRec* pdb_info, int quiet, int* model_number)
{
  CoordSet* cset = nullptr;
  pymol::vla<AtomInfoType> atInfo;
  int ok = true;
  ...
  while (repeatFlag) {
    ...
    if (ok) {
      ...
      if (ok)
        cset = ObjectMoleculePDBStr2CoordSet(G, start, &atInfo, &restart,
            segi_override, pdb_name, next_pdb, pdb_info, quiet, model_number);
      CHECKOK(ok, cset);
      if (ok) {
        nAtom = cset->NIndex;
      }
    ...
    if (ok) {
      ...
      if (isNew) {
        ...
        ok &= ObjectMoleculeMerge(
            I, std::move(atInfo), cset, true, aic_mask, true);
        /* NOTE: will release atInfo */
      }
      if (isNew)
        I->NAtom = nAtom;
      ...
      if (I->Symmetry) {
        /* check scale records */
        if (pdb_info && pdb_info->scale.flag[0] && pdb_info->scale.flag[1] &&
    ...
    const char* buffer, AtomInfoType** atInfoPtr, const char** restart)
{
  ...
  if (ok) {
    p = ncopy(cc, p, 6); // could this be too small for large molecules with
                         // #atoms > 999999 ?
    if (sscanf(cc, "%d", &nAtom) != 1)
...
void AtomInfoSettingGenerateSideEffects(
    PyMOLGlobals* G, ObjectMolecule* obj, int index, int id)
{
  switch (index) {
...
static int AtomInfoInOrder(
    PyMOLGlobals* G, const AtomInfoType* atom, int atom1, int atom2)
{
  return (AtomInfoCompare(G, atom + atom1, atom + atom2) <= 0);
}

static int AtomInfoInOrderIgnoreHet(
    PyMOLGlobals* G, const AtomInfoType* atom, int atom1, int atom2)
{
  return (AtomInfoCompareIgnoreHet(G, atom + atom1, atom + atom2) <= 0);
}

static int AtomInfoInOrigOrder(
    PyMOLGlobals* G, const AtomInfoType* atom, int atom1, int atom2)
{
...
int* AtomInfoGetSortedIndex(PyMOLGlobals* G, const ObjectMolecule* obj,
    const AtomInfoType* rec, int n, int** outdex)
{
  ...
  if (obj && obj->DiscreteFlag) {
    ...
    UtilSortIndexGlobals(G, n, rec, index,
        (UtilOrderFnGlobals*) (SettingGet_b(
                                   G, setting, nullptr, cSetting_retain_order)
                                   ? AtomInfoInOrigOrder
                               : SettingGet_b(G, setting, nullptr,
                                     cSetting_pdb_hetatm_sort)
                                   ? AtomInfoInOrder
