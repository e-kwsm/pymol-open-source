...
struct AtomInfoType_1_8_1 {
  ...
  {
    return anisou[0] || anisou[1] || anisou[2] || anisou[3] || anisou[4] ||
           anisou[5];
  }
...
public:
  AtomInfoTypeConverter(PyMOLGlobals* G_, int natom)
      : G(G_)
      , NAtom(natom)
  {
  }
  ...
  lexidx_int_t to_lexidx_int(const lexidx_t& idx) { return idx; }
  ...
  template <size_t N>
  inline void copy_attr_s(char (&dest)[N], const lexidx_t& src)
  {
