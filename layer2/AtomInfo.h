...
typedef struct AtomInfoType {
  ...
  // methods
  bool isHydrogen() const { return protons == cAN_H; }

  bool isMetal() const
  {
    return ((protons > 2 && protons < 5) || (protons > 10 && protons < 14) ||
            (protons > 18 && protons < 32) || (protons > 36 && protons < 51) ||
            (protons > 54 && protons < 85) || protons > 86);
  }
...
int AtomInfoMatch(PyMOLGlobals* G, const AtomInfoType* at1,
    const AtomInfoType* at2, bool, bool);
int AtomInfoCompare(
...
int AtomInfoSameSegmentP(
    PyMOLGlobals* G, const AtomInfoType* at1, const AtomInfoType* at2);
int AtomInfoSequential(PyMOLGlobals* G, const AtomInfoType* at1,
    const AtomInfoType* at2, int mode);
...
void AtomInfoBracketResidueFast(PyMOLGlobals* G, const AtomInfoType* ai0,
    int n0, int cur, int* st, int* nd);
...
void AtomInfoCombine(
    PyMOLGlobals* G, AtomInfoType* dst, AtomInfoType&& src, int mask);
...
void SettingSet(PyMOLGlobals* G, int index, V value, T* ai)
{
