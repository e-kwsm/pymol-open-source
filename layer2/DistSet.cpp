...
int DistSetMoveLabel(DistSet* I, int a1, float* v, int mode)
{
  ...
  if (!lp.mode) {
    ...
    const float* lab_pos = SettingGet_3fv(
        obj->G, nullptr, obj->Setting.get(), cSetting_label_position);
 ...
 * @param obj object molecule, can be nullptr so then all items in I will be
 * updated
  ...
  for (auto& listitem : I->MeasureInfo) {
    ...
    for (i = 0; i < N; i++) {
      ...
      if (ObjectMoleculeGetAtomVertex(
              eoo->obj, memb->state[i], eoo->atm, varDst + i * 3))
        rVal++;
    }
  }
    ...
    PyMOLGlobals* G, PyObject* list)
{
  ...
  for (i = 0; i < ll; i++) {
    ...
    if (val && PyList_Check(val) && PyList_Size(val) > 2) {
      ...
      CPythonVal_PConvPyListToIntArrayInPlace_From_List(
          G, val, 2, item->state, N);
      CPythonVal_Free(tmp);
...
DistSet* DistSetFromPyList(PyMOLGlobals* G, PyObject* list)
{
  ...
  ok_assert(1, CPythonVal_PConvPyListToFloatVLANoneOkay_From_List(
                   G, list, 1, &I->Coord));

  ok_assert(2, ll > 2);

  ok_assert(
      1, CPythonVal_PConvPyIntToInt_From_List(G, list, 3, &I->NAngleIndex));
  ok_assert(1, CPythonVal_PConvPyListToFloatVLANoneOkay_From_List(
                   G, list, 4, &I->AngleCoord));
  ok_assert(
      1, CPythonVal_PConvPyIntToInt_From_List(G, list, 5, &I->NDihedralIndex));
  ok_assert(1, CPythonVal_PConvPyListToFloatVLANoneOkay_From_List(
                   G, list, 6, &I->DihedralCoord));
...
PyObject* DistSetAsPyList(DistSet* I)
{
  ...
  if (I) {
    ...
    PyList_SetItem(
        result, 1, PConvFloatArrayToPyListNullOkay(I->Coord, I->NIndex * 3));
    PyList_SetItem(result, 2,
        PConvAutoNone(nullptr)); // I->LabCoord recalculated in RepDistLabelNew
    ...
    PyList_SetItem(result, 6,
        PConvFloatArrayToPyListNullOkay(
            I->DihedralCoord, I->NDihedralIndex * 3));
    // DistSet->Setting never gets set (removed BB 11/14), was state settings?
    PyList_SetItem(
        result, 7, PConvAutoNone(nullptr) /* SettingAsPyList(I->Setting) */);
    if (!I->LabPos.empty()) {
