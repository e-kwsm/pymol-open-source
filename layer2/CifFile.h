...
public:
  ...
  /// Data blocks
  const std::map<std::string, cif_data>& datablocks() const
  {
    return m_datablocks;
  }
};
...
public:
   ...
   * string, unlike as<const char*>() which returns nullptr as the default
   * value. If `pos >= size()` then return `d`.
   * @param pos element index (= row index in loop)
   * @param d default value for unknown/inapplicable elements
   */
  const char* as_s(unsigned pos = 0, const char* d = "") const
  {
    if (std::get_if<cif_detail::cif_str_array>(&m_array)) {
      ...
      m_internal_str_cache = std::visit(
          [](auto&& arg) -> std::string {
