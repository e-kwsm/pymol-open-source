...
struct CoordSet : CObjectState {
  ...
  float* coordPtr(int idx) { return Coord + idx * 3; }

  // read pointer to coordinate
  const float* coordPtr(int idx) const { return Coord + idx * 3; }

  float const* coordPtrSym(
      int idx, pymol::SymOp const& symop, float* v_out, bool inv = false) const;

  AtomInfoType* getAtomInfo(int idx) { return Obj->AtomInfo + IdxToAtm[idx]; }
  ...
  pymol::vla<BondType>
      TmpBond;          /* actual bond info is stored in ObjectMolecule */
  int NTmpLinkBond = 0; /* optional, temporary storage of linkage  info. */
  pymol::vla<BondType>
      TmpLinkBond; /* first atom is in obj, second is in cset */
...
bool CoordSetInsureOrthogonal(PyMOLGlobals* G, CoordSet* cset, const float* sca,
    const CCrystal* cryst = nullptr, bool quiet = true);
...
int CoordSetMerge(ObjectMolecule* OM, CoordSet* I,
    const CoordSet* cs); /* must be non-overlapping */
...
#define ATOMSTATEGETSETTINGARGS                                                \
  PyMOLGlobals *G, const ObjectMolecule *obj, const CoordSet *cs, int idx,     \
      const AtomInfoType *ai, int setting_id
...
template <typename V> V SettingGet(const CoordSet& cs, int index)
{
