...
/* RepCylinder -- This function is a helper function that generates a cylinder
 * for RepCylBond. Depending on the s1 and s2 arguments, it will either draw a
 * bond or a half bond.  Half bonds always have flat caps on the inside.  The
 * isRamped argument specifies whether the colors are interpolated only when s1
 * and s2 are set.
 *
 * PARAMS
 *
 ...
 * v2pickcolor - second pick color info (ptr to structure Pickable) specified
 * (optional)
 *
    ...
    Pickable* v2pickcolor = nullptr)
{
  ...
  if (s1 && s2) {
    ...
    if (v2color) {
      ok &= (bool) cgo->add<cgo::draw::shadercylinder2ndcolor>(
          cgo, v1, axis, tube_size, cap, v2color, v2pickcolor);
    } else {
      ok &=
          (bool) cgo->add<cgo::draw::shadercylinder>(v1, axis, tube_size, cap);
    }
  } else {
    ...
    if (s1) {
      ...
      ok &= (bool) cgo->add<cgo::draw::shadercylinder>(
          v1new, axis, tube_size, cap);
    }
  }
  return ok;
}
...
void RepCylBond::render(RenderInfo* info)
{
  ...
  if (ray) {
    ...
    if (I->renderCGO && (CGOCheckWhetherToFree(G, I->renderCGO) ||
                            ((bool) I->renderCGO->use_shader) != use_shader)) {
      CGOFree(I->renderCGO);
      I->renderCGO = 0;
    }
    ...
    if (pick) {
      ...
      if (I->renderCGO) {
        CGORenderPicking(I->renderCGO, info, &I->context, I->cs->Setting.get(),
            I->obj->Setting.get());
      }
    ...
    bool b2masked)
{
  ...
  float dgap = zradius * 6.f,
        dlen = zradius * 3.f; // dlen - dash length, dgap - gap length
  ...
  mult3f(naxis, dlen + dgap,
      adddtot); // adddtot - length of dash plus gap as x/y/z vector
  ...
  if (s1) {
    ...
    for (dplace = dgap; (dplace + dlen) < blen / 2.f;) {
      ...
      ok &= RepCylinder(I->primitiveCGO, true, true, false, placep, placep2,
          true, true, zradius);
    ...
    if (!s2) {
      if (dplace < blen / 2.f) {
        ...
        add3f(placep, adddlen, placep2);
        ok &= RepCylinder(I->primitiveCGO, s1, s2, false, placep, placep2, true,
            false, zradius);
        add3f(placep, adddtot, placep);
        dplace += dlen + dgap;
      }
    ...
    if (dplace < blen / 2.f) {
      ...
      ok &= RepCylinder(I->primitiveCGO, s1, s2, false, placep, placep2, false,
          true, zradius);
      add3f(placep, adddtot, placep);
      dplace += dlen + dgap;
    }
  ...
  if (s2) {
    if (dplace < blen / 2.f) {
      ...
      ok &= RepCylinder(I->primitiveCGO, true, true, false, placep, placep2,
          true, true, zradius, rgb2, &pickcolor2);
      add3f(placep, adddtot, placep);
      dplace += dlen + dgap;
    }
    ok &= CGOColorv(I->primitiveCGO, rgb2);
...
static int RepValence(RepCylBond* I, CGO* cgo, bool s1, bool s2, bool isRamped,
    const float* v1, const float* v2, const int* other, int a1, int a2,
    const float* coord, const float* color1, const float* color2, int ord,
    float tube_size, bool fixed_r, float scale_r, Pickable pickdata[] = nullptr)
{
  ...
  case 2: {
    ...
    if (ok)
      ok &= RepCylinder(cgo, s1, s2, isRamped, v1t, v2t, true, true, radius,
          color2, pickdataptr);
    subtract3f(v1, t, v1t);
    subtract3f(v2, t, v2t);
    if (ok)
  ...
  case 4: {
    ...
    if (double_sided) {
      ...
      if (ok)
        ok &=
            RepCylinder(cgo, s1, s2, isRamped, v1tt, v2tt, true, true, radius2);
    }
    ...
    if (ok)
      ok &= RepCylinder(cgo, s1, s2, isRamped, v1, v2, true, true, radius,
          color2, pickdataptr);
  } break;
  }
  return ok;
...
Rep* RepCylBondNew(CoordSet* cs, int state)
{
  ...
  valence =
      SettingGet_b(G, cs->Setting.get(), obj->Setting.get(), cSetting_valence);
  valence_flag = (valence != 0.0F);

  stick_color = SettingGet_color(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_color);
  cartoon_side_chain_helper = SettingGet_b(G, cs->Setting.get(),
      obj->Setting.get(), cSetting_cartoon_side_chain_helper);
  ribbon_side_chain_helper = SettingGet_b(G, cs->Setting.get(),
      obj->Setting.get(), cSetting_ribbon_side_chain_helper);

  transp = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_transparency);
  hide_long = SettingGet_b(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_hide_long_bonds);
  ...
  radius = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_radius);
  half_bonds = SettingGet_b(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_half_bonds);
  na_mode = SettingGet_i(G, cs->Setting.get(), obj->Setting.get(),
      cSetting_cartoon_nucleic_acid_mode);
  int na_mode_ribbon = SettingGet_i(G, cs->Setting.get(), obj->Setting.get(),
      cSetting_ribbon_nucleic_acid_mode);
  h_scale = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_h_scale);
  auto valence_zero_scale = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_valence_zero_scale);
  auto valence_zero_mode = SettingGet_i(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_valence_zero_mode);

  auto I = new RepCylBond(cs, state);

  I->primitiveCGO = CGONew(G);
  if (ok && obj->NBond) {
    stick_ball = SettingGet_b(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_ball);

    shader_mode = SettingGetGlobal_b(G, cSetting_use_shaders) &&
                  SettingGetGlobal_b(G, cSetting_stick_as_cylinders) &&
                  SettingGetGlobal_b(G, cSetting_render_as_cylinders) &&
                  SettingGetGlobal_b(G, cSetting_stick_use_shader);
    ...
    if (valence_found) { /* build list of up to 2 connected atoms for each atom
      ...
      CHECKOK(ok, other);
      if (ok) {
        fixed_radius = SettingGet_b(G, cs->Setting.get(), obj->Setting.get(),
            cSetting_stick_fixed_radius);
        scale_r = SettingGet_f(G, cs->Setting.get(), obj->Setting.get(),
            cSetting_stick_valence_scale);
      }
    }

    /* spheres for stick & balls */
    stick_ball_ratio = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_ball_ratio);
    stick_ball_color = SettingGet_color(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_ball_color);
    ...
    for (a = 0; ok && a < obj->NBond; ++a, ++b) {
      ...
      if ((a1 >= 0) && (a2 >= 0)) {
        ...
        auto bd_stick_color =
            BondSettingGetWD(G, b, cSetting_stick_color, stick_color);
      ...
      inv_sym_bond:
        ...
        if (hide_long && (s1 || s2)) {
          ...
          if (!within3f(vv1, vv2, cutoff)) /* atoms separated by more than 90%
                                              of the sum of their vdw radii */
            s1 = s2 = 0;
        }
        ...
        if ((s1 || s2) && (ati1->flags & ati2->flags & cAtomFlag_polymer)) {
          if ((cRepCartoonBit & ati1->visRep & ati2->visRep)) {
            bool sc_helper =
                AtomSettingGetWD(G, ati1, cSetting_cartoon_side_chain_helper,
                    cartoon_side_chain_helper) ||
                AtomSettingGetWD(G, ati2, cSetting_cartoon_side_chain_helper,
                    cartoon_side_chain_helper);
            if (sc_helper && SideChainHelperFilterBond(G, marked, ati1, ati2,
                                 b1, b2, na_mode, &c1, &c2))
              s1 = s2 = 0;
          }
          ...
          if ((s1 || s2) && (cRepRibbonBit & ati1->visRep & ati2->visRep)) {
            bool sc_helper =
                AtomSettingGetWD(G, ati1, cSetting_ribbon_side_chain_helper,
                    ribbon_side_chain_helper) ||
                AtomSettingGetWD(G, ati2, cSetting_ribbon_side_chain_helper,
                    ribbon_side_chain_helper);
            if (sc_helper && SideChainHelperFilterBond(G, marked, ati1, ati2,
                                 b1, b2, na_mode_ribbon, &c1, &c2))
              s1 = s2 = 0;
          }
        }

        /* This means that if stick_ball gets changed, the RepCylBond needs to
         * be completely invalidated */

        auto stick_ball_impl = [&](AtomInfoType* ati1, int b1, int c1,
                                   float const* vv1) {
          int stick_ball_1 =
              AtomSettingGetWD(G, ati1, cSetting_stick_ball, stick_ball);
          if (stick_ball_1) {
            float vdw = stick_ball_ratio *
                        ((ati1->protons == cAN_H) ? bd_radius : bd_radius_full);
            ...
            int sbc1 =
                (stick_ball_color == cColorDefault) ? c1 : stick_ball_color;
            float rgb1[3];
            ...
            CGOPickColor(
                I->primitiveCGO, b1, ati1->masked ? cPickableNoPick : a);
        ...
        if (s1 || s2) {
          ...
          if (s1)
            stick_ball_impl(ati1, b1, c1, vv1);
          if (s2)
            stick_ball_impl(ati2, b2, c2, vv2);
          ...
          if (!ord) {
            // zero order bonds
            ok &= RepZeroOrderBond(I, I->primitiveCGO, s1, s2, vv1, vv2,
                bd_radius, rgb1, rgb2, b1, b2, a, ati1->masked, ati2->masked);
          } else {
            all_zero_order_bond_atoms.erase(b1);
            all_zero_order_bond_atoms.erase(b2);

            bool bd_valence_flag =
                (ord > 1) && (ord < 5) &&
                BondSettingGetWD(G, b, cSetting_valence, valence_flag);
            ...
            if (bd_valence_flag) {
              ...
              ok &= RepValence(I, I->primitiveCGO, s1, s2, isRamped, vv1, vv2,
                  other, a1, a2, cs->Coord, rgb1, rgb2, ord, bd_radius,
                  fixed_radius, scale_r, pickdata);
            } else {
              ok &= CGOColorv(I->primitiveCGO, rgb1);
              ok &= CGOPickColor(
                  I->primitiveCGO, b1, ati1->masked ? cPickableNoPick : a);
              ...
              if (ok) {
                ...
                if (shader_mode) {
                  ...
                  if (drawcap1)
                    capdrawn[b1] = bd_radius;
                  if (drawcap2)
                    capdrawn[b2] = bd_radius;
                }
              }
            }
          }
    ...
    /* for all zero order bond atoms that do not have any other ordered bonds,
       a sphere should be rendered for it (a cylinder with vertices that are
       almost exactly the same is used to render a sphere so that we won't need
       to use the sphere shader excessively. */
    for (auto at : all_zero_order_bond_atoms) {
      ...
      // instead of using the sphere shader, we use a cylinder with 0 length (or
      // very close to zero, since the shader uses the axis for many
      // computations/checks
      add3f(v1, v, v2);
      ...
      ok &= CGOColorv(I->primitiveCGO, rgb1);
      ok &= CGOPickColor(
          I->primitiveCGO, at, ai1->masked ? cPickableNoPick : cPickableAtom);
      ok &= RepCylinder(
          I->primitiveCGO, true, true, false, v1, v2, true, true, radius);
    }
  }
  FreeP(other);
  FreeP(marked);
...
static void RepCylinderImmediate(const float* v1arg, const float* v2arg,
    int nEdge, int frontCapArg, int endCapArg, float overlap, float nub,
...
void RepCylBondRenderImmediate(CoordSet* cs, RenderInfo* info)
{
  ...
  if (info->ray || info->pick || (!(G->HaveGUI && G->ValidContext)))
    ...
    int nEdge = SettingGet_i(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_quality);
    float radius = fabs(SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_radius));
    float overlap = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_overlap);
    float nub = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_stick_nub);
        ...
        if (((ai1 = ai + b1)->visRep & cRepCylBit) &&
          ...
          if ((a1 >= 0) && (a2 >= 0)) {
            ...
            if (c1 == c2) { /* same colors -> one cylinder */
              ...
              /* overlap is half since it is one cylinder representing both
               * halfs of a bond */
              RepCylinderImmediate(
                  v1, v2, nEdge, 1, 1, overlap_r, nub_r, radius, nullptr);
              ...
              RepCylinderImmediate(
                  v1, avg, nEdge, 1, 0, overlap_r, nub_r, radius, &dir);
              ...
              RepCylinderImmediate(
                  v2, avg, nEdge, 1, 0, overlap_r, nub_r, radius, &dir);
              if (dir) {
                FreeP(dir);
                dir = 0;
              }
