...
struct ObjectMapState : public CObjectState {
  ...
  int Div[3]; /* NOTE: Div is only meaningful for maps defined relative to a
                 unit cell */
  ...
  float Corner[24];
  std::vector<int> Dim; /* this field is redundant and should be eliminated --
                           if exists, must match FDim */
...
ObjectMapState* ObjectMapNewStateFromDesc(
    PyMOLGlobals* G, ObjectMap* I, ObjectMapDesc* md, int state, int quiet);
...
std::vector<char> ObjectMapStateToCCP4Str(
    const ObjectMapState* ms, int quiet, int);

template <typename T>
std::vector<char> ObjectMapGetCCP4Str(
    PyMOLGlobals* G, const T* ptr, int state, int quiet, int format)
{
  return ObjectMapStateToCCP4Str(
      getObjectMapState(G, ptr, state), quiet, format);
}
