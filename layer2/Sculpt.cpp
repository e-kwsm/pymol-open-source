...
#define nb_hash(v)                                                             \
  (((((int) *(v)) >> 2) & 0x0003F) | ((((int) *(v + 1)) << 4) & 0x00FC0) |     \
      ((((int) *(v + 2)) << 10) & 0x3F000))

#define nb_hash_off_i0(v0i, d) ((((d) + v0i) >> 2) & 0x0003F)

#define nb_hash_off_i1(v1i, e) ((((e) + v1i) << 4) & 0x00FC0)

#define nb_hash_off_i2(v2i, f) ((((f) + v2i) << 10) & 0x3F000)
...
#define ex_hash_i0(a) (((a) ^ ((a) >> 5)) & 0x00FF)

#define ex_hash_i1(b) ((((b) << 5)) & 0xFF00)

#define ex_hash(a, b)                                                          \
  (((((a) ^ ((a) >> 5))) & 0x00FF) | (((((b) << 5))) & 0xFF00))

static float ShakerDoDist(
    float target, float* v0, float* v1, float* d0to1, float* d1to0, float wt)
{
  float d[3], push[3];
...
static float ShakerDoDistLimit(
    float target, float* v0, float* v1, float* d0to1, float* d1to0, float wt)
{
  float d[3], push[3];
...
static float ShakerDoDistMinim(
    float target, float* v0, float* v1, float* d0to1, float* d1to0, float wt)
{
  float d[3], push[3];
  float len, dev, dev_2, sc;
...
static void add_triangle_limits(
    ATLCall* ATL, int prev, int cur, float dist, int count)
{
  ATLCall* I = ATL;
  int n0;
  int n1;
    ...
    if (add_flag) {
      ...
      while ((atom1 = I->neighbor[n1]) >= 0) {
        if ((!I->ai[atom1].temp1) && (I->atom0 < atom1)) {
          ...
          if (((!I->discCSet) || ((I->cSet == I->discCSet[ref]) &&
                                     (I->cSet == I->discCSet[atom1]))) &&
            ...
            if ((ia >= 0) && (ib >= 0)) {
              ...
              ShakerAddDistCon(I->Shaker, I->atom0, atom1, dist_limit,
                  cShakerDistLimit, 1.0F);
            }
          }
          I->ai[atom1].temp1 = 1;
        }
    ...
    while ((atom1 = I->neighbor[n1]) >= 0) {
      if (I->ai[atom1].temp1 < 2) {
        ...
        if (!(count & 0x1)) { /* accumulate distances between even atoms only */
          if ((!I->discCSet) || ((I->cSet == I->discCSet[prev]) &&
                                    (I->cSet == I->discCSet[atom1]))) {
    ...
    int match_state, int match_by_segment)
{
  ...
  if ((state >= 0) && (state < obj->NCSet) && (obj->CSet[state])) {
    ...
    use_cache = SettingGet_i(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_memory);
    if (obj->NBond) {
      ...
      for (a = 0; a < obj->NBond; a++) {
        ...
        *(j++) = 2; /* 1-2 exclusion */
        ...
        if ((a1 >= 0) && (a2 >= 0)) {
          ...
          if (use_cache) {
            if (!SculptCacheQuery(G, cSculptBond, obj_atomInfo[b1].unique_id,
                    obj_atomInfo[b2].unique_id, 0, 0, &d))
              SculptCacheStore(G, cSculptBond, obj_atomInfo[b1].unique_id,
                  obj_atomInfo[b2].unique_id, 0, 0, d);
          }
      ...
      /* triangle relationships */
      {
        ...
        atl.min = SettingGet_i(
            G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_tri_min);
        atl.max = SettingGet_i(
            G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_tri_max);
        atl.mode = SettingGet_i(
            G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_tri_mode);
      ...
      if ((match_state >= 0) && (match_state < obj->NCSet) &&
          (!obj->DiscreteFlag)) {
        CoordSet* cs2 = obj->CSet[match_state];
        int n_site = 0;
        if (cs2) {
          float minim_min = SettingGet_f(G, cs->Setting.get(),
              obj->Setting.get(), cSetting_sculpt_min_min);
          float minim_max = SettingGet_f(G, cs->Setting.get(),
              obj->Setting.get(), cSetting_sculpt_min_max);
          float maxim_min = SettingGet_f(G, cs->Setting.get(),
              obj->Setting.get(), cSetting_sculpt_max_min);
          float maxim_max = SettingGet_f(G, cs->Setting.get(),
              obj->Setting.get(), cSetting_sculpt_max_max);
...

                {
                  ...
                  if ((i0a >= 0) && (i1a >= 0) && (i0b >= 0) && (i1b >= 0) &&
                    ...
                    if ((min_dist >= minim_min) && (min_dist <= minim_max)) {
                      ShakerAddDistCon(I->Shaker.get(), b0, b1, min_dist,
                          cShakerDistMinim, wt);
                    }
                    max_dist = (dist0 > dist1) ? dist0 : dist1;
                    if ((max_dist >= maxim_min) && (max_dist <= maxim_max)) {
                      ShakerAddDistCon(I->Shaker.get(), b0, b1, max_dist,
                          cShakerDistMaxim, wt);
                    }
                  }
                }
            ...
            if ((a0 >= 0) && (a1 >= 0) && (a2 >= 0)) {
              ...
              if (use_cache) {
                if (!SculptCacheQuery(G, cSculptAngl,
                        obj_atomInfo[b0].unique_id, obj_atomInfo[b1].unique_id,
                        obj_atomInfo[b2].unique_id, 0, &d))
                  SculptCacheStore(G, cSculptAngl, obj_atomInfo[b0].unique_id,
                      obj_atomInfo[b1].unique_id, obj_atomInfo[b2].unique_id, 0,
                      d);
              }

              ShakerAddDistCon(
                  I->Shaker.get(), b1, b2, d, cShakerDistAngle, 1.0F);
              ...
              if (linear[b0] && (linear[b1] || linear[b2])) {
                ...
                if (use_cache) {
                  if (!SculptCacheQuery(G, cSculptLine,
                    ...
                    SculptCacheStore(G, cSculptLine, obj_atomInfo[b1].unique_id,
                        obj_atomInfo[b0].unique_id, obj_atomInfo[b2].unique_id,
                        0, 0.0);
                }
              ...
              if ((a0 >= 0) && (a1 >= 0) && (a2 >= 0) && (a3 >= 0)) {
                ...
                if (use_cache) {
                  if (!SculptCacheQuery(G, cSculptPyra,
                    ...
                    SculptCacheStore(G, cSculptPyra, obj_atomInfo[b1].unique_id,
                        obj_atomInfo[b0].unique_id, obj_atomInfo[b2].unique_id,
                        obj_atomInfo[b3].unique_id, d);
                  if (!SculptCacheQuery(G, cSculptPyra + 1,
                    ...
                    SculptCacheStore(G, cSculptPyra + 1,
                        obj_atomInfo[b1].unique_id, obj_atomInfo[b0].unique_id,
                        obj_atomInfo[b2].unique_id, obj_atomInfo[b3].unique_id,
                        d2);
                }
                ...
                if ((b3 != b0) && (b3 > b1)) {
                  ...
                  if (planar[b0] && planar[b2]) {
                    ...
                    if (((obj_atomInfo[b1].protons == cAN_H) && single[b1] &&
                            (obj_atomInfo[b3].protons != cAN_H) &&
                            planar[b3]) ||
                        ((obj_atomInfo[b3].protons == cAN_H) && single[b3] &&
                            (obj_atomInfo[b1].protons != cAN_H) &&
                            planar[b1])) {
                      ...
                      if (!cycle) { /* don't add special amide constraints
                                       within small rings */
                        ...
                        if (((obj_atomInfo[b1].protons == cAN_H) &&
                                ...
                                (obj_atomInfo[b3].protons == cAN_O) &&
                                planar[b3]) ||
                            ((obj_atomInfo[b1].protons == cAN_H) &&
                                single[b3] &&
                                (obj_atomInfo[b2].protons == cAN_N) &&
                                (obj_atomInfo[b0].protons == cAN_C) &&
                                (obj_atomInfo[b1].protons == cAN_O) &&
                                planar[b1])) {
                          /* biased, asymmetric term for amides */
                          ShakerAddTorsCon(I->Shaker.get(), b1, b0, b2, b3,
                              cShakerTorsAmide);
                        } else {
                          /* biased, symmetric term for all others */
                          ShakerAddTorsCon(
                              I->Shaker.get(), b1, b0, b2, b3, cShakerTorsFlat);
                        }
                      }
                    }
                  }
                  ...
                  while (xoffset) {
                    ...
                    if ((abs(*(k + 3)) == 4) && (*(k + 1) == b1) &&
                        (*(k + 2) == b3)) {
                      if ((b0 != *(k + 4)) && (b2 != *(k + 5))) {
                        if (planar[b0] && planar[b2] && planar[*(k + 4)] &&
                            planar[*(k + 5)]) {
                    ...
                    if (planar[b0] && planar[b2]) {
...

                      {
                        ...
                        if (((obj_atomInfo[b0].protons != cAN_N) ||
                                (!WordMatchExact(G, obj_atomInfo[b0].resn,
                                    G->lex_const.PRO, true))) &&
                            ((obj_atomInfo[b2].protons != cAN_N) ||
                                (!WordMatchExact(G, obj_atomInfo[b2].resn,
                                    G->lex_const.PRO, true)))) {
                          ...
                          ShakerAddPlanCon(
                              I->Shaker.get(), b1, b0, b2, b3, d, cycle);

                          if (planar[b1] && planar[b3] &&
                              ((cycle == 5) || (cycle == 6))) {

                            /* also add minimum distance constraints to keep
                             * small rings from folding */

                            d = (float) diff3f(v1, v3);

                            ShakerAddDistCon(I->Shaker.get(), b1, b3, d,
                                cShakerDistBond, 1.0F);
                          }
                        }
                      }
                    }
      ...
      {
        /* longer-range exclusions (1-5,1-6,1-7,1-8,1-9) -- only locate & store
         * when needed */

        int mask = SettingGet_i(G, cs->Setting.get(), obj->Setting.get(),
            cSetting_sculpt_field_mask);
        int max_excl = SettingGet_i(
            G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_avd_excl);
        if (max_excl > 9)
          max_excl = 9;
...
static int SculptCheckBump(
    float* v1, float* v2, float* diff, float* dist, float cutoff)
{
  float d2;
  diff[0] = (v1[0] - v2[0]);
  diff[1] = (v1[1] - v2[1]);
...
static int SculptCGOBump(float* v1, float* v2, float vdw1, float vdw2,
    float cutoff, float min, float mid, float max, float* good_color,
    float* bad_color, int mode, CGO* cgo)
{
  float d2;
  float diff[3];
  float dist;
...

      {
        ...
        case 2:
          if (good_bad > mid) {
            ...
            {
              float* vertexVals =
                  CGODrawArrays(cgo, GL_LINES, CGO_VERTEX_ARRAY, 2);
...
static int SculptDoBump(float target, float actual, float* d, float* d0to1,
    float* d1to0, float wt, float* strain)
{
  float push[3];
...
static int SculptCheckAvoid(
    float* v1, float* v2, float* diff, float* dist, float avoid, float range)
{
  float d2, l2;
...
float SculptIterateObject(CSculpt* I, ObjectMolecule* obj, int state,
    int const n_cycle_arg, float* center)
{
  PyMOLGlobals* G = I->G;
  CShaker* shk;
  ...
  for (StateIterator iter(obj, state); iter.next();) {
    ...
    vdw = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_vdw_scale);
    vdw14 = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_vdw_scale14);
    vdw_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_vdw_weight);
    vdw_wt14 = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_vdw_weight14);
    bond_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_bond_weight);
    angl_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_angl_weight);
    pyra_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_pyra_weight);
    pyra_inv_wt = SettingGet_f(G, cs->Setting.get(), obj->Setting.get(),
        cSetting_sculpt_pyra_inv_weight);
    plan_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_plan_weight);
    line_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_line_weight);
    tri_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_tri_weight);
    tri_sc = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_tri_scale);

    min_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_min_weight);
    min_sc = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_min_scale);
    max_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_max_weight);
    max_sc = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_max_scale);

    mask = SettingGet_i(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_field_mask);
    hb_overlap = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_hb_overlap);
    hb_overlap_base = SettingGet_f(G, cs->Setting.get(), obj->Setting.get(),
        cSetting_sculpt_hb_overlap_base);
    tors_tole = SettingGet_f(G, cs->Setting.get(), obj->Setting.get(),
        cSetting_sculpt_tors_tolerance);
    tors_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_tors_weight);
    vdw_vis_mode = SettingGet_i(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_vdw_vis_mode);
    solvent_radius = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_solvent_radius);

    avd_wt = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_avd_weight);
    avd_gp = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_avd_gap);
    avd_rg = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_avd_range);
    avd_ex = SettingGet_i(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_avd_excl);
    if (avd_gp < 0.0F)
    ...
    if (vdw_vis_mode) {
      vdw_vis_min = SettingGet_f(G, cs->Setting.get(), obj->Setting.get(),
          cSetting_sculpt_vdw_vis_min);
      vdw_vis_mid = SettingGet_f(G, cs->Setting.get(), obj->Setting.get(),
          cSetting_sculpt_vdw_vis_mid);
      vdw_vis_max = SettingGet_f(G, cs->Setting.get(), obj->Setting.get(),
          cSetting_sculpt_vdw_vis_max);
    ...
    nb_skip = SettingGet_i(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sculpt_nb_interval);
    if (nb_skip > n_cycle)
      nb_skip = n_cycle;
    if (nb_skip < 0)
      nb_skip = 0;
    ...
    if (active_flag) {
          ...
          for (a = 0; a < ndc; a++) {
            ...
            if (eval_flag && !(exclude[b1] || exclude[b2])) {
              ...
              if ((a1 >= 0) && (a2 >= 0)) {
                ...
                switch (sdc_type) {
                case cShakerDistLimit:
                  strain = ShakerDoDistLimit(sdc->targ * tri_sc, v1, v2,
                      disp + b1 * 3, disp + b2 * 3, wt);
                  if (strain > 0.0F) {
                    cnt[b1]++;
                    cnt[b2]++;
                    total_strain += strain;
                  ...
                  break;
                case cShakerDistMaxim:
                  strain = ShakerDoDistLimit(sdc->targ * max_sc, v1, v2,
                      disp + b1 * 3, disp + b2 * 3, wt);
                  if (strain > 0.0F) {
                    cnt[b1]++;
                    cnt[b2]++;
                  ...
                  break;
                case cShakerDistMinim:
                  strain = ShakerDoDistMinim(sdc->targ * min_sc, v1, v2,
                      disp + b1 * 3, disp + b2 * 3, wt);
                  if (strain > 0.0F) {
                    cnt[b1]++;
                    cnt[b2]++;
                  ...
                  break;
                default:
                  total_strain += ShakerDoDist(
                      sdc->targ, v1, v2, disp + b1 * 3, disp + b2 * 3, wt);
                  cnt[b1]++;
                  cnt[b2]++;
                  total_count++;
                }
        ...
        if (cSculptLine & mask) {
          ...
          for (a = 0; a < nlc; a++) {
            ...
            if ((a0 >= 0) && (a1 >= 0) && (a2 >= 0) &&
              ...
              v1 = cs_coord + 3 * a1;
              v2 = cs_coord + 3 * a2;
              total_strain += ShakerDoLine(v0, v1, v2, disp + b0 * 3,
                  disp + b1 * 3, disp + b2 * 3, line_wt);
              total_count++;
            }
            slc++;
          }
        ...
        if (cSculptPyra & mask) {
          ...
          for (a = 0; a < npc; a++) {
            ...
            if ((a0 >= 0) && (a1 >= 0) && (a2 >= 0) && (a3 >= 0) &&
              ...
              v2 = cs_coord + 3 * a2;
              v3 = cs_coord + 3 * a3;
              total_strain += ShakerDoPyra(spc->targ1, spc->targ2, v0, v1, v2,
                  v3, disp + b0 * 3, disp + b1 * 3, disp + b2 * 3,
        ...
        if (cSculptPlan & mask) {
          ...
          for (a = 0; a < npc; a++) {
            ...
            if ((a0 >= 0) && (a1 >= 0) && (a2 >= 0) && (a3 >= 0) &&
              ...
              v2 = cs_coord + 3 * a2;
              v3 = cs_coord + 3 * a3;
              total_strain += ShakerDoPlan(v0, v1, v2, v3, disp + b0 * 3,
                  disp + b1 * 3, disp + b2 * 3, disp + b3 * 3, snc->target,
                  snc->fixed, plan_wt);
              total_count++;
              cnt[b0]++;
              cnt[b1]++;
        ...
        if (cSculptTors & mask) {
          ...
          for (a = 0; a < ntc; a++) {
            ...
            if ((a0 >= 0) && (a1 >= 0) && (a2 >= 0) && (a3 >= 0) &&
              ...
              v2 = cs_coord + 3 * a2;
              v3 = cs_coord + 3 * a3;
              total_strain += ShakerDoTors(stc->type, v0, v1, v2, v3,
                  disp + b0 * 3, disp + b1 * 3, disp + b2 * 3, disp + b3 * 3,
                  tors_tole, tors_wt);
              total_count++;
              cnt[b0]++;
              cnt[b1]++;
                      ...
                      while (offset) {
                        ...
                        if (b1 > b0) {
                          ...
                          if (ex > 3) {
                            ...
                            if (ex ==
                                10) { /* standard interaction -- no exclusion */
                              ...
                              if (cSculptVDW & mask) {
                                ...
                                if (vdw_vis_mode && cgo && (n_cycle < 1) &&
                                  ...
                                  SculptCGOBump(v0, v1, ai0->vdw, ai1->vdw,
                                      cutoff, vdw_vis_min, vdw_vis_mid,
                                      vdw_vis_max, good_color, bad_color,
                                      vdw_vis_mode, cgo);
                                }
                                if (SculptCheckBump(
                                        v0, v1, diff, &len, vdw_cutoff))
                    ...
                    for (l = -8; l < 9; l += 4) {
                      ...
                      while (offset) {
                        ...
                        if (b1 > b0) {
                          ...
                          if (ex > avd_ex) { /* either non-covalent or extended
                                                chain */
                            ai1 = obj->AtomInfo + b1;
        ...
        if (n_cycle >= 0) {
          ...
          for (aa = 0; aa < n_active; aa++) {
            if ((cnt_a = cnt[(a = *(a_ptr++))])) {
              ...
              int flags;
              if (!(ai->protekted != cAtomProtected_off ||
                      ((flags = ai->flags) & cAtomFlag_fix))) {
                v1 = disp + 3 * a;
                v2 = cs_coord + 3 * atm2idx[a];
      ...
      PRINTFB(G, FB_Sculpt, FB_Blather)
      " Sculpt: %2.5f seconds %8.3f %d %8.3f\n", task_time, total_strain,
          total_count, 100 * total_strain / total_count ENDFB(G);
