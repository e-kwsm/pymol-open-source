...
static int RepLine(CGO* cgo, bool s1, bool s2, bool isRamped, const float* v1,
    const float* v2, const float* v1color, unsigned int b1, unsigned int b2,
    int a, const float* v2color, bool b1masked, bool b2masked)
{
  int ok = true;
  if (s1 && s2) {
    ...
    {
      // if not ramped, then insert vertices so colors are not interpolated
      // since lines interpolate by default
      bool eq = equal3f(v1color, v2color);
      bool split = !eq || b1 != b2;
      if (split) {
        cgo->add<cgo::draw::splitline>(v1, v2, v2color, b2,
            b2masked ? cPickableNoPick : a, isRamped, b1 == b2, eq);
...
static void RepValence(CGO* cgo, bool s1, bool s2, bool isRamped,
    ...
    float tube_size, int fancy, unsigned int b1, unsigned int b2, int a,
    bool b1masked, bool b2masked)
{
  ...
  if (fancy) {
    ...
    RepLine(cgo, s1, s2, isRamped, vv1, vv2, color1, b1, b2, a, color2,
        b1masked, b2masked);
  } else {
    ...
    RepLine(cgo, s1, s2, isRamped, vv1[0], vv2[0], color1, b1, b2, a, color2,
        b1masked, b2masked);
    RepLine(cgo, s1, s2, isRamped, vv1[1], vv2[1], color1, b1, b2, a, color2,
        b1masked, b2masked);
  }
}
...
static void RepAromatic(CGO* cgo, bool s1, bool s2, bool isRamped,
    const float* v1, const float* v2, const int* other, int a1, int a2,
    const float* coord, const float* color1, const float* color2,
    float tube_size, int half_state, unsigned int b1, unsigned int b2, int a,
    bool b1masked, bool b2masked)
{
  float d[3], t[3], p0[3], p1[3], p2[3];
  int a3;
  ...
  if (s1) {
    ...
    float pt1[] = {(f_1[0] * v1[0] + f[0] * v2[0]),
        (f_1[0] * v1[1] + f[0] * v2[1]), (f_1[0] * v1[2] + f[0] * v2[2])};
    float pt2[] = {(f_1[1] * v1[0] + f[1] * v2[0]),
        (f_1[1] * v1[1] + f[1] * v2[1]), (f_1[1] * v1[2] + f[1] * v2[2])};
    float p1[3], p2[3];
    subtract3f(pt1, t, p1);
    subtract3f(pt2, t, p2);
...
void RepWireBondRenderImmediate(CoordSet* cs, RenderInfo* info)
{
  ...
  if (info->ray || info->pick || (!(G->HaveGUI && G->ValidContext)))
    ...
    float line_width, line_width_setting = SettingGet_f(G, cs->Setting.get(),
                          obj->Setting.get(), cSetting_line_width);
    ...
    glBegin(GL_LINES);
    {
      ...
      for (a = 0; a < nBond; a++) {
        ...
        bd++;
        if (GET_BIT((ai1 = ai + b1)->visRep, cRepLine) &&
            GET_BIT((ai2 = ai + b2)->visRep, cRepLine)) {
          active = true;
...
static int RepWireBondCGOGenerate(RepWireBond* I, RenderInfo* info)
{
  ...
  line_as_cylinders = SettingGetGlobal_b(G, cSetting_use_shaders) &&
...
void RepWireBond::render(RenderInfo* info)
{
  ...
  if (ray) {
    ...
    CGORenderRay(primitiveCGO, ray, info, nullptr, nullptr, cs->Setting.get(),
        cs->Obj->Setting.get());
    ...
    if (pick) {
      CGORenderPicking(use_shader ? I->shaderCGO : I->primitiveCGO, info,
          &I->context, nullptr, nullptr, I);
    } else { /* else not pick i.e., when rendering */
      short line_as_cylinders;
      line_as_cylinders = SettingGetGlobal_b(G, cSetting_render_as_cylinders) &&
                          SettingGetGlobal_b(G, cSetting_line_as_cylinders);
      if (I->shaderCGO &&
          (!use_shader || (line_as_cylinders ^ I->shaderCGO_has_cylinders))) {
        CGOFree(I->shaderCGO);
...
static bool IsBondTerminal(ObjectMolecule* obj, int b1, int b2)
{
...
static int RepWireZeroOrderBond(CGO* cgo, bool s1, bool s2, const float* v1,
    const float* v2, const float* rgb1, const float* rgb2, unsigned int b1,
    unsigned int b2, int a, float dash_gap, float dash_length, bool b1masked,
    bool b2masked)
{
  int ok = true;
  ...
  float remspace =
      blen - (ndashes * dash_length); // remaining space for first gaps
  float dgap =
      remspace / (ndashes - 1.f); // endpoints at each vertex, therefore only
                                  // account for ndashes-1 spaces
  float placep[3], placep2[3], adddlen[3], adddtot[3];
  float dplace;
  int ndashes_drawn = 0;
  bool color2_set = false;
  mult3f(
      naxis, dash_length, adddlen); // adddlen - length of dash as x/y/z vector
  mult3f(naxis, dash_length + dgap,
...
Rep* RepWireBondNew(CoordSet* cs, int state)
{
  ...
  valence = SettingGet_f(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_valence_size);
  ...
  line_stick_helper = SettingGet_b(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_line_stick_helper);
  line_color = SettingGet_color(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_line_color);
  line_width = SettingGet_f(
  ...
  fancy = SettingGet_i(G, cs->Setting.get(), obj->Setting.get(),
              cSetting_valence_mode) == 1;
    ...
    for (a = 0; ok && a < obj->NBond; ++a, ++b) {
      ...
      if ((a1 >= 0) && (a2 >= 0)) {
        ...
        if (s1 ^ s2) {
          if (!half_bonds) {
            if (line_stick_helper &&
                (((!s1) && (cRepCylBit & ati1->visRep) &&
                     !(cRepCylBit & ati2->visRep)) ||
                    ((!s2) && (cRepCylBit & ati2->visRep) &&
                        !(cRepCylBit & ati1->visRep))))
              s1 = s2 = 1; /* turn on line when both stick and line are
                              alternately shown */
            else {
              s1 = 0;
              s2 = 0;
      ...
      inv_sym_bond:
        ...
        if (hide_long && (s1 || s2)) {
          ...
          if (!within3f(v1, v2, cutoff)) /* atoms separated by more than 90% of
        ...
        if (s1 || s2) {
          ...
          auto bd_valence_flag =
              BondSettingGetWD(G, b, cSetting_valence, valence_flag);
          auto bd_line_color =
              BondSettingGetWD(G, b, cSetting_line_color, line_color);
          ...
          if (variable_width) {
            auto bd_line_width =
                BondSettingGetWD(G, b, cSetting_line_width, line_width);
            if (last_line_width != bd_line_width) {
              CGOSpecialWithArg(
                  I->primitiveCGO, LINEWIDTH_FOR_LINES, bd_line_width);
          ...
          if (line_stick_helper && (ati1->visRep & ati2->visRep & cRepCylBit)) {
            ...
            if ((cRepCartoonBit & ati1->visRep & ati2->visRep)) {
              ...
              if (!sc_helper)
                AtomSettingGetIfDefined(
                    G, ati2, cSetting_cartoon_side_chain_helper, &sc_helper);
            ...
            if ((s1 || s2) && (cRepRibbonBit & ati1->visRep & ati2->visRep)) {
              ...
              if (!sc_helper)
                AtomSettingGetIfDefined(
                    G, ati2, cSetting_ribbon_side_chain_helper, &sc_helper);
          ...
          if (s1 || s2) {
            ...
            if (!ord) {
              RepWireZeroOrderBond(I->primitiveCGO, s1, s2, v1, v2, rgb1, rgb2,
                  b1, b2, a, .15f, .15f, ati1->masked, ati2->masked);
            } else if (!bd_valence_flag || ord <= 1) {
              RepLine(I->primitiveCGO, s1, s2, isRamped, v1, v2, rgb1, b1, b2,
                  a, rgb2, ati1->masked, ati2->masked);
            } else {
              if (ord == 4) {
                RepAromatic(I->primitiveCGO, s1, s2, isRamped, v1, v2, other,
                    a1, a2, cs->Coord, rgb1, rgb2, valence, 0, b1, b2, a,
                    ati1->masked, ati2->masked);
              } else {
                RepValence(I->primitiveCGO, s1, s2, isRamped, v1, v2, other, a1,
                    a2, cs->Coord, rgb1, rgb2, ord, valence, fancy && !terminal,
                    b1, b2, a, ati1->masked, ati2->masked);
              }
            }
            line_counter++;
          }
