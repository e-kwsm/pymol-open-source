...
/* defer_builds_mode = 5 : Immediate mode for any sphere_mode
   ...
   1) rectangular points with the same size, that can be changed with
   sphere_point_size 2) rectangles with constant size relative to vdw and scene
   scale (i.e., changes when zoomed) maxed by a multiple of
   sphere_point_max_size (set it below 1 to see it influence, 3*pixel_scale max)
   3) same as 2 but with circles
   4) no longer available
   5) no longer available (was ARB impostor shader)
   6-8) same as 1-3 but with normals computed from close atoms to mimic nice
   lighting 9) GLSL Shader Spheres (only when shaders are available)
...
{
  CRay* ray = info->ray;
  float alpha = 1.0F - SettingGet_f(G, I->cs->Setting.get(),
                           I->obj->Setting.get(), cSetting_sphere_transparency);
  if (fabs(alpha - 1.0) < R_SMALL4)
    alpha = 1.0F;
  ray->transparentf(1.0 - alpha);
  if (I->spheroidCGO) {
    CGORenderRay(I->spheroidCGO, ray, info, nullptr, nullptr,
...
static bool RepSphereDetermineAtomVisibility(PyMOLGlobals* G,
    AtomInfoType* ati1, int cartoon_side_chain_helper,
    int ribbon_side_chain_helper)
{
...
static void RepSphereAddAtomVisInfoToStoredVC(RepSphere* I, ObjectMolecule* obj,
    ...
    float sphere_scale, int sphere_color, float transp, int* variable_alpha,
    float sphere_add, int const sphere_mode)
{
  PyMOLGlobals* G = cs->G;
  ...
  float at_sphere_scale =
      AtomSettingGetWD(G, ati1, cSetting_sphere_scale, sphere_scale);
  int at_sphere_color =
      AtomSettingGetWD(G, ati1, cSetting_sphere_color, sphere_color);

  if (AtomSettingGetIfDefined(
          G, ati1, cSetting_sphere_transparency, &at_transp))
    *variable_alpha = true;

  int trans_pick_mode = SettingGet<int>(G, cs->Setting.get(),
      obj->Setting.get(), cSetting_transparency_picking_mode);
...
static CGO* RepSphereGeneratespheroidCGO(
    ObjectMolecule* I, CoordSet* cs, SphereRec* sp, int state)
{
  int idx, a, b, c;
  int *q, *s;
  bool ok = true;
  float spheroid_scale = SettingGet_f(
      I->G, cs->Setting.get(), I->Setting.get(), cSetting_spheroid_scale);
  int sphere_color = SettingGet_color(
      I->G, cs->Setting.get(), I->Setting.get(), cSetting_sphere_color);
  float transp = SettingGet_f(
      I->G, cs->Setting.get(), I->Setting.get(), cSetting_sphere_transparency);
  ...
  for (idx = 0; idx < cs->NIndex; idx++) {
    ...
    q = sp->Sequence;
    s = sp->StripLen;
    RepSphereCGOSetSphereColorAndPick(
        I, cs, cgo, idx, state, transp, sphere_color);
    for (b = 0; ok && b < sp->NStrip; b++) {
      ...
      for (c = 0; c < (*s); c++) {
        ...
        CGOVertex(cgo, v0[0] + sphTmp * sp->dot[*q][0],
            v0[1] + sphTmp * sp->dot[*q][1], v0[2] + sphTmp * sp->dot[*q][2]);
        q++;
      }
      CGOEnd(cgo);
      s++;
 ...
 *
 */
static void RepSphereSetNormalForSphere(RepSphere* I, MapType* map,
    float* v_tmp, float* v, float cut_mult, int a, int* active, float* dot,
    int n_dot)
{
  int h, k, l, b, i, j;
  float v1[3];
  ...
  if (i) {
    ...
    while (j >= 0) {
      if (j != a) {
        vv = v_tmp + 3 * j;
        if (within3fret(
                vv, v, SPHERE_NORMAL_RANGE, SPHERE_NORMAL_RANGE2, v1, &dst)) {
...
Rep* RepSphereNew(CoordSet* cs, int state)
{
  ...
  if (!cs->Spheroid.empty())
    I->spheroidCGO =
        RepSphereGeneratespheroidCGO(obj, cs, GetSpheroidSphereRec(G), state);

  if (ok) {
    sphere_color = SettingGet_color(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sphere_color);
    ...
    transp = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sphere_transparency);
    sphere_scale = SettingGet_f(
        G, cs->Setting.get(), obj->Setting.get(), cSetting_sphere_scale);
  }
  ...
  if (ok) {
    if (SettingGet_i(G, cs->Setting.get(), obj->Setting.get(),
            cSetting_sphere_solvent)) { /* are we generating a solvent surface?
                                         */
      sphere_add = SettingGet_f(G, cs->Setting.get(), obj->Setting.get(),
  ...
  if (needNormals) {
    ...
    for (a = 0; ok && a < cs->NIndex; a++) {
      ...
      if (marked[a1]) {
        ati1 = obj->AtomInfo + a1;
        RepSphereSetNormalForSphere(
            I, map, v_tmp, &v_tmp[a * 3], cut_mult, a, active, dot, n_dot);
