...
static void AddOrthoOutputIfMatchesTags(PyMOLGlobals* G, int n_tags, int nAtom,
    const char* const* tag_start, const char* p, char* cc, int quiet)
{
...
static int populate_other(OtherRec* other, int at, const AtomInfoType* ai,
    const BondType* bd, const ObjectMolecule* obj)
{
...
static int append_index(
    int* result, int offset, int a1, int a2, int score, int ar_count)
{
  int c;
  c = result[a1];
  while (c < offset) {
...
int ObjectMoleculeAddPseudoatom(ObjectMolecule* I, int sele_index,
    const char* name, const char* resn, const char* resi, const char* chain,
    const char* segi, const char* elem, float vdw, int hetatm, float b, float q,
    ...
    int quiet)
{
  ...
  }
  {
    ...
    if (!quiet) {
      ...
      " ObjMol: created %s/%s/%s/%s`%d%c/%s\n", I->Name, LexStr(G, ai->segi),
          LexStr(G, ai->chain), LexStr(G, ai->resn), ai->resv,
          ai->getInscode(true), LexStr(G, ai->name) ENDFB(G);
    }
  }
  ...
  for (state = start_state; state < stop_state; state++) {
    ...
    if ((extant_only && (state < I->NCSet) && I->CSet[state]) || !extant_only) {
      ...
      if (sele_index >= 0) {
        ...
        if (op.i1) {
          ...
          if (vdw < 0.0F) {
            ...
            if (vdw >= 0.0F)
              ai->vdw =
                  vdw; /* NOTE: only uses vdw from first state selection... */
          }
        } else {
      ...
      if (pos) { /* only add coordinate to state if we have position for it */
        ...
        if (!ai_merged) {
          if (ok)
            ok &= ObjectMoleculeMerge(I, std::move(atInfo), cset, false,
                cAIC_AllMask, true); /* NOTE: will release atInfo */
          if (ok)
    ...
    if ((a1 >= 0) && (a2 >= 0)) {
      if (result[a1] < 0) {
        ...
        for (b = 0; b < o->n_cyclic_arom; b++) {
          a3 = o->cyclic_arom[b];
          offset =
              append_index(result, offset, a1, a3, 128 + other[a3].score, 1);
        }
        for (b = 0; b < o->n_arom; b++) {
          a3 = o->arom[b];
          offset =
              append_index(result, offset, a1, a3, 64 + other[a3].score, 1);
        }
        for (b = 0; b < o->n_high_val; b++) {
          a3 = o->high_val[b];
          offset =
              append_index(result, offset, a1, a3, 16 + other[a3].score, 0);
        }
      ...
      if (result[a2] < 0) {
        ...
        for (b = 0; b < o->n_cyclic_arom; b++) {
          a3 = o->cyclic_arom[b];
          offset =
              append_index(result, offset, a2, a3, 128 + other[a3].score, 1);
        }
        for (b = 0; b < o->n_arom; b++) {
          a3 = o->arom[b];
          offset =
              append_index(result, offset, a2, a3, 64 + other[a3].score, 1);
        }
        for (b = 0; b < o->n_high_val; b++) {
          a3 = o->high_val[b];
          offset =
              append_index(result, offset, a2, a3, 16 + other[a3].score, 0);
        }
        for (b = 0; b < o->n_cyclic; b++) {
          a3 = o->cyclic[b];
...
int ObjectMoleculeGetNearestBlendedColor(ObjectMolecule* I, const float* point,
    float cutoff, int state, float* dist, float* color, int sub_vdw)
{
  int result = -1;
            ...
            for (f = c - 1; f <= c + 1; f++) {
              ...
              while (j >= 0) {
                ...
                if (test < cutoff2) {
                  ...
                  const float* at_col =
                      ColorGet(I->G, I->AtomInfo[cs->IdxToAtm[j]].color);
...
int ObjectMoleculeGetNearestAtomIndex(
    ObjectMolecule* I, const float* point, float cutoff, int state, float* dist)
{
  int result = -1;
  float nearest = -1.0F;
...
int ObjectMoleculeGetPrioritizedOther(
    const int* other, int a1, int a2, int* double_sided)
{
  int a3 = -1;
  int lvl = -1, ck, ck_lvl;
  int offset;
 ...
 */
int ObjectMoleculeIsAtomBondedToName(
    ObjectMolecule* obj, int a0, const char* name, int same_res)
{
...
int ObjectMoleculeAreAtomsBonded2(
    ObjectMolecule* obj0, int a0, ObjectMolecule* obj1, int a1)
{
  if (obj0 == obj1 && a0 >= 0) {
    assert(a1 >= 0);
...
static void assign_pdb_known_residue(
    PyMOLGlobals* G, AtomInfoType* ai1, AtomInfoType* ai2, int* bond_order)
{
      ...
      case 0:
        ...
        if ((name1 == "C8" && name2 == "N7") ||
          ...
          order = 2;
                 ...
                 (name2 == "C2" && name1 == "N3"))
          order = 2;
        else if ((name1 == "P" && (name2 == "O1P" || name2 == "OP1")) ||
    ...
    ObjectMolecule* I, int sele1, int sele2, int invalidate)
{
  ...
  for (b = 0; b < I->NBond; b++) {
    ...
    if ((SelectorIsMember(I->G, s1, sele1) &&
        ...
        (SelectorIsMember(I->G, s2, sele1) &&
            SelectorIsMember(I->G, s1, sele2))) {
      order = -1;
      if (strlen(LexStr(I->G, ai1->resn)) <
          4) { /* Standard disconnected PDB residue */
...
int ObjectMoleculeAutoDisableAtomNameWildcard(ObjectMolecule* I)
{
  ...
  {
    const char* tmp =
        SettingGet_s(G, nullptr, I->Setting.get(), cSetting_atom_name_wildcard);
    if (tmp && tmp[0]) {
      wildcard = *tmp;
    } else {
  ...
  if (wildcard) {
    ...
    if (found_wildcard) {
      ExecutiveSetObjSettingFromString(
          G, cSetting_atom_name_wildcard, " ", I, -1, true, true);
    }
  }
...
static void ObjectMoleculePDBStr2CoordSetPASS1(PyMOLGlobals* G, int* ok,
    const char** restart_model, const char* p, int n_tags,
    const char* const* tag_start, int* nAtom, char cc[], int quiet,
    int* bogus_name_alignment, int* ssFlag, const char** next_pdb,
    PDBInfoRec* info, int only_read_one_model, int* ignore_conect,
    int* bondFlag, int* have_bond_order)
{
  int seen_end_of_atoms = false;
  *restart_model = nullptr;
  while (*ok && *p) {
    AddOrthoOutputIfMatchesTags(G, n_tags, *nAtom, tag_start, p, cc, quiet);
    if ((strstartswith(p, "ATOM ") || strstartswith(p, "HETATM")) &&
        (!*restart_model)) {
      ...
      if (*bogus_name_alignment) {
        ...
        if ((cc[0] == 32) &&
            (cc[1] != 32)) { /* check to see if indentation was followed
                                correctly, 32 - space */
      ...
      *ssFlag = true;
      ...
      if (*nAtom > 0) { /* if we've already found atom records, then this must
                           be a new pdb */
        (*next_pdb) = p;
        break;
      }
 ...
 */
static int sshash_register_rec(SSHash* hash, unsigned char ss_chain1,
    int ss_resv1, char ss_inscode1, unsigned char ss_chain2, int ss_resv2,
    char ss_inscode2, char SSCode)
{
  /* pretty confusing how this works... the following efficient (i.e.
     array-based) secondary structure lookup even when there are multiple
     insertion codes and where there may be multiple SS records for the residue
     using different insertion codes */
 ...
 * Assign ai->ssType
 */
static void sshash_lookup(
    SSHash* hash, AtomInfoType* ai, unsigned char ss_chain1)
{
  int index, ssi;
  SSEntry* sst = nullptr;
  ...
  if (hash->ss[ss_chain1]) {
    ...
    while (ssi) {
      ...
      if (ai->resv >= sst->resv1 && ai->resv <= sst->resv2 &&
 ...
 */
static bool parse_pqr_atom_line(
    PyMOLGlobals* G, const char*& p, AtomInfoType* ai, float* coord)
{
...
CoordSet* ObjectMoleculePDBStr2CoordSet(PyMOLGlobals* G, const char* buffer,
    AtomInfoType** atInfoPtr, const char** restart_model, char* segi_override,
    char* pdb_name, const char** next_pdb, PDBInfoRec* info, int quiet,
    int* model_number)
{
  ...
  const char *tags_in = SettingGetGlobal_s(G, cSetting_pdb_echo_tags),
             *tag_start[PDB_MAX_TAGS];
  int n_tags = 0;
  ...
  if (!atInfo)
    ErrFatal(G, "PDBStr2CoordSet",
        "need atom information record!"); /* failsafe for old version.. */

  if (buffer == *restart_model)
    only_read_one_model = true;
  else if (info && (info->multiplex > 0)) {
    only_read_one_model = true;
    if (!info->multi_object_status) { /* figure out if this is a multi-object
                                         (multi-HEADER) pdb file */
  ...
  /* INPUT USED:
     ...
     OUTPUT USED:
     bogus_name_alignment - whether all ATOM/HETATM has correct names in PDB
     (12-16, starting with space ssFlag - if PDB has HELIX and/or SHEET records
     both INPUT/OUTPUT:
     nAtom - returns the number of atoms read in, also uses it to detect
     multiple PDBs ignore_conect - do not set bondFlag if set
  ...
  *restart_model = nullptr;
  ...
  while (ok && *p) {
    ...
    if (strstartswith(p, "ATOM "))
      ...
      if (pdb_name) {
        if (atomCount > 0) {
          ...
          if (pdb_name[0] ==
              0) { /* if not, is there a plain name (for MERCK!) */
            p = nskip(p, 6);
            p = ntrim(cc, p, 44);
      ...
      if (ss_valid) {
        ...
        " ObjectMolecule: read HELIX %c %d%.1s %c %d%.1s\n", ss_chain1,
            ss_resv1, &ss_inscode1, ss_chain2, ss_resv2, &ss_inscode2 ENDFB(G);
        SSCode = 'H';
      }
      ...
      if (*restart_model)
        ...
        *restart_model = nextline(p);
        ...
        if (only_read_one_model) {
          ...
          pp = nextline(p);
          if (strstartswith(pp,
                  "END")) { /* END we're going to be starting a new object... */
            (*next_pdb) = check_next_pdb_object(nextline(pp), true);
            if (info && (info->multiplex == 0)) {
              /* multiplex == 0:  FORCED multimodel behavior with concatenated
               * PDB files */
          ...
          } else if (strstartswith(pp, "MODEL")) { /* not a new object...just a
                                                      new state (model) */
            if (info &&
                (info->multiplex > 0)) { /* end object if we're multiplexing */
      ...
      if ((strcmp("END", cc) == 0) && (!in_model)) { /* stop parsing here... */
        const char* pp;
        pp = nextline(
            p); /* ...unless we're in MODEL or next line is itself ENDMDL */
        p = ncopy(cc, p, 6);
        if (!((cc[0] == 'E') && (cc[1] == 'N') && (cc[2] == 'D') &&
                (cc[3] == 'M') && (cc[4] == 'D') &&
                (cc[5] == 'L'))) { /* NOTE: this test seems unnecessary given
                                      strcmp above... */
          if (!*next_pdb) {
            (*next_pdb) = check_next_pdb_object(pp, false);
          }
          if ((*next_pdb) && info && (!info->multiplex) && !(*restart_model)) {
            /* multiplex == 0:  FORCED multimodel behavior with concatenated PDB
             * files */
            (*restart_model) = (*next_pdb);
            (*next_pdb) = nullptr;
            foundNextModelFlag = true;
          ...
          if (*next_pdb) { /* we've found another object... */
            if (*restart_model)
              is_end_of_object =
                  false; /* however, if we're parsing multi-models, then we're
                            not yet at the end */
            else
              is_end_of_object = true;
            break;
    ...
    } else if (strstartswith(p, "SCALE") && (!*restart_model) &&
               info) { /* SCALEn */
      switch (p[5]) {
      case '1':
      case '2':
      case '3': {
        ...
        " PDBStrToCoordSet: SCALE%d %8.4f %8.4f %8.4f %8.4f\n", flag + 1,
            info->scale.matrix[offset], info->scale.matrix[offset + 1],
            info->scale.matrix[offset + 2],
            info->scale.matrix[offset + 3] ENDFB(G);
      } break;
      }
    } else if (strstartswith(p, "CONECT") && bondFlag && (!ignore_conect) &&
      ...
      if (sscanf(cc, "%d", &b1) == 1)
        while (1) {
          ...
          if (sscanf(cc, "%d", &b2) != 1)
            ...
            if ((b1 >= 0) && (b2 >= 0) &&
                (b1 != b2)) { /* IDs must be positive and distinct */
              VLACheck(bond, BondType, nBond);
              CHECKOK(ok, bond);
              if (ok) {
                if (b1 <= b2) {
                  bond[nBond].index[0] =
                      b1; /* temporarily store the atom indexes */
    ...
    if (ok && SSCode) {
      ss_found = sshash_register_rec(ss_hash, ss_chain1, ss_resv1, ss_inscode1,
    ...
    if (ok && AFlag && (!*restart_model)) {
      ...
      if (ai->name) {
          ...
          if (name_len > 3) {
            if ((ai_name[0] >= 'A') && ((ai_name[0] <= 'Z')) &&
              ...
              if (!(((ai_name[1] >= 'a') && (ai_name[1] <= 'z')) ||
                      ((ai_name[0] == 'C') &&
                          (ai_name[1] == 'L')) || /* try to be smart about */
                      ((ai_name[0] == 'B') &&
                          (ai_name[1] ==
                              'R')) || /* distinguishing common atoms */
                      ((ai_name[0] == 'C') &&
                          (ai_name[1] == 'A')) || /* in all-caps from typical */
                      ((ai_name[0] == 'F') &&
                          (ai_name[1] == 'E')) || /* nonatomic abbreviations */
            ...
            if ((ai_name[0] == 'H') && (ai_name[1] >= 'A') &&
                ((ai_name[1] <= 'Z')) && isdigit(ai_name[2])) {
              AtomInfoGetPDB3LetHydroName(
                  G, LexStr(G, ai->resn), ai_name, name);
        ...
        case 3: /* pdb compliant, but use IUPAC within PyMOL */
          ...
          break;
        case 4: /* simply read trim and write back out with 3-letter names
                   starting from the second column, and four-letter names
      ...
      if ((!info) || (!info->is_pqr_file())) { /* standard PDB file */
        ...
        if (!sscanf(cc, "%s", ai->elem))
          ai->elem[0] = 0;
                        ...
                        (ai->elem[0] <=
                            'z')) || /* don't get confused by PDB misuse */
        ...
        if (!ai->elem[0]) {
          if (((literal_name[0] == ' ') ||
                  ((literal_name[0] >= '0') && (literal_name[0] <= '9'))) &&
              (literal_name[1] >= 'A') &&
            ...
            ai->elem[1] = 0;
                     ...
                     (((literal_name[1] >= 'A') && (literal_name[1] <= 'Z')) ||
                         ((literal_name[1] >= 'a') &&
                             (literal_name[1] <=
                                 'z')))) { /* infer element from name column */
            ai->elem[0] = literal_name[0];
            ai->elem[2] = 0;
            if ((literal_name[1] >= 'A') &&
                (literal_name[1] <= 'Z')) { /* second letter is capitalized */
              if (bogus_name_alignment) {
  ...
  if (ok && bondFlag) {
    UtilSortInPlace(
        G, bond, nBond, sizeof(BondType), (UtilOrderFn*) BondInOrder);
    if (nBond) {
      ...
      if (ok) {
        ...
        for (a = 0; a < nBond; a++) {
          ...
          if ((ii1->index[0] >= 0) && ((ii1->index[1]) >= 0)) {
            if ((idx[ii1->index[0]] >= 0) &&
                (idx[ii1->index[1]] >=
                    0)) { /* in case PDB file has bad bonds */
    ...
    if ((*restart_model) && (!foundNextModelFlag)) {
      ...
      while (*p) {
        if (strstartswith(p, "HEADER")) {
          ...
          break;
        } else if (strstartswith(p, "MODEL ") || strstartswith(p, "ENDMDL")) {
          foundNextModelFlag = true;
          break;
        }
        p = nextline(p);
  ...
  if (ok) {
    ...
    if ((*restart_model) &&
        (*next_pdb)) { /* if we're mixing multistate objects and
...
void ObjectMoleculeInitHBondCriteria(PyMOLGlobals* G, HBondCriteria* hbc)
{
  ...
  hbc->maxDistAtMaxAngle =
      SettingGet_f(G, nullptr, nullptr, cSetting_h_bond_cutoff_edge);
  hbc->maxDistAtZero =
      SettingGet_f(G, nullptr, nullptr, cSetting_h_bond_cutoff_center);
  ...
  hbc->cone_dangle = (float) cos(
      PI * 0.5 * SettingGet_f(G, nullptr, nullptr, cSetting_h_bond_cone) /
      180.0F);
...
static int ObjectMoleculeTestHBond(float* donToAcc, float* donToH,
    float* hToAcc, float* accPlane, HBondCriteria* hbc)
{
  ...
  if (accPlane) { /* remember, plane need not exist if it's water... */
    ...
    if (dot_product3f(nHToAcc, nAccPlane) >
        (-hbc->cone_dangle)) /* don't allow H behind Acceptor plane */
      return 0;
  }
  ...
  if (hbc->maxDistAtMaxAngle != 0.0F) {
    ...
    cutoff =
        (hbc->maxDistAtMaxAngle * curve) + (hbc->maxDistAtZero * (1.0 - curve));
  } else {
...
static int ObjectMoleculeFindBestDonorH(ObjectMolecule* I, int atom, int state,
    float* dir, float* best, AtomInfoType** h_real)
{
  int result = 0;
  CoordSet* cs;
  ...
  if ((state >= 0) && (state < I->NCSet) && (cs = I->CSet[state]) &&
      (atom < I->NAtom)) {
    ...
    if (idx >= 0) {
      ...
      if ((nn < I->AtomInfo[atom].valence) ||
          I->AtomInfo[atom].hb_donor) { /* is there an implicit hydrogen? */
...
int ObjectMoleculeGetCheckHBond(AtomInfoType** h_real, float* h_crd_ret,
    ObjectMolecule* don_obj, int don_atom, int don_state,
    ObjectMolecule* acc_obj, int acc_atom, int acc_state, HBondCriteria* hbc)
{
  int result = 0;
  ...
  if ((don_state >= 0) && (don_state < don_obj->NCSet) &&
      (csD = don_obj->CSet[don_state]) && (acc_state >= 0) &&
      (acc_state < acc_obj->NCSet) && (csA = acc_obj->CSet[acc_state]) &&
    ...
    if ((idxA >= 0) && (idxD >= 0)) {
      ...
      if (ObjectMoleculeFindBestDonorH(
              don_obj, don_atom, don_state, donToAcc, bestH, h_real)) {

        subtract3f(bestH, vDon, donToH);
        subtract3f(vAcc, bestH, hToAcc);

        if (ObjectMoleculeGetAvgHBondVector(
                acc_obj, acc_atom, acc_state, accPlane, hToAcc) > 0.1) {
          vAccPlane = &accPlane[0];
        }
        result =
            ObjectMoleculeTestHBond(donToAcc, donToH, hToAcc, vAccPlane, hbc);
...
static PyObject* ObjectMoleculeBondAsPyList(ObjectMolecule* I)
{
  ...
  if (SettingGetGlobal_b(G, cSetting_pse_binary_dump) &&
      (!pse_export_version || pse_export_version >= 1765)) {
    /* For the pse_binary_dump, save entire Bond array to a binary string array
     */

    // supported versions
    auto version = (!pse_export_version || pse_export_version >= 1810) ? 181
                   : (pse_export_version >= 1770)                      ? 177
                                                                       : 176;
    ...
    PyList_SetItem(result, 1,
        PyBytes_FromStringAndSize(
            reinterpret_cast<const char*>(blob), blobsize));
...
static int ObjectMoleculeBondFromPyList(ObjectMolecule* I, PyObject* list)
{
  ...
  if (pse_binary_dump) {
    ...
    Copy_Into_BondType_From_Version(
        strval.data(), bondInfo_version, I->Bond.data(), I->NBond);
    ...
    for (a = 0; a < I->NBond; a++) {
      ...
      if (ok)
        if ((ok = CPythonVal_PConvPyIntToInt_From_List(
                 I->G, bond_list, 2, &stereo)))
          bond->order = stereo;
      if (ok && (ll > 5)) {
        int has_setting;
        if (ok)
        ...
        if (ok && bond->unique_id) { /* reserve existing IDs */
          bond->unique_id =
              SettingUniqueConvertOldSessionID(G, bond->unique_id);
        }
      }
      if (ll > 7) {
...
static PyObject* ObjectMoleculeAtomAsPyList(ObjectMolecule* I)
{
  ...
  if (SettingGetGlobal_b(G, cSetting_pse_binary_dump) &&
    ...
    for (a = 0; a < I->NAtom; a++) {
      if (ai->textType)
        lexIDs.insert(ai->textType);
      if (ai->chain)
        lexIDs.insert(ai->chain);
      if (ai->label)
        lexIDs.insert(ai->label);
      if (ai->custom)
        lexIDs.insert(ai->custom);
      if (ai->segi)
        lexIDs.insert(ai->segi);
      if (ai->resn)
        lexIDs.insert(ai->resn);
      if (ai->name)
        lexIDs.insert(ai->name);
      ++ai;
    }
    for (const auto& lexID :
        lexIDs) { // need to calculate totalstlen so we can allocate
    ...
    *(strval++) =
        lexIDs.size(); // first write number of strings into binary data string
    char* strpl = (char*) ((char*) strinfo + (1 + lexIDs.size()) * sizeof(int));
    /* write map of lex ids and strings into binary data string as an array of
       ids and null-terminated strings */
    ...
    PyList_SetItem(result, 2,
        PyBytes_FromStringAndSize(
            reinterpret_cast<const char*>(strinfo), strinfolen));
...
static int ObjectMoleculeAtomFromPyList(ObjectMolecule* I, PyObject* list)
{
  ...
  if (ll >= 3) {
    // checking if from pse_binary_dump
    // pse_binary_dump saves 3 values: atomInfo_version, AtomInfo binary, and
    // strings array
    CPythonVal* val1 = CPythonVal_PyList_GetItem(G, list, 1);
  ...
  if (pse_binary_dump) {
    ...
    int strcnt = nstrings;
    int stlen;
    // populate oldIDtoLexID with nstrings from binary string data (3rd entry in
    // list)
    while (strcnt) {
      lexidx_t idx =
          LexIdx(G, strpl); // increments ref count, need to take into account
      int oldidx = *(strval++);
    ...
    // go through AtomInfo array, swap new strings, convert colors, convert
    // settings (everything that AtomInfoFromPyList does except set properties,
    // which are currently
...
int ObjectMoleculeNewFromPyList(
    PyMOLGlobals* G, PyObject* list, ObjectMolecule** result)
{
  int ok = true;
...
PyObject* ObjectMoleculeAsPyList(ObjectMolecule* I)
{
  ...
  PyList_SetItem(result, 9,
      PyInt_FromLong(I->DiscreteFlag ? I->NAtom : 0 /* NDiscrete */));
  ...
  float pse_export_version =
      SettingGetGlobal_f(I->G, cSetting_pse_export_version);

  if (I->DiscreteFlag &&
      (pse_export_version ||
          !SettingGetGlobal_b(I->G, cSetting_pse_binary_dump)) &&
    ...
    PyList_SetItem(
        result, 14, PConvIntArrayToPyList(I->DiscreteAtmToIdx, I->NAtom));
...
static float connect_cutoff_adjustment(
    const AtomInfoType* ai1, const AtomInfoType* ai2)
{
 ...
 */
static bool is_distance_bonded(PyMOLGlobals* G, const CoordSet* cs,
    const AtomInfoType* ai1, const AtomInfoType* ai2, const float* v1,
    const float* v2, float cutoff, int connect_mode, int discrete_chains,
    bool connect_bonded, bool unbond_cations)
{
  ...
  if (unbond_cations &&
      (AtomInfoIsFreeCation(G, ai1) || AtomInfoIsFreeCation(G, ai2)))
    return false;
...
/*========================================================================*/
bool ObjectMoleculeConnect(ObjectMolecule* I, int& nBond,
    pymol::vla<BondType>& bondvla, struct CoordSet* cs, int bondSearchMode,
    int connectModeOverride, bool pbc)
{
  PyMOLGlobals* G = I->G;
...
#pragma omp parallel for
  ...
  /* if we have explicit connectivity, determine if we need to set
   * check_conect_all */
  if (cs->NTmpBond && cs->TmpBond) {
    ...
    " %s: incorporating explicit bonds. %d %d\n", __func__, nBond,
        cs->NTmpBond ENDFB(G);
    if ((nBond == 0) && (cs->NTmpBond > 0) && bondSearchMode &&
        (connect_mode == 0) && cs->NIndex) {
      ...
      if ((a1 >= 0) && (a2 >= 0) && (a1 < n_atom) && (a2 < n_atom)) {
        if (check_conect_all) {
          if ((!ai[a1].hetatm) && (!ai[a2].hetatm)) {
            /* found bond between non-HETATMs -- so tell PyMOL to CONECT all
             * ATOMs when writing out a PDB file */
            pdb_conect_all = true;
          }
        }
        ai[a1].bonded = true;
    ...
    if (pdb_conect_all) {
      ...
      if (!SettingGetIfDefined_b(
              G, I->Setting.get(), cSetting_pdb_conect_all, &dummy)) {
        {
...
void ObjectMoleculeConnectDiscrete(
    ObjectMolecule* I, int searchFlag, int connectModeOverride, bool pbc)
{
  for (int i = 0; i < I->NCSet; i++) {
    ...
    ObjectMoleculeConnect(
        I, nbond, bond, I->CSet[i], searchFlag, connectModeOverride, pbc);
