 ...
 */
static bool has_quotespace(const char* s, char quote)
{
 ...
 */
static bool cif_is_simpledatavalue(const char* s)
{
  return (
      ...
      !((s[0] == '.' || s[0] == '?') && !s[1]) &&
      // prefix is special
      strncasecmp("data_", s, 5) && strncasecmp("save_", s, 5) &&
      // reserved words
      strcasecmp("loop_", s) && strcasecmp("stop_", s) &&
...
const char* CifDataValueFormatter::quoted(const char* s)
{
  ...
  if (!quote) {
    quote = "\n;";
    if (strstr(s, quote)) {
      printf(" CIF-Warning: data value contains unquotable "
             "<newline><semicolon>\n");
      return "<UNQUOTABLE>";
    }
  }
