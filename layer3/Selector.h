...
SelectorCreateResult_t SelectorCreate(PyMOLGlobals* G, const char* name,
...
SelectorCreateResult_t SelectorCreateWithStateDomain(PyMOLGlobals* G,
    const char* name, const char* sele, ObjectMolecule* obj, int quiet,
...
SelectorID_t SelectorIndexByName(
    PyMOLGlobals* G, const char* sele, int ignore_case = -1);
...
int SelectorGetTmp2(
    PyMOLGlobals* G, const char* input, char* store, bool quiet = false);
int SelectorGetTmp(
    PyMOLGlobals* G, const char* input, char* store, bool quiet = false);
...
int SelectorCreateObjectMolecule(PyMOLGlobals* G, SelectorID_t sele,
    const char* name, int target_state, int state, int discrete, int zoom,
    int quiet, int singletons, int copy_properties);
int SelectorSubdivide(PyMOLGlobals* G, const char* pref, SelectorID_t sele1,
    SelectorID_t sele2, SelectorID_t sele3, SelectorID_t sele4,
...
pymol::Result<> SelectorUpdateCmd(PyMOLGlobals* G, SelectorID_t sele0,
    SelectorID_t sele1, int sta0, int sta1, int method, int quiet);
...
int* SelectorGetResidueVLA(
    PyMOLGlobals* G, SelectorID_t, int ca_only, ObjectMolecule* exclude);
...
int SelectorIsAtomBondedToSele(
    PyMOLGlobals* G, ObjectMolecule* obj, int sele1atom, int sele2);

int SelectorSetName(
    PyMOLGlobals* G, const char* new_name, const char* old_name);
...
public:
  SelectorTmp() = default;
  SelectorTmp(PyMOLGlobals* G, const char* sele)
      : m_G(G)
  {
  ...
  ~SelectorTmp() { SelectorFreeTmp(m_G, m_name); }
  const char* getName() const { return m_name; }
  int getAtomCount();
  SelectorID_t getIndex() const
  {
    return m_name[0] ? SelectorIndexByName(m_G, m_name, false)
                     : cSelectionInvalid;
  }
