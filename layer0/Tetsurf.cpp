...
#define O3(field, P1, P2, P3, offs)                                            \
  ((field)->get<float>(P1 + offs[0], P2 + offs[1], P3 + offs[2]))

#define O4Ptr(field, P1, P2, P3, P4, offs)                                     \
  ((field)->ptr<float>(P1 + offs[0], P2 + offs[1], P3 + offs[2], P4))
...
#define EdgePtPtr(field, P2, P3, P4, P5)                                       \
  ((field)->ptr<PointType>(P2, P3, P4, P5))
...
static int TetsurfFindActiveBoxes(CTetsurf* II, cIsosurfaceMode, int& n_strip,
    int n_vert, pymol::vla<int>& strip_l, pymol::vla<float>& vert,
...
static int ProcessTetrahedron(int* edge, int nv, int v0, int v1, int v2, int v3,
    int e01, int e02, int e03, int e12, int e13, int e23, int reflect)
{
...
#define cM_110_111 0x40000
  ...
  for (c = 0; c < 256; c++) {
    ...
    nv = ProcessTetrahedron(I->Edge, nv, v000, v001, v011, v111, cE_000_001,
        cE_000_011, cE_000_111, cE_001_011, cE_001_111, cE_011_111, 0);
    /* tetrahedron 1: 000, 001, 101, 111 */
    nv = ProcessTetrahedron(I->Edge, nv, v000, v001, v101, v111, cE_000_001,
        cE_000_101, cE_000_111, cE_001_101, cE_001_111, cE_101_111, 1);

    /* tetrahedron 2: 000, 010, 011, 111 */
    nv = ProcessTetrahedron(I->Edge, nv, v000, v010, v011, v111, cE_000_010,
        cE_000_011, cE_000_111, cE_010_011, cE_010_111, cE_011_111, 1);
    /* tetrahedron 3: 000, 010, 110, 111 */
    nv = ProcessTetrahedron(I->Edge, nv, v000, v010, v110, v111, cE_000_010,
        cE_000_110, cE_000_111, cE_010_110, cE_010_111, cE_110_111, 0);

    /* tetrahedron 4: 000, 100, 101, 111 */
    nv = ProcessTetrahedron(I->Edge, nv, v000, v100, v101, v111, cE_000_100,
        cE_000_101, cE_000_111, cE_100_101, cE_100_111, cE_101_111, 0);

    /* tetrahedron 5: 000, 100, 110, 111 */
    nv = ProcessTetrahedron(I->Edge, nv, v000, v100, v110, v111, cE_000_100,
        cE_000_110, cE_000_111, cE_100_110, cE_100_111, cE_110_111, 1);
    ...
    const CCrystal* cryst, const float* mn, const float* mx, int* range)
{
  ...
  " IsosurfGetRange: entered mn: %4.2f %4.2f %4.2f mx: %4.2f %4.2f %4.2f\n",
      mn[0], mn[1], mn[2], mx[0], mx[1], mx[2] ENDFD;

  for (a = 0; a < 3; a++) {
    rmn[a] = F4(field->points, 0, 0, 0, a);
    rmx[a] = F4(field->points, field->dimensions[0] - 1,
  ...
  for (a = 0; a < 3; a++) {
    if (imx[a] != imn[a]) { /* protect against div by zero */
      ...
      float cur;
      for (b = 0; b < 8; b++) {
        cur = ((field->dimensions[a] - 1) * (imix[a + 3 * b] - imn[a]) /
               (imx[a] - imn[a]));
  ...
  " IsosurfGetRange: returning range: %d %d %d %d %d %d\n", range[0], range[1],
      range[2], range[3], range[4], range[5] ENDFD;
}
...

  {
      ...
      for (i = 0; i < Steps[0]; i++)
        for (j = 0; j < Steps[1]; j++)
          for (k = 0; k < Steps[2]; k++) {
            ...
            /*
               for(c=0;c<3;c++)
               printf(" TetsurfVolume: c: %i I->CurOff[c]: %i I->Max[c]
               %i\n",c,I->CurOff[c],I->Max[c]);
             */

            if (ok) {
              if (TetsurfCodeVertices(I))
                n_vert = TetsurfFindActiveBoxes(
                    I, mode, n_strip, n_vert, num, vert, carvehelper, side);
            }
          }
      TetsurfPurge(I);
    }
    ...
    if (Feedback(G, FB_Isosurface, FB_Blather)) {
      if (static_cast<int>(mode) < 2) {
        printf(
            " TetsurfVolume: Surface generated using %d vertices.\n", n_vert);
      } else {
...
/*===========================================================================*/
inline static void TetsurfInterpolate2(
    float* pt, float* v0, float l0, float* v1, float l1, float level)
{
  float ratio;
...
/*===========================================================================*/
static void TetsurfInterpolate4(float* pt, float* v0, float l0, float* v1,
    float l1, float l2, float l3, float level)
{
  float ratio;
  float v[3], l;
  average3f(v0, v1, v);
...
/*===========================================================================*/
static void TetsurfInterpolate8(float* pt, float* v0, float l0, float* v1,
    float l1, float l2, float l3, float l4, float l5, float l6, float l7,
    float level)
{
  float ratio;
  float v[3], l;
...
/*===========================================================================*/
static int TetsurfFindActiveBoxes(CTetsurf* II, cIsosurfaceMode mode,
    int& n_strip, int n_vert, pymol::vla<int>& strip_l,
    pymol::vla<float>& vert_, const CarveHelper* carvehelper,
    cIsosurfaceSide side)
{
  ...
  float *g000 = nullptr, *g001 = nullptr, *g010 = nullptr, *g011 = nullptr,
        *g100 = nullptr, *g101 = nullptr, *g110 = nullptr, *g111 = nullptr;
  ...
  for (i = 0; i < (I->Max[0] - 1); i++)
    for (j = 0; j < (I->Max[1] - 1); j++)
      for (k = 0; k < (I->Max[2] - 1); k++) {
        ...
        if ((i000 != i001) || (i001 != i010) || (i010 != i011) ||
            (i011 != i100) || (i100 != i101) || (i101 != i110) ||
          ...
          if (i000 != i001) {
            if (!(I3(I->ActiveEdges, i, j, k) & cM_000_001)) {
              ...
              TetsurfInterpolate2(
                  e[cE_000_001]->Point, c000, d000, c001, d001, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_000_001]->Normal, g000, d000, g001, d001, I->Level);
            }
            active |= cM_000_001;
          }
          if (i000 != i010) {
            if (!(I3(I->ActiveEdges, i, j, k) & cM_000_010)) {
              ...
              TetsurfInterpolate2(
                  e[cE_000_010]->Point, c000, d000, c010, d010, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_000_010]->Normal, g000, d000, g010, d010, I->Level);
            }
            active |= cM_000_010;
          }
          if (i000 != i011) {
            if (!(I3(I->ActiveEdges, i, j, k) & cM_000_011)) {
              ...
              TetsurfInterpolate4(e[cE_000_011]->Point, c000, d000, c011, d011,
                  d001, d010, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate4(e[cE_000_011]->Normal, g000, d000, g011,
                    d011, d001, d010, I->Level);
            }
            active |= cM_000_011;
          }
          if (i000 != i100) {
            if (!(I3(I->ActiveEdges, i, j, k) & cM_000_100)) {
              ...
              TetsurfInterpolate2(
                  e[cE_000_100]->Point, c000, d000, c100, d100, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_000_100]->Normal, g000, d000, g100, d100, I->Level);
            }
            active |= cM_000_100;
          }
          if (i000 != i101) {
            if (!(I3(I->ActiveEdges, i, j, k) & cM_000_101)) {
              ...
              TetsurfInterpolate4(e[cE_000_101]->Point, c000, d000, c101, d101,
                  d100, d001, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate4(e[cE_000_101]->Normal, g000, d000, g101,
                    d101, d100, d001, I->Level);
            }
            active |= cM_000_101;
          }
          if (i000 != i110) {
            if (!(I3(I->ActiveEdges, i, j, k) & cM_000_110)) {
              ...
              TetsurfInterpolate4(e[cE_000_110]->Point, c000, d000, c110, d110,
                  d100, d010, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate4(e[cE_000_110]->Normal, g000, d000, g110,
                    d110, d100, d010, I->Level);
            }
            active |= cM_000_110;
          }
          if (i000 != i111) {
            if (!(I3(I->ActiveEdges, i, j, k) & cM_000_111)) {
              ...
              TetsurfInterpolate8(e[cE_000_111]->Point, c000, d000, c111, d111,
                  d001, d010, d011, d100, d101, d110, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate8(e[cE_000_111]->Normal, g000, d000, g111,
                    d111, d001, d010, d011, d100, d101, d110, I->Level);
            }
            active |= cM_000_111;
          }
          if (i001 != i011) {
            if (!(I3(I->ActiveEdges, i, j, k + 1) & cM_000_010)) {
              ...
              TetsurfInterpolate2(
                  e[cE_001_011]->Point, c001, d001, c011, d011, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_001_011]->Normal, g001, d001, g011, d011, I->Level);
            }
            active |= cM_001_011;
          }
          if (i001 != i101) {
            if (!(I3(I->ActiveEdges, i, j, k + 1) & cM_000_100)) {
              ...
              TetsurfInterpolate2(
                  e[cE_001_101]->Point, c001, d001, c101, d101, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_001_101]->Normal, g001, d001, g101, d101, I->Level);
            }
            active |= cM_001_101;
          }
          if (i001 != i111) {
            if (!(I3(I->ActiveEdges, i, j, k + 1) & cM_000_110)) {
              ...
              TetsurfInterpolate4(e[cE_001_111]->Point, c001, d001, c111, d111,
                  d101, d011, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate4(e[cE_001_111]->Normal, g001, d001, g111,
                    d111, d101, d011, I->Level);
            }
            active |= cM_001_111;
          }
          if (i010 != i011) {
            if (!(I3(I->ActiveEdges, i, j + 1, k) & cM_000_001)) {
              ...
              TetsurfInterpolate2(
                  e[cE_010_011]->Point, c010, d010, c011, d011, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_010_011]->Normal, g010, d010, g011, d011, I->Level);
            }
            active |= cM_010_011;
          }
          if (i010 != i110) {
            if (!(I3(I->ActiveEdges, i, j + 1, k) & cM_000_100)) {
              ...
              TetsurfInterpolate2(
                  e[cE_010_110]->Point, c010, d010, c110, d110, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_010_110]->Normal, g010, d010, g110, d110, I->Level);
            }
            active |= cM_010_110;
          }
          if (i010 != i111) {
            if (!(I3(I->ActiveEdges, i, j + 1, k) & cM_000_101)) {
              ...
              TetsurfInterpolate4(e[cE_010_111]->Point, c010, d010, c111, d111,
                  d110, d011, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate4(e[cE_010_111]->Normal, g010, d010, g111,
                    d111, d110, d011, I->Level);
            }
            active |= cM_010_111;
          }
          if (i100 != i101) {
            if (!(I3(I->ActiveEdges, i + 1, j, k) & cM_000_001)) {
              ...
              TetsurfInterpolate2(
                  e[cE_100_101]->Point, c100, d100, c101, d101, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_100_101]->Normal, g100, d100, g101, d101, I->Level);
            }
            active |= cM_100_101;
          }
          if (i100 != i110) {
            if (!(I3(I->ActiveEdges, i + 1, j, k) & cM_000_010)) {
              ...
              TetsurfInterpolate2(
                  e[cE_100_110]->Point, c100, d100, c110, d110, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_100_110]->Normal, g100, d100, g110, d110, I->Level);
            }
            active |= cM_100_110;
          }
          if (i100 != i111) {
            if (!(I3(I->ActiveEdges, i + 1, j, k) & cM_000_011)) {
              ...
              TetsurfInterpolate4(e[cE_100_111]->Point, c100, d100, c111, d111,
                  d101, d110, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate4(e[cE_100_111]->Normal, g100, d100, g111,
                    d111, d101, d110, I->Level);
            }
            active |= cM_100_111;
          }
          if (i011 != i111) {
            if (!(I3(I->ActiveEdges, i, j + 1, k + 1) & cM_000_100)) {
              ...
              TetsurfInterpolate2(
                  e[cE_011_111]->Point, c011, d011, c111, d111, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_011_111]->Normal, g011, d011, g111, d111, I->Level);
            }
            active |= cM_011_111;
          }
          if (i101 != i111) {
            if (!(I3(I->ActiveEdges, i + 1, j, k + 1) & cM_000_010)) {
              ...
              TetsurfInterpolate2(
                  e[cE_101_111]->Point, c101, d101, c111, d111, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_101_111]->Normal, g101, d101, g111, d111, I->Level);
            }
            active |= cM_101_111;
          }
          if (i110 != i111) {
            if (!(I3(I->ActiveEdges, i + 1, j + 1, k) & cM_000_001)) {
              ...
              TetsurfInterpolate2(
                  e[cE_110_111]->Point, c110, d110, c111, d111, I->Level);
              if (mode == cIsosurfaceMode::triangles_grad_normals)
                TetsurfInterpolate2(
                    e[cE_110_111]->Normal, g110, d110, g111, d111, I->Level);
            }
            active |= cM_110_111;
          }
          ...
          if (active) {
            ...
            case cIsosurfaceMode::triangles_grad_normals:
              code = (i000 ? 0x01 : 0) | (i001 ? 0x02 : 0) | (i010 ? 0x04 : 0) |
                     (i011 ? 0x08 : 0) | (i100 ? 0x10 : 0) | (i101 ? 0x20 : 0) |
                     (i110 ? 0x40 : 0) | (i111 ? 0x80 : 0);
