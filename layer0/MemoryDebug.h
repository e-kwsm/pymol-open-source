...
#define VLAlloc(type, init_size)                                               \
  (type*) VLAMalloc(init_size, sizeof(type), 5, 0)
#define VLACalloc(type, init_size)                                             \
  (type*) VLAMalloc(init_size, sizeof(type), 5, 1)
...
#define VLAInsert(ptr, type, index, count)                                     \
  {                                                                            \
    ptr = (type*) VLAInsertRaw(ptr, index, count);                             \
  }
#define VLADelete(ptr, type, index, count)                                     \
  {                                                                            \
    ptr = (type*) VLADeleteRaw(ptr, index, count);                             \
  }
...
#define ReallocForSure(ptr, type, size)                                        \
  (type*) MemoryReallocForSure(ptr, sizeof(type) * (size))
 ...
 */
template <typename T> T* VLACopy2(const T* vla)
{
 ...
 * insertion. If not, a resize will be attempted.
 ...
 * Note: Use of this function should be limited. Used for safe replacement of
 * VLACheck Note: This function can throw.
 */
template <typename T> void VecCheck(std::vector<T>& vec, std::size_t i)
{
  if (i >= vec.size()) {
    vec.resize(i + 1);
  }
...
template <typename T> T* VLACheck2(T*& ptr, size_t pos)
{
...
template <typename T> void VLASize2(T*& ptr, size_t size)
{
  ptr = static_cast<T*>(VLASetSize(ptr, size));
}

template <typename T> void VLASizeForSure2(T*& ptr, size_t size)
{
  ptr = static_cast<T*>(VLASetSizeForSure(ptr, size));
}

template <typename T> void VLAFreeP(T*& ptr)
{
  if (ptr) {
    VLAFree(ptr);
    ptr = nullptr;
