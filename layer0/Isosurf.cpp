...
#define O3Ptr(field, P1, P2, P3, offs)                                         \
  ((field)->ptr<float>((P1) + offs[0], (P2) + offs[1], (P3) + offs[2]))
...
static int IsosurfGradients(PyMOLGlobals* G, CSetting* set1, CSetting* set2,
    CIsosurf* II, Isofield* field, int* range, float min_level,
    float max_level);
...
/*===========================================================================*/
inline static void IsosurfInterpolate(
    CIsosurf* I, float* v1, float* l1, float* v2, float* l2, float* pt)
{
  float ratio;
...
Isofield* IsosurfNewFromPyList(PyMOLGlobals* G, PyObject* list)
{
  ...
  if (ok)
    ok = PConvPyListToIntArrayInPlace(
        PyList_GetItem(list, 0), result->dimensions, 3);
  if (ok)
    ...
    for (a = 0; a < dim[0]; a += (dim[0] - 1)) {
      ...
      for (b = 1; b < (dim[1] - 1); b++) {
        for (c = 1; c < (dim[2] - 1); c++) {
          if (!a) {
            F4(gradients, a, b, c, 0) =
                (F3(data, a + 1, b, c) - F3(data, a, b, c));
          } else {
            F4(gradients, a, b, c, 0) =
                (F3(data, a, b, c) - F3(data, a - 1, b, c));
          }
          F4(gradients, a, b, c, 1) =
              (F3(data, a, b + 1, c) - F3(data, a, b - 1, c)) / 2.0F;
          F4(gradients, a, b, c, 2) =
      ...
      for (b = 0; b < dim[1]; b += (dim[1] - 1)) {
        for (c = 0; c < dim[2]; c++) {
          ...
          if (!b) {
            F4(gradients, a, b, c, 1) =
                (F3(data, a, b + 1, c) - F3(data, a, b, c));
          } else {
            F4(gradients, a, b, c, 1) =
                (F3(data, a, b, c) - F3(data, a, b - 1, c));
          }

          if (!c) {
            F4(gradients, a, b, c, 2) =
                (F3(data, a, b, c + 1) - F3(data, a, b, c));
          } else if (c < (dim[2] - 1)) {
            F4(gradients, a, b, c, 2) =
                (F3(data, a, b, c + 1) - F3(data, a, b, c - 1)) / 2.0F;
          } else {
            F4(gradients, a, b, c, 2) =
                (F3(data, a, b, c) - F3(data, a, b, c - 1));
          }
        }
      }
              ...
              if ((a >= 0) && (b >= 0) && (c >= 0) &&
...

                {
                  ...
                  if ((x <= sloppy_1) && (y <= sloppy_1) && (z <= sloppy_1)) {
                    ...
                    /* found at least one point obtained through symmetry or
                     * priodicity */
                    if (x > 1.0F)
                      x = 1.0F;
                    if (y > 1.0F)
                    ...
                    if (z > 1.0F)
                      z = 1.0F;
                    average +=
                        FieldInterpolatef(field1->data.get(), a, b, c, x, y, z);
                    cnt++;
                  } else {
                    ...
                    if (((x - 1.0F) < sloppy_1) && ((y - 1.0F) < sloppy_1) &&
                        ((z - 1.0F) < sloppy_1)) {
                      if (x > 1.0F)
                        x = 1.0F;
                      if (y > 1.0F)
                      ...
                      if (z > 1.0F)
                        z = 1.0F;
                      extrapolate_average += FieldInterpolatef(
                          field1->data.get(), a, b, c, x, y, z);
                      extrapolate_cnt++;
                    }
                  }
                }
              ...
              if (ok) {
                ...
                if (ok)
                  ...
                  case cIsomeshMode::isomesh: /* standard mode - want lines */
                    ok = IsosurfCurrent(I);
                    break;
                  case cIsomeshMode::isodot: /* point mode - just want points on
                                                the isosurface */
                    ok = IsosurfPoints(I);
                    break;
                  }
...
static int IsosurfGradients(PyMOLGlobals* G, CSetting* set1, CSetting* set2,
    CIsosurf* II, Isofield* field, int* range, float min_level, float max_level)
{
  CIsosurf* I = II;
  int ok = true;
  ...
  if (field->gradients) {
    ...
    CHECKOK(ok, order);
    if (order && flag && (range_dim[0] > 1) && (range_dim[1] > 1) &&
        (range_dim[2] > 1)) {

      {
        ...
        average_cell_axis_dist =
            (float) ((diff3f(pos[0], pos[1]) + diff3f(pos[0], pos[2]) +
                         diff3f(pos[0], pos[3])) /
                     3.0);
...

      {
        ...
        for (a = 0; a < range_size; a++) {
          ...
          float symmetry_max =
              FLT_MIN; /* if we're trying to get symmetric segments */
          float symmetry_min = FLT_MAX;

          for (pass = 0; pass < 2;
               pass++) { /* one pass down the gradient, one up */

            int have_prev =
                false; /* flag & storage for previous gradient & locus */
            ...
            for (;;) {
...

              {
                ...
                for (b = 0; b < 3; b++) {
                  ...
                  while (
                      locus[b] > (range[b + 3] - 2)) { /* above range? done */
                    if (fract[b] <= 0.0F) {
                      locus[b]--;
                      fract[b] += 1.0F;
              ...
              if ((!have_prev) ||
                  (have_prev && ((locus[0] != prev_locus[0]) ||
                                    (locus[1] != prev_locus[1]) ||
                                    (locus[2] != prev_locus[2])))) {
                /* above: prev_locus may be nullptr, so relying upon shortcut
                 * logic eval */
...

              {
                ...
                float level = FieldInterpolatef(i_data, locus[0], locus[1],
                    locus[2], fract[0], fract[1], fract[2]);
...

              {
...

                {
                  ...
                  if (have_prev) {
                    if (dot_product3f(interp_gradient, prev_grad_normal) <
                        min_dot)
                      break;
                  }
            ...
            if (n_vert < 2) { /* quash isolated vertices */
              if (n_vert) {
                n_line = i_num[n_seg];
                       ...
                       i_num[n_seg]) { /* or retain count of new line segment */
          ...
          }
          {
            ...
            if (symmetry_flag) {
              if ((symmetry_max * symmetry_min) >=
                  0.0F) /* abort if not both +/- pot. sampled */
                abort_segment = true;
              else {
                float symmetry_ratio =
                    (float) (fabs(symmetry_max) / fabs(symmetry_min));
                if (symmetry_ratio > 1.0F)
                  symmetry_ratio = 1.0F / symmetry_ratio;
                if (symmetry_ratio < symmetry) /* abort if +/- weren't close
                                                  enough in magnitude */
                  abort_segment = true;
              }
            }
...
static int IsosurfDrawPoints(CIsosurf* II)
{
  ...
  if (ok) {
    for (i = 0; i < (I->Max[0] - 1); i++) {
      for (j = 0; j < I->Max[1]; j++) {
        for (k = 0; k < I->Max[2]; k++) {
          if ((I3(I->VertexCodes, i, j, k)) &&
              (!I3(I->VertexCodes, i + 1, j, k))) {
            IsosurfInterpolate(I, O4Ptr(I->Coord, i, j, k, 0, I->CurOff),
            ...
            *a = *b;
            I->NLine++;
          } else if (!(I3(I->VertexCodes, i, j, k)) &&
                     (I3(I->VertexCodes, i + 1, j, k))) {
            IsosurfInterpolate(I, O4Ptr(I->Coord, i, j, k, 0, I->CurOff),
                O3Ptr(I->Data, i, j, k, I->CurOff),
                O4Ptr(I->Coord, i + 1, j, k, 0, I->CurOff),
