...
int CShaderPrg::Enable()
{
  ...
  glUseProgram(id);
  // uniform
  Set1i("isPicking", SettingGetGlobal_b(G, cSetting_pick_shading)
                         ? 1
                         : G->ShaderMgr->is_picking);
  return 1;
}
...
int CShaderPrg::Link()
{
  ...
  if (!IsLinked()) {
    if (G && G->Option && !G->Option->quiet) {
      ...
      " CShaderPrg_Link-Error: Shader program failed to link name='%s'; "
      "GL_MAX_VARYING_FLOATS=%d log follows.\n",
          name.c_str(), maxVarFloats ENDFB(G);
...
void CShaderPrg::SetAttrib4fLocation(
...
void CShaderPrg::Set_AnaglyphMode(int mode)
{
  ...
  SetMat3fc("matL", G->ShaderMgr->stereo_flag < 0 ? anaglyphL_constants[mode]
...
void CShaderPrg::Set_Specular_Values()
{
  ...
  if (trans_oblique > R_SMALL4) {
    ...
    Set1f("oblique_power",
        SettingGetGlobal_f(G, cSetting_ray_transparency_oblique_power));
  }
  ...
  float settingSpecReflect, settingSpecDirect, settingSpecDirectPower,
      settingSpecPower;

  SceneGetAdjustedLightValues(G, &settingSpecReflect, &settingSpecPower,
      &settingSpecDirect, &settingSpecDirectPower);
...
void CShaderPrg::SetBgUniforms()
{
  auto bg_image_tilesize =
      SettingGet<const float*>(G, cSetting_bg_image_tilesize);
  ...
  Set2f("tiledSize", bg_image_tilesize[0] / (float) scene_width,
      bg_image_tilesize[1] / (float) scene_height);
  Set2f("tileSize", 1.f / (float) bg_image_tilesize[0],
      1.f / (float) bg_image_tilesize[1]);
  Set2f("viewImageSize", bg_width / (float) scene_width,
      bg_height / (float) scene_height);
  ...
  if (SettingGet<bool>(G, cSetting_chromadepth) &&
    ...
    Set2f("clippingplanes", SceneGetCurrentFrontSafe(G),
