...
void get_system1f3f(
    float* x, float* y, float* z); /* make system in direction of x */
void get_system2f3f(float* x, float* y,
    float* z); /* make system in direction of x, perp to x,y */
...
bool is_allclosef(int nrow, const float* m1, int ncol1, const float* m2,
    int ncol2, float threshold = 1.0E-6F);
bool is_diagonalf(
    int nrow, const float* m, int ncol = 0, float threshold = 1.0E-6F);
...
void transform33Tf3f(
    const float* m1, const float* m2, float* m3); /* uses transpose */
...
/* A 4x4 TTT matrix is really a 3x3 rotation matrix with two translation
   vectors: (1) a pre-translation stored in forth row, first three columns. (2)
   and a post-translation stored in forth column, first three rows. There are
   certain cases where this representation is more convenient.
 */
...
void multiply44d44d44d(
    const double* left, const double* right, double* product);
...
template <typename T> inline void zero3(T* v1)
{
  v1[0] = 0;
  v1[1] = 0;
  v1[2] = 0;
...
template <typename S, typename D> void copyN(const S* src, D* dst, int N)
{
  for (int i = 0; i < N; ++i)
    dst[i] = src[i];
}

template <typename S, typename D> void copy2(const S* src, D* dst)
{
  dst[0] = src[0];
  dst[1] = src[1];
}

template <typename S, typename D> void copy3(const S* src, D* dst)
{
  dst[0] = src[0];
  dst[1] = src[1];
  dst[2] = src[2];
...
template <typename S, typename D> void copy4(const S* src, D* dst)
{
  dst[0] = src[0];
  dst[1] = src[1];
  dst[2] = src[2];
...
/**
 * Get the shortest position vector for a plane with position v1 and normal
 * vector v2.
