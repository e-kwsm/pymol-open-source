...
public:
  ...
  /*
   * Create a derivative copy. This will reload with
   * ShaderMgr::Reload_Derivatives.
   */
  ...
  void SetAttrib4fLocation(
      const char* name, float f1, float f2, float f3, float f4);
...
public:
  ...
  template <typename T, typename... TArgs> T* newGPUBuffer(TArgs&&... args)
  {
  ...
  template <typename T> T* getGPUBuffer(size_t hashid)
  {
  ...
  CShaderPrg* GetShaderPrg(std::string name, short set_current_shader = 1,
  ...
  CShaderPrg* Enable_DefaultShaderWithSettings(
  ...
  CShaderPrg* Get_CylinderNewShader(
      RenderPass pass, short set_current_shader = 1);
  ...
  CShaderPrg* Setup_DefaultShader(
  ...
  void SetPreprocVar(
      const std::string& key, bool value, bool invshaders = true);
...
private:
  ...
  void CollectDependantFileNames(
...
public:
  ...
  short stereo_blend; /* 0 - no blend, 1 - blend  : for right eye stereo in
                         full-screen e.g., anaglyph */
...
public:
   ...
   */
  GLFramebufferConfig bindOffscreenOrtho(
      const Extent2D& extent, bool clear = true);
   ...
   */
  GLFramebufferConfig bindOffscreenSizedImage(
      const Extent2D& extent, bool clear = true);
   ...
   */
  std::vector<unsigned char> readPixelsFrom(PyMOLGlobals* G, const Rect2D& rect,
      const GLFramebufferConfig& srcConfig);
