...
static void DieOutOfMemory(void)
{
  printf("*********************************************************************"
         "*******\n");
  printf("*** EEK!  PyMOL just ran out of memory and crashed.  To get around "
         "this, ***\n");
  printf("*** you may need to reduce the quality, size, or complexity of the "
         "scene ***\n");
  printf("*** that you are viewing or rendering.    Sorry for the "
         "inconvenience... ***\n");
  printf("*********************************************************************"
         "*******\n");
  ...
  if (ptr) {
    ...
    if ((index >= 0) && (count > 0) && (index <= old_size)) {
      ...
      if (ptr) {
        ...
        if (vla->auto_zero)
          ov_os_memset(
              base + index * vla->unit_size, 0, vla->unit_size * count);
      }
    }
  }
  return ptr;
