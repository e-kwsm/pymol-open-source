...
typedef void (*AttribOpFuncDataFunctionPtr)(
    void* varData, const float* pc, void* globalData, int idx);

/* AttribOpFuncData : This structure holds information a callback that
   sets/post-processes data for a particular attribute.  Currently, a list of
   these functions are attached to the AttribOp so that when vertices are
   created (i.e., incr_vertices > 0) then for each vertex, this function is
   called for the particular attribute attribName.

   funcDataConversion - pointer to function that sets/post-processes the
   attribute data funcDataGlobalArg - pointer to global structure that can be
   used in each call to the callback attribName - attribute name this function
   is processing. (this calling function specifies the name, and the
                CGOConvertToShader() sets attrib to the associated AttribDesc.
 */
struct AttribOpFuncData {
  void (*funcDataConversion)(void* varData, const float* pc, void* globalData,
      int idx); // if set, should be called on every output value for this
                // attribute
  ...
  AttribOpFuncData(AttribOpFuncDataFunctionPtr _funcDataConversion,
      void* _funcDataGlobalArg, const char* _attribName)
      : funcDataConversion(_funcDataConversion)
      , funcDataGlobalArg(_funcDataGlobalArg)
 ...
 * order - the order for this operation to be executed for the given CGO
 * operation offset - the offset into the CGO operation to copy conv_type - type
 * of copy (can be general or specific, see above, e.g. FLOAT3_TO_FLOAT3,
 * UB1_TO_INTERP) incr_vertices - the number of vertices (if any) that are
 * generated for the VBO after this operation is executed.
 *
 */
struct AttribOp {
  AttribOp(unsigned short _op, size_t _order, size_t _conv_type, size_t _offset,
      size_t _incr_vertices = 0, int _copyFromAttr = -1)
      : op(_op)
      , order(_order)
      , offset(_offset)
...
/**
 * defines an attribute that is used in a shader.  this description has all of
 * the necessary information for our "optimize" function to generate either an
 * array for input into the VBO or a call to the related glVertexAttrib() call
 * when this attribute has the same value throughout the CGO.
 *
 * attr_name - the name of this attribute inside the shaders
 * order - order of attribute used in VBO
 * attrOps - all AttribOp for this particular attribute.  This allows the user
 * to define how this attribute gets populated from the primitive CGO's one or
 * many CGO operations. default_value - pointer to the default value of this
 * attribute (optional, needs to be the same size of the attribute's type)
 * repeat_value/repeat_value_length - specified if the attribute has repeating
 * values repeat_value - a pointer to the type and data for repeat values
 *                                    repeat_value_length - number of repeat
 * values type_size - size of type for this attribute (e.g., GL_FLOAT,
 * GL_UNSIGNED_BYTE) type_dim - number of primitives (i.e., type_size) for each
 * vertex of this attribute data_norm - whether this attribute is normalized
 * when passed to the VBO (GL_TRUE or GL_FALSE)
 *
 */
struct AttribDesc {
  AttribDesc(
 ...
 * INTERLEAVED:
 *   vbo [ data1[0], data2[0], ..., dataN[0] | ... | data1[M], data2[M], ...,
 *dataN[M] ]
 */
...
namespace rbo
{
enum storage { DEPTH16 = 0, DEPTH24, COUNT };
...
namespace tex
{
enum class dim : int { D1 = 0, D2, D3, COUNT };
enum class format : int { R = (int) dim::COUNT, RG, RGB, RGBA, COUNT };
...
enum class env_name : int { ENV_MODE = (int) wrap::COUNT, COUNT };
enum class env_param : int { REPLACE = (int) env_name::COUNT, COUNT };
...
public:
  ...
  textureBuffer_t(tex::format format, tex::data_type type, tex::filter mag,
      tex::filter min, tex::wrap wrap_s)
      : _dim(tex::dim::D1)
      , _format(format)
      , _type(type)
  ...
  textureBuffer_t(tex::format format, tex::data_type type, tex::filter mag,
      tex::filter min, tex::wrap wrap_s, tex::wrap wrap_t)
      : _dim(tex::dim::D2)
      , _format(format)
      , _type(type)
  ...
  textureBuffer_t(tex::format format, tex::data_type type, tex::filter mag,
      tex::filter min, tex::wrap wrap_s, tex::wrap wrap_t, tex::wrap wrap_r)
      : _dim(tex::dim::D3)
      , _format(format)
      , _type(type)
      , _sampling(
            {(int) mag, (int) min, (int) wrap_s, (int) wrap_t, (int) wrap_r})
  {
    genBuffer();
  };
  ~textureBuffer_t() { freeBuffer(); }
...
namespace fbo
{
enum attachment { COLOR0 = 0, COLOR1, COLOR2, COLOR3, DEPTH, COUNT };
...
public:
  frameBuffer_t() { genBuffer(); }
  ~frameBuffer_t() { freeBuffer(); }
...
public:
  ...
  renderTarget_t(shape_type size)
      : _size(size)
  {
  }
  renderTarget_t(int width, int height)
      : _size(width, height)
  {
  }
  ~renderTarget_t();
  ...
  void layout(
      std::vector<rt_layout_t>&& desc, renderBuffer_t* with_rbo = nullptr);
  ...
  const std::vector<textureBuffer_t*>& textures() const noexcept
  {
    return _textures;
  }
