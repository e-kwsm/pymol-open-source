...
int UtilShouldWePrintQuantity(int quantity)
{
  if (quantity < 10)
    return 1;
  if ((quantity > 0) &&
      (quantity < 0x07FFFFFF)) /* avoids overflow, just in case */ {
    int factor = 10;
...
void UtilNConcat(char* dst, const char* src, ov_size n)
...
void UtilExpandArrayElements(
    void* src, void* dst, int n_entries, int old_rec_size, int new_rec_size)
{
...
void UtilApplySortedIndices(int n, int* x, int rec_size, void* src, void* dst)
{
  int a;
  for (a = 0; a < n; a++) {
    memcpy(((char*) dst) + (a * rec_size), ((char*) src) + (x[a] * rec_size),
        rec_size);
  }
}
...
void UtilSortIndex(int n, void* array, int* x, UtilOrderFn* fOrdered)
{
  ...
  while (1) {
    ...
    a = l << 1;
    while (a <= r) {
      if (a < r && (!fOrdered(array, x[a + 1] - 1, x[a] - 1)))
        a++;
    ...
    UtilOrderFnGlobals* fOrdered)
{
  ...
  while (1) {
    ...
    a = l << 1;
    while (a <= r) {
      if (a < r && (!fOrdered(G, array, x[a + 1] - 1, x[a] - 1)))
        a++;
...
int UtilSemiSortFloatIndexWithNBins(
    int n, int nbins, float* array, int* destx, int forward)
{
  int* start1 = pymol::calloc<int>(n + nbins);
  int ret = UtilSemiSortFloatIndexWithNBinsImpl(
      start1, n, nbins, array, destx, forward);
  mfree(start1);
  return ret;
}

int UtilSemiSortFloatIndexWithNBinsImpl(
    int* start1, int n, int nbins, float* array, int* destx, int forward)
{
   ...
   */
  /* the next1 array will hold a link to most of the indexes, which are
   * traversed */
...
void UtilSortInPlace(PyMOLGlobals* G, void* array, int nItem,
    unsigned int itemSize, UtilOrderFn* fOrdered)

{
  ...
  if (nItem > 0) {
    ...
    for (a = 0; a < nItem; a++)
      index[a]++; /* ^tricky index adjustment to avoid flag array */
    for (a = 0; a < nItem; a++) {
      ia = abs(index[a]) - 1; /* ^ */
      if (ia != a) {
