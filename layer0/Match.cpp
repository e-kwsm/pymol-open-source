...
CMatch* MatchNew(
    PyMOLGlobals* G, unsigned int na, unsigned int nb, int dist_mats)
{
  unsigned int dim[2];
  auto I = new CMatch();
...

  {
    ...
    for (i = 0; i < dim[0]; i++) {
      I->smat[i][i] =
          10.0F; /* these values will be overwritten by BLOSUM, etc. */
    }
...
int MatchResidueToCode(CMatch* I, int* vla, int n)
{
  ...
  char res[][4] = {
      ...
      "MET", "M", "MSE", "M", /* selenomet */

      "ASN", "N", "PRO", "P", "GLN", "Q", "ARG", "R",

      "SER", "S", "SEP", "S", /* phosphoserine */
      "THR", "T", "TPO", "T", /* phosphothreonine */
      "VAL", "V",

      "TRP", "W", "TYR", "Y", "PTR", "Y", /* phosphotyrosine */
      "TYS", "Y"                          /* sulfotyrosine */
  };
...
int MatchAlign(CMatch* I, float gap_penalty, float ext_penalty, int max_gap,
    int max_skip, int quiet, int window, float ante)
{
  PyMOLGlobals* G = I->G;
  int a, b, f, g;
  int nf, ng;
