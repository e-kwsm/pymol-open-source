...
PyObject* FieldAsNumPyArray(CField* field, short copy)
{
  ...
  if (copy) {
    if ((result = PyArray_SimpleNew(field->n_dim(), dims, typenum)))
      memcpy(PyArray_DATA((PyArrayObject*) result), field->data.data(),
          field->size());
  } else {
    result = PyArray_SimpleNewFromData(
        field->n_dim(), dims, typenum, field->data.data());
  }
...
PyObject* FieldAsPyList(PyMOLGlobals* G, CField* I)
{
  ...
  int pse_export_version =
      SettingGetGlobal_f(G, cSetting_pse_export_version) * 1000;
  bool dump_binary = (!pse_export_version || pse_export_version > 1776) &&
                     SettingGetGlobal_b(G, cSetting_pse_binary_dump);
  ...
  PyList_SetItem(
      result, 4, PConvIntArrayToPyList((int*) I->dim.data(), I->n_dim()));
  PyList_SetItem(
      result, 5, PConvIntArrayToPyList((int*) I->stride.data(), I->n_dim()));
  n_elem = I->data.size() / I->base_size;
  switch (I->type) {
  case cFieldInt:
    PyList_SetItem(result, 6,
        PConvIntArrayToPyList((int*) I->data.data(), n_elem, dump_binary));
    break;
  case cFieldFloat:
    PyList_SetItem(result, 6,
        PConvFloatArrayToPyList((float*) I->data.data(), n_elem, dump_binary));
    break;
  default:
...
float FieldInterpolatef(
    CField* I, int a, int b, int c, float x, float y, float z)
{
...

  {
    ...
    if ((product1 = x * y * z1) != 0.0F)
      result1 +=
          product1 * (*((float*) (data + a_bs + a_st + b_bs + b_st + c_bs)));
    if ((product2 = x1 * y * z) != 0.0F)
      result2 +=
          product2 * (*((float*) (data + a_bs + b_bs + b_st + c_bs + c_st)));
    if ((product1 = x * y1 * z) != 0.0F)
      result1 +=
          product1 * (*((float*) (data + a_bs + a_st + b_bs + c_bs + c_st)));
    if ((product2 = x * y * z) != 0.0F)
      result2 += product2 *
                 (*((float*) (data + a_bs + a_st + b_bs + b_st + c_bs + c_st)));
  }
...
float FieldExtrapolatef(CField *I,int a,int b,int c,float x,float y,float z)
{
  ...
  {
    float base = FieldInterpolatef(I,a,b,c,ix,iy,iz) -
FieldInterpolatef(I,a,b,c,0.0F,0.0F,0.0F); float offset =
FieldInterpolatef(I,a,b,c,x,y,z); return base + offset;
  }
}
*/
...
void FieldInterpolate3f(CField* I, int* locus, float* fract, float* result)
{
...

  {
    ...
    for (d = 0; d < 3; d++) {
      ...
      if ((product2 = x * y1 * z1) != 0.0F)
        result2 +=
            product2 * (*((float*) (data + a_bs + a_st + b_bs + c_bs + d_bs)));
      if ((product1 = x1 * y * z1) != 0.0F)
        result1 +=
            product1 * (*((float*) (data + a_bs + b_bs + b_st + c_bs + d_bs)));
      if ((product2 = x1 * y1 * z) != 0.0F)
        result2 +=
            product2 * (*((float*) (data + a_bs + b_bs + c_bs + c_st + d_bs)));
      if ((product1 = x * y * z1) != 0.0F)
        result1 +=
            product1 *
            (*((float*) (data + a_bs + a_st + b_bs + b_st + c_bs + d_bs)));
      if ((product2 = x1 * y * z) != 0.0F)
        result2 +=
            product2 *
            (*((float*) (data + a_bs + b_bs + b_st + c_bs + c_st + d_bs)));
      if ((product1 = x * y1 * z) != 0.0F)
        result1 +=
            product1 *
            (*((float*) (data + a_bs + a_st + b_bs + c_bs + c_st + d_bs)));
      if ((product2 = x * y * z) != 0.0F)
        result2 += product2 * (*((float*) (data + a_bs + a_st + b_bs + b_st +
                                           c_bs + c_st + d_bs)));
...
int FieldSmooth3f(CField* I)
{
  ...
  if (data) {
    for (a = 0; a < na; a++)
      for (b = 0; b < nb; b++)
        for (c = 0; c < nc; c++) {
          ...
          tot = tot / cnt;
          *((float*) (data + (a) *I->stride[0] + (b) *I->stride[1] +
    ...
    inp_stdev =
        (float) sqrt1d((inp_sumsq - (inp_sum * inp_sum / n_pts)) / (n_pts - 1));

    out_mean = (float) (out_sum / n_pts);
    out_stdev =
        (float) sqrt1d((out_sumsq - (out_sum * out_sum / n_pts)) / (n_pts - 1));
...
CField::CField(PyMOLGlobals* G, const int* const dim, int n_dim,
    unsigned int base_size, cField_t type)
    : type(type)
    , base_size(base_size)
{
  auto I = this;
