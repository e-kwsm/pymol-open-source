...
inline bool Feedback(PyMOLGlobals* G, unsigned int sysmod, unsigned char mask)
{
...
#define PRINTF                                                                 \
  {                                                                            \
    FeedbackLineType _FBstr; (snprintf)( _FBstr, FEEDBACK_MAX_OUTPUT,
#define ENDF(G)  );                                                            \
  G->Feedback->add(_FBstr);                                                    \
  }
#endif

/* debugging: goes to stderr */

#define PRINTFD(G, sysmod)                                                     \
  {                                                                            \
    if (G->Feedback->testMask(sysmod, FB_Debugging)) { fprintf(stderr,
#define ENDFD   );                                                             \
  fflush(stderr);                                                              \
  }                                                                            \
  }
