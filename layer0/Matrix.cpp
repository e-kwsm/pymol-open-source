...
{
  xx_float64
      stack_A_tmp[XX_MATRIX_STACK_STORAGE_MAX * XX_MATRIX_STACK_STORAGE_MAX];
...

        {
          ...
          for (p = 0; p < (size - 1); p++) {
            for (q = p + 1; q < size; q++) {
              ...
              if ((i > 3) && ((xx_fabs(e_val[p]) + g) == xx_fabs(e_val[p])) &&
...
static xx_word xx_matrix_decompose(
    xx_float64* matrix, xx_word size, xx_word* permute, xx_word* parity)
{
...
static void xx_matrix_back_substitute(
    xx_float64* result, xx_float64* decomp, xx_word size, xx_word* permute)
{
  {
    xx_word i, ii;
    ii = -1;
...
xx_boolean xx_matrix_invert(
    xx_float64* result, const xx_float64* input, xx_word size)
{
  xx_float64
      stack_mat_tmp[XX_MATRIX_STACK_STORAGE_MAX * XX_MATRIX_STACK_STORAGE_MAX];
...
int MatrixInvTransformExtentsR44d3f(const double* matrix, const float* old_min,
    const float* old_max, float* new_min, float* new_max)
{
...
int MatrixTransformExtentsR44d3f(const double* matrix, const float* old_min,
    const float* old_max, float* new_min, float* new_max)
{
  /* just brute-forcing this for now... */
  int a;
...
#define MAT(m, r, c) (m)[(c) * 4 + (r)]
  ...
  r0[3] = MAT(m, 0, 3), r0[4] = 1.0, r0[5] = r0[6] = r0[7] = 0.0F,
  r1[0] = MAT(m, 1, 0), r1[1] = MAT(m, 1, 1), r1[2] = MAT(m, 1, 2),
  r1[3] = MAT(m, 1, 3), r1[5] = 1.0, r1[4] = r1[6] = r1[7] = 0.0F,
  r2[0] = MAT(m, 2, 0), r2[1] = MAT(m, 2, 1), r2[2] = MAT(m, 2, 2),
  r2[3] = MAT(m, 2, 3), r2[6] = 1.0, r2[4] = r2[5] = r2[7] = 0.0F,
  r3[0] = MAT(m, 3, 0), r3[1] = MAT(m, 3, 1), r3[2] = MAT(m, 3, 2),
  r3[3] = MAT(m, 3, 3), r3[7] = 1.0, r3[4] = r3[5] = r3[6] = 0.0F;
...
/*========================================================================*/
void MatrixTransformTTTfN3f(
    unsigned int n, float* q, const float* m, const float* p)
{
...
/*========================================================================*/
void MatrixGetRotationC44f(float* m44, float angle, float x, float y, float z)
{
  float m33[9];
...
/*========================================================================*/
float MatrixGetRMS(
    PyMOLGlobals* G, int n, const float* v1, const float* v2, float* wt)
{
...
/*========================================================================*/
float MatrixFitRMSTTTf(PyMOLGlobals* G, int n, const float* v1, const float* v2,
    const float* wt, float* ttt)
{
  /*
  ...
  if (n > 1) {
    ...
    if (fit_kabsch) {
      ...
      multiply33d33d(
          (double*) (void*) At, (double*) (void*) aa, (double*) (void*) AtA);
...

      {
        ...
        if (xx_matrix_jacobi_solve((xx_float64*) (void*) e_vec,
                (xx_float64*) (void*) e_val, &n_rot, (xx_float64*) (void*) AtA,
                3)) {
          double V[3][3], Vt[3][3];
          ...
          if ((e_val[0] >= 0.0) && (e_val[1] >= 0.0) && (e_val[2] >= 0.0)) {
            ...
            case 1: {
              ...
              if (xx_matrix_invert(
                      (xx_float64*) (void*) Ai, (xx_float64*) (void*) aa, 3)) {
                ...
                { /* is the rotation matrix left-handed? Then swap the
                  ...
                  double cp[3], dp;
                  cross_product3d(
                      (double*) (void*) m, (double*) (void*) m + 3, cp);
                  dp = dot_product3d((double*) (void*) m + 6, cp);
                  if ((1.0 - fabs(dp)) > R_SMALL4) { /* not orthonormal? */
                    got_kabsch = false;

                    PRINTFB(G, FB_Matrix, FB_Warnings)
                    "Matrix-Warning: Kabsch matrix not orthonormal: falling "
                    "back on iteration.\n" ENDFB(G);

                  } else if (dp < 0.0F) {
                    multiply33d33d((double*) (void*) rootD, (double*) (void*) V,
                        (double*) (void*) sqrtAtA);
                    multiply33d33d((double*) (void*) Vt,
                        (double*) (void*) sqrtAtA, (double*) (void*) sqrtAtA);
                ...
                PRINTFB(G, FB_Matrix, FB_Warnings)
                "Matrix-Warning: Kabsch matrix inversion failed: falling back "
                "on iteration.\n" ENDFB(G);
              }
            } break;
            case 2: {
              ...
              if ((e_val[0] > R_SMALL8) && (e_val[1] > R_SMALL8) &&
                  (e_val[2] > R_SMALL8)) {
                ...
                { /* cis the rotation matrix left-handed? Then swap the
                  ...
                  double cp[3], dp;
                  cross_product3d(
                      (double*) (void*) Mi, (double*) (void*) Mi + 3, cp);
                  ...
                  if ((1.0 - fabs(dp)) > R_SMALL4) { /* not orthonormal? */
                    ...
                    multiply33d33d((double*) (void*) invRootD,
                        (double*) (void*) V, (double*) (void*) Mi);
                ...
                PRINTFB(G, FB_Matrix, FB_Warnings)
                "Matrix-Warning: Kabsch matrix degenerate: falling back on "
                "iteration.\n" ENDFB(G);
              }
            } break;
            }
            ...
            if (got_kabsch) {
              ...
              if ((fabs(length3d(m[0]) - 1.0) < R_SMALL4) &&
                ...
                PRINTFB(G, FB_Matrix, FB_Warnings)
                "Matrix-Warning: Kabsch matrix not normal: falling back on "
                "iteration.\n" ENDFB(G);
              }
            }
          } else {
            PRINTFB(G, FB_Matrix, FB_Warnings)
            "Matrix-Warning: Kabsch eigenvalue(s) negative: falling back on "
            "iteration.\n" ENDFB(G);
          }
        } else {
          PRINTFB(G, FB_Matrix, FB_Warnings)
          "Matrix-Warning: Kabsch decomposition failed: falling back on "
          "iteration.\n" ENDFB(G);
        }
      }
    }
    ...
    if (!got_kabsch) {
      ...
      while (1) {
        ...
        if (iters >= maxiter) {
          ...
          " Matrix: Warning: no convergence (%1.8f<%1.8f after %d "
          "iterations).\n",
     ...
     the weighted RMS error. */
  {
    ...
    for (c = 0; c < n; c++) {
      etmp = 0.0;
      for (a = 0; a < 3; a++) {
        tmp = m[a][0] * (vv2[0] - t2[0]) + m[a][1] * (vv2[1] - t2[1]) +
              m[a][2] * (vv2[2] - t2[2]);
        tmp = (vv1[a] - t1[a]) - tmp;
        etmp += tmp * tmp;
      }
 ...
 * @return Error code
 */
int MatrixEigensolveC33d(
    PyMOLGlobals* G, const double* a, double* wr, double* wi, double* v)
{
  ...
  if (G && Feedback(G, FB_Matrix, FB_Blather)) {
    ...
    printf(
        " Eigensolve: eigenvalues  %8.3f %8.3f %8.3f\n", wr[0], wr[1], wr[2]);
    printf(
        " Eigensolve:              %8.3f %8.3f %8.3f\n", wi[0], wi[1], wi[2]);
  }
