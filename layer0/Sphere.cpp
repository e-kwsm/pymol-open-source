...
static const float start_points[13][3] = {{tau, one, 0}, {-tau, one, 0},
    {-tau, -one, 0}, {tau, -one, 0}, {one, 0, tau}, {one, 0, -tau},
    {-one, 0, -tau}, {-one, 0, tau}, {0, tau, one}, {0, -tau, one},
    {0, -tau, -one}, {0, tau, -one}};

static const int icosahedron[21][3] = {{4, 8, 7}, {4, 7, 9}, {5, 6, 11},
    {5, 10, 6}, {0, 4, 3}, {0, 3, 5}, {2, 7, 1}, {2, 1, 6}, {8, 0, 11},
    {8, 11, 1}, {9, 10, 3}, {9, 2, 10}, {8, 4, 0}, {11, 0, 5}, {4, 9, 3},
    {5, 3, 10}, {7, 8, 1}, {6, 1, 11}, {7, 2, 9}, {6, 10, 2}};
#endif

static const int mesh[30][2] = {{0, 3}, {0, 4}, {0, 5}, {0, 8}, {0, 11}, {1, 2},
    {1, 6}, {1, 7}, {1, 8}, {1, 11}, {2, 6}, {2, 7}, {2, 9}, {2, 10}, {3, 4},
    {3, 5}, {3, 9}, {3, 10}, {4, 7}, {4, 8}, {4, 9}, {5, 6}, {5, 10}, {5, 11},
    {6, 10}, {6, 11}, {7, 8}, {7, 9}, {8, 11}, {9, 10}};
...
static void SphereDumpAll(CSphere* I)
{
  ...
  fprintf(
      f, "static int Sphere_StripLen_start[%d] = {\n", NUMBER_OF_SPHERE_LEVELS);
  strip_total = 0;
  for (i = 0; i < NUMBER_OF_SPHERE_LEVELS; i++) {
  ...
  fprintf(
      f, "static int Sphere_Sequence_start[%d] = {\n", NUMBER_OF_SPHERE_LEVELS);
  seq_total = 0;
  for (i = 0; i < NUMBER_OF_SPHERE_LEVELS; i++) {
...
// MAXDOT : 12, 42, 162, 642, 2562 ... :: 12 + (30 + 120 + 480 + 1920 + ... ) ::
// 12 + ( 30 + (30*4) + (30*4*4) + (30*4*4*4) + ... ) MAXTRI : 80, 320, 1280,
// 5120, ... :: 20*1 + 20*4 + 20*4*4 + 20*4*4*4 + 20*4*4*4*4 ::
...
typedef int EdgeCol[MAXDOT]; /* should move these into dynamic storage to save
                                3MB  mem */
...
static SphereRec* MakeDotSphere(PyMOLGlobals* G, int level)
{
  ...
  for (a = 0; a < S->NTri; a++) {
    ...
    if (dot_product3f(vt, result->dot[S->Tri[a][0]]) <
        0.0) { /* if wrong, then interchange */
      it = S->Tri[a][2];
      S->Tri[a][2] = S->Tri[a][1];
      S->Tri[a][1] = it;
    }
  ...
  for (a = 0; a < S->NTri; a++) {
    area =
        (float) (SphericalAngle(S, S->Tri[a][0], S->Tri[a][1], S->Tri[a][2]) +
                 SphericalAngle(S, S->Tri[a][1], S->Tri[a][0], S->Tri[a][2]) +
                 SphericalAngle(S, S->Tri[a][2], S->Tri[a][0], S->Tri[a][1]) -
                 cPI);
  ...
  /* tesselate the sphere in a semi-efficient fashion...this could definitely be
   * improved */
...
void SphereRender(PyMOLGlobals* G, int level, const float* centroid,
    const float* color, float alpha, float radius)
{
