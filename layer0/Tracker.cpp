    ...
    if (iterIndexIt != I->id2info.end()) {
      ...
      if ((member_index = iter_info->first)) {
        ...
        iter_info->first = member->list_next;
                         ...
                         iter_info->last)) { /* first is zero, so try last */
        auto member = &I->member[member_index];
        if (member->list_next) {
      ...
      if (!already_linked) {
        ...
        if (member_index) {
          ...
          if (!hash_start) {
            ...
            /* else, member is sole member of the hash chain, so no links are
             * required */
...
int TrackerUnitTest(PyMOLGlobals* G)
{
...

  {
    ...
    for (a = 0; a < N_ID; a++) {
      ...
      if (!TrackerUnlink(I, cand_id[a], list_id[a])) {
        fprintf(
            stderr, "TRACKER_UNIT_TEST FAILED AT LINE %d; a=%d\n", __LINE__, a);
      }
    }
      ...
      if (cand_id[cand_idx]) {
        ...
        if (len >= 0) {
          if (TrackerDelCand(I, cand_id[cand_idx])) {
            ...
            fprintf(
                stderr, "TRACKER_UNIT_TEST FAILED AT LINE %d\n\n", __LINE__);
        }
      }
  ...
  if (TrackerGetNCand(I) != N_ID) {
    fprintf(stderr, "TRACKER_UNIT_TEST FAILED AT LINE %d; %d!=%d\n", __LINE__,
        N_ID, TrackerGetNCand(I));
  }

  if (TrackerGetNList(I) != N_ID) {
    fprintf(stderr, "TRACKER_UNIT_TEST FAILED AT LINE %d; %d!=%d\n", __LINE__,
        N_ID, TrackerGetNList(I));
  }
...

  {
    ...
    {
      int cnt = 0;
      const int expected_cnt =
          6119; /* THIS TEST IS FRAGILE -- result depends on
