...
void GenericBuffer::bufferSubData(
    size_t offset, size_t size, void* data, size_t index)
{
...
void GenericBuffer::bufferReplaceData(
...
const static int rbo_lut[rbo::storage::COUNT] = {
...
void renderBuffer_t::genBuffer()
{
  ...
  glRenderbufferStorage(
      GL_RENDERBUFFER, rbo_lut[(int) _storage], _width, _height);
  glCheckOkay();
}

void renderBuffer_t::freeBuffer()
{
  glDeleteRenderbuffers(1, &_id);
}

void renderBuffer_t::bind() const
{
  glBindRenderbuffer(GL_RENDERBUFFER, _id);
}

void renderBuffer_t::unbind() const
{
  glBindRenderbuffer(GL_RENDERBUFFER, 0);
}
...
#ifdef PURE_OPENGL_ES_2
    GL_TEXTURE_2D, GL_TEXTURE_2D, GL_TEXTURE_2D, GL_RGBA, GL_RGBA, GL_RGB,
#else
    GL_TEXTURE_1D, GL_TEXTURE_2D, GL_TEXTURE_3D, GL_RED, GL_RG, GL_RGB,
#endif
    GL_RGBA, GL_UNSIGNED_BYTE, GL_FLOAT, GL_FLOAT, GL_NEAREST, GL_LINEAR,
...
template <typename T> static int tex_tab(T val)
{
  return tex_lut[(int) val];
}

void tex::env(tex::env_name name, tex::env_param param)
{
#ifdef PURE_OPENGL_ES_2
  fprintf(stderr, "No Support for Texture Env\n");
#else
...
void textureBuffer_t::freeBuffer()
{
...
{
#ifdef PURE_OPENGL_ES_2
  fprintf(stderr, "No support for 1D textures\n");
#else
  using namespace tex;
  _width = width;
  bind();
  ...
  case data_type::HALF_FLOAT:
    glTexImage1D(GL_TEXTURE_1D, 0, tex_format_internal_half_float(_format),
        _width, 0, tex_tab(_format), tex_tab(tex::data_type::FLOAT), data);
    break;
  case data_type::FLOAT:
    glTexImage1D(GL_TEXTURE_1D, 0, tex_format_internal_float(_format), _width,
        0, tex_tab(_format), tex_tab(_type), data);
    break;
...
void textureBuffer_t::texture_data_2D(int width, int height, const void* data)
{
  ...
  switch (_type) {
  case data_type::HALF_FLOAT:
    glTexImage2D(GL_TEXTURE_2D, 0, tex_format_internal_half_float(_format),
        _width, _height, 0, tex_tab(_format), tex_tab(tex::data_type::FLOAT),
        data);
    break;
  case data_type::FLOAT:
...
void textureBuffer_t::texture_data_3D(
    int width, int height, int depth, const void* data)
{
#ifdef PURE_OPENGL_ES_2
  fprintf(stderr, "No support for 3D textures\n");
#else
  _width = width;
  _height = height;
  _depth = depth;
  ...
  case tex::data_type::HALF_FLOAT:
    glTexImage3D(GL_TEXTURE_3D, 0, tex_format_internal_half_float(_format),
        _width, _height, _depth, 0, tex_tab(_format),
        tex_tab(tex::data_type::FLOAT), data);
    break;
  case tex::data_type::FLOAT:
    glTexImage3D(GL_TEXTURE_3D, 0, tex_format_internal_float(_format), _width,
        _height, _depth, 0, tex_tab(_format), tex_tab(tex::data_type::FLOAT),
        data);
    break;
...
void textureBuffer_t::bind() const
{
  glBindTexture(tex_tab(_dim), _id);
}
...
void textureBuffer_t::unbind() const
{
  glBindTexture(tex_tab(_dim), 0);
}
/***********************************************************************
 * FRAMEBUFFER
 ***********************************************************************/
const static int fbo_lut[fbo::attachment::COUNT] = {GL_COLOR_ATTACHMENT0,
#if defined(PURE_OPENGL_ES_2) && !defined(_WEBGL)
    GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT0,
#else
    GL_COLOR_ATTACHMENT1, GL_COLOR_ATTACHMENT2, GL_COLOR_ATTACHMENT3,
#endif
    GL_DEPTH_ATTACHMENT};

template <typename T> static int fbo_tab(T val)
{
  return fbo_lut[(int) val];
}
...
void frameBuffer_t::genBuffer()
{
  glGenFramebuffers(1, &_id);
}

void frameBuffer_t::freeBuffer()
{
  glDeleteFramebuffers(1, &_id);
}

void frameBuffer_t::attach_texture(
    textureBuffer_t* texture, fbo::attachment loc)
{
  size_t id = texture->get_hash_id();
  _attachments.emplace_back(id, loc);
  bind();
  glFramebufferTexture2D(GL_FRAMEBUFFER, fbo_tab(loc),
      GL_TEXTURE_2D, // tex_tab(texture->_dim),
      texture->_id, 0);
  checkStatus();
}

void frameBuffer_t::attach_renderbuffer(
    renderBuffer_t* renderbuffer, fbo::attachment loc)
{
  size_t id = renderbuffer->get_hash_id();
  _attachments.emplace_back(id, loc);
  bind();
  glFramebufferRenderbuffer(
      GL_FRAMEBUFFER, fbo_tab(loc), GL_RENDERBUFFER, renderbuffer->_id);
  checkStatus();
}
...
void renderTarget_t::layout(
    std::vector<rt_layout_t>&& desc, renderBuffer_t* with_rbo)
{
  ...
  for (auto& d : desc) {
    ...
    _textures.push_back(new textureBuffer_t(format, type, tex::filter::LINEAR,
        tex::filter::LINEAR, tex::wrap::CLAMP, tex::wrap::CLAMP));
