...
struct CTexture {
  std::unordered_set<int>
      texturedCharIDs; // set of char ids that have been textured
...
static void TextureInitTextTextureImpl(PyMOLGlobals* G, int textureSize);
    ...
    }
    {
      ...
      if (buffer) {
...

        {
          ...
          if ((I->ypos + h) >=
              I->text_texture_dim) { // only need to check y since x gets reset
                                     // above
            int nrefreshes;
            I->xpos = POS_START;
            I->ypos = 0;
            ...
            /* Also need to reload the selection markers into the texture, since
               we are wiping everything out from the texture and starting from
               the origin */
            if ((nrefreshes = SceneIncrementTextureRefreshes(G)) > 1) {
              /* Texture was refreshed more than once for this frame, increase
               * size of texture */
        ...
        if (I->texture) {
          ...
          if (is_new) {
            ...
            xoff = I->xpos;
            yoff = I->ypos;
            I->texture->texture_subdata_2D(
                xoff, yoff, w, h, temp_buffer.data());
#ifdef _WEBGL
        ...
        if (I->ypos + h > I->maxypos) {
          I->maxypos =
              I->ypos + h +
              1; // added space for running on Ipad/Iphone (weird artifacts)
        }
        if (I->xpos + w > tex_dim) {
          I->xpos = 0;
          I->ypos = I->maxypos;
          ...
          I->xpos +=
              w + 1; // added space for running on Ipad/Iphone (weird artifacts)
        }
...
void TextureGetPlacementForNewSubtexture(PyMOLGlobals* G, int new_texture_width,
    int new_texture_height, int* new_texture_posx, int* new_texture_posy)
{
  CTexture* I = G->Texture;
  ...
  if (I->ypos + new_texture_height > I->maxypos) {
    I->maxypos = I->ypos + new_texture_height +
                 1; // added space for running on Ipad/Iphone (weird artifacts)
  }
  *new_texture_posx = I->xpos;
  *new_texture_posy = I->ypos;
  I->xpos += new_texture_width +
             1; // added space for running on Ipad/Iphone (weird artifacts)
}

void TextureFillNewSubtexture(
    PyMOLGlobals* G, int width, int height, int x, int y, const void* data)
{
  CTexture* I = G->Texture;
  if (I->texture) {
