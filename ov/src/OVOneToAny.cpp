...
void OVOneToAny_Dump(OVOneToAny* up)
{
  ...
  if (up && up->mask) {
    for (a = 0; a <= up->mask; a++) {
      if (up->forward[a]) {
        fprintf(stderr, " OVOneToAny_Dump: Hashes forward[0x%02x]->%d\n",
    ...
    for (a = 0; a < up->size; a++)
      if (up->elem[a].active) {
        fprintf(stderr, " OVOneToAny_Dump: Elements %d:    %d (->%d)    %d \n",
...
#ifdef DEBUG_OVOneToAny
      fprintf(
          stderr, "OVOneToAnyGetKey-Debug: hash %d index %d\n", hash, index);
#endif
...
OVstatus OVOneToAny_SetKey(
    OVOneToAny* up, ov_word forward_value, ov_word reverse_value)
{
#ifdef DEBUG_OVOneToAny
  fprintf(
      stderr, "OVOneToAnySetKey-Debug: %d,%d\n", forward_value, reverse_value);
#endif
  if (!up) {
    return_OVstatus_NULL_PTR;
  } else {
    ...
    if (!mask) {
      ...
      { /* find elements if they exist, and detect erroneous conditions */
...
#ifdef DEBUG_OVOneToAny
        {
          ...
          for (a = 0; a < up->size; a++) {
            fprintf(stderr,
                "OVOneToAnySet-Debug: on entry %d forward_next: %d\n", a + 1,
                elem[a].forward_next);
          }
