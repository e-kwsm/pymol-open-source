...
#define HASH(value, mask)                                                      \
  (((value ^ (value >> 24)) ^ ((value >> 8) ^ (value >> 16))) & mask)
  ...
  if (up && up->mask) {
    for (a = 0; a <= up->mask; a++) {
      if (up->forward[a] || up->reverse[a]) {
        fprintf(stderr,
            " OVOneToOne_Dump: Hashes forward[0x%02x]->%d    "
            "reverse[0x%02x]->%d\n",
            (unsigned int) a, (int) up->forward[a], (unsigned int) a,
            (int) up->reverse[a]);
        empty = OV_FALSE;
      }
    }
    ...
    for (a = 0; a < up->size; a++)
      if (up->elem[a].active) {
        fprintf(stderr,
            ...
            (int) a + 1, (int) up->elem[a].forward_value,
      ...
      if (!rev) {
        ...
        if (rev_elem) {
          ...
          if (rev && (rev == fwd)) {
            ...
            up->n_inactive++;
            if (up->n_inactive >
                (up->size >> 1)) /* over half of bits are inactive */
              OVOneToOne_Pack(up);
            return_OVstatus_SUCCESS;
          }
        }
...
void OVOneToOne_Stats(OVOneToOne* up)
{
  if (up && up->mask) {
    ...
    fprintf(stderr, "active=%d n_inactive=%d ",
        (int) (up->size - up->n_inactive), (int) up->n_inactive);
    fprintf(stderr, "mask=0x%x n_alloc=%lu\n", (unsigned int) up->mask,
        (unsigned long) OVHeapArray_GET_SIZE(up->elem));
  }
}

OVstatus OVOneToOne_Set(
    OVOneToOne* up, ov_word forward_value, ov_word reverse_value)
{
  if (!up) {
    return_OVstatus_NULL_PTR;
  } else {
    ...
    if (!mask) {
      ...
      fprintf(stderr, "OVOneToOneSet-Debug: set %d,%d\n", forward_value,
          reverse_value);
      fprintf(stderr,
          "OVOneToOneSet-Debug: fwd_hash %d rev_hash %d mask %d size %d\n",
      ...
      { /* find elements if they exist, and detect erroneous conditions */
...
#ifdef DEBUG_OVOneToOne
        {
          ...
          for (a = 0; a < up->size; a++) {
            fprintf(stderr,
                "OVOneToOneSet-Debug: on entry %d forward_next: %d\n", a + 1,
                elem[a].forward_next);
          }

          for (a = 0; a <= up->mask; a++) {
            fprintf(stderr,
                "OVOneToOneSet-Debug: on entry %d forward hash %d: "
                "reverse_hash: %d\n",
    ...
    if (!(fwd || rev)) {
      ...
      if (up->n_inactive) {
        ...
        if (up->elem && (!OVHeapArray_CHECK(up->elem, up_element, up->size))) {
          ...
          if (OVreturn_IS_ERROR(
                  result = Recondition(up, up->size + 1, OV_FALSE))) {
            return result;
          } else {
      ...
      }
      {
...
#ifdef DEBUG_OVOneToOne
        {
          ...
          for (a = 0; a <= up->mask; a++) {
            fprintf(stderr,
                "OVOneToOneSet-Debug: forward[%d]=%d, reverse[%d]=%d\n", a,
                up->forward[a], a, up->reverse[a]);
          }
        }
#endif
