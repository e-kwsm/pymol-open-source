...
void OVHeap_Dump(OVHeap* I, ov_uint32 flags)
{
    ...
    while (rec) {
      {
        ...
        if (flags & OVHeap_DUMP_NO_ADDRESSES) {
          ...
          fprintf(OVHeap_ERROR_LOG,
#endif
              "OVHeap: (%7x) %c %s", (unsigned int) rec->size, type[rec->type],
              rec->file);
          ...
          fprintf(OVHeap_ERROR_LOG,
#endif
              "OVHeap:%8x -%8x (%7x) %c %s:%-4d", (unsigned int) rec + 1,
              (unsigned int) ((char*) (rec + 1) + rec->size),
              (unsigned int) rec->size, type[rec->type], rec->file, rec->line);
        }
  ...
  fprintf(OVHeap_ERROR_LOG,
      "OVHeap: Summary: Total bytes allocated 0x%x (%0.3f MB).\n", tot,
      tot / (1024.0 * 1024));
  fprintf(OVHeap_ERROR_LOG, "=================================================="
                            "======================\n");
  fflush(OVHeap_ERROR_LOG);
...
/*
   Ustatus OVHeap_Wrap(Ustatus input,void **ptrptr,char *file, int line, int
   type)
   {
   DebugRec *rec,*cur;
   int hash;
...
void* _OVHeap_Malloc(
    OVHeap* I, ov_size size, const char* file, int line, int type)
{
  DebugRec* rec;
...
void* _OVHeap_Calloc(
    OVHeap* I, ov_size num, ov_size size, const char* file, int line, int type)
{
  DebugRec* rec;
  ...
  if (!rec) {
    if (size) {
      fprintf(OVHeap_ERROR_LOG,
          "OVHeap_Malloc-Error: calloc failed %lu x %lu.\n",
...
void* _OVHeap_Realloc(
    OVHeap* I, void* ptr, ov_size size, const char* file, int line, int type)
{
  ...
  if (!ptr)
    ...
    if (!rec) {
      fprintf(OVHeap_ERROR_LOG,
          "OVHeap_Realloc-Error: realloc() corrupted heap or bad ptr! (%s:%i "
          "@%p)\n",
          file, line, ptr);
#ifdef OVHeap_ABORT_ON_ERROR
      OVHeap_Dump(false);
      ...
      if (!new_rec) {
        fprintf(OVHeap_ERROR_LOG,
            "OVHeap_Realloc-Error: realloc() failed! (%s:%i)\n", file, line);
        HandleOutOfMemory();
