...
static OVstatus OVLexicon_CheckStorage(
    OVLexicon* uk, ov_word entry_size, ov_size data_size)
{
...
static ov_word _GetCStringHash(ov_uchar8* str)
{
  ...
  while ((c = *(p++))) {
    x = (x << 5) + x +
        c; /*  djb2 (G5 = 2.8, P3 = 18.7, P4=2.9) FASTEST OVERALL */
    len++;
  }
  x ^= len;
  return x;
...
OVstatus OVLexicon_Pack(OVLexicon* uk)
{
  ...
  if (uk->entry && uk->data && uk->n_entry && uk->data_unused) {
    ...
    if ((!new_count) &&
        (!new_size)) { /* if lexicon is completely empty, then purge both */
      ...
      if (OVreturn_IS_ERROR(
              status = OVLexicon_CheckStorage(uk, uk->n_entry, new_size))) {
        uk->data = old_data;
        return status;
      }
...
{
  if ((!uk->entry) || (id < 1) ||
      (id > (ov_word) uk->n_entry)) { /* range checking */
    if (id)
      printf(
          "OVLexicon_DecRef-Warning: key %zd not found, this might be a bug\n",
          id);
    ...
    if (ref_cnt < 0) {
      printf("OVLexicon_DecRef-Warning: key %zd with ref_cnt %zd, this might "
             "be a bug\n",
          id, ref_cnt);
      ...
      if (OVreturn_IS_OK(result)) {
        ...
        if (index != id) {
          ...
          if (cur_entry->next) {
            OVOneToOne_Set(uk->up, cur_entry->hash,
                cur_entry->next); /* NOTE: no error checking performed! */
          }
        }
      }
      ...
      uk->n_active--;
      if (uk->data_unused >=
          (uk->data_size >> 1)) /* if 50% unutilized, then pack */
        OVLexicon_Pack(uk);
    }
    return_OVstatus_SUCCESS;
  }
...
OVreturn_word OVLexicon_GetFromCString(OVLexicon* uk, const ov_char8* str)
{
  ...
  if (!index) {
    ...
    OVstatus status;
    {
      ...
      if (OVreturn_IS_ERROR(
              status = OVLexicon_CheckStorage(uk, new_n_entry, new_size))) {
    ...
    if (!cur_index) {
      ...
      if (OVreturn_IS_ERROR(status = OVOneToOne_Set(uk->up, hash, index))) {
        OVreturn_word result;
        uk->entry[index].next =
            uk->free_index; /* record this as a free entry */
        uk->free_index = index;
        uk->n_active--;
        result.status = status.status;
        result.word = 0;
...
OVreturn_word OVLexicon_BorrowFromCString(OVLexicon* uk, const ov_char8* str)
{
  ...
  /* error is something like OVstatus_NULL_PTR, OVstatus_NOT_FOUND; see
   * ov/src/OVreturns.h */
