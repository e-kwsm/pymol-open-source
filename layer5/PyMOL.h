...
void PyMOL_SetClickReady(CPyMOL* I, const char* name, int index, int button,
    int mod, int x, int y, const float* pos, int state, int bond = -1);
void PyMOL_SetPassive(CPyMOL* I, int onOff);
void PyMOL_NeedReshape(
    CPyMOL* I, int mode, int x, int y, int width, int height);
...
void PyMOL_Button(
    CPyMOL* I, int button, int state, int x, int y, int modifiers);
...
void PyMOL_SwapBuffers(
    CPyMOL* I); /* only works if above function has been set */
...
/* asynchronous processing (only useful for Python-based multithreaded builds
 * right now) */
...
void PyMOL_SetModalDraw(
    CPyMOL* I, PyMOLModalDrawFn* fn); /* for internal use only */
...
PyMOLreturn_status PyMOL_CmdLoadCGO(CPyMOL* I, const float* content,
    int content_length, const char* object_name, int state, int quiet,
    int zoom);
...
PyMOLreturn_status PyMOL_CmdRampNew(CPyMOL* I, const char* name,
    const char* map, float* range, int n_range, const char* color, int state,
...
PyMOLreturn_status PyMOL_CmdPseudoatom(CPyMOL* I, const char* object_name,
    ...
    int set_xyz, float x, float y, float z, int state, int mode, int quiet);
...
PyMOLreturn_int_array PyMOL_GetRepsInSceneForObject(
    CPyMOL* I, const char* name);
PyMOLreturn_int_array PyMOL_GetRepsForObject(CPyMOL* I, const char* name);
PyMOLreturn_status PyMOL_SetButton(
    CPyMOL* I, const char* button, const char* modifier, const char* action);
...
PyMOLreturn_float_array PyMOL_Spectrum(CPyMOL* I, const char* expression,
    const char* palette, const char* selection, float minimum, float maximum,
    int byres, int quiet);
...
AtomPropertyInfo* PyMOL_GetAtomPropertyInfo(
    CPyMOL* I, const char* atompropname);
